...
struct FAMILY
{
	...
	int fmnum;		  // 7家族人数，包含申请退出、加入人数
	int fmjoinnum;	  // 8真正加入家族人数
	int fmacceptflag; // 9是否接收成员 0:不接受 1:接受
	int fmsetupflag;  // 10 0:尚未成立 1:已成立
	char fmrule[256]; // 11家族主旨
	int fmsprite;	  // 12 0:光明精灵 1:黑暗精灵
	int fmpointindex; // 13家族据点 index
	int fmpopular;	  // 14家族综合声望
#ifdef _PERSONAL_FAME // 15Arminius: 家族个人声望
	int fmtotalfame;  // 16家族综合 + 家族个人声望总合
#endif
#ifdef _NEW_MANOR_LAW
	int fmmomentum; // 17家族气势
#endif
	int fmgold;				 // 18家族基金
	int fmmemberindexmaxnum; // 19家族目前最大人数
	int apply_time;			 // 20家族申请时间
	int setup_time;			 // 21家族成立时间
	int predel_time;		 // 22预计删除家族时间

	int memonowwritenum;						   // 23目前留言位置
	int memonum;								   // 24留言数量
	int fmadv;									   // 25冒险任务声望
	int fmfeed;									   // 26饲育声望
	int fmsynthesize;							   // 27合成加工声望
	int fmdealfood;								   // 28料理声望
	int fmpk;									   // 29ＰＫ声望
	char memolist[FAMILY_MEMONUM][FAMILY_MEMOLEN]; // 30留言内容
	MEMBERDATA fmmemberindex[MAX_MEMBERNUM];	   // 31成员资料
#ifdef _SET_FMPOINT
	int fmpointtime; // 32申请庄园时间
#endif
#ifdef _NB_FAMILYBADGE
	int fmbadge; //33 家族徽章
#endif
...
int fmtotalfamecompar(const void *indexa, const void *indexb)
{
	...
	if (CheckFMUse(*(int *)indexa) == 0)
		return 1; // empty record
	if (CheckFMUse(*(int *)indexb) == 0)
		return -1;
...
void AddFMMaintainSort(int index)
{
	...
	// modify popular sort
	while ((family[fmpopularindex[i]].fmpopular >= family[index].fmpopular) && (i < MAX_FAMILY) && CheckFMUse(fmpopularindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmpopularindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmadvindex[i]].fmadv >= family[index].fmadv) && (i < MAX_FAMILY) && CheckFMUse(fmadvindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmadvindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmfeedindex[i]].fmfeed >= family[index].fmfeed) && (i < MAX_FAMILY) && CheckFMUse(fmfeedindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmfeedindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmsynthesizeindex[i]].fmsynthesize >= family[index].fmsynthesize) && (i < MAX_FAMILY) && CheckFMUse(fmsynthesizeindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmsynthesizeindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmdealfoodindex[i]].fmdealfood >= family[index].fmdealfood) && (i < MAX_FAMILY) && CheckFMUse(fmdealfoodindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmdealfoodindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmpkindex[i]].fmpk >= family[index].fmpk) && (i < MAX_FAMILY) && CheckFMUse(fmpkindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmpkindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmtotalfameindex[i]].fmtotalfame >= family[index].fmtotalfame) && (i < MAX_FAMILY) && CheckFMUse(fmtotalfameindex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmtotalfameindex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
	...
	j = MAX_FAMILY;
	k = 0;
	while ((family[fmMomentumIndex[i]].fmmomentum >= family[index].fmmomentum) && (i < MAX_FAMILY) && CheckFMUse(fmMomentumIndex[i]))
	{
		if (i == MAX_FAMILY - 1)
			break;
		i++;
	}
	while ((fmMomentumIndex[j] != index) && (j > 0))
		j--;
	if (j > i)
	{
		for (k = j; k > i; k--)
...
void DelFMMaintainSort(int index)
{
	int i = 0, j;
	// modify popular sort
	while ((fmpopularindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmadvindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmfeedindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmsynthesizeindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmdealfoodindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmpkindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmtotalfameindex[i] != index) && (i < MAX_FAMILY))
		i++;
	...
	i = 0;
	j = 0;
	while ((fmMomentumIndex[i] != index) && (i < MAX_FAMILY))
		i++;
...
int readFamily(char *dir)
{
	...
	{
		int i, j;
		unsigned int ttfame = 0;
		for (i = 0; i < MAX_FAMILY; i++)
		{
			family[i].fmpopular = family[i].fmadv + family[i].fmfeed + family[i].fmsynthesize + family[i].fmdealfood + family[i].fmpk;
			...
			for (j = 0; j < MAX_MEMBERNUM; j++)
			{
				...
				{
					ttfame = family[i].fmtotalfame + family[i].fmmemberindex[j].popular;
					if (ttfame > 2000000000)
					{
						family[i].fmtotalfame = 2000000000;
						break;
					}
					else
					{
...
int writeFamily(char *dir)
{
	...
	for (i = 0; i < MAX_FAMILY; i++)
	{
		...
		if ((family[i].fmindex != -1) && (strcmp(family[i].fmname, "") != 0))
		{
			...
			{
				if ((strcmp(family[i].fmmemberindex[k].charname, "") != 0) && (strcmp(family[i].fmmemberindex[k].charid, "") != 0) && (family[i].fmmemberindex[k].charflag != FMMEMBER_NONE))
				{
#ifdef _FMVER21
...
int getblankindex()
{
	int i = 0;
	for (i = 0; i < MAX_FAMILY; i++)
		if (((family[i].fmindex <= 0) || (family[i].fmindex > MAX_FAMILY)) && (strcmp(family[i].fmname, "") == 0))
			return i;
	return -1;
}
...
int getindex(int fmindex, char *fmname)
{
	...
	{
		if ((family[i].fmindex == fmindex) && (strcmp(family[i].fmname, fmname) == 0))
			return i;
	}
	return -1;
}
...
void delovertimeFMMem(int time)
{
	...
	for (i = 0; i <= fmindexmaxnum; i++)
	{
		if (CheckFMUse(i) == 0)
			continue;
		if (((family[i].predel_time < time) && (family[i].predel_time != -1) && (family[i].fmsetupflag != 1)) || (family[i].fmjoinnum == 0))
		{
		...
		for (j = 1; j < family[i].fmmemberindexmaxnum; j++)
		{
			if (family[i].fmmemberindex[j].charflag == FMMEMBER_NONE)
				continue;
			if ((family[i].fmmemberindex[j].predeltime < time) && (family[i].fmmemberindex[j].predeltime != -1))
			{
#ifdef _FMVER21
...
#endif
{
	...
	if (CheckFM(&index, fmname, fmindex) < 0)
		return -1;
	if (family[index].fmacceptflag == 0)
		return -2;
	if (family[index].fmnum >= MAX_MEMBERNUM || family[index].fmjoinnum >= MAX_MEMBERNUM)
		return -3;

	// 防止族长加入自己的家族  code:shan
	if ((strcmp(family[index].fmmemberindex[0].charname, charname) == 0) && (strcmp(family[index].fmmemberindex[0].charid, charid) == 0))
		return -1;

	for (i = 1; i < MAX_MEMBERNUM; i++)
	{
		if ((strcmp(family[index].fmmemberindex[i].charname, "") == 0) && (strcmp(family[index].fmmemberindex[i].charid, "") == 0))
		{
			...
			if (family[index].fmmemberindexmaxnum <= i)
				family[index].fmmemberindexmaxnum = i + 1;
			db_familyupdate[index] = 1;
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
			// 更新玩家资料
			  ...
			  char *charname, char *charid)
{
	...
	{
		if ((strcmp(family[index].fmmemberindex[i].charname, charname) == 0) && (strcmp(family[index].fmmemberindex[i].charid, charid) == 0))
		{
#ifdef _FMVER21
...
int ACFixFMData(int index, char *fmname, int fmindex, int kindflag, int charindex, char *data1, char *data2)
{
	...
	else if (kindflag == 2)
	{
		...
		if (family[index].fmpk < MINFMPOPNUM)
			family[index].fmpk = MINFMPOPNUM;
		if (family[index].fmpk > MAXFMPOPNUM)
			family[index].fmpk = MAXFMPOPNUM;
	}
	else if (kindflag == 3)
	{
		if ((strcmp(data1, "") == 0) || (strcmp(data2, "") == 0))
			return -1;
	...
	{
		if (family[index].fmgold + recvdata < 0)
			return -1;
	...
	else if (kindflag == 7)
	{
		...
		if (family[index].fmadv < 0)
			family[index].fmadv = 0;
		if (family[index].fmadv > MAXFMPOPNUM)
			family[index].fmadv = MAXFMPOPNUM;
#ifdef _NEW_MANOR_LAW
		family[index].fmmemberindex[charindex].momentum += recvdata;
		if (family[index].fmmemberindex[charindex].momentum < 0)
			family[index].fmmemberindex[charindex].momentum = 0;
		if (family[index].fmmemberindex[charindex].momentum > MAXFMPOPNUM)
			family[index].fmmemberindex[charindex].momentum = MAXFMPOPNUM;
	...
	else if (kindflag == 8)
	{
		...
		if (family[index].fmfeed < 0)
			family[index].fmfeed = 0;
		if (family[index].fmfeed > MAXFMPOPNUM)
			family[index].fmfeed = MAXFMPOPNUM;
#ifdef _NEW_MANOR_LAW
		family[index].fmmemberindex[charindex].momentum += recvdata;
		if (family[index].fmmemberindex[charindex].momentum < 0)
			family[index].fmmemberindex[charindex].momentum = 0;
	...
	else if (kindflag == 9)
	{
		...
		if (family[index].fmsynthesize < 0)
			family[index].fmsynthesize = 0;
		if (family[index].fmsynthesize > MAXFMPOPNUM)
			family[index].fmsynthesize = MAXFMPOPNUM;
	}
	else if (kindflag == 10)
	{
		family[index].fmdealfood = family[index].fmdealfood + recvdata;
		if (family[index].fmdealfood < 0)
			family[index].fmdealfood = 0;
		if (family[index].fmdealfood > MAXFMPOPNUM)
	...
	else if (kindflag == 11)
	{
		...
		if (strcmp(family[index].fmmemberindex[charindex].charname, data2) != 0 || strcmp(family[index].fmmemberindex[charindex].charid, "") == 0)
			return -1;
		// shan 2002/01/04
	...
	else if (kindflag == 12)
	{
		...
		if (family[index].fmmemberindex[charindex].momentum < 0)
			family[index].fmmemberindex[charindex].momentum = 0;
		if (family[index].fmmemberindex[charindex].momentum > MAXFMMOMENTUM)
			family[index].fmmemberindex[charindex].momentum = MAXFMMOMENTUM;
	}
	else if (kindflag == 13)
	{
...
#endif
	if ((kindflag == 2) || (kindflag == 7) || (kindflag == 8) || (kindflag == 9) || (kindflag == 10)
#ifdef _NEW_MANOR_LAW
		|| (kindflag == 12) || (kindflag == 13)
#endif
	)
...
#endif
		// 计算家族声望
		family[index].fmpopular = family[index].fmadv + family[index].fmfeed + family[index].fmsynthesize + family[index].fmdealfood + family[index].fmpk;
		{
			int j;
			...
			for (j = 0; j < MAX_MEMBERNUM; j++)
			{
				if ((family[index].fmmemberindex[j].charflag != FMMEMBER_NONE) && (family[index].fmmemberindex[j].charflag != FMMEMBER_APPLY))
				{
					ttfame = family[index].fmtotalfame + family[index].fmmemberindex[j].popular;
					if (ttfame > 2000000000)
					{
						family[index].fmtotalfame = 2000000000;
						break;
					}
					else
					{
...
{
	int award = 0;
	if (CheckFM(&winindex, winfmname, winfmindex) < 0)
		return -1;
	if (CheckFM(&loseindex, losefmname, losefmindex) < 0)
		return -1;
	...
	{
		family[winindex].fmpopular = family[winindex].fmadv + family[winindex].fmfeed + family[winindex].fmsynthesize + family[winindex].fmdealfood + family[winindex].fmpk;
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
		{
			...
			for (j = 0; j < MAX_MEMBERNUM; j++)
			{
				...
				{
					ttfame = family[winindex].fmtotalfame + family[winindex].fmmemberindex[j].popular;
					if (ttfame > 2000000000)
					{
						family[winindex].fmtotalfame = 2000000000;
						break;
					}
					else
					{
		...
		family[loseindex].fmpopular = family[loseindex].fmadv + family[loseindex].fmfeed + family[loseindex].fmsynthesize + family[loseindex].fmdealfood + family[loseindex].fmpk;
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
		{
			...
			for (j = 0; j < MAX_MEMBERNUM; j++)
			{
				...
				{
					ttfame = family[loseindex].fmtotalfame + family[loseindex].fmmemberindex[j].popular;
					if (ttfame > 2000000000)
					{
						family[loseindex].fmtotalfame = 2000000000;
						break;
					}
					else
					{
...
)
{
	...
	*badge = family[index].fmbadge;
	printf("index:%d | badge:%d", index, badge);
	for (i = 0; i < family[index].fmmemberindexmaxnum; i++)
	{
		char msg[256];
		if ((strcmp(family[index].fmmemberindex[i].charname, "") != 0) && (strcmp(family[index].fmmemberindex[i].charid, "") != 0) && (family[index].fmmemberindex[i].charflag != FMMEMBER_NONE))
		{
#ifdef _FM_MODIFY
#ifdef _NEW_MANOR_LAW
...
int ACFMDetail(int index, char *fmname, int fmindex, char *data)
{
	...
	strcpy(data, "");
#ifdef _NB_FAMILYBADGE
	sprintf(data, "%d|%d|%s|%s|%d|%s|%s|%d|%d|%s|%d|%d"
#else
...
#endif
{
	...
	if (result < 0)
		return -1;
	if (meindex < 0 || meindex > MAX_MEMBERNUM)
		return -1;
#endif
	if (strcmp(family[index].fmmemberindex[charindex].charname, charname) == 0)
	{
		if (family[index].fmjoinnum >= MAX_MEMBERNUM)
			return -1;
		time(&t1);
		...
		if (family[index].fmmemberindexmaxnum <= charindex)
			family[index].fmmemberindexmaxnum = charindex + 1;
		if (family[index].fmjoinnum >= MIN_MEMBERNUM)
		{
			if (family[index].fmjoinnum == MIN_MEMBERNUM)
				family[index].setup_time = t1;
			family[index].fmsetupflag = 1;
			family[index].predel_time = -1;
		}
		if (family[index].fmjoinnum >= MAX_MEMBERNUM)
			family[index].fmacceptflag = 0;
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
		{
			int j;
			unsigned int ttfame = 0;
...
#endif
		if (family[index].fmmemberindexmaxnum == charindex)
			family[index].fmmemberindexmaxnum--;
		if (family[index].fmjoinnum < MIN_MEMBERNUM)
		{
			time(&t1);
				  ...
				  char *charname, int charindex, int result)
{
	...
	for (i = 0; i < MAX_MEMBERNUM; i++)
	{
		if (family[index].fmmemberindex[i].charflag == result && result != FMMEMBER_MEMBER)
			count++;
	}
	if (count >= FMELDERNUM)
	{
	...
	for (i = 0; i < family[index].fmmemberindexmaxnum; i++)
	{
		if ((strcmp(family[index].fmmemberindex[i].charname, charname) == 0) && (strcmp(family[index].fmmemberindex[i].charid, charid) == 0))
		{
			...
			*joinflag = family[index].fmmemberindex[i].charflag;
			...
			if (i == 0)
				*joinflag = 1;
			else if ((family[index].fmmemberindex[i].charflag == 1) || (family[index].fmmemberindex[i].charflag == 3))
				*joinflag = 2;
			else
				*joinflag = 0;
#endif
						...
						char *charname, char *charid, int *charindex)
{
	...
	for (i = 0; i < MAX_FAMILY; i++)
	{
		if (family[i].fmindex <= 0)
			continue;
		for (j = 0; j < MAX_MEMBERNUM; j++)
		{
			if (family[i].fmmemberindex[j].charflag <= 0)
				continue;
			if ((strcmp(family[i].fmmemberindex[j].charname, charname) == 0) && (strcmp(family[i].fmmemberindex[j].charid, charid) == 0))
			{
				strcpy(fmname, "");
...
int ACFMWriteMemo(int index, char *fmname, int fmindex, char *data)
{
	...
	{
		fmsmemo.num++;
		if (fmsmemo.num > FMSMEMONUM)
			fmsmemo.num = FMSMEMONUM;
		strcpy(fmsmemo.memodata[fmsmemo.fmsnowwritenum], data);
		fmsmemo.fmsnowwritenum++;
		if (fmsmemo.fmsnowwritenum >= FMSMEMONUM)
			fmsmemo.fmsnowwritenum = 0;
		db_fmsmemoupdate = 1;
		return 0;
	}
	if ((index > MAX_FAMILY) || (index < 0))
	...
	if (strlen(data) > FAMILY_MEMOLEN)
		return -1;
	family[index].memonum++;
	if (family[index].memonum > FAMILY_MEMONUM)
		family[index].memonum = FAMILY_MEMONUM;
	strcpy(family[index].memolist[family[index].memonowwritenum], data);
	family[index].memonowwritenum++;
	if (family[index].memonowwritenum >= FAMILY_MEMONUM)
		family[index].memonowwritenum = 0;
	db_familyupdate[index] = 1;
	return 0;
}
				 ...
				 int fl, int x, int y)
{
	...
	if (family[index].fmpointindex >= 0)
		return -2;
#endif
	if ((fmpoint[fmpointindex].x != x) || (fmpoint[fmpointindex].y != y))
		return -1;
	if (fmpoint[fmpointindex].hadfmindex > 0)
		return -4;
	if (family[index].fmjoinnum < MINAPPLYPNUM)
		return -5;
#ifdef _SET_FMPOINT
	if (family[index].fmpointtime > (int)time(NULL))
		return -6;
#endif
				 ...
				 char *losefmname, int losefmindex, int village)
{
	...
	if (winindex == -1 || winfmindex == -1 || loseindex == -1 || losefmindex == -1)
	{
		...
		family[fmpoint[village - 1].hadfmindex - 1].fmpointtime = (int)time(NULL) + 60 * 30;
	...
	if (fmpoint[village - 1].hadfmindex == winfmindex)
	{
		...
		for (i = 0; i < MAX_MEMBERNUM; i++)
		{
			if (strcmp(family[winindex].fmmemberindex[i].charname, "") == 0)
				continue;
	...
	if (loseindex >= 0)
	{
		...
		for (i = 0; i < MAX_MEMBERNUM; i++)
		{
			if (strcmp(family[loseindex].fmmemberindex[i].charname, "") == 0)
				continue;
#ifdef _NEW_MANOR_LAW
			// 庄园战後,个人气势全归零
		...
		for (i = 0; i < familymaxint; i++)
		{
			if (strcmp(cmd, FAMILY_INTDATA[i]) == 0)
			{
				...
				case 21:
					...
					if (family[index].fmmomentum > 2000000000)
					{
						family[index].fmmomentum = 200000000;
					}
					if (family[index].fmmomentum < 0)
					{
						family[index].fmmomentum = 0;
					}
					break;
				...
				for (i = 0; i < MAX_MEMBERNUM; i++)
				{
					if ((family[index].fmmemberindex[i].charflag != FMMEMBER_NONE) &&
						(family[index].fmmemberindex[i].charflag != FMMEMBER_APPLY))
					{
						ttfame = family[index].fmtotalfame + family[index].fmmemberindex[i].popular;
						if (ttfame > 2000000000)
						{
							family[index].fmtotalfame = 2000000000;
							break;
				...
				case 5:
					...
					for (i = 0; i < MAX_MEMBERNUM; i++)
					{
							...
							(family[index].fmmemberindex[i].charflag != FMMEMBER_APPLY))
						{
							...
							{
								family[index].fmtotalfame += family[i].fmmemberindex[i].popular;
							}
						}
					}
					break;
...
int readOneFamily(char *filename, int i)
{
	...
	if (family[i].fmgold > FMMAXGOLD)
		family[i].fmgold = FMMAXGOLD;
	if (family[i].fmgold < 0)
		family[i].fmgold = 0;
