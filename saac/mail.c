...
static int reallocMailBuf(void)
{
    ...
    if (previous)
        memcpy((char *)newbuf, (char *)previous,
...
void receiveMailAck(char *id, char *charname, int a, int mesgid)
{
    ...
    for (i = 0; i < mailbufsize; i++)
    {
        ...
        {
            if (mailbuf[i].use && mailbuf[i].id_charname_hash == h && strcmp(mailbuf[i].id_to, id) == 0 && strcmp(mailbuf[i].charname_to, charname) == 0 && mailbuf[i].state == MS_WAIT_ACK)
            {
                char savefile[1024];
                char childname[1000];
                snprintf(childname, sizeof(childname), "%u",
