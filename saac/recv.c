...
void saacproto_ACCharList_recv(int ti, char *id, char *pas, char *ip, char *mac1, char *mac2, char *mac3, int mesgid, int charlistflg)
{
	...
	if (!is_game_server_login(ti))
	{
		saacproto_ACCharList_send(ti, FAILED, "�޷���½��Ϸ��", mesgid);
		return;
	}

	if (strlen(id) == 0 || strlen(pas) == 0)
	{
		saacproto_ACCharList_send(ti, FAILED, "�˺����벻��Ϊ�գ�", mesgid);
		return;
	}
...
void saacproto_ACCharDelete_recv(int ti, char *id, char *pas, char *charname, char *option, int mesgid)
{
	...
	if (sasql_ItemPetLocked_Char(id, option) != 1)
	{
...
#ifdef _FAMILY
	int r = 0;
	printf("saacproto_ACFixFMData_recv\n");
	...
	for (i = 0; i < MAXCONNECTION; i++)
	{
		if (gs[i].use && gs[i].name[0])
		{
			saacproto_S_UpdataStele_send(i, ocdkey, oname,
										 ncdkey, nname, title, level, trns, floor);
...
void saacproto_ACMissionTable_recv(int fd, int num, int type, char *data, char *angelinfo)
{
	...
	{ // change data flag
		...
		for (i = 0; i < MAXMISSIONTABLE; i++)
		{
			if (missiontable[i].angelinfo[0] == '\0')
				continue;
			if (strcmp(missiontable[i].angelinfo, angelinfo) && strcmp(missiontable[i].heroinfo, angelinfo))
				continue;
...
void saacproto_ItemPetLocked_recv(int fd, int clifd, char *id, char *safepasswd)
{
	...
	if (strcmp(data, "��ȫ���Ѿ��ɹ�������") == 0)
	{
		saacproto_ItemPetLocked_send(fd, clifd, 1, data);
	}
	else if (strcmp(data, "����δ���ð�ȫ���������룬Ϊ��ȷ����ȫ��������һ����λ����������Ϊ��ȫ�����벢���ͼ�ס��") == 0)
	{
...
void saacproto_FormulateAutoPk_recv(int fd, int clifd, char *id, int point)
{
	...
	if (pkpoint == -1)
	{
		sprintf(data, "�����������%d�޷�ת����SQL��������Ա��ϵ��", point);
	}
	else
	{
		sprintf(data, "�ѽ������������%d��ת����SQL��㵱ǰ������ֹ�%d", point, pkpoint);
	}
...
void saacproto_AllServSend_recv(char *data, int color)
{
	...
	for (i = 0; i < MAXCONNECTION; i++)
	{
		if (gs[i].use && gs[i].name[0])
		{
			saacproto_AllServSend_send(i, data, color);
		}
	}
}
#endif
...
void saacproto_ACFmPkOnlineSn_recv(int fd, int manorindex, int flag)
{
	extern gmsv gs[MAXCONNECTION];
	int i;
	char *gsname = gs[fd].name;
	for (i = 0; i < MAXCONNECTION; i++)
	{
		if (i == fd)
			continue;
		if (gs[i].use && gs[i].name[0])
		{
			saacproto_ACFmPkOnlineSn_send(i, manorindex, gsname, flag);
		}
	}
}
#endif
...
void saacproto_gm_shutdown_recv(int fd, int itype, char *msg)
{
	int i;
	for (i = 0; i < MAXCONNECTION; i++)
	{
		if (is_game_server_login(i))
		{
			saacproto_gm_shutdown_send(i, itype, msg); //����GMSV
		}
	}
}
#endif
