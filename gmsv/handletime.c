...
void LSTimeToRealTime(LSTIME *lstime, long *t) {
  *t = (long)((lstime->hour * LSTIME_DAYS_PER_YEAR + lstime->day) /* 凛棉 */
...
void Assess_SysEfficacy(int flg) {
  if (flg == 0) {
    ...
    if (Cnum % 500 == 0) {
      ...
      SaveCheck_TotalClock = 0;
      GMBroadCast_SysTime =
          (float)(GMBroadCast_TotalClock / Cnum) / CLOCKS_PER_SEC;
      GMBroadCast_TotalClock = 0;
#endif
      Cnum = 0;
    }
...
void ASSESS_getSysEfficacy(float *TVsec) { *TVsec = SysTime; }
