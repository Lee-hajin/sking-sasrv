...
#ifdef _CHAR_PROFESSION // WON ADD 人物职业
int SKILL_getRealInt(Skill *skill, int element) {
...
static SKILL_table SKILL_tbl[] = {
    ...
    {100, NULL},         // 1
    {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL},
    {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL},
    {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL}, {100, NULL}, // 16
#else
...
static void SKILL_setitemlimit(int charaindex, Skill *sk) {
  static int itemlimit[9] = {
      ...
      7 + 8 + 4 * 5, 7 + 8 + 4 * 6, 7 + 8 + 4 * 7, 7 + 8 + 4 * 8,
  };
  int level;
  if (!CHAR_CHECKINDEX(charaindex))
    return;
  if (sk->data[SKILL_IDENTITY] != SKILL_ALOTOFTHINGS)
    return;
  level = sk->data[SKILL_LEVEL];
  if (level < 0)
    level = 0;
  if (level >= arraysizeof(itemlimit))
    level = arraysizeof(itemlimit) - 1;
}
...
static void SKILL_setmerchant(int charaindex, Skill *sk) {
  ...
  if (!CHAR_CHECKINDEX(charaindex))
    return;
  if (sk->data[SKILL_IDENTITY] != SKILL_MERCHANT)
    return;
...
char *SKILL_makeSkillStatusString(Skill *skill, int charaindex, int skill_num) {
  ...
  snprintf(ITEM_statusStringBuffer, sizeof(ITEM_statusStringBuffer),
           ...
           PROFESSION_SKILL_getInt(Pskill, PROFESSION_SKILL_ICON), cost_mp,
           skill_level, PROFESSION_SKILL_getChar(Pskill, PROFESSION_SKILL_NAME),
...
char *SKILL_makeSkillStatusString(Skill *skill) {
  snprintf(ITEM_statusStringBuffer, sizeof(ITEM_statusStringBuffer), "%d|%d",
           SKILL_getInt(skill, SKILL_IDENTITY),
...
char *SKILL_makeSkillCoolTimeString(Skill *skill, int charaindex,
                                    int skill_num) {
  int skillindex = SKILL_getInt(skill, SKILL_IDENTITY);
  int Pskill = PROFESSION_SKILL_getskillArray(skillindex);
  snprintf(ITEM_statusStringBuffer, sizeof(ITEM_statusStringBuffer), "%d",
           PROFESSION_COOL_TIME(charaindex, skill_num));
  return ITEM_statusStringBuffer;
}
#endif
...
BOOL SKILL_CHECKID(int skillid) {
  if (SKILL_NUM <= skillid && skillid > 0)
    return FALSE;
  return TRUE;
}
...
int SKILL_levelup(Skill *sk) {
  ...
  if (!SKILL_CHECKID(id))
    return -1;
  sk->data[SKILL_LEVEL]++;
  sk->data[SKILL_LEVEL] = min(sk->data[SKILL_LEVEL], SKILL_tbl[id].maxlevel);
  return TRUE;
}
...
BOOL SKILL_getUpableSkillID(int charaindex, char *buf, int buflen) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    ...
    if (chsk && chsk->use && SKILL_CHECKID(chsk->skill.data[SKILL_IDENTITY]) &&
...
void SKILL_skillEffect(int charaindex) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    ...
    if (cskill == NULL || cskill->use == FALSE)
      continue;
...
int PROFESSION_MAGIC_COST_MP(int charaindex, int skill_num) {
  ...
  if ((strcmp(skill_name, "PROFESSION_VOLCANO_SPRINGS")) == 0) { // 火山泉
    if (skill_level >= 10)
      ...
      dec_mp = 10;
  } else if ((strcmp(skill_name, "PROFESSION_FIRE_BALL")) == 0) { // 火星球
    if (skill_level >= 9)
      dec_mp = 50;
    else if (skill_level >= 7)
      dec_mp = 45;
      ...
      dec_mp = 30;
  } else if ((strcmp(skill_name, "PROFESSION_SUMMON_THUNDER")) == 0) { // 召雷术
    if (skill_level >= 8)
      dec_mp = 30;
    else if (skill_level >= 5)
      dec_mp = 25;
      ...
      dec_mp = 10;
  } else if ((strcmp(skill_name, "PROFESSION_CURRENT")) == 0) { // 电流术
    ...
    if (skill_level >= 9)
      ...
      dec_mp = 30;
#endif
  } else if ((strcmp(skill_name, "PROFESSION_STORM")) == 0) { // 暴风雨
#ifdef _PROFESSION_ADDSKILL
    if (skill_level > 8)
      dec_mp = 50;
    else if (skill_level > 6)
    ...
    if (skill_level >= 10)
      ...
      dec_mp = 50;
#endif
  } else if ((strcmp(skill_name, "PROFESSION_ICE_ARROW")) == 0) { // 冰箭术
    if (skill_level >= 8)
      dec_mp = 20;
    else if (skill_level >= 4)
      dec_mp = 15;
      ...
      dec_mp = 10;
  } else if ((strcmp(skill_name, "PROFESSION_ICE_CRACK")) == 0) { // 冰爆术
#ifdef _PROFESSION_ADDSKILL
    if (skill_level >= 10)
      dec_mp = 80;
    else if (skill_level > 8)
    ...
    if (skill_level >= 9)
      ...
      dec_mp = 30;
#endif
  } else if ((strcmp(skill_name, "PROFESSION_DOOM")) == 0) { // 世界末日
    if (skill_level > 8)
      dec_mp = 150;
    else if (skill_level > 4)
      dec_mp = 100;
      ...
      dec_mp = 50;
  } else if ((strcmp(skill_name, "PROFESSION_FIRE_SPEAR")) == 0) { // 火龙枪
    if (skill_level > 8)
      dec_mp = 80;
    else if (skill_level > 6)
      dec_mp = 70;
      ...
      dec_mp = 30;
  } else if ((strcmp(skill_name, "PROFESSION_BLOOD_WORMS")) == 0) { // 嗜血蛊
    if (skill_level >= 10)
      dec_mp = 15;
    else if (skill_level >= 5)
      dec_mp = 10;
      ...
      dec_mp = 5;
  } else if ((strcmp(skill_name, "PROFESSION_SIGN")) == 0) { // 一针见血
  ...
  } else if ((strcmp(skill_name, "PROFESSION_ENCLOSE")) == 0) { // 附身术
    if (skill_level >= 10)
      dec_mp = 80;
    else if (skill_level >= 8)
      dec_mp = 70;
      ...
      dec_mp = 50;
  } else if ((strcmp(skill_name, "PROFESSION_ICE_MIRROR")) == 0) { // 冰镜术
    if (skill_level >= 9)
      dec_mp = 40;
    else if (skill_level >= 7)
      dec_mp = 35;
      ...
      dec_mp = 20;
  } else if ((strcmp(skill_name, "PROFESSION_FIRE_ENCLOSE")) == 0) { // 火附体
    if (skill_level >= 10)
      dec_mp = 50;
    else if (skill_level >= 7)
      dec_mp = 40;
      ...
      dec_mp = 20;
  } else if ((strcmp(skill_name, "PROFESSION_ICE_ENCLOSE")) == 0) { // 冰附体
    if (skill_level >= 10)
      dec_mp = 50;
    else if (skill_level >= 7)
      dec_mp = 40;
      ...
      dec_mp = 20;
  } else if ((strcmp(skill_name, "PROFESSION_THUNDER_ENCLOSE")) ==
             0) { // 雷附体
    if (skill_level >= 10)
      dec_mp = 50;
    else if (skill_level >= 7)
      ...
      dec_mp = 20;
  } else if ((strcmp(skill_name, "PROFESSION_TRANSPOSE")) == 0) { // 移形换位
    if (skill_level >= 10)
      dec_mp = 50;
    else if (skill_level >= 9)
      dec_mp = 40;
    ...
    if (skill_level >= 10)
      ...
      dec_mp = 5;
  } else if ((strcmp(skill_name, "PROFESSION_CALL_NATURE")) == 0) { // 号召自然
    ...
    dec_mp = 50;
  } else if ((strcmp(skill_name, "PROFESSION_BOUNDARY")) == 0) { // 四属性结界
    char *pszP = NULL;
    if (skill_level > 9)
      dec_mp = 20;
    else if (skill_level > 6)
    ...
    //破除结界耗损mp与其他结界不同
    if ((pszP =
             strstr(PROFESSION_SKILL_getChar(Pskillid, PROFESSION_SKILL_OPTION),
                    "破结界")) != NULL) { // 技能的参数
      if (skill_level >= 9)
        dec_mp = 20;
      else if (skill_level > 4)
...
#endif
#ifdef _PRO_KILLME
  else if ((strcmp(skill_name, "PROFESSION_KILLME")) == 0) {
    dec_mp = 25;
  }
#endif
  else {
...
int PROFESSION_CHANGE_SKILL_LEVEL_M(int skill_level) {
  if (skill_level > 90)
    skill_level = 10;
...
int PROFESSION_CHANGE_SKILL_LEVEL_A(int skill_level) {
  if (skill_level >= 100)
    skill_level = 10;
...
int PROFESSION_COOL_TIME(int charaindex, int skill_num) {
  ...
  skill_name =
      PROFESSION_SKILL_getChar(Pskillid, PROFESSION_SKILL_FUNCNAME); // 技能名称

  if (strcmp(skill_name, "PROFESSION_ABSOLUTE_DEFENSE") == 0) //绝对防御
    return GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ABSOLUTE2));
  else if (strcmp(skill_name, "PROFESSION_CRACK_BONE") == 0) //裂骨断筋
    return GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_CRACKBONE2));
  else if (strcmp(skill_name, "PROFESSION_PACIFY") == 0) //抚慰心灵
    return GETFIRSTVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_PACIFY));
  else if (strcmp(skill_name, "PROFESSION_DARK_SEAL") == 0) //灵封印
    return GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL));
  else if (strcmp(skill_name, "PROFESSION_DEMON_SPEECH") == 0) //恶魔诅咒
    return GETTHIRDVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH));
  else if (strcmp(skill_name, "PROFESSION_FIRST_AID") == 0) //战场急救
    return CHAR_getWorkInt(charaindex, CHAR_WORK_FIRST_AID);
  else if (strcmp(skill_name, "PROFESSION_ROAR") == 0) //战狼怒吼
    return GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ROAR));
  else if (strcmp(skill_name, "PROFESSION_MEDICATION") == 0) //制药学
    return CHAR_getWorkInt(charaindex, CHAR_WORK_MEDICATION);
