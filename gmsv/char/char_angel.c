...
char *getMissionNameInfo(int charaindex, char *nameinfo) {
  sprintf(nameinfo, "%s:%s", CHAR_getChar(charaindex, CHAR_CDKEY),
...
void addAngelData(int angelindex, int heroindex, int mission, int flag) {
  ...
  //	return;
  // sprintf( angelinfo, "%s:%s", CHAR_getChar( angelindex, CHAR_CDKEY),
  // CHAR_getChar( angelindex, CHAR_NAME) );
  getMissionNameInfo(angelindex, angelinfo);
  // sprintf( heroinfo, "%s:%s", CHAR_getChar( heroindex, CHAR_CDKEY),
  // CHAR_getChar( heroindex, CHAR_NAME) );
...
void selectAngel(int charaindex, int heroindex, int mission, int gm_cmd) {
  ...
  sprintf(msg, " 使者资格检查: %s %s ", CHAR_getChar(charaindex, CHAR_CDKEY),
          CHAR_getChar(charaindex, CHAR_NAME));
  print(msg);
  LogAngel(msg);
  ...
  if (gm_cmd == FALSE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_LV) < 30 ||
        !NPC_EventCheckFlg(charaindex, 4)) {
      print(" ANGEL资格不符 ");
      return;
    }
  ...
  if (heroindex == -1) {
    ...
    while (1) {
      if (findindex == startindex - 1)
        break;
      findindex = (findindex + 1) % max_char;
      if (!CHAR_CHECKINDEX(findindex))
        continue;
      if (CHAR_getInt(findindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
        continue;
      if (findindex == charaindex)
        continue;
      if (checkIfAngel(findindex) != -1)
        continue; // 是否天使或勇者
      if (CHAR_getInt(findindex, CHAR_LV) < 80 ||
          !NPC_EventCheckFlg(findindex, 4))
        continue; // 勇者的条件
      if (rand() % 3 == 0)
        continue; // 勇者的机率 2/3
...

  {
    sprintf(
        msg,
        " 产生 %s 天使候补: %s %s Lv:%d 勇者候补: %s %s Lv:%d 任务:%d ci=%d "
        "hi=%d ",
        gm_cmd ? ("(GM指令)") : (" "), CHAR_getChar(charaindex, CHAR_CDKEY),
        CHAR_getChar(charaindex, CHAR_NAME), CHAR_getInt(charaindex, CHAR_LV),
        CHAR_getChar(heroindex, CHAR_CDKEY), CHAR_getChar(heroindex, CHAR_NAME),
        CHAR_getInt(heroindex, CHAR_LV), mission, charaindex, heroindex);
    print(msg);
    LogAngel(msg);
  }
}
...
int AngelCreate(int angelindex) {
  ...
  if (mindex == -1 || missiontable[mindex].flag == MISSION_TIMEOVER) {
    ...
    CHAR_talkToCli(angelindex, -1, "很抱歉，你太晚回答，已经逾时了。",
                   CHAR_COLORYELLOW);

    sprintf(msgbuf, " 回答逾时或无资料 i:%d 使者:%s %s ", mindex,
  ...
  if (CHAR_findEmptyItemBoxNo(angelindex) < 2) {
    CHAR_talkToCli(angelindex, -1, "空间栏位不足。至少要有两个空道具栏位。",
                   CHAR_COLORYELLOW);
    return TRUE;
  }
  ...
  if (angeltokenindex != -1) {
    ...
    // sprintf( nameinfo, "%s:%s", CHAR_getChar( angelindex, CHAR_CDKEY),
    // CHAR_getChar( angelindex, CHAR_NAME ) );
    ITEM_setChar(angeltokenindex, ITEM_ANGELINFO,
    ...
    // sprintf( msgbuf, "%s(%s)", ITEM_getChar( angeltokenindex, ITEM_NAME),
    // CHAR_getChar( angelindex, CHAR_NAME ) ); ITEM_setChar( angeltokenindex,
    // ITEM_NAME, msgbuf); ITEM_setChar( angeltokenindex, ITEM_SECRETNAME,
    // msgbuf); sprintf( msgbuf, "这是使者 %s 与勇者 %s 专属的信物",
    // CHAR_getChar( angelindex, CHAR_NAME), heroname );
    ...
    LogItem(CHAR_getChar(angelindex, CHAR_NAME),
            CHAR_getChar(angelindex, CHAR_CDKEY), angeltokenindex,
            "制作使者信物", CHAR_getInt(angelindex, CHAR_FLOOR),
            CHAR_getInt(angelindex, CHAR_X), CHAR_getInt(angelindex, CHAR_Y),
            ITEM_getChar(angeltokenindex, ITEM_UNIQUECODE),
            ITEM_getChar(angeltokenindex, ITEM_NAME),
            ITEM_getInt(angeltokenindex, ITEM_ID), angelindex);
  ...
  if (herotokenindex != -1) {
    ...
    // sprintf( nameinfo, "%s:%s", CHAR_getChar( angelindex, CHAR_CDKEY),
    // CHAR_getChar( angelindex, CHAR_NAME ) );
    ITEM_setChar(herotokenindex, ITEM_ANGELINFO,
                 missiontable[mindex].angelinfo);
    ...
    // sprintf( msgbuf, "%s(%s)", ITEM_getChar( herotokenindex, ITEM_NAME),
    // heroname ); ITEM_setChar( herotokenindex, ITEM_NAME, msgbuf);
    // ITEM_setChar( herotokenindex, ITEM_SECRETNAME, msgbuf);
    // sprintf( msgbuf, "这是勇者 %s 与使者 %s 专属的信物", heroname,
    // CHAR_getChar( angelindex, CHAR_NAME) );
    ...
    LogItem(CHAR_getChar(angelindex, CHAR_NAME),
            CHAR_getChar(angelindex, CHAR_CDKEY), herotokenindex,
            "制作勇者信物", CHAR_getInt(angelindex, CHAR_FLOOR),
            CHAR_getInt(angelindex, CHAR_X), CHAR_getInt(angelindex, CHAR_Y),
            ITEM_getChar(herotokenindex, ITEM_UNIQUECODE),
            ITEM_getChar(herotokenindex, ITEM_NAME),
            ITEM_getInt(herotokenindex, ITEM_ID), angelindex);
  ...
  // sprintf( nameinfo, "%s:%s", CHAR_getChar( angelindex, CHAR_CDKEY),
  // CHAR_getChar( angelindex, CHAR_NAME ) );
  sprintf(msgbuf, "%s|%d", missiontable[mindex].angelinfo,
          missionlist[missiontable[mindex].mission].limittime);
  saacproto_ACMissionTable_send(acfd, MISSION_DOING, 4, msgbuf, "");

  // CHAR_talkToCli( angelindex, -1,
  // "天之声：非常感谢你答应帮忙，那我就将信物交给你了，请将勇者的信物转交给勇者。",
  // CHAR_COLORYELLOW);
  ...
  for (i = 0; i < maxchar; i++) {
    ...
    if (CHAR_getInt(i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
      continue;
    if (strcmp(herocdkey, CHAR_getChar(i, CHAR_CDKEY)))
      continue;
    if (strcmp(heroname, CHAR_getChar(i, CHAR_NAME)))
      continue;

    if (ADDRESSBOOK_AutoaddAddressBook(angelindex, i) == TRUE) {
      char token[256];
      sprintf(token, "系统自动为你和英雄 %s 交换名片。",
    ...
    if (ADDRESSBOOK_AutoaddAddressBook(i, angelindex) == TRUE) {
      char token[256];
      sprintf(token, "系统自动为你和使者 %s 交换名片。",
              CHAR_getChar(angelindex, CHAR_NAME));
  ...
  sprintf(msgbuf, " 使者答应帮忙了 i:%d 使者:%s 勇者:%s ci=%d ", mindex,
          missiontable[mindex].angelinfo, missiontable[mindex].heroinfo,
          angelindex);
  print(msgbuf);
  LogAngel(msgbuf);
...
void Use_AngelToken(int charaindex, int toindex, int haveitemindex) {
  ...
  if (mindex == -1 ||
      (strcmp(nameinfo, ITEM_getChar(itemindex, ITEM_ANGELINFO)) &&
       strcmp(nameinfo, ITEM_getChar(itemindex, ITEM_HEROINFO)))) {
    // 路人甲使用时
    CHAR_talkToCli(charaindex, -1, "这并不是属於你的信物，不可随便使用喔。",
                   CHAR_COLORRED);
    return;
  }
  ...
  if (strcmp(ITEM_getChar(itemindex, ITEM_ANGELINFO),
    ...
    CHAR_talkToCli(charaindex, -1, "这是无用的信物，还是丢掉吧。",
                   CHAR_COLORRED);
    return;
  }
  ...
  if (!strcmp(nameinfo, missiontable[mindex].angelinfo)) { // 使者使用时
    ...
    lefttime = missiontable[mindex].time +
               missiontable[mindex].limittime * 60 * 60 - (int)time(NULL);
    hour = lefttime / (60 * 60);
    ...
    if (missiontable[mindex].flag == MISSION_DOING) {
      ...
      sprintf(
          msg,
          "勇者 %s 的使命已经完成了，你可以去领奖了，时间还剩余%d小时%d分。",
          tokenbuf, hour, min);
      CHAR_talkToCli(charaindex, -1, msg, CHAR_COLORRED);
    ...
    if (checkUnlawWarpFloor(CHAR_getInt(charaindex, CHAR_FLOOR))) {
      CHAR_talkToCli(charaindex, -1, "你所在的地方无法传送。",
                     CHAR_COLORYELLOW);
      return;
    }
    ...
    for (i = 0; i < maxchar; i++) {
      ...
      if (CHAR_getInt(i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
        continue;
      // sprintf( tempinfo, "%s:%s", CHAR_getChar( i, CHAR_CDKEY), CHAR_getChar(
      // i, CHAR_NAME));
      if (strcmp(cdkey, CHAR_getChar(i, CHAR_CDKEY)))
        continue;
      if (strcmp(name, CHAR_getChar(i, CHAR_NAME)))
        continue;
      ...
      if (checkUnlawWarpFloor(fl)) {
        CHAR_talkToCli(charaindex, -1, "对象所在的地方无法传送。",
                       CHAR_COLORYELLOW);
        return;
      }
...
void Use_HeroToken(int charaindex, int toindex, int haveitemindex) {
  ...
  if (!strcmp(nameinfo, missiontable[mindex].heroinfo)) { // 勇者使用时
    ...
    if (CheckDropatLogout(charaindex) == TRUE) {
      CHAR_talkToCli(charaindex, -1, "携带登出後消失的道具时无法使用。",
                     CHAR_COLORYELLOW);
      return;
    }
    ...
    for (i = 0; i < maxchar; i++) {
      ...
      if (checkUnlawWarpFloor(fl)) {
        CHAR_talkToCli(charaindex, -1, "对象所在的地方无法传送过去。",
                       CHAR_COLORYELLOW);
        return;
      }
