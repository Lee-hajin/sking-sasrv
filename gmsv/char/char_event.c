...
BOOL CHAR_allprewalk(int index, int *dir, int *mode) {
  ...
  if (poisonvalue >= 1) {
    CHAR_setInt(index, CHAR_HP, CHAR_getInt(index, CHAR_HP) - poisonvalue);
...
void CHAR_allpostwalk(int index) { return; }
...
static void CHAR_makeCADAMAGEStringFromSpecifiedCoordinate(int x, int y,
                                                           char *buf,
                                                           int buflen, int ef1,
                                                           int damagevalue) {
  snprintf(buf, buflen, "%d|%d|%d|%d|%d|%d|%d", -1, x, y, CHAR_ACTDAMAGE, 0,
           ef1, damagevalue);
}

BOOL CHAR_makeCAOPT1String(int objindex, char *buf, int buflen, int act,
                           int opt1) {
  char objindexbuf[64];
  if (CHECKOBJECT(objindex) == FALSE)
    return FALSE;
...
BOOL CHAR_makeCAOPTString(int objindex, char *buf, int buflen, int act,
                          char *string) {
  char objindexbuf[64];
...
BOOL CHAR_makeCAOPT3String(int objindex, char *buf, int buflen, int act,
  ...
  snprintf(buf, buflen, "%s|%d|%d|%d|%d|%d|%d|%d",
           cnv10to62(objindex, objindexbuf, sizeof(objindexbuf)),
           OBJECT_getX(objindex), OBJECT_getY(objindex), act,
           CHAR_getInt(OBJECT_getIndex(objindex), CHAR_DIR), opt1, opt2, opt3);
  return TRUE;
}

void CHAR_playerWatchfunc(int objmeindex, int objmoveindex, CHAR_ACTION act,
                          int x, int y, int dir, int *opt, int optlen) {
  int fd;
  int meindex;
  if (!CHECKOBJECTUSE(objmeindex))
    return;
  if (!CHECKOBJECTUSE(objmoveindex))
    return;
  if (OBJECT_getType(objmeindex) != OBJTYPE_CHARA)
    return;

  meindex = OBJECT_getIndex(objmeindex);
  if (CHAR_getInt(meindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
    return;
  ...
  case OBJTYPE_CHARA: {
    ...
    case CHAR_ACTTRADE:
      if (optlen == 1)
        if (CHAR_makeCAOPT1String(objmoveindex, buf, sizeof(buf), act, opt[0]))
          CONNECT_appendCAbuf(fd, buf, strlen(buf));
      break;
    ...
    case CHAR_STREETVENDOR_OPEN:
      if (CHAR_makeCAOPTString(
              objmoveindex, buf, sizeof(buf),
#ifdef _MULTI_STREETVENDOR
              act, CHAR_get_StreetVendor_AName(OBJECT_getIndex(objmoveindex))))
#else
              act,
              CHAR_getWorkChar(OBJECT_getIndex(objmoveindex),
    ...
    case CHAR_ACTBATTLE:
      if (optlen == 3)
        if (CHAR_makeCAOPT3String(objmoveindex, buf, sizeof(buf), act, opt[0],
                                  opt[1], opt[2]))
          CONNECT_appendCAbuf(fd, buf, strlen(buf));
      break;
    default:
...
void CHAR_sendWallDamage(int charaindex, int x, int y, int damage) {
  ...
  CHAR_makeCADAMAGEStringFromSpecifiedCoordinate(x, y, cabuf, sizeof(cabuf),
                                                 SPR_kmydam, damage);
  ...
  for (i = 0; i < arraysizeof(pShow); i++) {
    if (CHAR_getInt(charaindex, pShow[i].paramindex) > 0) {
      ...
      old = CHAR_setInt(charaindex, pShow[i].paramindex,
                        CHAR_getInt(charaindex, pShow[i].paramindex) - 1);
      if (old == 1) {
        if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
          CHAR_talkToCli(charaindex, -1, pShow[i].offmessage, CHAR_COLORWHITE);
        }
        cure = TRUE;
      }
    }
    ...
    for (j = centery - CHAR_CLAIRVOYANCEWIDTH / 2;
      ...
      for (object = MAP_getTopObj(fl, i, j); object;
        ...
        if (OBJECT_getType(objindex) == OBJTYPE_NOUSE)
          continue;
        if (OBJECT_getType(objindex) == OBJTYPE_CHARA &&
            OBJECT_getIndex(objindex) == charaindex)
          continue;
        for (loop = 0; loop < clairvoyancelevel; loop++) {
          ...
          if (ret == TRUE) {
            ...
            snprintf(onebuf, sizeof(onebuf), "%d|%d|%c|", i, j, id);
...
int CHAR_die(int charaindex) {
  CHAR_DischargeParty(charaindex, 0);
  {
    int bymonster = 0;
    int attackindex =
        CHAR_getWorkInt(charaindex, CHAR_WORKLASTATTACKCHARAINDEX);
    if (attackindex == -2)
      bymonster = 0;
              ...
              CHAR_getInt(charaindex, CHAR_DEADCOUNT) + 1);
  {
    ...
    if (CHAR_setInt(charaindex, CHAR_POISON, 0) > 0)
      old = TRUE;
    if (old)
...
void CHAR_playerTalkedfunc(int charaindex, int talkindex, char *message,
  ...
  if ((channel > -1) && (fmindex > 0)) {
    if (channel == 0) {
      ...
      snprintf(
          lastbuf, sizeof(lastbuf), "P|[何练]%s",
#else
      snprintf(
          lastbuf, sizeof(lastbuf), "P|F|[何练]%s",
#endif
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)));
    }
#ifdef _FMVER21
    else if (channel == FAMILY_MAXCHANNEL &&
             CHAR_getInt(talkindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER) {
#else
    else if (channel == FAMILY_MAXCHANNEL &&
             CHAR_getInt(talkindex, CHAR_FMLEADERFLAG) == FMMEMBER_MEMBER) {
#endif
#ifndef _CHANNEL_MODIFY
      snprintf(
          lastbuf, sizeof(lastbuf), "P|[族长广播]%s",
#else
      snprintf(
          lastbuf, sizeof(lastbuf), "P|F|[族长广播]%s",
#endif
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)));
      ...
      snprintf(
          lastbuf, sizeof(lastbuf), "P|[%d]%s", channel,
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)));
    }
    ...
    if (CHAR_getFlg(talkindex, CHAR_ISPARTYCHAT) &&
        (CHAR_getWorkInt(talkindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE))
      snprintf(
          lastbuf, sizeof(lastbuf), "P|T|[颇萍]%s",
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)));
    else
#ifdef _FONT_SIZE
      snprintf(
          lastbuf, sizeof(lastbuf), "P|P|%s|%d",
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)),
          CHAR_getWorkInt(talkindex, CHAR_WORKFONTSIZE));
#else
      snprintf(
          lastbuf, sizeof(lastbuf), "P|P|%s",
          makeEscapeString(CHAR_appendNameAndTitle(talkindex, message, mesgbuf,
                                                   sizeof(mesgbuf)),
                           escapebuf, sizeof(escapebuf)));
#endif
    ...
    snprintf(lastbuf, sizeof(lastbuf), "P|%s",
             makeEscapeString(CHAR_appendNameAndTitle(talkindex, message,
                                                      mesgbuf, sizeof(mesgbuf)),
                              escapebuf, sizeof(escapebuf)));
#endif
  }
  lssproto_TK_send(fd, CHAR_getWorkInt(talkindex, CHAR_WORKOBJINDEX), lastbuf,
                   color);
}
