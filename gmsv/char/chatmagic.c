...
void CHAR_CHAT_DEBUG_setmp(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    sprintf(token, "[GM]%s把你的MP设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(setmp) / 100);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的MP的设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(setmp) / 100);
...
void CHAR_CHAT_DEBUG_str(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    CHAR_complianceParameter(i);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(i, CHAR_P_STRING_STR | CHAR_P_STRING_MAXHP |
                                    CHAR_P_STRING_ATK);
    sprintf(token, "[GM]%s把你的腕力设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(str) / 100);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的腕力设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(str) / 100);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
    ...
    CHAR_send_P_StatusString(
        charindex, CHAR_P_STRING_STR | CHAR_P_STRING_MAXHP | CHAR_P_STRING_ATK);
...
void CHAR_CHAT_DEBUG_tgh(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    CHAR_complianceParameter(i);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(i, CHAR_P_STRING_TOUGH | CHAR_P_STRING_MAXHP |
                                    CHAR_P_STRING_DEF);
    sprintf(token, "[GM]%s把你的耐力设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(tgh) / 100);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的耐力设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(tgh) / 100);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
    ...
    CHAR_complianceParameter(charindex);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(charindex, CHAR_P_STRING_TOUGH |
                                            CHAR_P_STRING_MAXHP |
                                            CHAR_P_STRING_DEF);
...
void CHAR_CHAT_DEBUG_dex(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    CHAR_complianceParameter(i);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(i, CHAR_P_STRING_DEX | CHAR_P_STRING_MAXHP |
                                    CHAR_P_STRING_ATK | CHAR_P_STRING_QUICK);
    sprintf(token, "[GM]%s把你的敏捷设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(dex) / 100);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的敏捷设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(dex) / 100);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
...
void CHAR_CHAT_DEBUG_vital(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    CHAR_complianceParameter(i);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(i, CHAR_P_STRING_VITAL | CHAR_P_STRING_MAXHP |
                                    CHAR_P_STRING_DEF);
    sprintf(token, "[GM]%s把你的体力设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(vital) / 100);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的体力设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(vital) / 100);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
    ...
    CHAR_complianceParameter(charindex);
    // CHAR_sendStatusString( charindex , "P");
    CHAR_send_P_StatusString(charindex, CHAR_P_STRING_VITAL |
                                            CHAR_P_STRING_MAXHP |
                                            CHAR_P_STRING_DEF);
...
void CHAR_CHAT_DEBUG_luck(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    sprintf(token, "[GM]%s把你的运气设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), luck);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_gold(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    sprintf(token, "[GM]%s把你的金钱设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), money);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    ...
    for (i = 0; i < num; i++) {
      ...
      if (itemindex != -1) {
        ...
        LogItem(CHAR_getChar(j, CHAR_NAME), CHAR_getChar(j, CHAR_CDKEY),
                itemindex, "GM制作道具", CHAR_getInt(j, CHAR_FLOOR),
                CHAR_getInt(j, CHAR_X), CHAR_getInt(j, CHAR_Y),
                ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                ITEM_getChar(itemindex, ITEM_NAME),
                ITEM_getInt(itemindex, ITEM_ID), j);
        snprintf(msgbuf, sizeof(msgbuf), "[GM]捞 %s焊陈嚼聪促",
                 CHAR_getChar(charindex, CHAR_NAME),
                 ITEM_getChar(itemindex, ITEM_NAME));
        CHAR_talkToCli(j, -1, msgbuf, CHAR_COLORYELLOW);
        snprintf(msgbuf, sizeof(msgbuf), "%s 罐促 %s",
                 CHAR_getChar(j, CHAR_NAME),
                 ITEM_getChar(itemindex, ITEM_NAME));
        CHAR_talkToCli(charindex, -1, msgbuf, CHAR_COLORYELLOW);
      } else {
        sprintf(msgbuf, "酒捞袍积己坷幅。");
        CHAR_talkToCli(charindex, -1, msgbuf, CHAR_COLORYELLOW);
      }
    }
  } else {
    for (i = 0; i < num; i++) {
      ...
      if (itemindex != -1) {
        ...
        LogItem(CHAR_getChar(charindex, CHAR_NAME),
                CHAR_getChar(charindex, CHAR_CDKEY), itemindex, "GM制作道具",
                CHAR_getInt(charindex, CHAR_FLOOR),
                CHAR_getInt(charindex, CHAR_X), CHAR_getInt(charindex, CHAR_Y),
                ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                ITEM_getChar(itemindex, ITEM_NAME),
                ITEM_getInt(itemindex, ITEM_ID), charindex);
        snprintf(msgbuf, sizeof(msgbuf), "%s 裙垫沁嚼聪促",
                 ITEM_getChar(itemindex, ITEM_NAME));
        CHAR_talkToCli(charindex, -1, msgbuf, CHAR_COLORYELLOW);
      } else {
        sprintf(msgbuf, "酒捞袍积己坷幅。");
...
void CHAR_CHAT_DEBUG_info(int charindex, char *message) {
  ...
  snprintf(line, sizeof(line),
           "name:%s i:%d B:%d BB:%d LV:%d EXP:%d MakeSeq:%d Cdkey:%s",
           CHAR_getChar(index, CHAR_NAME), charindex,
           CHAR_getInt(index, CHAR_BASEIMAGENUMBER),
           CHAR_getInt(index, CHAR_BASEBASEIMAGENUMBER),
           CHAR_getInt(index, CHAR_LV), CHAR_getInt(index, CHAR_EXP),
           CHAR_getCharMakeSequenceNumber(index), message);

  CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);

  snprintf(line, sizeof(line), "Gold:%d HP:%d MP/MHP:%d/%d",
           CHAR_getInt(index, CHAR_GOLD), CHAR_getInt(index, CHAR_HP),
           CHAR_getInt(index, CHAR_MP), CHAR_getInt(index, CHAR_MAXMP));
  CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);
  ...
  snprintf(line, sizeof(line), "土:%d 水:%d 火:%d 风:%d",
  ...
  snprintf(line, sizeof(line), "Sk Itm:%d ChV:%d Rad:%d",
           CHAR_getMaxInventory(index), CHAR_getInt(index, CHAR_CHATVOLUME),
           CHAR_getInt(index, CHAR_RADARSTRENGTH));
  CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);

  snprintf(line, sizeof(line), "Status 毒:%d 麻:%d 默:%d 石:%d 暗:%d 乱:%d",
           CHAR_getInt(index, CHAR_POISON), CHAR_getInt(index, CHAR_PARALYSIS),
           CHAR_getInt(index, CHAR_SLEEP), CHAR_getInt(index, CHAR_STONE),
           CHAR_getInt(index, CHAR_DRUNK), CHAR_getInt(index, CHAR_CONFUSION));
  CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);
  ...
  snprintf(line, sizeof(line),
           ...
           CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
           CHAR_getInt(index, CHAR_Y), CHAR_getInt(index, CHAR_DIR),
...
#else
  snprintf(line, sizeof(line), "TitInd:%d FL:%d X:%d Y:%d DIR:%d",
           CHAR_getInt(index, CHAR_INDEXOFEQTITLE),
           CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
           CHAR_getInt(index, CHAR_Y), CHAR_getInt(index, CHAR_DIR));
#endif
  ...
  snprintf(line, sizeof(line), "walk:%d dead:%d ItemMergecnt:%d",
  ...
  if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
    snprintf(line, sizeof(line), "petindex [%d][%d][%d][%d][%d] ",
             CHAR_getCharPet(index, 0), CHAR_getCharPet(index, 1),
             CHAR_getCharPet(index, 2), CHAR_getCharPet(index, 3),
             CHAR_getCharPet(index, 4));
  } else {
    snprintf(line, sizeof(line), "PetskillIndex [%d][%d][%d][%d][%d][%d][%d] ",
             CHAR_getPetSkill(index, 0), CHAR_getPetSkill(index, 1),
             CHAR_getPetSkill(index, 2), CHAR_getPetSkill(index, 3),
             CHAR_getPetSkill(index, 4), CHAR_getPetSkill(index, 5),
             CHAR_getPetSkill(index, 6));
  }

  CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);
  if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
    snprintf(line, sizeof(line), "poolpetindex [%d][%d][%d][%d][%d] ",
             CHAR_getCharPoolPet(index, 0), CHAR_getCharPoolPet(index, 1),
             CHAR_getCharPoolPet(index, 2), CHAR_getCharPoolPet(index, 3),
             CHAR_getCharPoolPet(index, 4));
    CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);

    snprintf(line, sizeof(line), "poolpetindex [%d][%d][%d][%d][%d] ",
             CHAR_getCharPoolPet(index, 5), CHAR_getCharPoolPet(index, 6),
             CHAR_getCharPoolPet(index, 7), CHAR_getCharPoolPet(index, 8),
             CHAR_getCharPoolPet(index, 9));
    CHAR_talkToCli(charindex, -1, line, CHAR_COLORWHITE);
  }

  snprintf(line, sizeof(line), "levelup [%d], 累积 %d   赌场积分[%d]",
...
void CHAR_CHAT_DEBUG_sysinfo(int charindex, char *message) {
  ...
  snprintf(msg, sizeof(msg),
           "Object Char:%d Item:%d Gold:%d Nouse:%d Item use:%d", chars, items,
           golds, nouses, itemuse);

  CHAR_talkToCli(charindex, -1, msg, CHAR_COLORYELLOW);

  snprintf(msg, sizeof(msg), "player : %d  pet : %d  others  : %d", players,
           pets, others);
  ...
  snprintf(msg, sizeof(msg), "start: %d days %d hours %d min %d sec。", dd, hh,
           mm, ss);
  CHAR_talkToCli(charindex, -1, msg, CHAR_COLORYELLOW);
  ...
  snprintf(msg, sizeof(msg), "NPC_createnum:%d。", NPC_createnum);
  CHAR_talkToCli(charindex, -1, msg, CHAR_COLORYELLOW);
}
...
void CHAR_CHAT_DEBUG_announce(int charindex, char *message) {
  ...
  int playernum = CHAR_getPlayerMaxNum();
  char token[256];
  sprintf(token, "%s辑滚傍瘤 %s", getGameservername(), message);
...
void CHAR_CHAT_DEBUG_delitem(int charindex, char *message) {
  ...
  if (strcmp(message, "all") == 0) {
    int i;
    for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(charindex); i++) {
      itemid = CHAR_getItemIndex(charindex, i);
      if (itemid != -1) {
        {
          LogItem(CHAR_getChar(charindex, CHAR_NAME),
                  CHAR_getChar(charindex, CHAR_CDKEY), itemid, "GM删除道具",
                  CHAR_getInt(charindex, CHAR_FLOOR),
                  CHAR_getInt(charindex, CHAR_X),
                  CHAR_getInt(charindex, CHAR_Y),
                  ITEM_getChar(itemid, ITEM_UNIQUECODE),
                  ITEM_getChar(itemid, ITEM_NAME), ITEM_getInt(itemid, ITEM_ID),
                  charindex);
        }
    ...
    if (equipplace < CHAR_STARTITEMARRAY ||
        equipplace > CHAR_getMaxInventory(charindex) - 1) {
...
void CHAR_CHAT_DelMap(int charaindex, char *message) {
  ...
  for (x = 0; x < xx; x++) {
    for (y = 0; y < yy; y++) {
      if (MAP_getTileAndObjData(ff, x, y, &tile, &obj) == FALSE)
        continue;
...
void CHAR_CHAT_Fukuwa(int charaindex, char *message) {
  ...
  for (obj = MAP_getTopObj(ff, fx, fy); obj; obj = NEXT_OBJECT(obj)) {
    ...
    CHAR_Talk(vs_fd, vs_index, szBuffer, CHAR_COLORWHITE, 3);
    break;
  }
}
#endif
...
#define CHAT_CHAT_DEBUG_SUPERMAN_PARAMDEF 100000
  ...
  if (strlen(cdkey) > 0) {
    ...
    CHAR_setInt(charaindex, CHAR_MP, 999);
    CHAR_setInt(charaindex, CHAR_MAXMP, 999);
...
void CHAR_CHAT_DEBUG_petmake(int charaindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    LogPet(CHAR_getChar(j, CHAR_NAME), CHAR_getChar(j, CHAR_CDKEY),
           CHAR_getChar(ret, CHAR_NAME), CHAR_getInt(ret, CHAR_LV),
           "GM命令制作", CHAR_getInt(j, CHAR_FLOOR), CHAR_getInt(j, CHAR_X),
           CHAR_getInt(j, CHAR_Y), CHAR_getChar(ret, CHAR_UNIQUECODE), ret, j);

    if (vit > 0)
      CHAR_setInt(ret, CHAR_VITAL, vit * 100);
    if (str > 0)
      CHAR_setInt(ret, CHAR_STR, str * 100);
    if (tgh > 0)
      CHAR_setInt(ret, CHAR_TOUGH, tgh * 100);
    if (dex > 0)
    ...
    snprintf(token, sizeof(token), "%s积己肯丰!", ENEMY_getChar(i, ENEMY_NAME));
    ...
    LogPet(CHAR_getChar(charaindex, CHAR_NAME),
           ...
           CHAR_getInt(charaindex, CHAR_Y), CHAR_getChar(ret, CHAR_UNIQUECODE),
           ret, charaindex);
...
void CHAR_CHAT_DEBUG_reset(int charaindex, char *message) {
  int i;
  char mess[MAXMESS_LISTNUM][256] = {
      "enemy", "encount", "magic", "warppoint", "petskill", "pettalk", "all"};
  ...
  if (i >= MAXMESS_LISTNUM) {
    CHAR_talkToCli(charaindex, -1, "舅荐绝绰疙飞绢涝聪促",
  ...
  case 0:
    if (!ENEMYTEMP_reinitEnemy() || !ENEMY_reinitEnemy()) {
      CHAR_talkToCli(charaindex, -1, "檬扁拳肯丰!", CHAR_COLORRED);
      return;
    }
    if (i != ALLRESET)
      break;
  ...
  CHAR_talkToCli(charaindex, -1, "檬扁拳肯丰!", CHAR_COLORYELLOW);
}
...
void CHAR_CHAT_DEBUG_dp(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    sprintf(token, "[GM]%s把你的DP设置为%d!",
            CHAR_getChar(charindex, CHAR_NAME), (int)atoi(dp));
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "玩家%s的DP设置为%d!", CHAR_getChar(i, CHAR_NAME),
            (int)atoi(dp));
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
...
void CHAR_CHAT_DEBUG_manorpk(int charaindex, char *message) {
  ...
  if (strcmp(cmd, "allpeace") == 0) {
    ...
    manorid = atoi(cmd);
#ifdef FAMLYNUM_CF
    if ((manorid < 1) || (manorid > getfamlynum())) {
      sprintf(buf, "参数不正确。 id 必须在 1~%d 之间。", getfamlynum());
#else
    if ((manorid < 1) || (manorid > MANORNUM)) {
      sprintf(buf, "参数不正确。 id 必须在 1~10 之间。");
#endif
      CHAR_talkToCli(charaindex, -1, buf, CHAR_COLORWHITE);
      return;
    }
...
void CHAR_CHAT_DEBUG_fixfmpk(int charaindex, char *message) {
  int ID = atoi(message);
  saacproto_ACSendFmPk_send(acfd, -1, 0, ID, "0|-1||-1||0|0|0|");
}
...
void CHAR_CHAT_DEBUG_petlevelup(int charaindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    for (j = 1; j <= level; j++) { //升级
      CHAR_PetLevelUpNoFame(petindex);
      CHAR_PetAddVariableAi(petindex, AI_FIX_PETLEVELUP);
      CHAR_setInt(petindex, CHAR_LV, CHAR_getInt(petindex, CHAR_LV) + 1);
    }
    CHAR_complianceParameter(petindex);
    ...
    CHAR_setInt(petindex, CHAR_HP, CHAR_getWorkInt(petindex, CHAR_WORKMAXHP));
    sprintf(token, "其飘锅龋 % d阑设%d 饭骇肺 函版窍看嚼聪促!", petid + 1,
            CHAR_getInt(petindex, CHAR_LV));
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    CHAR_complianceParameter(petindex);
    snprintf(token, sizeof(token), "K%d", petid);
...
void CHAR_CHAT_DEBUG_gmkick(int charindex, char *message) {
  char cdkey[32];
  char buf1[32];
  char gmset[][32] = {"LSLOCK", "KICK", "DEUNLOCK", "UNLOCKALL",
                      "LOCK",   "TYPE", "UNLOCK"};
  int act = 1;
...
void CHAR_CHAT_DEBUG_sendeffect(int charaindex, char *message) {
  ...
  for (i = 0; i <= CHAR_effectnum; i++) {
    if (CHAR_effect[i].floor) {
      ...
      snprintf(CHAR_effect[i].month, sizeof(CHAR_effect[i].month), "%d",
               tm1.tm_mon + 1);
      snprintf(CHAR_effect[i].day, sizeof(CHAR_effect[i].day), "%d",
               tm1.tm_mday);
      snprintf(CHAR_effect[i].hour, sizeof(CHAR_effect[i].hour), "%d",
               tm1.tm_hour);
      snprintf(CHAR_effect[i].min, sizeof(CHAR_effect[i].min), "%d",
               tm1.tm_min);
  ...
  if (strcmp(type, "1") != 0) {
    ...
    for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
      ...
      if (skillid > 0) {
        ...
        memset(token, 0, sizeof(token));
        sprintf(token, "P%d(%s) ", i + 1,
                PROFESSION_SKILL_getChar(array, PROFESSION_SKILL_NAME));
        if (strcmp(token, "") != 0)
          strcat(msg, token);
      }
    }
  ...
  for (i = 0; i < MAX_USER; i++) {
    ...
    if (i_use) {
      ...
      if (strcmp(token, cdkey) == 0) {
        ...
        // print("\n<Set TRUE");
#ifdef _GM_KING
        snprintf(line, sizeof(line), "管理员[%s]将 %s 禁言%d分钟。",
                 CHAR_getChar(index, CHAR_NAME), CHAR_getChar(index, CHAR_NAME),
                 minu);
#else
        snprintf(line, sizeof(line), "管理员将 %s 禁言%d分钟。",
...
void CHAR_CHAT_DEBUG_help(int charindex, char *message) {
  ...
  for (i = 0; i < MaxMagicNum; i++) {
    ...
    if (type == 1) {
      ...
      if (strstr(buf1, buf) != NULL || strstr(buf2, buf) != NULL) {
...
void CHAR_CHAT_DEBUG_set_manor_owner(int charindex, char *message) {
...
#ifdef FAMLYNUM_CF
  if (atoi(szId) < 1 || atoi(szId) > getfamlynum()) {
    sprintf(szToken, "庄园ID值不正确(1~%d)", getfamlynum());
#else
  if (atoi(szId) < 1 || atoi(szId) > MANORNUM) {
    sprintf(szToken, "庄园ID值不正确(1~10)");
#endif
...
#ifdef FAMLYNUM_CF
    sprintf(szToken,
            "参数不正确1 [set_manor_owner 庄园id(1~%d) 使用旗标(0:名称 1:id) "
            "家族名称或id]",
            getfamlynum());
#else
    sprintf(szToken, "参数不正确1 [set_manor_owner 庄园id(1~10) "
                     "使用旗标(0:名称 1:id) 家族名称或id]");
#endif
    CHAR_talkToCli(charindex, -1, szToken, CHAR_COLORRED);
    return;
  }
  ...
  if (iUseFlag < 0 || iUseFlag > 1) {
    CHAR_talkToCli(charindex, -1, "使用旗标值不正确(0:名称 1:id)",
                   CHAR_COLORRED);
    return;
  }
...
#ifdef FAMLYNUM_CF
    sprintf(szToken,
            "参数不正确2 [set_manor_owner 庄园id(1~%d) 使用旗标(0:名称 1:id) "
            "家族名称或id]",
            getfamlynum());
#else
    sprintf(szToken, "参数不正确2 [set_manor_owner 庄园id(1~10) "
                     "使用旗标(0:名称 1:id) 家族名称或id]");
#endif
    CHAR_talkToCli(charindex, -1, szToken, CHAR_COLORRED);
    return;
...
void CHAR_CHAT_DEBUG_set_schedule_time(int charindex, char *message) {
  ...
  if (message == NULL || getStringFromIndexWithDelim(message, " ", 1, szId,
                                                     sizeof(szId)) == FALSE) {
#ifdef FAMLYNUM_CF
    sprintf(szMsg, "参数不正确1 [set_schedule_time 庄园id(1~%d) 时间(分)]",
            getfamlynum());
#else
    sprintf(szMsg, "参数不正确1 [set_schedule_time 庄园id(1~10) 时间(分)]");
#endif
...
#ifdef FAMLYNUM_CF
  if (atoi(szId) < 1 || atoi(szId) > getfamlynum()) {
    sprintf(szMsg, "庄园ID值不正确 庄园id(1~%d)", getfamlynum());
#else
  if (atoi(szId) < 1 || atoi(szId) > MANORNUM) {
    sprintf(szMsg, "庄园ID值不正确 庄园id(1~10)");
#endif
    CHAR_talkToCli(charindex, -1, szMsg, CHAR_COLORRED);
    return;
  }
  if (getStringFromIndexWithDelim(message, " ", 2, szTime, sizeof(szTime)) ==
      FALSE) {
#ifdef FAMLYNUM_CF
    sprintf(szMsg, "参数不正确2 [set_schedule_time 庄园id(1~%d) 时间(分)]",
            getfamlynum());
#else
    sprintf(szMsg, "参数不正确2 [set_schedule_time 庄园id(1~10) 时间(分)]");
#endif
    CHAR_talkToCli(charindex, -1, szMsg, CHAR_COLORRED);
    return;
  }
  for (i = 0; i < char_max; i++) {
    if (CHAR_getCharUse(i) &&
        (CHAR_getInt(i, CHAR_WHICHTYPE) == CHAR_TYPEMANORSCHEDULEMAN)) {
...
void CHAR_CHAT_DEBUG_angelinfo(int charindex, char *message) // 显示Mission资料
{
  ...
  if (message[0] == '\0' ||
      getStringFromIndexWithDelim(message, " ", 1, cdkey, sizeof(cdkey)) ==
          FALSE) {
    CHAR_talkToCli(charindex, -1, "参数不正确 [angelinfo 帐号(all) (clean)]",
                   CHAR_COLORRED);
    return;
  }
  if (getStringFromIndexWithDelim(message, " ", 2, buf, sizeof(buf)) != FALSE) {
  ...
  if (!strcmp(cdkey, "all")) { // 显示全部资料
    for (i = 0; i < MAXMISSIONTABLE; i++) {
      ...
      sprintf(msg, "%d 使者:%s 勇者:%s MS:%d FLG:%d TIME:%d LT:%d ", i,
              missiontable[i].angelinfo, missiontable[i].heroinfo,
...
void CHAR_CHAT_DEBUG_angelclean(int charindex, char *message) // 清除Mission资料
{
  ...
  sprintf(
      msg,
      " GM指令 清除任务资料 i:%d 使者:%s 勇者:%s m:%d flg:%d time:%d lt:%d ",
...
void CHAR_CHAT_DEBUG_angelcreate(int charindex,
                                 char *message) // 制造使者与勇者配对
{
...
void CHAR_CHAT_DEBUG_missionreload(int charindex, char *message) {
  ...
  print("done\n");
  CHAR_talkToCli(charindex, -1, "酒捞袍 府悸 肯丰", CHAR_COLORYELLOW);
}
...
void CHAR_CHAT_DEBUG_itemreload(int charindex, char *message) {
  ...
  CHAR_talkToCli(charindex, -1, "酒捞袍 府悸吝...", CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_rejobdaily(int charaindex, char *message) {
  ...
  print("done\n");
  CHAR_talkToCli(charaindex, -1, "酒捞袍 府悸 肯丰", CHAR_COLORYELLOW);
}
#endif
...
void CHAR_CHAT_DEBUG_SETRIDE(int charindex, char *message) {
  ...
  tagNewRideCode NewRides[12] = {
      {"帖拉所伊朵", RIDE_PET0}, {"玛恩摩洛斯", RIDE_PET1},
      {"朵拉比斯", RIDE_PET2},   {"拉奇鲁哥", RIDE_PET3},
      {"扬奇洛斯", RIDE_PET4},   {"卡达鲁卡斯", RIDE_PET5},
      {"卡卡金宝", RIDE_PET6},   {"格尔格", RIDE_PET7},
      {"左迪洛斯", RIDE_PET8},   {"巴朵兰恩", RIDE_PET9},
      {"史卡鲁", RIDE_PET10},    {"罗多克雷", RIDE_PET11}};
  if (strlen(cdkey) > 0) {
    for (i = 0; i < playernum; i++) {
      if (CHAR_CHECKINDEX(i) == FALSE)
        continue;
    ...
    sprintf(token, "[GM]%s给你骑乘骑宠%s的资格!",
            CHAR_getChar(charindex, CHAR_NAME), NewRides[rideindex - 1].name);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "给玩家%s骑乘骑宠%s的资格!", CHAR_getChar(i, CHAR_NAME),
...
void CHAR_CHAT_DEBUG_MVRIDE(int charindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
    ...
    sprintf(token, "[GM]%s移除你骑乘骑宠%s的资格!",
            CHAR_getChar(charindex, CHAR_NAME), NewRides[rideindex - 1].name);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "移除玩家%s骑乘骑宠%s的资格!!", CHAR_getChar(i, CHAR_NAME),
            NewRides[rideindex - 1].name);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
    int LRCode = CHAR_getInt(charindex, CHAR_LOWRIDEPETS);
...
void CHAR_CHAT_DEBUG_petmakecf(int charaindex, char *message) {
  ...
  if (strlen(cdkey) > 0) {
...

    {
      ...
      for (i = 0; i < 160; i++) {
        fRand =
            (float)RAND(RankRandTbl[petrank].min, RankRandTbl[petrank].max) *
            0.01;
      ...
      rnd = ((cstr - str) / 80.0 / 160.0 + (ctough - tough) / 80.0 / 160.0 +
             (cdex - dex) / 100.0 / 160.0);
...

    {
      ...
      rnd = ((cstr - (float)str) / 80.0 / 160.0 +
             (ctough - (float)tough) / 80.0 / 160.0 +
             (cdex - (float)dex) / 100.0 / 160.0);
...
void CHAR_CHAT_DEBUG_petmakeabi(int charaindex, char *message) {
  ...
  if (CHAR_CHECKINDEX(ret) == TRUE) {
    CHAR_setInt(ret, CHAR_VITAL, vital * 100);
    CHAR_setInt(ret, CHAR_STR, str * 100);
    CHAR_setInt(ret, CHAR_TOUGH, tough * 100);
    CHAR_setInt(ret, CHAR_DEX, dex * 100);
    //设置等级
...
void CHAR_CHAT_DEBUG_DelJiaRen(int charaindex, char *message) {
  int floor, x, y, i, j, k, l, area;
  char areabuf[5];
  char buf[256];
  OBJECT object;
  char line[256];
  ...
  int getnum = 0;
  int jiarenindex[5];
  int jiarenpetindex = -1;

  getStringFromIndexWithDelim(message, " ", 1, areabuf, sizeof(areabuf));
  if (strstr(areabuf, "all") != NULL) {
    int charmaxnum = CHAR_getCharNum();
    int startindex = CHAR_getPlayerMaxNum() + CHAR_getPetMaxNum();
    for (i = startindex; i <= charmaxnum; i++) {
      if (CHAR_CHECKINDEX(i) == FALSE)
        continue;
      if (CHAR_getInt(i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC)
        continue;
      CHAR_CharaDeleteHavePet(i);
      CHAR_CharaDelete(i);
    ...
    for (j = x - area; j <= x + area; j++) {
      for (object = MAP_getTopObj(floor, j, i); object;
        ...
        if (OBJECT_getType(o) == OBJTYPE_CHARA) {
          ...
          if (CHAR_CHECKINDEX(chara_index) == FALSE)
            continue;
          if (CHAR_getInt(chara_index, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC ||
              CHAR_getWorkInt(chara_index, CHAR_WORKPARTYMODE) !=
                  CHAR_PARTY_LEADER)
            continue;
          // if( chara_index == charaindex ) continue;  //找到自己了
          getnum++;
          // snprintf( line, sizeof( line),
          // "第%d个找到[%s]的帐号是[%s]",getnum,CHAR_getChar(chara_index,
          // CHAR_NAME),CHAR_getChar(chara_index, CHAR_CDKEY));
          // CHAR_talkToCli( charaindex, -1,line, CHAR_COLORRED);
          for (k = 0; k < 5; k++) {
            jiarenindex[k] =
                CHAR_getWorkInt(chara_index, CHAR_WORKPARTYINDEX1 + k);
            if (CHAR_CHECKINDEX(jiarenindex[k]) == FALSE ||
                CHAR_getInt(jiarenindex[k], CHAR_WHICHTYPE) !=
                    CHAR_TYPEPLAYERNPC) {
              continue;
            }
          }
          for (l = 0; l < 5; l++) {
            CHAR_CharaDeleteHavePet(jiarenindex[l]);
            CHAR_CharaDelete(jiarenindex[l]);
            luaplayernum = luaplayernum - 1;
          }
        }
      }
...
void CHAR_CHAT_DEBUG_ServerRunTime(int charaindex, char *message) {
  char timebuf[32];
  serverruntimenum = 0;
  getStringFromIndexWithDelim(message, " ", 1, timebuf, sizeof(timebuf));
  if (atoi(timebuf) > 0)
    serverruntimenum = atoi(timebuf);
}

#ifdef _NB_GM_SERVERLIST
void CHAR_CHAT_DEBUG_SERVERGM(int charindex, char *message) {
  int itype;
  char token[128];
  char buf[256];
  //最少2个参数
  easyGetTokenFromString(message, 1, token, sizeof(token));
  if (strstr(message, "shutdown") != NULL || strstr(message, "关服") != NULL) {
    itype = 1;
  } else if (strstr(message, "reset") != NULL ||
             strstr(message, "重读设置") != NULL) {
    itype = 2;
  } else if (strstr(message, "itemreload") != NULL ||
             strstr(message, "重读道具") != NULL) {
    itype = 3;
  } else if (strstr(message, "reloadcf") != NULL ||
             strstr(message, "重读CF") != NULL) {
    itype = 4;
  } else if (strstr(message, "reloadlua") != NULL ||
             strstr(message, "重读LUA") != NULL) {
    itype = 5;
  } else if (strstr(message, "gmreload") != NULL ||
             strstr(message, "重读GM设置") != NULL) {
    itype = 6;
  }

  if ((itype <= 0) || (itype > 6)) {
    sprintf(buf, "%s", "参数不正确！[请输入跨服指令编号，编号范围1-6]");
    CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
    sprintf(
        buf, "%s",
        "1、关服  2、重读设置 3、重读道具 4、重读CF 5、重读LUA 6、重读GM设置");
    CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
    return;
  }
  ...
  if ((itype < 2) || (itype > 5)) {
    if (strlen(token) == 0) {
      sprintf(buf, "%s", "参数不正确！[缺乏命令参数]");
      CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
      sprintf(buf, "%s",
              "关服命令需输入时间，2-4指令编号输入0 重读LUA输入路径");
      CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
      return;
    }
  }
  saacproto_gm_shutdown_send(itype, token); //整理待发送给SAAC

  if (itype == 1) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[关服]指令成功！~",
                   CHAR_COLORRED);
  } else if (itype == 2) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[重读设置]指令成功！~",
                   CHAR_COLORRED);
  } else if (itype == 3) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[重读道具]指令成功！~",
                   CHAR_COLORRED);
  } else if (itype == 4) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[重读CF]指令成功！~",
                   CHAR_COLORRED);
  } else if (itype == 5) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[重读LUA]指令成功！~",
                   CHAR_COLORRED);
  } else if (itype == 6) {
    CHAR_talkToCli(charindex, -1, "[跨服指令]发送[重读GM设置]指令成功！~",
                   CHAR_COLORRED);
  }
}
#endif
#ifdef _NB_GMCHECKEVENT
void CHAR_CHAT_DEBUG_checkevent(int charaindex, char *message) {
  char cdkey[CDKEYLEN];
  getStringFromIndexWithDelim(message, " ", 1, cdkey, sizeof(cdkey));
  int toindex = -1, i;
  if (strlen(cdkey) > 0) {
  ...
  CHAR_setWorkInt(toindex, CHAR_WORK_GMEVENT, 1);
  CHAR_talkToCli(charaindex, -1, "设置任务条件查看成功", CHAR_COLORRED);
}
#endif

void CHAR_CHAT_DEBUG_setpoint(int charaindex, char *message) {
  char token[128];
  char itype[256];
  int getpoint, fd;
  char cdkey[CDKEYLEN];

  getStringFromIndexWithDelim(message, " ", 1, itype, sizeof(itype));
  getStringFromIndexWithDelim(message, " ", 2, token, sizeof(token));
  getpoint = atoi(token);
  getStringFromIndexWithDelim(message, " ", 3, cdkey, sizeof(cdkey));
  int toindex = -1, i;
  if (strlen(cdkey) > 0) {
    int playernum = CHAR_getPlayerMaxNum();
    for (i = 0; i < playernum; i++) {
  ...
  int mypoint, maxpoint;

  if (strstr(itype, "胶沛") != NULL || atoi(itype) == 1) {
    mypoint = CHAR_getInt(toindex, CHAR_GOLD);
    maxpoint = CHAR_getMaxHaveGold(toindex);
    if (mypoint + getpoint > maxpoint) {
      getpoint = maxpoint - mypoint;
    }
    CHAR_setInt(toindex, CHAR_GOLD, mypoint + getpoint);
    CHAR_complianceParameter(toindex);
    CHAR_send_P_StatusString(toindex, CHAR_P_STRING_GOLD);
    if (toindex != charaindex) {
      sprintf(token, "[GM]%s丛膊辑 [%d]胶沛阑 焊郴林菌嚼聪促!",
              CHAR_getChar(charaindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
      sprintf(token, "[%s]丛膊 [%d]胶沛阑 傈价窍看嚼聪促!",
              CHAR_getChar(toindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    } else {
      sprintf(token, "[%d]胶沛阑 裙垫窍看嚼聪促!", getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    }

  } else if (strstr(itype, "声望") != NULL || atoi(itype) == 2) {
    mypoint = CHAR_getInt(toindex, CHAR_FAME);
    maxpoint = MAX_PERSONALFAME;
    if (mypoint + (getpoint * 100) > maxpoint) {
      getpoint = (maxpoint - mypoint) / 100;
    }
    CHAR_setInt(toindex, CHAR_FAME, mypoint + (getpoint * 100));

    if (toindex != charaindex) {
      sprintf(token, "[GM] %s 给你的声望增加%d!",
              CHAR_getChar(charaindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
      sprintf(token, "玩家 %s 的声望增加%d!", CHAR_getChar(toindex, CHAR_NAME),
              getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    } else {
      sprintf(token, "声望增加%d!", getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    }
    char buf[128];
    fd = getfdFromCharaIndex(toindex);
    sprintf(buf, "%d", getpoint);
    saacproto_ACFixFMData_send(
        acfd, CHAR_getChar(toindex, CHAR_FMNAME),
        CHAR_getInt(toindex, CHAR_FMINDEX),
        CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI), FM_FIX_FMFEED, buf, "",
        CHAR_getWorkInt(toindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));

  } else if (strstr(itype, "活力") != NULL || atoi(itype) == 3) {
    mypoint = CHAR_getInt(toindex, CHAR_ACTIVE);
    maxpoint = MAX_PERSONALFAME;
    if (mypoint + getpoint > maxpoint) {
      getpoint = maxpoint - mypoint;
    }
    CHAR_setInt(toindex, CHAR_ACTIVE, mypoint + getpoint);
    if (toindex != charaindex) {
      sprintf(token, "[GM] %s 给你的活力增加%d!",
              CHAR_getChar(charaindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
      sprintf(token, "玩家 %s 的活力增加%d!", CHAR_getChar(toindex, CHAR_NAME),
              getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    } else {
      sprintf(token, "活力增加%d!", getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    }
  } else if (strstr(itype, "金币") != NULL || atoi(itype) == 4) {
    mypoint = sasql_getVipPoint(toindex);
    maxpoint = MAX_PERSONALFAME;
    if (mypoint + getpoint > maxpoint) {
      getpoint = maxpoint - mypoint;
    }
    sasql_setVipPoint(toindex, mypoint + getpoint);
    if (toindex != charaindex) {
      sprintf(token, "[GM]%s丛膊辑 [%d]瘤盔器牢飘甫 焊郴林菌嚼聪促!",
              CHAR_getChar(charaindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
      sprintf(token, "[%s]丛膊 [%d]瘤盔器牢飘甫 傈价窍看嚼聪促!",
              CHAR_getChar(toindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    } else {
      sprintf(token, "[%d]瘤盔器牢飘甫 裙垫窍看嚼聪促!", getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    }
  } else if (strstr(itype, "积分") != NULL || atoi(itype) == 5) {
    mypoint = CHAR_getInt(toindex, CHAR_AMPOINT);
    maxpoint = MAX_PERSONALFAME;
    if (mypoint + getpoint > maxpoint) {
      getpoint = maxpoint - mypoint;
    }
    CHAR_setInt(toindex, CHAR_AMPOINT, mypoint + getpoint);
    if (toindex != charaindex) {
      sprintf(token, "[GM] %s 给你的积分增加%d!",
              CHAR_getChar(charaindex, CHAR_NAME), getpoint);
      CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
      sprintf(token, "玩家 %s 的积分增加%d!", CHAR_getChar(toindex, CHAR_NAME),
              getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    } else {
      sprintf(token, "积分增加%d!", getpoint);
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    }
  } else {
    CHAR_talkToCli(charaindex, -1, "找不到类型！", CHAR_COLORYELLOW);
    return;
  }
}

#ifdef _NB_CreateTJMap
void CHAR_CHAT_DEBUG_CreateTJMap(int charaindex, char *message) {
  char token[128];
  getStringFromIndexWithDelim(message, " ", 1, token, sizeof(token));
  if (strstr(token, "mk") != NULL) { //单地图创建
    getStringFromIndexWithDelim(message, " ", 2, token, sizeof(token));
    int floorid = atoi(token);
    sprintf(token, "写出地图编号%d的地图格式文件...", floorid);
    MAP_walkAbleForJL(charaindex, floorid);
    CHAR_talkToCli(charaindex, -1, "写出完成！", CHAR_COLORYELLOW);
  } else if (strstr(token, "all") != NULL) { //全部地图写出
    CHAR_talkToCli(charaindex, -1, "地图格式写出中...", CHAR_COLORYELLOW);
    CHAR_CHAT_DEBUG_MAP_walkAbleForJL(charaindex);
    CHAR_talkToCli(charaindex, -1, "写出完成！", CHAR_COLORYELLOW);
  }
}
#endif
...
void CHAR_CHAT_DEBUG_alcheplus(int charaindex, char *message) {
  char token[64];
  char sendmsg[1024];
  int i;
  int alcheType, alchePlus;

  strcpy(sendmsg, "");
  if (strlen(message) == 0) {
    for (i = 0; i < 25; i++) {
      sprintf(token, " %d:%s=%d ", i, alchetest_list[i],
              CHAR_getInt(charaindex, CHAR_ITEMCHECK01 + i));
      strcat(sendmsg, token);
    }
    CHAR_talkToCli(charaindex, -1, sendmsg, CHAR_COLORYELLOW);
    return;
  }

  if (getStringFromIndexWithDelim(message, " ", 1, token, sizeof(token)) ==
      FALSE) {
    CHAR_talkToCli(charaindex, -1, "参数错误", CHAR_COLORYELLOW);
    return;
  }
  alcheType = atoi(token);
  if (alcheType < 0 || alcheType >= 25) {
    CHAR_talkToCli(charaindex, -1, "代号错误", CHAR_COLORYELLOW);
    return;
  }
  ...
  alchePlus = atoi(token);
  if (alchePlus < 0)
    alchePlus = 0;
  if (alchePlus > 100 * 1000)
    alchePlus = 100 * 1000;

  CHAR_setInt(charaindex, CHAR_ITEMCHECK01 + alcheType, alchePlus);
  sprintf(token, "修改熟练度 %s=%d ", alchetest_list[alcheType],
          CHAR_getInt(charaindex, CHAR_ITEMCHECK01 + alcheType));
  CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
  lssproto_ALCHEPLUS_recv(CHAR_getWorkInt(charaindex, CHAR_WORKFD));
}
#endif

#ifdef _CHAT_MERGE
void CHAR_CHAT_DEBUG_setmergeAdditem(int charaindex, char *message) {
  int emptyitemindexinchara, itemindex;
  int i, ret;
  int num = 0;
  ...
  if (emptyitemindexinchara < 0) {
    sprintf(msgbuf, "酒捞袍 芒捞 何练钦聪促.");

    CHAR_talkToCli(charaindex, -1, msgbuf, CHAR_COLORRED);
  ...
  if (itemindex != -1) {
    ...
    ITEM_setInt(itemindex, ITEM_MERGEFLG, 1);
    ITEM_setInt(itemindex, ITEM_MODIFYATTACK, Str);
    ITEM_setInt(itemindex, ITEM_MODIFYDEFENCE, Tgh);
    ITEM_setInt(itemindex, ITEM_MODIFYQUICK, Dex);
    ITEM_setInt(itemindex, ITEM_MODIFYCHARM, Charm);
    ITEM_setInt(itemindex, ITEM_MODIFYAVOID, Avoid);
    ITEM_setInt(itemindex, ITEM_CRITICAL, Critical);
    ...
    snprintf(msgbuf, sizeof(msgbuf), "[%s] 钦己酒捞袍阑 积己 沁嚼聪促.",
             ITEM_getChar(itemindex, ITEM_NAME));
    CHAR_talkToCli(charaindex, -1, msgbuf, CHAR_COLORYELLOW);
  } else {
    sprintf(msgbuf, "钦己酒捞袍 积己俊 角菩沁嚼聪促.");
    CHAR_talkToCli(charaindex, -1, msgbuf, CHAR_COLORRED);
  }
}
#endif
#ifdef _HIGHRANK_PETMAKE
void CHAR_CHAT_DEBUG_spet(int charaindex, char *message) {
  int ret;
  int enemynum;
  int enemyid;
  int i, j;
  ...
  if (strlen(cdkey) > 0) {
    ...
    if (j >= playernum) {
      CHAR_talkToCli(charaindex, -1, "敲饭捞绢啊 坷橇扼牢 惑怕 涝聪促.",
                     CHAR_COLORYELLOW);
      return;
    }
    enemynum = ENEMY_getEnemyNum();
    ...
    ret = ENEMY_createPets(j, i);
    if (!CHAR_CHECKINDEX(ret))
      return;

    sprintf(token, "%s丛膊辑 其飘[%s]甫 瘤鞭 窍看嚼聪促.",
            CHAR_getChar(charaindex, CHAR_NAME), ENEMY_getChar(i, ENEMY_NAME));
    CHAR_talkToCli(j, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "%s丛俊霸 其飘[%s]甫 瘤鞭 窍看嚼聪促.",
            CHAR_getChar(j, CHAR_NAME), ENEMY_getChar(i, ENEMY_NAME));
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    for (i = 0; i < CHAR_MAXPETHAVE; i++) {
      if (CHAR_getCharPet(j, i) == ret)
        break;
    ...
    ret = ENEMY_createPets(charaindex, i);
    if (!CHAR_CHECKINDEX(ret))
      return;

    snprintf(token, sizeof(token), "其飘[%s]甫 己傍利栏肺 积己窍看嚼聪促.",
             ENEMY_getChar(i, ENEMY_NAME));
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_VipPoint(int charindex, char *message) {
  int i, MaxGold, point;
  char cdkey[CDKEYLEN];
  char buf[64];
  char token[128];
  ...
  if (strlen(cdkey) > 0) {
    ...
    if (i >= playernum) {
      CHAR_talkToCli(charindex, -1, "秦寸 牢盔篮 泅犁 立加吝捞 酒凑聪促.",
                     CHAR_COLORWHITE);
      return;
    }
    CHAR_setInt(i, CHAR_AMPOINT, CHAR_getInt(i, CHAR_AMPOINT) + point);
    CHAR_complianceParameter(i);
    sprintf(token, "%s(捞)啊 %dVIP POINT甫 利赋 秦林菌嚼聪促.",
            CHAR_getChar(charindex, CHAR_NAME), point);
    CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
    sprintf(token, "%s(俊)霸 %dVIP POINT甫 利赋 沁嚼聪促.",
            CHAR_getChar(i, CHAR_NAME), point);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  } else {
    CHAR_setInt(charindex, CHAR_AMPOINT,
                CHAR_getInt(charindex, CHAR_AMPOINT) + point);
    CHAR_complianceParameter(charindex);
    sprintf(token, "%dVIP POINT啊 利赋 登菌嚼聪促.", point);
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  }
}
#endif