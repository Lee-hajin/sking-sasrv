...
#define TRADE_WAIT "[%s]丛膊 背券脚没 窍看嚼聪促."
#define TRADE_NONE "芭贰 惑措啊 粮犁窍瘤 臼嚼聪促！"
#define TRADE_OVERPLAYER "茄 疙狼 敲饭捞绢俊霸父 啊瓷 钦聪促."
#define TRADE_NOWILL "惑措规狼 背券捞 凯妨乐瘤 臼疽嚼聪促."
#define TRADE_TRADING "[%s]丛膊辑绰 促弗荤恩苞 背券 吝 涝聪促."
#define TRADE_VANISHITEM "官蹿俊 滚副 荐 绝绰 拱前篮 背券捞 阂啊瓷 钦聪促."
#define TRADE_PETFULL "[%s]丛狼 其飘芒捞 啊垫瞒辑 芭贰啊 秒家 邓聪促."
#define TRADE_ITEMFULL "[%s]丛狼 酒捞袍沫捞 啊垫瞒辑 芭贰啊 秒家 邓聪促."
#define TRADE_CANCEL "[%s]丛捞 背券阑 秒家窍看嚼聪促."
#define TRADE_POOR "[%s]丛 胶沛捞 何练窍咯 芭贰啊 阂啊瓷 钦聪促."
#define TRADE_RICH "[%s]丛 焊蜡胶沛捞 檬苞登绰 芭贰甫 窍看嚼聪促."
#define TRADE_TOORICH "[%s]丛狼 罐阑 荐 乐绰 陛咀捞 檬苞 窍看嚼聪促."
#define TRADE_POORLV "[%s]丛狼 饭骇捞 何练窍咯 杭 荐 绝嚼聪促."
#define TRADE_LOCK "泪陛惑怕"
#define TRADE_SUCCESS "背券捞 肯丰 登菌绢夸!"
#define TRADE_FAILED "背券捞 秒家 登菌绢夸!"
#define TRADE_FMPET "啊练 荐龋咙铰篮 芭贰甫 窍角 荐 绝嚼聪促."
#ifdef _LOCK_PET_ITEM
#define TRADE_LOCKPET "泪变 其飘绰 芭贰啊 阂啊瓷钦聪促！"
#define TRADE_LOCKITEM "泪变 酒捞袍篮 芭贰啊 阂啊瓷钦聪促！"
#define TRADE_LOCKPETITEM "其飘客 酒捞袍捞 葛滴 泪败乐绢 芭贰啊 阂啊瓷钦聪促！"
#endif
#define TRADE_NOITEM "焊蜡酒捞袍捞 粮犁窍瘤 臼嚼聪促！"
#define TRADE_NOPET "焊蜡其飘啊 粮犁窍瘤 臼嚼聪促！"
#define TRADE_RDCANCEL "背券阑 秒家沁绢夸！"
...
BOOL TRADE_setTradeListIndex(int ti, int meindex, int toindex) {
  ...
  if (ti < 0 || ti >= MAX_TRADELISTNUM)
    return FALSE;
  if (TradeList[ti][0].use == 1 || TradeList[ti][1].use == 1)
    return FALSE;
...
void TRADE_CheckTradeListUser(void) {
  ...
  for (i = 0; i < MAX_TRADELISTNUM; i++) {
    ...
    if (CHAR_CHECKINDEX(TradeList[i][0].charaindex) &&
        CHAR_getWorkInt(TradeList[i][0].charaindex, CHAR_WORKTRADEMODE) !=
            CHAR_TRADE_FREE &&
        CHAR_CHECKINDEX(TradeList[i][1].charaindex) &&
        CHAR_getWorkInt(TradeList[i][1].charaindex, CHAR_WORKTRADEMODE) !=
            CHAR_TRADE_FREE) {
      usenum++;
      continue;
    } else {
...
int TRADE_getMyTarget(int meindex) {
  ...
  if (ti < 0 || ti >= MAX_TRADELISTNUM)
    return -1;
  if (TradeList[ti][0].use == 0 || TradeList[ti][1].use == 0)
    return -1;

  for (i = 0; i < 2; i++) {
    if (TradeList[ti][i].charaindex == meindex && TradeList[ti][i].fd == fd) {
      side = (i + 1) % 2;
      if (!CHAR_CHECKINDEX(TradeList[ti][side].charaindex))
        return -1;
      if (TradeList[ti][side].charaindex == meindex)
        return -1;
      if (TradeList[ti][side].fd !=
          getfdFromCharaIndex(TradeList[ti][side].charaindex))
        return -1;
...
BOOL TRADE_addSomeTradeList(int meindex, int usTi, int *num, int type) {
  ...
  case TRADE_ITEM: {
    ...
    if (usTi < CHAR_STARTITEMARRAY || usTi >= CHAR_getMaxInventory(meindex))
      return FALSE;
    ...
    if (ITEM_getInt(itemindex, ITEM_VANISHATDROP) == 1) {
      char buf[256];
      sprintf(buf, "[%s](篮)绰 芭贰且 荐 绝绰 酒捞袍涝聪促.",
    ...
    if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
      char buf[256];
      sprintf(buf, "【温馨提示】%s 此道具为绑定道具，无法交易，请妥 票９堋?",
    ...
    if (ITEM_getInt(itemindex, ITEM_CANBEPILE) == 1) { //可堆叠
      ...
      // ITEM_USEPILENUMS
      for (i = 0; i < 15; i++) {
        if (TradeList[ti][side].ItemTi[i] != usTi)
          continue;
        if (TradeList[ti][side].ItemNum[i] >= MeMaxPile ||
            TradeList[ti][side].ItemNum[i] >= itemMaxPile)
          return FALSE;

        TradeList[ti][side].ItemNum[i] += 1;
        if (num != NULL)
          *num = TradeList[ti][side].ItemNum[i];
        return TRUE;
      }
    }
    for (i = 0; i < 15; i++) {
      if (TradeList[ti][side].ItemTi[i] != -1)
        continue;
  ...
  case TRADE_PET: {
    int petindex;
    if (usTi < 0 || usTi >= CHAR_MAXPETHAVE)
      return FALSE;
...
#ifdef _LOCK_PET_ITEM
    {
...
#ifdef _PET_ITEM
      {
        ...
        for (i = 0; i < CHAR_MAXPETITEMHAVE; i++) {
          ...
          if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
            CHAR_talkToCli(meindex, -1, "宠物 砩嫌邪蠖ǖ牡谰呶薹交易?",
                           CHAR_COLORYELLOW);
            return FALSE;
          }
        }
    ...
    for (i = 0; i < 5; i++) {
      if (TradeList[ti][side].PetTi[i] == usTi)
        return FALSE;
    }
    for (i = 0; i < 5; i++) {
      if (TradeList[ti][side].PetTi[i] != -1)
        continue;
...
BOOL TRADE_CheckTradeList(int meindex, STradeList *temp1, int toindex,
                          STradeList *temp2);
#endif
...
void TRADE_SwapItem(int meindex, int toindex, char *message, int fd,
                    char *mycharaname, int tofd, char *tocharaname);

int TRADE_CheckItembuf(int fd, int meindex, int toindex, int tofd,
                       char *mycharaname, char *tocharaname);
// BOOL TRADE_ChangeItem(int meindex, int toindex, char *a, char *b, int item1,
// int item2, int itemindex1, int itemindex2);
...
BOOL TRADE_Search(int fd, int meindex, char *message) {
  ...
  // 若玩家在摆摊中不处理交易
  if (CHAR_getWorkInt(meindex, CHAR_WORKSTREETVENDOR) > -1)
    return FALSE;
#endif
  ...
  if (getStringFromIndexWithDelim(message, "|", 2, token, sizeof(token)) ==
      FALSE)
    return FALSE;
    //  瓒ㄗ刺交易请求?//   CHAR_setWorkInt(meindex, CHAR_WORKTRADEMODE,
    //  CHAR_TRADE_SENDING);
  ...
  CHAR_getCoordinationDir(CHAR_getInt(meindex, CHAR_DIR),
                          ...
                          CHAR_getInt(meindex, CHAR_Y), 1, &front_x, &front_y);
  ...
  for (i = 0; i < found_count; i++) {
    ...
    if (CHAR_getWorkInt(index, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE)
      continue;
    if (CHAR_getFlg(index, CHAR_ISTRADE) == FALSE)
      continue;
  ...
  if (cnt == 1) {
    ...
    if (!CHAR_CHECKINDEX(toindex))
      return FALSE;
#ifdef _NB_CDKEYLOCK
    char *cdkey1 = CHAR_getChar(meindex, CHAR_CDKEY);
    char *cdkey2 = CHAR_getChar(toindex, CHAR_CDKEY);
    if (sasql_getLock(cdkey1) > 0) {
      CHAR_talkToCli(meindex, -1, "你的帐号存在非法操作，无法进行交易！", 6);
      return FALSE;
    } else if (sasql_getLock(cdkey2) > 0) {
      CHAR_talkToCli(meindex, -1, "对方的帐号存在非法操作，无法进行交易！", 6);
      return FALSE;
    }
#endif
#ifdef _ITEM_PET_LOCKED
    if (CHAR_getInt(meindex, CHAR_LOCKED) == 1 && getItemPetLocked() == 1) {
      char message[256];
      char buf[256];
      sprintf(message, "【为了确保您的账号财产安全】\n【请输入您的安全码进行解"
                       "锁】\n\n「PS」快捷解锁命令：/safe 安全密码");
      lssproto_WN_send(
          getfdFromCharaIndex(meindex), WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT,
...
void TRADE_ShowItem(int fd, int meindex, char *message) {
  ...
  if (getStringFromIndexWithDelim(message, "|", 3, tocharaname,
                                  sizeof(tocharaname)) == FALSE)
    return;
  ...
  case 'i': {
    ...
    if (TRADE_HandleItem(meindex, showindex, message, buf1) == FALSE) {
      print(" TRADE_Close_Err3:%d,%d,%s,%s ", meindex, showindex, message,
            buf1);
      TRADE_Close(fd, meindex, message);
      return;
    } else {
      snprintf(msg, sizeof(msg), "T|%d|%s|I|%d|%s", fd, mycharaname, showindex,
               buf1);
      lssproto_TD_send(tofd, msg);
    }
  } break;
  case 'g': {
    ...
    if (TRADE_HandleGold(meindex, showindex, message, buf1) == FALSE) {
      print(" TRADE_Close_Err4:%d,%d,%s,%s ", meindex, showindex, message,
            buf1);
      TRADE_Close(fd, meindex, message);
      return;
    } else { // 将欲交易之金钱传给对方
      snprintf(msg, sizeof(msg), "T|%d|%s|G|%d|%s", fd, mycharaname, showindex,
               buf1);
      lssproto_TD_send(tofd, msg);
    }
  } break;
  case 'p': {
    ...
    if (TRADE_HandlePet(meindex, showindex, message, buf1) == FALSE) {
      print(" TRADE_Close_Err5:%d,%d,%s,%s ", meindex, showindex, message,
            buf1);
      TRADE_Close(fd, meindex, message);
      return;
    } else {
      snprintf(msg, sizeof(msg), "T|%d|%s|P|%d|%s", fd, mycharaname, showindex,
               buf1);
      lssproto_TD_send(tofd, msg);
    }
  } break;
  case 'k': {
    if ((CONNECT_get_confirm(fd) == TRUE) &&
      ...
      if (CHAR_getWorkInt(meindex, CHAR_WORKTRADEMODE) == CHAR_TRADE_LOCK) {
        // andy_log
        print("ANDY err 滴锅掳 柳涝 吝!\n");
        return; //防止第二次进入
      }
      CHAR_setWorkInt(meindex, CHAR_WORKTRADEMODE, CHAR_TRADE_LOCK);
      //检查对方是否按下 ok
#ifdef _TRADESYSTEM2
      if (CHAR_getWorkInt(toindex, CHAR_WORKTRADEMODE) != CHAR_TRADE_LOCK)
        return;
#endif
      snprintf(msg, sizeof(msg), "T|%d|%s|A", fd, mycharaname);
      lssproto_TD_send(tofd, msg);
      TRADE_SwapItem(meindex, toindex, message, fd, mycharaname, tofd,
                     tocharaname);
    }
...
void TRADE_SwapItem(int meindex, int toindex, char *message, int fd,
  ...
  result =
      TRADE_CheckItembuf(fd, meindex, toindex, tofd, mycharaname, tocharaname);
#ifndef _ITEM_PILEFORTRADE
  toindex = CONNECT_getCharaindex(tofd);
#endif
  ...
  if (result == 1) {
    if (CHAR_charSaveFromConnect(meindex, FALSE)) {
      CHAR_talkToCli(meindex, -1, "拌沥沥焊 历厘 己傍!!", CHAR_COLORRED);
    }

    if (CHAR_charSaveFromConnect(toindex, FALSE)) {
      CHAR_talkToCli(toindex, -1, "拌沥沥焊 历厘 己傍!!", CHAR_COLORRED);
    }
  }
#endif
}
...
int TRADE_HandleTrade_DelItem(int charaindex, int MyMaxPile, int MaxPile,
                              int *Item, int *nums, int *indexlist) {
  int i, k;
...
BOOL TRADE_HandleTrade_AddPet(int charaindex, int *Pet) {
  ...
  for (i = 0; i < 5; i++) {
    ...
    CHAR_setChar(Pet[i], CHAR_OWNERCHARANAME,
                 CHAR_getChar(charaindex, CHAR_NAME));
    CHAR_complianceParameter(Pet[i]);

    SQL_UpdatePetRank(Pet[i]);
...
BOOL TRADE_HandleTrade(int meindex, STradeList *temp1, int toindex,
                       STradeList *temp2) {
...
#ifdef _NB_FIX_RIDE_PET_TRADE //骑乘中的宠物不能交易，防止改封包
  int j = 0;
  for (j = 0; j < 5; j++) {
    if (temp1->PetTi[j] != -1) {
      if (CHAR_getInt(meindex, CHAR_RIDEPET) == temp1->PetTi[j]) {
        CHAR_talkToCli(meindex, -1, "骑乘中的宠物不能交易！", CHAR_COLORYELLOW);
        CHAR_talkToCli(toindex, -1, "骑乘中的宠物不能交易！", CHAR_COLORYELLOW);
        return FALSE;
      }
    }
    if (temp2->PetTi[j] != -1) {
      if (CHAR_getInt(toindex, CHAR_RIDEPET) == temp2->PetTi[j]) {
        CHAR_talkToCli(meindex, -1, "骑乘中的宠物不能交易！", CHAR_COLORYELLOW);
        CHAR_talkToCli(toindex, -1, "骑乘中的宠物不能交易！", CHAR_COLORYELLOW);
        return FALSE;
      }
  ...
  /*
  {
          ...
          sprintf( buf, "%s|%s|%s|%s|%d,%d,%d",
                  CHAR_getChar( meindex, CHAR_NAME), CHAR_getChar(meindex,
  CHAR_CDKEY), CHAR_getChar( toindex, CHAR_NAME), CHAR_getChar(toindex,
  CHAR_CDKEY), CHAR_getInt( meindex, CHAR_FLOOR), CHAR_getInt( meindex, CHAR_X),
  CHAR_getInt( meindex, CHAR_Y) ); LogTrade(buf);
  }
  */
  //移除
  if ((ret = TRADE_HandleTrade_DelItem(meindex, MeMaxPile, ToMaxPile,
                                       temp1->ItemTi, temp1->ItemNum,
                                       ToItem)) == -1)
    return FALSE;
  if ((ret = TRADE_HandleTrade_DelItem(toindex, ToMaxPile, MeMaxPile,
                                       temp2->ItemTi, temp2->ItemNum,
                                       MeItem)) == -1)
    return FALSE;

  if ((ret = TRADE_HandleTrade_DelPet(meindex, temp1->PetTi, ToPet)) == -1)
    return FALSE;
  if ((ret = TRADE_HandleTrade_DelPet(toindex, temp2->PetTi, MePet)) == -1)
    return FALSE;
  if (TRADE_HandleTrade_DelGold(meindex, temp1->Golds, &ToGold) == FALSE)
    return FALSE;
  if (TRADE_HandleTrade_DelGold(toindex, temp2->Golds, &MeGold) == FALSE)
    return FALSE;
  //加入
  if (TRADE_HandleTrade_AddItem(meindex, MeItem) == FALSE)
    return FALSE;
  if (TRADE_HandleTrade_AddItem(toindex, ToItem) == FALSE)
    return FALSE;
  if (TRADE_HandleTrade_AddPet(meindex, MePet) == FALSE)
    return FALSE;
  if (TRADE_HandleTrade_AddPet(toindex, ToPet) == FALSE)
    return FALSE;
  ...
  {
    char buf1[4096], buf2[4096], buf[4096], buf3[4096];
    int i;
    ...
    for (i = 0; i < 50; i++) {
      if (ITEM_CHECKINDEX(ToItem[i])) {
        ...
        sprintf(buf3, "%s[%s](%d*%d),", ITEM_getChar(ToItem[i], ITEM_NAME),
                ITEM_getChar(ToItem[i], ITEM_UNIQUECODE),
                ITEM_getInt(ToItem[i], ITEM_ID),
                ITEM_getInt(ToItem[i], ITEM_USEPILENUMS));
        strcat(buf1, buf3);
      }
      if (ITEM_CHECKINDEX(MeItem[i])) {
        memset(buf3, 0, sizeof(buf3));
        sprintf(buf3, "%s[%s](%d*%d),", ITEM_getChar(MeItem[i], ITEM_NAME),
                ITEM_getChar(MeItem[i], ITEM_UNIQUECODE),
                ITEM_getInt(ToItem[i], ITEM_ID),
    ...
    for (i = 0; i < 5; i++) {
      if (CHAR_CHECKINDEX(ToPet[i])) {
        memset(buf3, 0, sizeof(buf3));
        sprintf(buf3, "%s[%s](%d),", CHAR_getChar(ToPet[i], CHAR_NAME),
                CHAR_getChar(ToPet[i], CHAR_UNIQUECODE),
                CHAR_getInt(ToPet[i], CHAR_LV));
        strcat(buf1, buf3);
      }
      if (CHAR_CHECKINDEX(MePet[i])) {
        memset(buf3, 0, sizeof(buf3));
        sprintf(buf3, "%s[%s](%d),", CHAR_getChar(MePet[i], CHAR_NAME),
                CHAR_getChar(MePet[i], CHAR_UNIQUECODE),
                CHAR_getInt(MePet[i], CHAR_LV));
        strcat(buf2, buf3);
      }
    }
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "%s|%s|%s|%s|%d,%d,%d|%d|%d|%s|%s",
            ...
            CHAR_getInt(meindex, CHAR_FLOOR), CHAR_getInt(meindex, CHAR_X),
            CHAR_getInt(meindex, CHAR_Y), ToGold, MeGold, buf1, buf2);
    // print(buf);
    LogTrade(buf);
  }
...
int TRADE_CheckItembuf(int fd, int meindex, int toindex, int tofd,
  ...
  if (TRADE_getMyTarget(meindex) != toindex)
    return -1;
  Tradeti = CONNECT_getTradeList(fd);
  if (Tradeti < 0 || Tradeti >= MAX_TRADELISTNUM)
    return -1;
  if (TradeList[Tradeti][0].use != 1 || TradeList[Tradeti][1].use != 1)
    return -1;
  if (TRADE_getSomeTradeList(meindex, &TradeListTemp[0]) == FALSE)
    return -1;
  if (TRADE_getSomeTradeList(toindex, &TradeListTemp[1]) == FALSE)
    return -1;
  //确定空位
  if (TRADE_CheckTradeList(meindex, &TradeListTemp[0], toindex,
                           &TradeListTemp[1]) == FALSE)
    return -1;
    //移除人物 砩?	//加入人物 砩?
#ifdef _TRADE_PK
  if (CHAR_getInt(meindex, CHAR_FLOOR) == 50000 &&
      CHAR_getInt(toindex, CHAR_FLOOR) == 50000) {
    BATTLE_CreateVsPlayerForTrade(TradeListTemp[0], TradeListTemp[1]);
    // PK交易结束动作
    return 3;
  }
#endif
  if (TRADE_HandleTrade(meindex, &TradeListTemp[0], toindex,
                        &TradeListTemp[1]) == FALSE)
    return -1;
  //交易结束动作
  return 1;
...
int TRADE_CheckItembuf(int fd, int meindex, int toindex, int tofd,
  ...
  // 比对双方之最後交易协定
  if (getStringFromIndexWithDelim(itembuf, "|", 5, token, sizeof(token)) ==
      TRUE)
    strcpy(a, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 6, token, sizeof(token)) ==
      TRUE)
    itemindex1 = atoi(token);
  if (getStringFromIndexWithDelim(itembuf, "|", 7, token, sizeof(token)) ==
      TRUE)
    strcpy(b, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 8, token, sizeof(token)) ==
      TRUE)
    itemindex2 = atoi(token);
  if (getStringFromIndexWithDelim(itembuf, "|", 9, token, sizeof(token)) ==
      TRUE)
    strcpy(c, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 10, token, sizeof(token)) ==
      TRUE)
    itemindex3 = atoi(token);
  if (getStringFromIndexWithDelim(itembuf, "|", 11, token, sizeof(token)) ==
      TRUE)
    strcpy(d, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 12, token, sizeof(token)) ==
      TRUE)
    itemindex4 = atoi(token);
  if (getStringFromIndexWithDelim(itembuf, "|", 13, token, sizeof(token)) ==
      TRUE)
    strcpy(e, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 14, token, sizeof(token)) ==
      TRUE)
    itemindex5 = atoi(token);
  if (getStringFromIndexWithDelim(itembuf, "|", 15, token, sizeof(token)) ==
      TRUE)
    strcpy(f, token);
  if (getStringFromIndexWithDelim(itembuf, "|", 16, token, sizeof(token)) ==
      TRUE)
    itemindex6 = atoi(token);

  if (getStringFromIndexWithDelim(toitembuf, "|", 5, token, sizeof(token)) ==
      TRUE)
    strcpy(g, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 6, token, sizeof(token)) ==
      TRUE)
    toitemindex1 = atoi(token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 7, token, sizeof(token)) ==
      TRUE)
    strcpy(h, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 8, token, sizeof(token)) ==
      TRUE)
    toitemindex2 = atoi(token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 9, token, sizeof(token)) ==
      TRUE)
    strcpy(i, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 10, token, sizeof(token)) ==
      TRUE)
    toitemindex3 = atoi(token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 11, token, sizeof(token)) ==
      TRUE)
    strcpy(j, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 12, token, sizeof(token)) ==
      TRUE)
    toitemindex4 = atoi(token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 13, token, sizeof(token)) ==
      TRUE)
    strcpy(k, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 14, token, sizeof(token)) ==
      TRUE)
    toitemindex5 = atoi(token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 15, token, sizeof(token)) ==
      TRUE)
    strcpy(l, token);
  if (getStringFromIndexWithDelim(toitembuf, "|", 16, token, sizeof(token)) ==
      TRUE)
  ...
  if ((strcmp(a, j) == 0) && (strcmp(b, k) == 0) && (strcmp(c, l) == 0) &&
    ...
    if (swapitem2 > CHAR_findTotalEmptyItem(toindex))
      return -3;
    // 检查双方金钱
    if ((strcmp(a, "G") == 0) && (itemindex1 != -1))
      gold1 += itemindex1;
    if ((strcmp(b, "G") == 0) && (itemindex2 != -1))
      gold1 += itemindex2;
    if ((strcmp(d, "G") == 0) && (itemindex4 != -1))
      gold2 += itemindex4;
    if ((strcmp(e, "G") == 0) && (itemindex5 != -1))
      gold2 += itemindex5;
    if (gold1 > CHAR_getMaxHaveGold(meindex))
      return -4;
    if (gold1 > CHAR_getInt(meindex, CHAR_GOLD))
      return -5;
    if (gold2 > CHAR_getMaxHaveGold(toindex))
      return -6;
    if (gold2 > CHAR_getInt(toindex, CHAR_GOLD))
      return -7;
    if ((gold2 + CHAR_getInt(meindex, CHAR_GOLD) - gold1) >
        CHAR_getMaxHaveGold(meindex))
      return -8;
    if ((gold1 + CHAR_getInt(toindex, CHAR_GOLD) - gold2) >
    ...
    if ((pet1 > 1) || (pet2 > 1))
      return -10;
    if ((pet1 == 1) && (CHAR_getCharPetElement(meindex) == -1))
      return -10;
    if ((pet2 == 1) && (CHAR_getCharPetElement(toindex) == -1))
      return -11;
    if ((strcmp(a, "I") == 0) && (itemindex1 != -1))
    ...
    if ((ITEM_getInt(item1, ITEM_VANISHATDROP) == 1) ||
      ...
      return -12;
#ifdef _LOCK_PET_ITEM
    if (ITEM_getInt(item1, ITEM_ItemLock) == 1) {
      return -17;
    }
    if (ITEM_getInt(item2, ITEM_ItemLock) == 1) {
      return -17;
    }
    if (ITEM_getInt(item4, ITEM_ItemLock) == 1) {
      return -17;
    }
    if (ITEM_getInt(item5, ITEM_ItemLock) == 1) {
      return -17;
    }
    ...
    if (CHAR_getWorkInt(meindex, CHAR_PickAllPet) != TRUE) {
      if ((pet6 != -1) &&
          (CHAR_getInt(pet6, CHAR_LV) - CHAR_getInt(meindex, CHAR_LV) > 5) &&
    ...
    if (CHAR_getWorkInt(toindex, CHAR_PickAllPet) != TRUE) {
      ...
      if ((pet3 != -1) &&
          (CHAR_getInt(pet3, CHAR_LV) - CHAR_getInt(toindex, CHAR_LV) > 5) &&
          (CHAR_getInt(toindex, CHAR_TRANSMIGRATION) <= 0))
        return -14;
    }
    ...
    {
      if (pet6 != -1) {
        if (CHAR_getInt(pet6, CHAR_PETLOCK) == 1) {
          return -16;
        }
#ifdef _PET_ITEM
        {
      ...
      if (pet3 != -1) {
        if (CHAR_getInt(pet3, CHAR_PETLOCK) == 1) {
          return -16;
        }
#ifdef _PET_ITEM
        {
    ...
    TRADE_ChangeItem(meindex, toindex, a, d, item1, item4, itemindex1,
                     itemindex4);
    TRADE_ChangeItem(meindex, toindex, b, e, item2, item5, itemindex2,
                     itemindex5);
    TRADE_ChangeItem(meindex, toindex, c, f, pet3, pet6, itemindex3,
                     itemindex6);
    ...
    // 写入 trade.log
    {
      ...
      if ((strcmp(a, "I") == 0) && (item1 != -1))
        logitem1 = ITEM_getInt(item1, ITEM_ID);
      if ((strcmp(b, "I") == 0) && (item2 != -1))
        logitem2 = ITEM_getInt(item2, ITEM_ID);
      if ((strcmp(a, "G") == 0) && (item1 == -1))
        logitem1 = itemindex1;
      if ((strcmp(b, "G") == 0) && (item2 == -1))
        logitem2 = itemindex2;
      ...
      if ((strcmp(d, "I") == 0) && (item4 != -1))
        logitem4 = ITEM_getInt(item4, ITEM_ID);
      if ((strcmp(e, "I") == 0) && (item5 != -1))
        logitem5 = ITEM_getInt(item5, ITEM_ID);
      if ((strcmp(d, "G") == 0) && (item4 == -1))
        logitem4 = itemindex4;
      if ((strcmp(e, "G") == 0) && (item5 == -1))
        logitem5 = itemindex5;
      sprintf(buf,
              "%s\t%s\t(%s[%d,%s],%s[%d,%s],%s[%s,%d,%s]) <-> "
              "%s\t%s\t(%s[%d,%s],%s[%d,%s],%s[%s,%d,%s])",
              CHAR_getChar(meindex, CHAR_CDKEY), mycharaname, a, logitem1,
              ITEM_getChar(item1, ITEM_UNIQUECODE), b, logitem2,
              ITEM_getChar(item2, ITEM_UNIQUECODE), c, petname1, pet1lv,
              CHAR_getChar(pet3, CHAR_UNIQUECODE),
              CHAR_getChar(toindex, CHAR_CDKEY), tocharaname, d, logitem4,
              ITEM_getChar(item4, ITEM_UNIQUECODE), e, logitem5,
              ITEM_getChar(item5, ITEM_UNIQUECODE), f, petname2, pet2lv,
              CHAR_getChar(pet6, CHAR_UNIQUECODE));
      LogTrade(buf);
    }
...
BOOL TRADE_ChangeItem(int meindex, int toindex, char *a, char *b, int item1,
  ...
  if ((strcmp(a, "I") == 0) && (strcmp(b, "I") == 0)) {
    ...
    if ((itemindex1 == -1) && (itemindex2 > 0)) {
      if (ITEM_CHECKINDEX(item2) == FALSE)
        return FALSE;
    ...
    // 道具 <-> 无
      ...
      if (ITEM_CHECKINDEX(item1) == FALSE)
        return FALSE;
  ...
  // 宠物 <-> 宠物
    ...
    char category[8];
    if ((item1 != -1) && (CHAR_CHECKINDEX(item1) == FALSE))
      return FALSE;
    if ((item2 != -1) && (CHAR_CHECKINDEX(item2) == FALSE))
      return FALSE;

    if ((itemindex1 != -1) &&
        (itemindex1 == CHAR_getInt(meindex, CHAR_DEFAULTPET))) {
      int fd;
    ...
    if ((itemindex2 != -1) &&
        (itemindex2 == CHAR_getInt(toindex, CHAR_DEFAULTPET))) {
      int tofd;
    ...
    if ((itemindex1 != -1) && (item1 != -1)) {
      ...
      CHAR_setChar(item1, CHAR_OWNERCDKEY, CHAR_getChar(toindex, CHAR_CDKEY));
    ...
    if ((itemindex2 != -1) && (item2 != -1)) {
      ...
      CHAR_setChar(item2, CHAR_OWNERCDKEY, CHAR_getChar(meindex, CHAR_CDKEY));
...
BOOL TRADE_HandleItem(int meindex, int showindex, char *message,
                      char *outmess) {
  char token[256];
  int item, itemindex;
  ...
  if (getStringFromIndexWithDelim(message, "|", 6, token, sizeof(token)) ==
    ...
    return FALSE;
  item = atoi(token);
  if (item < CHAR_STARTITEMARRAY || item > CHAR_getMaxInventory(meindex)) {
    return FALSE;
  }
...

  {
    ...
    if (crushe >= 0)
      snprintf(token, sizeof(token), "%d%%", crushe);
#endif
    ...
    sprintf(outmess, "%d|%s|%s|%d|%s",
            ...
            makeEscapeString(ITEM_getChar(itemindex, ITEM_NAME), tmpbuf1,
                             sizeof(tmpbuf1)),
#ifdef _ITEM_PILENUMS
...
BOOL TRADE_HandleGold(int meindex, int showindex, char *message,
                      char *outmess) {
  int gold, tmpgold;
  char token[256];
  int fd = getfdFromCharaIndex(meindex);
  ...
  if (gold > tmpgold)
    return FALSE;
#ifdef _ITEM_PILEFORTRADE
  if (TRADE_addSomeTradeList(meindex, gold, NULL, TRADE_GOLD) == FALSE)
    return FALSE;
#endif
...
BOOL TRADE_HandlePet(int meindex, int showindex, char *message, char *outmess) {
...
#ifdef _ITEM_PILEFORTRADE
  if (TRADE_addSomeTradeList(meindex, havepetindex, NULL, TRADE_PET) == FALSE)
    return FALSE;
#endif
  sprintf(token, "%d|%s|%d|%d|%d|%d|%d|%d",
          ...
          CHAR_getWorkInt(petindex, CHAR_WORKQUICK), havepetindex,
...
#endif
        ,
        token, CHAR_getWorkInt(petindex, CHAR_WORKMAXHP),
        CHAR_getInt(petindex, CHAR_SLOT), skillname[0], skillname[1],
        skillname[2], skillname[3], skillname[4], skillname[5], skillname[6],
...
BOOL TRADE_CheckTradeList(int meindex, STradeList *temp1, int toindex,
  ...
  MeMaxPile = CHAR_getMyMaxPilenum(meindex); //郴啊 掘阑荐乐绰 沫捞 绝澜
  ToMaxPile = CHAR_getMyMaxPilenum(toindex); //惑措规捞 掘阑荐 乐绰 沫捞 绝澜

  MeSurplus =
      CHAR_findSurplusItemBox(meindex); //郴啊 啊龙荐乐绰 酒捞袍沫捞 绝澜
  ToSurplus = CHAR_findSurplusItemBox(toindex); //其飘
  ...
  if (MeSurplus < MeNeeds) {
    CHAR_talkToCli(meindex, -1, "酒捞袍 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW); //郴 酒捞袍沫捞 啊垫谩澜
    CHAR_talkToCli(toindex, -1, "惑措规狼 酒捞袍 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW); //惑措规 酒捞袍沫捞 啊垫谩澜
    return FALSE;
  }
  if (ToSurplus < ToNeeds) {
    CHAR_talkToCli(toindex, -1, "酒捞袍 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW); //惑措规捞 好阑锭 郴 酒捞袍沫捞 啊垫谩澜
    CHAR_talkToCli(meindex, -1, "惑措规狼 酒捞袍 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW); //郴 酒捞袍沫捞 啊垫 谩澜
    return FALSE;
  }
  ...
  if (MeSurplus < MeNeeds) {
    CHAR_talkToCli(meindex, -1, "惑措狼 其飘 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW);
    CHAR_talkToCli(toindex, -1, "其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
    return FALSE;
  }
  if (ToSurplus < ToNeeds) {
    CHAR_talkToCli(meindex, -1, "其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
    CHAR_talkToCli(toindex, -1, "惑措狼 其飘 沫捞 啊垫 谩绢夸!",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
  ...
  MeNeeds = temp2->Golds;
  if (MeSurplus < MeNeeds) {
    CHAR_talkToCli(meindex, -1, "芭贰 陛咀捞 惑茄急俊 档崔窍看嚼聪促.",
                   CHAR_COLORYELLOW);
    CHAR_talkToCli(toindex, -1, "芭贰 陛咀捞 惑茄急俊 档崔窍看嚼聪促.",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
  if (ToSurplus < ToNeeds) {
