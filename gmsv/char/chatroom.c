...
BOOL ChatCheck_BeMaster(int myindex, int chatnum) {
  ...
  if (CHAR_getWorkInt(myindex, CHAR_WORKCHATROOMNUM) == 0)
    return FALSE;
...
BOOL ChatCheck_Free(int myindex) {
  ...
  if (CHAR_getInt(myindex, CHAR_LV) < 30 &&
    ...
    CHAR_talkToCli(myindex, -1, "成立聊天室需０转３０级以上！",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
...
BOOL ChatRoom_Create(int myindex, char *message) {
  ...
  if (ChatCheck_Free(myindex) == FALSE)
    return FALSE;
  ...
  CHAR_talkToCli(myindex, -1, "聊天室已满无法建立新的聊天频道！",
                 CHAR_COLORYELLOW);
  return FALSE;
}
...
void ChatRoom_Make(int myindex, int toindex) {
  ...
  if (CHAR_getWorkInt(myindex, CHAR_WORKCHATROOMTYPE) == 1) {
    if (Num != CHAR_getWorkInt(toindex, CHAR_WORKCHATROOMNUM))
      ...
      CHAR_talkToCli(myindex, -1, "您选择的继任人物等级不足以担任室长！",
                     CHAR_COLORRED);
    }
  }
  ChatRoom_Refresh(Num);
      ...
      if (ChatRoom[Num].MemberList[i] != myindex &&
        ...
        if (CHAR_CHECKINDEX(ChatRoom[Num].MemberList[i])) {
          if (CHAR_getInt(ChatRoom[Num].MemberList[i], CHAR_LV) >= 30 ||
              CHAR_getInt(ChatRoom[Num].MemberList[i], CHAR_TRANSMIGRATION) >=
                  1) {
...
void ChatRoom_Join(int myindex, int num) {
  ...
  if (ChatRoom[num].useFlag == TRUE &&
      ChatRoom[num].NowPeople < MAX_PPLINROOM) {
...
void ChatRoom_Agree(int myindex, int toindex, int YesNo) {
  ...
  if (CHAR_getWorkInt(myindex, CHAR_WORKCHATROOMTYPE) == 1) {
    if (ChatRoom[Num].NowPeople < MAX_PPLINROOM && YesNo == 1) {
      ...
      CHAR_talkToCli(toindex, -1, "您申请的聊天室室长拒绝您的加入！",
                     CHAR_COLORRED);
    } else if (ChatRoom[Num].NowPeople >= MAX_PPLINROOM) {
...
void ChatRoom_List(int fd) {
  ...
  for (i = 0; i < MAX_CHATROOM; i++) {
    if (ChatRoom[i].useFlag == TRUE) {
      sprintf(buf, "聊天室%2d资讯：室名=>%20s , 室长=>%16s , 人数=>%2d", i,
              ChatRoom[i].RoomName, CHAR_getChar(ChatRoom[i].Maker, CHAR_NAME),
              ChatRoom[i].NowPeople);

      sprintf(buf, "r%d|%s|%d|%s|p%d|", i, ChatRoom[i].RoomName,
              ChatRoom[i].Maker, CHAR_getChar(ChatRoom[i].Maker, CHAR_NAME),
              ChatRoom[i].NowPeople);
...
void ChatRoom_Refresh(int Num) {
  ...
  char buf[64];
  if (ChatRoom[Num].useFlag == TRUE) {
    sprintf(token, "R|r%d|%s|%d|%s|p%d|", Num, ChatRoom[Num].RoomName,
            ChatRoom[Num].Maker, CHAR_getChar(ChatRoom[Num].Maker, CHAR_NAME),
            ChatRoom[Num].NowPeople);
    for (i = 0; i < MAX_PPLINROOM; i++) {
      if (ChatRoom[Num].MemberList[i] != -1) {
        sprintf(buf, "I%d|%s|%s|", ChatRoom[Num].MemberList[i],
  ...
  for (i = 0; i < MAX_PPLINROOM; i++) {
    if (ChatRoom[Num].MemberList[i] != -1) {
      if (CHAR_getWorkInt(ChatRoom[Num].MemberList[i], CHAR_WORKCHATROOMTYPE) ==
          1)
        RoomLeader = 1;
      else
        RoomLeader = 0;
