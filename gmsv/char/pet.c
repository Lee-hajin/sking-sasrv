...
CHAR_talkToCli(charaindex, -1, "绑定的宠物无法丢出！", CHAR_COLORYELLOW);
return	FALSE;
}
*/
    ...
    if(strlen(owntile)>0){
            if(CHAR_getInt(petindex,CHAR_LV)>=100 && strcmp(name,owntile)!=0){
                    CHAR_talkToCli( charaindex, -1,
    "【宠物保护系统】有昵称的100级以上宠物无法丢出，如需操作请把宠物名称改为原名称.",
...

    {
      int i1;
      int cancelflg = 0;
      for (i1 = 0; i1 < CHAR_MAXPETITEMHAVE; i1++) {
        int itemindex = CHAR_getItemIndex(petindex, i1);
        if (itemindex >= 0) {
          if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
            char TM_SayBuff[64];
            snprintf(TM_SayBuff, sizeof(TM_SayBuff),
                     "装备绑定道具的宠物无法丢出！");
            CHAR_talkToCli(charaindex, -1, TM_SayBuff, CHAR_COLORYELLOW);
            cancelflg = 1;
            break;
          }
        }
  ...
  snprintf(szPet, sizeof(szPet),
           "[%s]其飘 [%d]饭骇 其飘啊 %d檬 饶俊 荤扼笼聪促。",
...
int PET_dropPet(int charaindex, int havepetindex) {
  ...
  if (_PET_dropPet(charaindex, havepetindex, -1, -1, -1) == TRUE) {
    ...
    SQL_DeletePetInRank(petindex);
    LogPet(CHAR_getChar(charaindex, CHAR_NAME), /* 平乓仿   */
           CHAR_getChar(charaindex, CHAR_CDKEY),
           CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
           "丢宠", CHAR_getInt(charaindex, CHAR_FLOOR),
...
int PET_createPetFromCharaIndex(int charaindex, int enemyindex) {
  ...
  CharNew.data[CHAR_CRITIAL] = CHAR_getInt(enemyindex, CHAR_CRITIAL); //会心率
  CharNew.data[CHAR_CRIT_RATE] =
      CHAR_getInt(enemyindex, CHAR_CRIT_RATE); //会心率
  CharNew.data[CHAR_COUNTER] = CHAR_getInt(enemyindex, CHAR_COUNTER); //反击率
  CharNew.data[CHAR_HIT_RATE] = CHAR_getInt(enemyindex, CHAR_HIT_RATE); //反击率
  CharNew.data[CHAR_PETMAILEFFECT] = RAND(0, PETMAIL_EFFECTMAX);

  CharNew.data[CHAR_DATAALL] = CHAR_getInt(enemyindex, CHAR_DATAALL);
  CharNew.data[CHAR_DATAVIT] = CHAR_getInt(enemyindex, CHAR_DATAVIT);
  CharNew.data[CHAR_DATASTR] = CHAR_getInt(enemyindex, CHAR_DATASTR);
  CharNew.data[CHAR_DATATGH] = CHAR_getInt(enemyindex, CHAR_DATATGH);
  CharNew.data[CHAR_DATADEX] = CHAR_getInt(enemyindex, CHAR_DATADEX);
...
#ifdef _CHAR_PETDATA
  if (CHAR_getInt(enemyindex, CHAR_LV) == 1) {
    CHAR_setInt(newindex, CHAR_PETDATAH,
                CHAR_getWorkInt(enemyindex, CHAR_WORKMAXHP));
    CHAR_setInt(newindex, CHAR_PETDATAS,
                CHAR_getWorkInt(enemyindex, CHAR_WORKATTACKPOWER));
    CHAR_setInt(newindex, CHAR_PETDATAT,
                CHAR_getWorkInt(enemyindex, CHAR_WORKDEFENCEPOWER));
    CHAR_setInt(newindex, CHAR_PETDATAD,
                CHAR_getWorkInt(enemyindex, CHAR_WORKQUICK));
  }
#endif
...
int PET_dropPetFollow(int charaindex, int havepetindex, int tofl, int tox,
  ...
  LogPet(CHAR_getChar(charaindex, CHAR_NAME),
         CHAR_getChar(charaindex, CHAR_CDKEY),
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "溜宠", CHAR_getInt(charaindex, CHAR_FLOOR),
         CHAR_getInt(charaindex, CHAR_X), CHAR_getInt(charaindex, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, charaindex);
  return 1;
}
...
void PET_showEditBaseMsg(int charaindex, int toindex, int itemindex,
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (work[i] > maxnums) {
      ...
      if (type > 0) {
        if (type > 2) {
          sprintf(buf1, "%s %s 。", buf2[i], buf3[0]);
        } else {
          sprintf(buf1, "%s %s 。", buf2[i], buf3[1]);
        }
      } else if (type < 0) {
        sprintf(buf1, "%s %s 。", buf2[i], buf3[2]);
      }
    }
    if (strcmp(buf1, "\0")) {
