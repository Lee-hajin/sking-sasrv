...
void FAMILY_Add(int fd, int meindex, char *message) // 成立家族
{
  ...
  if ((strstr(fmname, " ")) || (strcmp(fmname, "") == 0) ||
      (strstr(fmname, "　"))) {
    lssproto_WN_send(
        fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
  ...
  fmbadge = atoi(token);
  if (fmbadge < FamilyBadgeStart || fmbadge > FamilyBadgeEnd) {
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1,
                     -1,
                     makeEscapeString("\n家族徽章设置错误", buf, sizeof(buf)));
    return;
  ...
  // print("%s %s %s %d %s %s %s %d %d\n", fmname, charname, charid, charlv,
  // petname,petattr, fmrule, fmsprite, chargrano);
#ifdef _PERSONAL_FAME
#ifdef _NB_FAMILYBADGE
  saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv, petname,
                         petattr, fmrule, fmsprite, chargrano,
                         CHAR_getInt(meindex, CHAR_FAME), fmbadge,
                         CONNECT_getFdid(fd));
#else
  saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv, petname,
                         petattr, fmrule, fmsprite, chargrano,
  ...
  // print("ACAddFM acfd:%d meindex:%d fmname:%s charname:%s fame:%d
  // Connectfd:%d fd:%d\n",acfd, meindex, fmname, charname, CHAR_getInt(meindex,
  // CHAR_FAME), CONNECT_getFdid(fd), fd);
#else
  saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv, petname,
                         petattr, fmrule, fmsprite, chargrano,
                         CONNECT_getFdid(fd));
...
void ACAddFM(int fd, int result, int fmindex, int index) {
  ...
  if (result == 1) {
    ...
    CHAR_talkToCli(meindex, -1, "俺牢 扁咯档啊 0栏肺 汲沥登菌嚼聪促",
                   CHAR_COLORYELLOW);
#endif
    lssproto_WN_send(
        fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
        makeEscapeString("\n何练捞 汲赋 登菌嚼聪促\n老林老 捞郴肺 啊练 牢盔捞 "
                         "10疙阑 给逞辨 版快 何练篮 磊悼栏肺 秦眉邓聪促。",
                         buf, sizeof(buf)));
...
#endif
    LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
              CHAR_getInt(meindex, CHAR_FMINDEX),
              CHAR_getChar(meindex, CHAR_NAME),
              CHAR_getChar(meindex, CHAR_CDKEY), "ADDFAMILY(成立家族)", "");
  } else {
    int i = 0, petindex = 0;
    char tmpbuf[256];
    ...
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1,
                     -1, makeEscapeString(tmpbuf, buf, sizeof(buf)));
  }
...
void FAMILY_Join(int fd, int meindex, char *message) {
  ...
  if (CHAR_getInt(meindex, CHAR_FMTIMELIMIT) > (int)time(NULL)) {
    char buff[255];
    sprintf(buff, "\n如之前退出家族，\n需满%d小时才能再加入家族喔！",
            (CHAR_getInt(meindex, CHAR_FMTIMELIMIT) - (int)time(NULL)) / 3600 +
                1);
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1,
                     -1, makeEscapeString(buff, buf, sizeof(buf)));
    return;
  }
#endif
  ...
  // print("JoinFM index:%d fmindex:%d fmname:%s charname:%s charid:%s charlv:%d
  ...
  saacproto_ACJoinFM_send(acfd, fmname, fmindex, charname, charid, charlv,
                          index, CHAR_getInt(meindex, CHAR_FAME),
                          CONNECT_getFdid(fd));
#else
  // print("charfdid:%d\n", CONNECT_getFdid(fd));
  saacproto_ACJoinFM_send(acfd, fmname, fmindex, charname, charid, charlv,
...
void FAMILY_Leave(int fd, int meindex, char *message) {
...

  {
    ...
    for (i = 1; i <= MANORNUM; i++) {
      ...
      if (fmpks[fmpks_pos + 1].host_index > -1 &&
          fmpks[fmpks_pos + 1].guest_index > -1) {
#ifdef _FMVER21
        if (CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER) {
#else
        if (CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == 1) {
  ...
  if (result == 1) {
...
#endif
    {
      ...
      // print("LeaveFM index:%d fmindex:%d fmname:%s charname:%s
...
void ACLeaveFM(int fd, int result, int resultflag) {
  ...
  if (result == 1) {
    ...
    LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
              CHAR_getInt(meindex, CHAR_FMINDEX),
              CHAR_getChar(meindex, CHAR_NAME),
              CHAR_getChar(meindex, CHAR_CDKEY), "LEAVEFAMILY(离开家族)", "");
...
#ifdef _FM_JOINLIMIT
    CHAR_setInt(meindex, CHAR_FMTIMELIMIT,
                (int)time(NULL) + getJoinFamilyTime() * (60 * 60));
#endif
#ifdef _NEW_MANOR_LAW
    CHAR_setInt(meindex, CHAR_MOMENTUM, 0);
    CHAR_talkToCli(meindex, -1, "俺牢 扁技啊 0栏肺 汲沥登菌嚼聪促.",
                   CHAR_COLORYELLOW);
#endif
...
void ACDelFM(int fd, int result) {
  ...
  if (result == 1) {
    ...
    LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
              CHAR_getInt(meindex, CHAR_FMINDEX),
              CHAR_getChar(meindex, CHAR_NAME),
              CHAR_getChar(meindex, CHAR_CDKEY), "DELFAMILY(家族解散)", "");
...
#endif
{
  ...
  if (result == 1) {
    ...
    if (charfame != CHAR_getInt(meindex, CHAR_FAME)) {
      sprintf(buf, "server fame:%d,ac fame:%d", CHAR_getInt(meindex, CHAR_FAME),
              charfame);
      LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
                CHAR_getInt(meindex, CHAR_FMINDEX),
                CHAR_getChar(meindex, CHAR_NAME),
                CHAR_getChar(meindex, CHAR_CDKEY), "ACFMCharLogin", buf);
    }
...
#endif
    {
      ...
      */
      ...
      saacproto_ACFixFMData_send(
          acfd, CHAR_getChar(meindex, CHAR_FMNAME),
          CHAR_getInt(meindex, CHAR_FMINDEX),
    ...
    if (flag == 0) {
      ...
      CHAR_setInt(meindex, CHAR_FMTIMELIMIT,
                  (int)time(NULL) + getJoinFamilyTime() * (60 * 60));
#endif
      CHAR_talkToCli(meindex, -1, "你已经退出家族或家族已经不存在了！",
                     CHAR_COLORYELLOW);
      CHAR_setWorkInt(meindex, CHAR_WORKFMFLOOR, -1);
      CHAR_setInt(meindex, CHAR_RIDEPET, -1);
      CHAR_setInt(meindex, CHAR_BASEIMAGENUMBER,
...
void FAMILY_Detail(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "P") == 0) {
...
#ifdef _NEW_MANOR_LAW
    sprintf(sendbuf, "你目前的个人气势点数为：%d",
            CHAR_getInt(meindex, CHAR_MOMENTUM) / 100);
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORCYAN);
#endif

#ifdef _POINT_AMPOINT
    sprintf(sendbuf, "你目前的个人积分点数为：%d",
            CHAR_getInt(meindex, CHAR_AMPOINT));
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
#endif

#ifdef _ONLINE_SHOP
    sprintf(sendbuf, "你目前的个人贝币点数为：%d",
            CHAR_getInt(meindex, CHAR_BJ));
...
#ifdef _MISSION_TRAIN
    {
      ...
      if (num < 0) {
        CHAR_talkToCli(meindex, -1, "你目前的连续完成任务链：未接任务",
                       CHAR_COLORYELLOW);
      } else {
    ...
    if (CHAR_getInt(meindex, CHAR_TITLE_LV) > 0) {
      if (CHAR_getInt(meindex, CHAR_TITLE_TIME) > time(NULL)) {
        ...
        sprintf(sendbuf,
                "你目前的英雄称号有效期：%4d年%2d月%2d日 %2d:%2d:%2d\n",
    ...
    if (CHAR_getInt(meindex, CHAR_MISSION_TIME) > time(NULL)) {
      sprintf(sendbuf, "你目前副本任务时间剩余：%d分钟",
              (CHAR_getInt(meindex, CHAR_MISSION_TIME) - time(NULL)) / 60);
      CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORGREEN);
    }
#endif
#ifdef _ACTIVE_GAME
    sprintf(sendbuf, "你目前的个人活力数目为：%d",
            CHAR_getInt(meindex, CHAR_ACTIVE));
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORCYAN);
#endif
    {
      ...
      if (lefttime > 0) {
        sprintf(sendbuf,
                "你目前的经验上升效果为：经验上升 %d\% 剩余时间 %d 分 %d 秒",
                CHAR_getWorkInt(meindex, CHAR_WORKITEM_ADDEXP), lefttime / 60,
                lefttime % 60);
        CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
      }
    }
#ifdef _SASQL
    sprintf(sendbuf, "你目前的金币为：%d", sasql_getVipPoint(meindex));
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
#else
#ifdef _NEW_VIP_SHOP
...
#endif
#ifdef _OFFLINE_TIME
    sprintf(sendbuf, "你目前的可离线时间分钟：%d",
            CHAR_getInt(meindex, CHAR_OFFLINE_TIME));
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
#endif
#endif
  }
  ...
  if (strcmp(token, "D") == 0) {
    ...
    saacproto_ACFMDetail_send(acfd, fmname, fmindex, tempindex,
                              CONNECT_getFdid(fd));
  }
  ...
  // shan begin
...
#endif
    {
      ...
      if (fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_MANOR_BATTLEBEGIN ||
        ...
        if ((fmpks[fmpks_pos + 1].host_index == index &&
          ...
          sprintf(tmpbuf, "%d/%d %d:%d【%s】 %s ｖｓ %s", tm1->tm_mon + 1,
...
#ifdef _NEW_MANOR_LAW
#ifdef _NB_FAMILYBADGE
    sprintf(sendbuf, "%s|%d|%s|%d|%d|%d|%d|%d|%s|%d|%d|%d",
#else
    ...
    sprintf(sendbuf, "%s|%d|%s|%d|%d|%d|%d|%d|%s",
#endif
            CHAR_getChar(meindex, CHAR_FMNAME),
            memberlist[fmindex_wk].fmjoinnum, leadernamebuf, h + 1,
#ifdef _FMVER21
            fmdptop.fmtopdp[h],
#else
            ...
            CHAR_getInt(meindex, CHAR_FMSPRITE), tmpbuf
#ifdef _NEW_MANOR_LAW
            ,
...
#ifdef _NB_FAMILYBADGE
            ,
            memberlist[fmindex_wk].badge
#endif
    );

    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_FAMILYDETAIL, WINDOW_BUTTONTYPE_OK,
                     -1, -1, makeEscapeString(sendbuf, buf, sizeof(buf)));
  }
  // shan end
}
...
void ACFMDetail(int ret, char *data, int clifd) {
  ...
  */
  ...
  // from saac data
  /*
  sprintf(data, "%d|%d|%s|%s|%d|%s|%s|%d|%d|%s|%d", index,
  family[index].fmindex, family[index].fmname, family[index].fmleadername,
...
void FAMILY_CheckMember(int fd, int meindex, char *message) {
...
#ifdef _FMVER21
  {
    ...
    LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
              CHAR_getInt(meindex, CHAR_FMINDEX),
              CHAR_getChar(meindex, CHAR_NAME),
              CHAR_getChar(meindex, CHAR_CDKEY), "CheckMember", sbuf);
    // shan end

    saacproto_ACMemberJoinFM_send(
        acfd, CHAR_getChar(meindex, CHAR_FMNAME),
...
#else
  {
    saacproto_ACMemberJoinFM_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                                  CHAR_getInt(meindex, CHAR_FMINDEX), charname,
                                  charindex,
                                  CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
                                  result, CONNECT_getFdid(fd));
  }
#endif
#ifdef _FMVER21
...
#endif
  {
...
#endif
    {
      ...
      if ((fmpks[fmpks_pos].host_index == index &&
        ...
        lssproto_WN_send(
            fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
            makeEscapeString("\n家族目前正在战斗中，所以无法审核成员。", buf,
                             sizeof(buf)));
        //      	 	print("fmpks_pos:%d index:%d host:%d
        //      guest:%d\n", fmpks_pos, 	 		index, fmpks[fmpks_pos].host_index,
...
#ifdef _FMVER21
    {
      ...
      LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
                ...
                CHAR_getChar(meindex, CHAR_CDKEY), "CheckMember", sbuf);
      // shan end
      saacproto_ACMemberLeaveFM_send(
          acfd, CHAR_getChar(meindex, CHAR_FMNAME),
          CHAR_getInt(meindex, CHAR_FMINDEX), charname, charindex,
          CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
          CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
...
#else
    {
      saacproto_ACMemberLeaveFM_send(
          acfd, CHAR_getChar(meindex, CHAR_FMNAME),
          CHAR_getInt(meindex, CHAR_FMINDEX), charname, charindex,
          CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI), CONNECT_getFdid(fd));
    }
#endif
  }
#ifdef _FMVER21
...
#ifdef _FMVER21
    else if (channel == FAMILY_MAXCHANNEL &&
             CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER)
#else
    else if (channel == FAMILY_MAXCHANNEL &&
             CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == 1)
...
void FAMILY_Bank(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "G") == 0) {
    ...
    if (((cash - toBank) >= 0) && ((cash - toBank) <= MaxGold) &&
        ((((bank + toBank) >= 0) &&
          ((bank + toBank) <= CHAR_MAXBANKGOLDHAVE)) ||
         toBank < 0)) {
      // shan add
      ...
      if (toBank >= 0) {
        ...
        LogFamilyBankStone(
            CHAR_getChar(meindex, CHAR_NAME), CHAR_getChar(meindex, CHAR_CDKEY),
            toBank, CHAR_getInt(meindex, CHAR_GOLD), "存款(家族个人银行)",
            CHAR_getInt(meindex, CHAR_FLOOR), CHAR_getInt(meindex, CHAR_X),
            CHAR_getInt(meindex, CHAR_Y), CHAR_getInt(meindex, CHAR_BANKGOLD));
        ...
        LogFamilyBankStone(
            CHAR_getChar(meindex, CHAR_NAME), CHAR_getChar(meindex, CHAR_CDKEY),
            toBank, CHAR_getInt(meindex, CHAR_GOLD), "提款(家族个人银行)",
            CHAR_getInt(meindex, CHAR_FLOOR), CHAR_getInt(meindex, CHAR_X),
            CHAR_getInt(meindex, CHAR_Y), CHAR_getInt(meindex, CHAR_BANKGOLD));
      }
      // Syu ADD 新增家族个人银行存取Log (不含家族银行)
      LogStone(-1, CHAR_getChar(meindex, CHAR_NAME), /* 平乓仿   */
               CHAR_getChar(meindex, CHAR_CDKEY),    /* 交□扒□ID */
               -toBank,                              /* 嗯喊 */
               CHAR_getInt(meindex, CHAR_GOLD), "家族个人银行",
               CHAR_getInt(meindex, CHAR_FLOOR), CHAR_getInt(meindex, CHAR_X),
  ...
  if (strcmp(token, "T") == 0) {
    ...
    if (toTax > 0) { //+存款
      if (((mygold - toTax) < 0) ||
          (familyTax[FMindex] + toTax) > CHAR_MAXFMBANKGOLDHAVE) {
        return;
      }
    } else if (toTax < 0) { //-取款
...
void FAMILY_SetPoint(int fd, int meindex, char *message) {
  ...
  y = atoi(token);
  /*
     print("SetFMPoint charname:%s fmindex:%d index:%d pointindex:%d fl:%d x:%d
     y:%d\n", CHAR_getChar(meindex, CHAR_FMNAME), CHAR_getInt(meindex,
     CHAR_FMINDEX), CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI), fmpointindex,
     fl, x, y);
  */
  saacproto_ACSetFMPoint_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                              CHAR_getInt(meindex, CHAR_FMINDEX),
                              CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
...
void ACSetFMPoint(int ret, int r, int clifd) {
  ...
  if (ret == 0) {
    if (r == -1)
      ...
      sprintf(
          message,
          "您的家族人数未达申请标准唷！最少必须拥有10个成员"); //成员数在saac/acfamly.h里修改
    else if (r == -6)
  ...
  lssproto_WN_send(clifd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1,
...
void ACFMAnnounce(int ret, char *fmname, int fmindex, int index, int kindflag,
    ...
    if (chindex >= 0) {
      if (CHAR_getCharUse(chindex)) {
        if (kindflag == 1) {
          ...
          CHAR_talkToCli(chindex, -1,
                         "由於您的家族在七天之内没有召收到１０名家族成员，所以"
                         "被迫解散了！",
                         CHAR_COLORRED);
        }
      } else
  ...
  if (kindflag == 4) {
    ...
    if (!CHAR_CHECKINDEX(meindex))
      return;
    //      print("Here2\n");
    lssproto_WN_send(clifd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK,
                     -1, -1, makeEscapeString(data, buf, sizeof(buf)));
  }
}
...
void FAMILY_SetAcceptFlag(int fd, int meindex, char *message) {
  ...
  if ((result == 0) || (result == 1)) {
    saacproto_ACFixFMData_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                               CHAR_getInt(meindex, CHAR_FMINDEX),
                               CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
                               FM_FIX_ACCEPTFLAG, token, "",
...
void FAMILY_FixRule(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "R") == 0) {
    ...
    saacproto_ACFixFMData_send(
        acfd, CHAR_getChar(meindex, CHAR_FMNAME),
        CHAR_getInt(meindex, CHAR_FMINDEX),
        CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI), FM_FIX_FMRULE, buf, "",
        CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
    return;
  }
#ifdef _NB_FAMILYBADGE
  if (strcmp(token, "B") == 0) {
    ...
    saacproto_ACFixFMData_send(
        acfd, CHAR_getChar(meindex, CHAR_FMNAME),
        CHAR_getInt(meindex, CHAR_FMINDEX),
        CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI), FM_FIX_BADGE, buf, "",
        CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
    saacproto_ACShowMemberList_send(
        acfd, CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI));
    return;
  }
#endif
  if (strcmp(token, "P") == 0) {
...
int FAMILY_RidePet(int fd, int meindex, char *message) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORKTRADEMODE) != CHAR_TRADE_FREE) {
    CHAR_talkToCli(meindex, -1, "交易中不可骑宠！", CHAR_COLORYELLOW); //芭贰
    return 0;
  }
  ...
  if (CHAR_getInt(meindex, CHAR_BECOMEPIG) > -1) { //处於乌力化状态
    ...
    CHAR_complianceParameter(meindex);
    CHAR_send_P_StatusString(meindex, CHAR_P_STRING_RIDEPET);
    CHAR_talkToCli(meindex, -1, "目前你处于乌力化状态，不能骑乘宠物。",
                   CHAR_COLORYELLOW);
    return 0;
  }
#endif
  ...
  if (strcmp(token, "P") == 0) {
    ...
    if (atoi(token2) != -1) {
...
#ifdef _RIDELEVEL
      int RideLevel = 5 + (CHAR_getInt(meindex, CHAR_TRANSMIGRATION) * 5);
      if (CHAR_getInt(meindex, CHAR_LV) + RideLevel <
          CHAR_getInt(petindex, CHAR_LV)) {
        // char buff[255];
      ...
      if (rideGraNo != 0) {
        ...
        CHAR_send_P_StatusString(meindex, CHAR_P_STRING_ATK);
        CHAR_send_P_StatusString(meindex, CHAR_P_STRING_DEF);
        CHAR_send_P_StatusString(meindex, CHAR_P_STRING_QUICK);
...
void getNewFMList() { saacproto_ACShowFMList_send(acfd); }
  ...
  if (strcmp(token, "CHANGE") == 0) {
    ...
    if (strcmp(token2, "L") == 0) {
      ...
      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_LEADERSELECT,
                       WINDOW_BUTTONTYPE_OK, -1, -1, sendbuf);
    }
    // 询问族长候选人是否愿意接受
    if (strcmp(token2, "Q") == 0) {
      ...
      sprintf(
          sendbuf, "%s|%d",
          makeEscapeString(CHAR_getChar(meindex, CHAR_NAME), buf, sizeof(buf)),
          meindex);

      lssproto_WN_send(CHAR_getWorkInt(toindex, CHAR_WORKFD),
                       WINDOW_MESSAGETYPE_LEADERSELECTA, WINDOW_BUTTONTYPE_OK,
                       -1, -1, sendbuf);
    }
    // 候选人的答覆
    if (strcmp(token2, "A") == 0) {
      ...
      if (CHAR_getInt(leaderindex, CHAR_FMLEADERFLAG) != 1)
        return;
#endif
      if (CHAR_getInt(meindex, CHAR_FMINDEX) !=
          CHAR_getInt(leaderindex, CHAR_FMINDEX))
        return;
      ...
      if (answerflag == 0) {
        ...
        lssproto_WN_send(
            CHAR_getWorkInt(leaderindex, CHAR_WORKFD),
            WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
      ...
      if (answerflag == 1) {
        ...
        saacproto_ACFixFMData_send(
            acfd, CHAR_getChar(meindex, CHAR_FMNAME),
            CHAR_getInt(meindex, CHAR_FMINDEX),
            CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI), FM_FIX_FMLEADERCHANGE,
            buf, tmpbuf, CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX),
            CONNECT_getFdid(fd));
        // "", CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX),
        // CONNECT_getFdid(fd));
        return;
...
void ACFMJob(int fd, int ret, char *data1, char *data2) {
  ...
  if (1) {
    ...
    // Robin 10/02 debug
    if (CHAR_getInt(leaderindex, CHAR_FMINDEX) !=
    ...
    // || CHAR_getInt( leaderindex, CHAR_FMLEADERFLAG) != 1
      ...
      sprintf(buf, "leaderindex:%d:%s\n", leaderindex,
              CHAR_getChar(leaderindex, CHAR_NAME));
      LogFamily(CHAR_getChar(charaindex, CHAR_FMNAME),
                CHAR_getInt(charaindex, CHAR_FMINDEX),
                CHAR_getChar(charaindex, CHAR_NAME),
                CHAR_getChar(charaindex, CHAR_CDKEY),
                "LEADERCHANGE_ERROR(族长让位失败)", buf);
      return;
    }
    ...
    sprintf(buf2, "\n辛苦你了！你已经将族长的位子交给%s了。",
            CHAR_getChar(charaindex, CHAR_NAME));
    lssproto_WN_send(CHAR_getWorkInt(leaderindex, CHAR_WORKFD),
                     WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
                     makeEscapeString(buf2, buf, sizeof(buf)));

    //		print(" LeaderChange!! [%s]->[%s] ", CHAR_getChar(leaderindex,
    //CHAR_CDKEY), CHAR_getChar(charaindex, CHAR_CDKEY) );

    sprintf(buf, "%s\t%s\t%s", CHAR_getChar(leaderindex, CHAR_FMNAME),
            CHAR_getChar(leaderindex, CHAR_NAME),
    ...
    LogFamily(CHAR_getChar(charaindex, CHAR_FMNAME),
              CHAR_getInt(charaindex, CHAR_FMINDEX),
              CHAR_getChar(charaindex, CHAR_NAME),
              CHAR_getChar(charaindex, CHAR_CDKEY), "LEADERCHANGE(族长让位)",
              buf);
  }
}
...
int Battle_RidePet(int meindex, int defNo, int cooltimeidx, int cooltime) {
  ...
  if (petno != -1) {
    ...
    CHAR_setWorkInt(cooltimeidx, CHAR_WORK_RIDE,
                    MAKE2VALUE(GETHIGHVALUE(temp1), cooltime));
    temp1 = CHAR_getWorkInt(meindex, CHAR_WORK_RIDE);
    CHAR_setWorkInt(meindex, CHAR_WORK_RIDE,
                    MAKE2VALUE(-1, GETLOWVALUE(temp1)));
    ...
    CHAR_setInt(meindex, CHAR_RIDENAMEFLG, 0);
    rideGraNo = FamilyRideFunction(meindex, petindex, petno);
    if (rideGraNo <= 0) {
      CHAR_talkToCli(meindex, -1, "进入战斗时未设置骑宠，无法进行骑乘！",
                     CHAR_COLORYELLOW);
      return -1;
    }
...
void setFamilyBadge(int charaindex) {
  if (CHAR_CHECKINDEX(index)) {
    int fmindex_wk = CHAR_getWorkInt(index, CHAR_WORKFMINDEXI);
    memberlist[fmindex_wk].badge;
  }
