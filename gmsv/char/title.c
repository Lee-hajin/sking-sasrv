...
char *TITLE_makeTitleStatusString(int charaindex, int havetitleindex) {
  ...
  case TITLE_FUNCTYPENONE:
    snprintf(TITLE_statusStringBuffer, sizeof(TITLE_statusStringBuffer), "%s",
...
BOOL TITLE_initTitleConfig(char *filename) {
  ...
  TITLE_ConfigTable =
      allocateMemory(sizeof(struct tagTITLE_configTable) * TITLE_titlecfgnum);
      ...
      for (i = 1;; i++) {
        ...
        if (strncmp("TITLE", token, 5) == 0) {
          ...
          for (j = 0; j < arraysizeof(TITLE_param); j++) {
            if (strncmp(TITLE_param[j].paramname, token,
          ...
          if (charInclude(&token[comppos + 1], "<>=") != -1) {
            ...
            if (j == 3) {
              ...
              ret = TITLE_getParamData(titlecfg_readlen, i - 1,
                                       &token[comppos + 2]);
              if (!ret) {
                fprint("文件语法错误:%s 第%d行\n", filename, linenum);
                TITLE_initTitleData(titlecfg_readlen);
 ...
 *  FALSE:窒手  井匀凶［
 *------------------------------------------------------------*/
static BOOL TITLE_TitleCheck_Main(int charaindex, BOOL mode, int *addcnt,
                                  int *delcnt) {
  int i, j, k, ret;
  ...
  *delcnt = 0;
  ...
  for (i = 0; i < TITLE_titlecfgnum; i++) {
    ...
    for (j = 0; j < arraysizeof(TITLE_ConfigTable[i].param) &&
      ...
      if (TITLE_param[index].checkfunc != NULL) {
        rc = TITLE_param[index].checkfunc(
            charaindex, TITLE_param[index].element,
            TITLE_ConfigTable[i].param[j], TITLE_ConfigTable[i].compareflg[j]);
        if (rc != TRUE)
          break;
      }
    }
...
#define TITLE_MSGUNIT2 "KO"
  ...
  if (rc) {
    if (delcnt > 0) {
      snprintf(msgbuf, sizeof(msgbuf), "失去%d%s 称号！", delcnt,
               delcnt < 10 ? TITLE_MSGUNIT1 : TITLE_MSGUNIT2);
      CHAR_talkToCli(charaindex, -1, msgbuf, CHAR_COLORYELLOW);
    }
    if (addcnt > 0) {
      snprintf(msgbuf, sizeof(msgbuf), "获得%d%s 称号！", addcnt,
 ...
 *  FALSE:窒手  井匀凶［
 *------------------------------------------------------------*/
BOOL TITLE_TitleCheck_Nomsg(int charaindex, BOOL mode, int *addcnt,
                            int *delcnt) {
 ...
 * -------------------------------------*/
static int TITLE_ItemCheckMain(int charaindex, int itemhaveindex, int *data,
                               int flg) {
  int i, j;
  int rc = FALSE;
...
static int TITLE_ItemCheck(int charaindex, int elem, int *data, int flg) {
  return (TITLE_ItemCheckMain(charaindex, CHAR_getMaxInventory(charaindex),
                              data, flg));
}
...
static int TITLE_SexCheck(int charaindex, int elem, int *data, int flg) {
  ...
  struct {
    int sex[25];
  } s_sex[] = {{{30008, 30009, 30010, 30011, 30012, 30013, 30014, 30015, 30020,
                 30021, 30022, 30023, 30024, 30025, 30026, 30027, 30028, 30029,
                 30030, 30031, 30032, 30033, 30034, 30035, -1}},
               {{30000, 30001, 30002, 30003, 30004, 30005, 30006, 30007, 30016,
                 30017, 30018, 30019, 30036, 30037, 30038, 30039, -1,    -1,
                 -1,    -1,    -1,    -1,    -1,    -1,    -1}}};
  for (i = 0; i < 2; i++) {
