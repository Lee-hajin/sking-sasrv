...
/* 愤坌及蟆卞簿手中卅井匀凶及匹｝失玉伊旦皮永弁卞馨笛匹五卅井匀凶
   午五及裔烂丢永本□斥 */
#define ADDRESSBOOK_CANTADD "疙窃背券阑 且 荐 绝嚼聪促。"
#define ADDRESSBOOK_CANTADD2 "疙窃背券阑 且 荐 绝嚼聪促。"

/* 簿井毛笛尹月仇午互匹五凶午五｝笛尹方丹午仄凶谛卞霜耨允月丢永本□斥*/
#define ADDRESSBOOK_ADDED "[%s]丛苞 疙窃背券阑 窍看嚼聪促 。"

/* 簿井卞涌毛创尹日木凶日 */
#define ADDRESSBOOK_BEINGADDED "[%s]丛苞 疙窃背券阑 窍看嚼聪促 。"
...
/* 丢永本□斥毛霜耨允月及卞岳  仄凶午五 */
#define ADDRESSBOOK_SENT "[%s]丛膊 皋老阑 傈价窍看嚼聪促 。"
...
#define ADDRESSBOOK_RETURNED1 "从%s寄来信件，但由於没有%s的名片又将信件退回。"

#define ADDRESSBOOK_RETURNED2                                                  \
  "寄信件给%s，但由於%s 没有名片，所以信件又被退回来了。"

#define PETMAIL_RETURNED1 "%s不在这个世界里，所以无法寄送信件给他。"
...
BOOL ADDRESSBOOK_sendMessage(int cindex, int aindex, char *text, int color) {
  ...
  snprintf(textbuffer, sizeof(textbuffer), "%2d/%02d %2d:%02d|%s|-1",
           tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, text);
  ...
  for (i = 0; i < playernum; i++) {
    if (CHAR_CHECKINDEX(i) &&
      ...
      int index_to_my_info =
          ADDRESSBOOK_getIndexInAddressbook(i, mycd, mycharaname);
      ...
      if (index_to_my_info < 0) {
        ...
        /* 霜曰潜卞手丢永本□斥 */
        CHAR_talkToCli(cindex, -1, tmpmsg, CHAR_COLORYELLOW);
        return FALSE;
      }
  ...
  saacproto_Message_send(acfd, mycd, mycharaname, ae->cdkey, ae->charname,
                         textbuffer, color);
  CHAR_setInt(cindex, CHAR_SENDMAILCOUNT,
              CHAR_getInt(cindex, CHAR_SENDMAILCOUNT) + 1);
...
#define ADDRESSBOOK_SYSTEM "system"
  ...
  for (i = 0; i < playernum; i++) {
    if (CHAR_CHECKINDEX(i) &&
      ...
      index_to_my_info =
          ADDRESSBOOK_getIndexInAddressbook(i, fromcdkey, fromcharaname);
      if (index_to_my_info < 0) {
        /*
         * 锹澎互愤坌毛坫壅仄化仄引匀化月［
         */

        snprintf(tmpmsg, sizeof(tmpmsg), ADDRESSBOOK_RETURNED2, tocharaname,
                 tocharaname);

        /* 霜曰潜卞手丢永本□斥 */
        saacproto_Message_send(acfd, ADDRESSBOOK_SYSTEM, ADDRESSBOOK_SYSTEM,
                               fromcdkey, fromcharaname, tmpmsg,
                               CHAR_COLORYELLOW);
        ...
        if (fd != -1) {
          ...
          /* 夫弘午曰 */
          printl(LOG_TALK, "CD=%s\tNM=%s\tT=%s", fromcdkey, fromcharaname,
                 text);
        }
      }
      break;
...
BOOL ADDRESSBOOK_addEntry(int meindex) {
  ...
  for (i = 0; i < found_count; i++) {
    ...
    if (ADDRESSBOOK_getIndexInAddressbook(
            meindex, tocd, CHAR_getChar(index, CHAR_NAME)) >= 0 &&
        ADDRESSBOOK_getIndexInAddressbook(
            index, mycd, CHAR_getChar(meindex, CHAR_NAME)) >= 0) {
      continue;
    }
  ...
  if (cnt == 1) {
    ADDRESSBOOK_addAddressBook(meindex, CONNECT_getTradecardcharaindex(fd, 0));
    return TRUE;
  } else if (cnt > 1) {
    int strlength;
    char msgbuf[1024];
    ...
    for (i = 0; CONNECT_getTradecardcharaindex(fd, i) != -1 &&
                i < CONNECT_WINDOWBUFSIZE;
         i++) {
      char *a = CHAR_getChar(CONNECT_getTradecardcharaindex(fd, i), CHAR_NAME);
      char buf[256];
    ...
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_SELECT, WINDOW_BUTTONTYPE_CANCEL,
                     CHAR_WINDOWTYPE_SELECTTRADECARD, -1,
...
static int ADDRESSBOOK_findBlankEntry(int cindex) {
  ...
  if (CHAR_CHECKINDEX(cindex) == FALSE)
    return -1;
  ...
  for (i = 0; i < playernum; i++) {
    if (CHAR_CHECKINDEX(i) && i != cindex) {
      ...
      for (j = 0; j < ADDRESSBOOK_MAX; j++) {
        ...
        if (ae && ae->use == TRUE && strcmp(ae->cdkey, cd) == 0 &&
  ...
  if (flg == 0) {
    ...
    for (i = 0; i < ADDRESSBOOK_MAX; i++) {
      ...
      if (j == playernum) {
        ...
        // saacproto_DBGetEntryString_send( acfd, DB_ADDRESSBOOK, escapebuf,
        // 0,0);
      }
    }
...
BOOL ADDRESSBOOK_sendAddressbookTableOne(int cindex, int num) {
  ...
  if (ae && ae->use) {
    char tmp[CHARNAMELEN + 32];
    char charname_escaped[CHARNAMELEN * 2];
    makeEscapeString(ae->charname, charname_escaped, sizeof(charname_escaped));
...
char *ADDRESSBOOK_makeAddressbookString(ADDRESSBOOK_entry *a) {
  ...
  snprintf(ADDRESSBOOK_returnstring, sizeof(ADDRESSBOOK_returnstring),
           "%s|%s|%d|%d|%d|%d", work1, work2, a->level, a->duelpoint,
           a->graphicsno, a->transmigration);
...
void ADDRESSBOOK_addAddressBook(int meindex, int toindex) {
  char tmpstring[CHARNAMELEN + ADDRESSBOOK_FIXEDMESSAGE_MAXLEN];
  ...
  myaddindex = ADDRESSBOOK_getIndexInAddressbook(
      meindex, cdkey, CHAR_getChar(toindex, CHAR_NAME));
  cdkey = CHAR_getChar(meindex, CHAR_CDKEY);
  toaddindex = ADDRESSBOOK_getIndexInAddressbook(
      toindex, cdkey, CHAR_getChar(meindex, CHAR_NAME));
  if (myaddindex < 0) {
  ...
  if (toaddindex < 0) {
    ...
    if (myaddindex < 0) {
      snprintf(tmpstring, sizeof(tmpstring), ADDRESSBOOK_TAKEADDRESS2,
               CHAR_getChar(meindex, CHAR_NAME));
      CHAR_talkToCli(toindex, -1, tmpstring, CHAR_COLORWHITE);
    }
  }
