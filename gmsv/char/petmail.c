...
BOOL PETMAIL_sendPetMail(int cindex, int aindex, int havepetindex,
                         int haveitemindex, char *text, int color) {
  struct tm tm1;
  Char *ch;
  ...
  if (CHAR_getWorkInt(cindex, CHAR_WORKTRADEMODE) != CHAR_TRADE_FREE) {
    CHAR_talkToCli(cindex, -1, "交易状态中无法传递宠物邮件。",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
#endif
  ...
  if (CHAR_getInt(cindex, CHAR_RIDEPET) == havepetindex) {
    CHAR_talkToCli(cindex, -1, "骑乘中的宠物无法传递宠物邮件！",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
  if (CHAR_getWorkInt(cindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE) {
    CHAR_talkToCli(cindex, -1, "战斗状态中无法传递宠物邮件。",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
...
#ifdef _PET_ITEM
    {
      ...
      for (i = 0; i < CHAR_MAXPETITEMHAVE; i++) {
        ...
        if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
          CHAR_talkToCli(cindex, -1, "宠物装备有绑定的道具无法传递邮件！",
                         CHAR_COLORYELLOW);
          return FALSE;
        }
      }
...
// end
#ifdef _MAP_TIME
  if ((CHAR_getInt(cindex, CHAR_FLOOR) >= 30017 &&
       CHAR_getInt(cindex, CHAR_FLOOR) <= 30021)) {
    char msgbuf[512];
    snprintf(msgbuf, sizeof(msgbuf), "此地区不得寄送道具！");
    CHAR_talkToCli(cindex, -1, msgbuf, CHAR_COLORWHITE);
    return FALSE;
  }
  if ((CHAR_getInt(i, CHAR_FLOOR) >= 30017 &&
       CHAR_getInt(i, CHAR_FLOOR) <= 30021)) {
    char msgbuf[512];
  ...
  // Syu Add 2003/05/30
  if ((CHAR_getInt(cindex, CHAR_FLOOR) >= 8200 &&
       CHAR_getInt(cindex, CHAR_FLOOR) <= 8213)) {
    char msgbuf[512];
  ...
  if (CHAR_getWorkInt(cindex, CHAR_PETMAILNUMS) > 6) {
    CHAR_talkToCli(cindex, -1, "邮件中宠物过多，暂时无法传递宠物邮件！",
                   CHAR_COLORYELLOW);
    return FALSE;
  } else if (PETMAIL_CheckIsMyOffmsg(cindex, ae->cdkey, ae->charname) > 5) {
    CHAR_talkToCli(cindex, -1,
                   "收件者目前邮件宠物超过5封，暂时无法传递宠物邮件！",
                   CHAR_COLORYELLOW);
    return FALSE;
    ...
    CHAR_talkToCli(cindex, -1, "目前系统邮件过多，请稍後再寄。",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
#endif
...
#endif
  {
    ...
    if (!PET_dropPetFLXY(cindex, havepetindex, PETMAIL_SPOOLFLOOR,
                         PETMAIL_SPOOLX, PETMAIL_SPOOLY)) {
    ...
    int offmsgindex;
    offmsgindex =
        PETMAIL_addOffmsg(cindex, ae->cdkey, ae->charname, text, color);
    if (offmsgindex == -1) {
    ...
    CHAR_sendPMEToArroundCharacter(cindex, petindex, 0,
                                   CHAR_getInt(petindex, CHAR_PETMAILEFFECT));
    ...
    CHAR_setInt(petindex, CHAR_PETMAILFROMFLOOR,
    ...
    LogPet(CHAR_getChar(cindex, CHAR_NAME), CHAR_getChar(cindex, CHAR_CDKEY),
           CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
           "寄宠邮", CHAR_getInt(cindex, CHAR_FLOOR),
           CHAR_getInt(cindex, CHAR_X), CHAR_getInt(cindex, CHAR_Y),
           CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, cindex);
#endif
    if (haveitemindex != -1) {
      ...
      LogItem(CHAR_getChar(cindex, CHAR_NAME), CHAR_getChar(cindex, CHAR_CDKEY),
              itemindex, "宠邮->寄送道具", CHAR_getInt(cindex, CHAR_FLOOR),
              CHAR_getInt(cindex, CHAR_X), CHAR_getInt(cindex, CHAR_Y),
              ITEM_getChar(itemindex, ITEM_UNIQUECODE),
              ITEM_getChar(itemindex, ITEM_NAME),
              ITEM_getInt(itemindex, ITEM_ID), cindex);

#ifdef _SAMETHING_SAVEPOINT
      if (CHAR_charSaveFromConnect(cindex, FALSE)) {
        CHAR_talkToCli(cindex, -1, "矫胶袍阑 磊悼栏肺 历厘钦聪促!",
                       CHAR_COLORRED);
      }
#endif
    }
    {
      ...
      sprintf(token, "寄送宠物邮件(%s)给%s。", CHAR_getUseName(petindex),
              CHAR_getUseName(tocharaindex));
...
BOOL PETMAIL_initOffmsgBuffer(int count) {
  ...
  snprintf(filename, sizeof(filename), "%s/%s", getStoredir(),
           PETMAILOFFMSGFILE);
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    ...
    strcpysafe(PETMAIL_offmsgbuf[index].text,
               sizeof(PETMAIL_offmsgbuf[index].text), buf);
    ...
    strcpysafe(PETMAIL_offmsgbuf[index].destcd,
               sizeof(PETMAIL_offmsgbuf[index].destcd), buf);
    ...
    strcpysafe(PETMAIL_offmsgbuf[index].srccharname,
               sizeof(PETMAIL_offmsgbuf[index].srccharname), buf);
    linenum++;
  }
  fclose(fp);
  return TRUE;
...
PETMAIL_offmsg *PETMAIL_getOffmsg(int offmsgindex) {
  if (offmsgindex < 0 || offmsgindex >= PETMAIL_offmsg_max)
    return NULL;
...
BOOL storePetmail(void) {
  ...
  for (i = 0; i < PETMAIL_offmsg_max; i++) {
    if (PETMAIL_offmsgbuf[i].use) {
      fprintf(fp, "%d|%ud|%d|%s|%s|%s|%s|%s|", i,
              (unsigned int)PETMAIL_offmsgbuf[i].send_tm,
              PETMAIL_offmsgbuf[i].color, PETMAIL_offmsgbuf[i].text,
              PETMAIL_offmsgbuf[i].destcd,
              makeEscapeString(PETMAIL_offmsgbuf[i].destcharname, escapebuf1,
                               sizeof(escapebuf1)),
              PETMAIL_offmsgbuf[i].srccd,
              makeEscapeString(PETMAIL_offmsgbuf[i].srccharname, escapebuf2,
                               sizeof(escapebuf2)));
    }
  }
  fclose(fp);
...
static int PETMAIL_getIdleTime(int index) {
  /*
  #define		PETMAIL_DIVRANGE		25		//
  坌喃允月汹
  #define		PETMAIL_IDLELEVELRANGE	10		//	坌喃允月
  区 #define		PETMAIL_IDLEUNITTIME	3		//	棉厥 int
  d; int		dex = CHAR_getWorkInt( index, CHAR_WORKQUICK) ;
...
static void PETMAIL_sendPetmail(int index, int tocharaindex) {
  ...
  int itemindex, ret;
  index_to_my_info = ADDRESSBOOK_getIndexInAddressbook(
      tocharaindex, CHAR_getChar(index, CHAR_OWNERCDKEY),
  ...
  if (index_to_my_info < 0) {
    ...
    snprintf(msgbuf, sizeof(msgbuf),
             "%s的%s 终於来了！"
             "由於对方没有您的名片，所以信件被退回了。",
             CHAR_getChar(index, CHAR_OWNERCHARANAME), CHAR_getUseName(index));
  ...
  // Syu Add 06/16
    ...
    if (ret < 0 || ret >= CHAR_getMaxInventory(tocharaindex)) {
      ...
      LogItem(CHAR_getChar(tocharaindex, CHAR_NAME),
              CHAR_getChar(tocharaindex, CHAR_CDKEY), itemindex,
              "宠邮->道具栏已满，放置地上", CHAR_getInt(index, CHAR_FLOOR),
              CHAR_getInt(index, CHAR_X), CHAR_getInt(index, CHAR_Y),
              ITEM_getChar(itemindex, ITEM_UNIQUECODE),
              ITEM_getChar(itemindex, ITEM_NAME),
              ITEM_getInt(itemindex, ITEM_ID), tocharaindex);
    } else {
      CHAR_setItemIndex(index, CHAR_STARTITEMARRAY, -1);
      CHAR_sendItemDataOne(tocharaindex, ret);
      LogItem(CHAR_getChar(tocharaindex, CHAR_NAME),
              CHAR_getChar(tocharaindex, CHAR_CDKEY), itemindex,
              "宠邮->收到道具", CHAR_getInt(index, CHAR_FLOOR),
              CHAR_getInt(index, CHAR_X), CHAR_getInt(index, CHAR_Y),
              ITEM_getChar(itemindex, ITEM_UNIQUECODE),
              ITEM_getChar(itemindex, ITEM_NAME),
              ITEM_getInt(itemindex, ITEM_ID), tocharaindex);
    }
    offmsg = PETMAIL_getOffmsg(CHAR_getInt(index, CHAR_PETMAILBUFINDEX));
    memcpy(&tm1, localtime((time_t *)&NowTime.tv_sec), sizeof(tm1));
    snprintf(
        textbuffer, sizeof(textbuffer), "%2d/%02d %2d:%02d|%s|%d|%d|%s|%d",
        tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, offmsg->text,
        CHAR_getInt(index, CHAR_BASEIMAGENUMBER), CHAR_getInt(index, CHAR_LV),
        makeEscapeString(CHAR_getUseName(index), escapebuf, sizeof(escapebuf)),
        ITEM_getInt(itemindex, ITEM_BASEIMAGENUMBER));
    fd = getfdFromCharaIndex(tocharaindex);
    if (fd != -1)
      lssproto_MSG_send(fd, index_to_my_info, textbuffer, offmsg->color);
                ...
                CHAR_getInt(index, CHAR_PETSENDMAILCOUNT) + 1);
    {
      ...
      if (cdkey)
        printl(LOG_TALK, "CDKEY=%s\tTEXT=%s", cdkey, offmsg->text);
      else
        printl(LOG_TALK, "CDKEY=(null)\tTEXT=%s", offmsg->text);
    }
  }
}
...
static void PETMAIL_IdleProc1(int index) {
  ...
  if (!CHAR_CHECKINDEX(tocharaindex)) {
    ...
    if (CHAR_getInt(tocharaindex, CHAR_FLOOR) == 8215) { //客服活动 andy
      warp = TRUE;
    } else if (CHAR_getWorkInt(tocharaindex, CHAR_WORKTRADEMODE) !=
               CHAR_TRADE_FREE) { //交易中不收邮件
      warp = TRUE;
    } else if (CHAR_getWorkInt(tocharaindex, CHAR_WORKBATTLEMODE) !=
  ...
  if (warp) {
    if (CHAR_getInt(index, CHAR_FLOOR) != PETMAIL_SPOOLFLOOR) {
      CHAR_warpToSpecificPoint(index, PETMAIL_SPOOLFLOOR, PETMAIL_SPOOLX,
                               PETMAIL_SPOOLX);
  ...
  if (tocharaindex == -1) {
    ...
    if (CHAR_getWorkInt(tocharaindex, CHAR_WORKBATTLEMODE) ==
      ...
      if (distance > CHAR_DEFAULTSEESIZ / 2) {
        ...
        warp = TRUE;
        /*				CHAR_setInt( index,
           CHAR_MAILMODE,CHAR_PETMAIL_IDLE5); CHAR_setInt( index,
           CHAR_PETMAILIDLETIME, NowTime.tv_sec); CHAR_setInt( index,
           CHAR_LOOPINTERVAL, PETMAIL_LOOPINTERVAL2); PETMAIL_returnMail( index,
           tocharaindex);
        */
      }
    }
...
static void PETMAIL_IdleProc4(int index) {
  unsigned int t = CHAR_getInt(index, CHAR_PETMAILIDLETIME);
  {
    ...
    if (tocharaindex != -1) {
      if (CHAR_getWorkInt(tocharaindex, CHAR_WORKBATTLEMODE) ==
              BATTLE_CHARMODE_NONE &&
          CHAR_getWorkInt(tocharaindex, CHAR_WORKTRADEMODE) ==
              CHAR_TRADE_FREE) {
      ...
      if (NowTime.tv_sec > t + PETMAIL_IDLE_RETURNOWNER) { //检查等待主人时间
        ...
        itemindex =
            CHAR_getItemIndex(index, CHAR_STARTITEMARRAY); //删除附带道具
        if (ITEM_CHECKINDEX(itemindex)) {
          CHAR_DropItem(index, CHAR_STARTITEMARRAY);

          LogItem(CHAR_getChar(index, CHAR_OWNERCHARANAME),
                  CHAR_getChar(index, CHAR_OWNERCDKEY), itemindex,
                  "宠邮->逾时删除道具", CHAR_getInt(index, CHAR_FLOOR),
                  CHAR_getInt(index, CHAR_X), CHAR_getInt(index, CHAR_Y),
                  ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                  ITEM_getChar(itemindex, ITEM_NAME),
                  ITEM_getInt(itemindex, ITEM_ID), index);
        }
      }
    }
  }
...
static void PETMAIL_IdleProc5(int index) {
  ...
  if (NowTime.tv_sec > t + PETMAIL_IDLEDISCARD) {
    ...
    LogPet("系统", "Watchfunc", CHAR_getChar(index, CHAR_NAME),
           CHAR_getInt(index, CHAR_LV), "系统扣留-宠邮自由宠",
           CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
           CHAR_getInt(index, CHAR_Y), CHAR_getChar(index, CHAR_UNIQUECODE),
           index, -1);
    // print("系统扣留-宠邮自由宠:%s\n", CHAR_getUseName( index));
    CHAR_CharaDelete(index);
#else
    LogPet(CHAR_getChar(index, CHAR_OWNERCHARANAME),
           CHAR_getChar(index, CHAR_OWNERCDKEY), CHAR_getChar(index, CHAR_NAME),
           CHAR_getInt(index, CHAR_LV), "宠邮变回自由宠",
           CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
           CHAR_getInt(index, CHAR_Y), CHAR_getChar(index, CHAR_UNIQUECODE),
           index, -1);
#endif
...
static void PETMAIL_returnMail(int index, int tocharaindex) {
  ...
  if (ITEM_CHECKINDEX(itemindex)) {
    ...
    if (ret < 0 || ret >= CHAR_getMaxInventory(tocharaindex)) {
      ...
      LogItem(CHAR_getChar(tocharaindex, CHAR_NAME),
              CHAR_getChar(tocharaindex, CHAR_CDKEY), itemindex,
              "宠邮->道具已满将道具放置地", CHAR_getInt(index, CHAR_FLOOR),
              CHAR_getInt(index, CHAR_X), CHAR_getInt(index, CHAR_Y),
              ITEM_getChar(itemindex, ITEM_UNIQUECODE),
              ITEM_getChar(itemindex, ITEM_NAME),
              ITEM_getInt(itemindex, ITEM_ID), tocharaindex);
  ...
  if (CHAR_getInt(index, CHAR_PETMAILBUFINDEX) != -1) {
    ...
    snprintf(msgbuf, sizeof(msgbuf), "%s 回来了！", CHAR_getUseName(index));
    CHAR_talkToCli(tocharaindex, -1, msgbuf, CHAR_COLORWHITE);
  }
...
void CHAR_AutoPickupMailPet(int charaindex, int petindex) {
  ...
  if (CHAR_getWorkInt(charaindex, CHAR_WORKTRADEMODE) != CHAR_TRADE_FREE) {
    CHAR_talkToCli(charaindex, -1, "交易状态中无法自动拾回邮件宠物！",
                   CHAR_COLORYELLOW);
    return;
  }
#endif
  ...
  if (havepetindex == -1) {
    ...
    CHAR_talkToCli(charaindex, -1, "宠物栏已满！无法自动拾回邮件宠物！",
                   CHAR_COLORYELLOW);
    // WON ADD
    LogPet(CHAR_getChar(charaindex, CHAR_NAME),
           CHAR_getChar(charaindex, CHAR_CDKEY),
           CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
           "宠物栏已满，无法拾起邮寄宠", CHAR_getInt(charaindex, CHAR_FLOOR),
  ...
  LogPet(CHAR_getChar(charaindex, CHAR_NAME),
         CHAR_getChar(charaindex, CHAR_CDKEY),
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "自动收回邮宠", CHAR_getInt(charaindex, CHAR_FLOOR),
         CHAR_getInt(charaindex, CHAR_X), CHAR_getInt(charaindex, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, charaindex);
  return;
}
...
int PETMAIL_getPetMailTotalnums() {
  ...
  if (PetMailTotalnums < 0)
    PetMailTotalnums = 0;
  if (PetMailTotalnums >= PETMAIL_DEFTOTALNUM)
    PetMailTotalnums = PETMAIL_DEFTOTALNUM;
...
void PETMAIL_setPetMailTotalnums(int numflg) {
  ...
  if (PetMailTotalnums < 0)
    PetMailTotalnums = 0;
  if (PetMailTotalnums > PETMAIL_DEFTOTALNUM + 10)
    PetMailTotalnums = PETMAIL_DEFTOTALNUM + 5;
}
