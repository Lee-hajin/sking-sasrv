...
BOOL PetTalk_CheckMyType(int meindex, int toindex, int kosuu, int flg,
                         int Type);
...
#define RAND(x, y)                                                             \
  ...
  for (i = 0; i < PETTALK_MAXID; i++) {
    if (pettalktext[i].ID >= 0 &&
        pettalktext[i].ID == CHAR_getInt(meindex, CHAR_PETID)) {
      if (strcmp(pettalktext[i].DATA, "\0") &&
          strlen(pettalktext[i].DATA) > 0) {
  ...
  //设定为非主人不得与PET互动
  if (strcmp(CHAR_getChar(meindex, CHAR_OWNERCDKEY),
             CHAR_getChar(talkerindex, CHAR_CDKEY)) ||
      strcmp(CHAR_getChar(meindex, CHAR_OWNERCHARANAME),
             CHAR_getChar(talkerindex, CHAR_NAME))) {
#ifdef _PET_TALKPRO
    if (NPC_Util_GetStrFromStrWithDelim(pettalktext[tPage].DATA, "NoPlayerMsg",
                                        buf3, sizeof(buf3)) != NULL) {
#else
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "NoPlayerMsg", buf3,
                                        sizeof(buf3)) != NULL) {
#endif
    } else {
...
#endif
    talkNo++;
    if ((strstr(buf2, TalkType[Type]) == NULL) &&
        (strstr(buf2, TalkType[2]) == NULL))
      continue;
    if (NPC_Util_GetStrFromStrWithDelim(buf2, "FLOOR", buf3, sizeof(buf3)) !=
        NULL) { //判断房间号
      if (PetTalk_CheckMyFloor(meindex, talkerindex, buf3, 0) == FALSE)
        continue;
    }
    if (NPC_Util_GetStrFromStrWithDelim(buf2, "PET", buf3, sizeof(buf3)) !=
        NULL) { //判断宠物FREE条件
      if (PetTalk_CheckFree(meindex, meindex, buf3) != TRUE)
        continue;
    }
    if (NPC_Util_GetStrFromStrWithDelim(buf2, "FREE", buf3, sizeof(buf3)) ==
        NULL)
      continue;
    if (NPC_ActionPassCheck(meindex, talkerindex, buf3) == FALSE)
      continue; //判断玩家FREE条件
    //		if( ActionNpc_CheckFree( meindex, talkerindex, buf2, 0) == FALSE )
    //continue;
    if (PetTalk_CheckPetEvent(meindex, talkerindex, buf2) == FALSE)
      continue;
    FREEs = TRUE; //条件成立
    strcpy(AllTalk[j++], buf2);
    if (j > PETTALK_MAXID - 1)
      break;
  }
  talkNo = 0;
  ...
  if (j > 0) {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf2, "TalkMsg", buf3, sizeof(buf3)) ==
        NULL) {
...
BOOL PetTalk_RunEvent(int meindex, int talker, char *buf) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "RandItem", buf1, sizeof(buf1)) !=
      NULL) {
    if (RAND(0, 10) > 9) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "LimitLevel", buf1, sizeof(buf1)) !=
      NULL) {
        ...
        if (ITEM_CHECKINDEX(itemindex)) {
          ...
          if (itemno == id) {
            ...
            LogItem(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
                    ...
                    "NPC收道具后传至某点", CHAR_getInt(talker, CHAR_FLOOR),
...
BOOL PetTalk_BSCheck(int meindex, int talker, char *buf) {
  ...
  if (strstr(buf, "<") != NULL) {
    ...
    } else if (PetTalk_FreeIfCheck(meindex, talker, buff2, kosuu, flg, temp) ==
               TRUE) {
      return TRUE;
    }
  }
...
BOOL PetTalk_CheckMyPet(int meindex, int talker, int petLv, int flg,
                        int petid) {
  ...
  if (petsel == CHAR_MAXPETHAVE) {
    ...
    if (PetTalk_BigSmallLastCheck(petLv, CHAR_getInt(petindex, CHAR_LV), flg) ==
        TRUE)
      return TRUE;
  }
  return FALSE;
...
int PET_CleanPetdeletetime(int objmeindex) {
  ...
  if (OBJECT_getType(objmeindex) != OBJTYPE_CHARA)
    return 0;
  ...
  } else if (CHAR_getWorkInt(pindex, CHAR_WORKPETFOLLOWMODE) ==
             CHAR_PETFOLLOW_NOW) {
  } else {
#ifdef _PET_LOSTPET
//		CHAR_CharSaveLostPet( pindex, 0);
#endif
    LogPet("系统", "Watchfunc", CHAR_getChar(pindex, CHAR_NAME),
           CHAR_getInt(pindex, CHAR_LV), "系统清除-地上自由宠",
           CHAR_getInt(pindex, CHAR_FLOOR), CHAR_getInt(pindex, CHAR_X),
           CHAR_getInt(pindex, CHAR_Y), CHAR_getChar(pindex, CHAR_UNIQUECODE),
           pindex, -1);
...
void PET_CHECKFreePetIsIt(int petindex) {
  ...
  if (NowTime.tv_sec < (petputtime + getPetdeletetime()))
    return;
#ifdef _PET_LOSTPET
//	CHAR_CharSaveLostPet( petindex, 0);
#endif

  LogPet("系统", "Watchfunc", CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "系统清除-地上自由宠",
         CHAR_getInt(petindex, CHAR_FLOOR), CHAR_getInt(petindex, CHAR_X),
         CHAR_getInt(petindex, CHAR_Y), CHAR_getChar(petindex, CHAR_UNIQUECODE),
         petindex, -1);
...
void PET_Watchfunc(int objmeindex, int objmoveindex, CHAR_ACTION act, int x,
  ...
  if (CHAR_getInt(petindex, CHAR_MAILMODE) == CHAR_PETMAIL_IDLE3 ||
    ...
    if (act == CHAR_ACTWALK && CHAR_CHECKINDEX(workpindex) &&
        workpindex == pindex) {
  ...
  if (CHAR_getInt(petindex, CHAR_MAILMODE) != CHAR_PETMAIL_NONE) {
    //宠邮不处理
  } else if (CHAR_getWorkInt(petindex, CHAR_WORKPETFOLLOWMODE) ==
             CHAR_PETFOLLOW_NOW) {
    if (NowTime.tv_sec >= (petputtime + 60 * 60)) {
      ...
      if (CHAR_CHECKINDEX(ownerindex)) {
        ...
        CHAR_talkToCli(ownerindex, -1, "溜宠太久，宠物走失了！！",
                       CHAR_COLORYELLOW);
      }
#ifdef _PET_LOSTPET
      CHAR_CharSaveLostPet(petindex, 1);
      LogPet("系统", "Watchfunc", CHAR_getChar(petindex, CHAR_NAME),
             CHAR_getInt(petindex, CHAR_LV), "系统扣留-溜宠自由宠",
             CHAR_getInt(petindex, CHAR_FLOOR), CHAR_getInt(petindex, CHAR_X),
             CHAR_getInt(petindex, CHAR_Y),
             CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, -1);
      ...
      LogPet(CHAR_getChar(pindex, CHAR_NAME), CHAR_getChar(pindex, CHAR_CDKEY),
             CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
             "溜宠太久，宠物走失", CHAR_getInt(pindex, CHAR_FLOOR),
             CHAR_getInt(pindex, CHAR_X), CHAR_getInt(pindex, CHAR_Y),
             CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, pindex);
#endif
      return;
    }
