...
static BOOL ENEMY_CHECKINTDATAINDEX(int index) {
  if (index < 0 || index >= ENEMY_DATAINTNUM)
    return FALSE;
  return TRUE;
}

static BOOL ENEMY_CHECKCHARDATAINDEX(int index) {
...
int *ENEMY_getIntdata(int index) { return ENEMY_enemy[index].intdata; }
...
static BOOL ENEMYTEMP_CHECKINTDATAINDEX(int index) {
  if (index < 0 || index >= E_T_DATAINTNUM)
    return FALSE;
  return TRUE;
}

static BOOL ENEMYTEMP_CHECKCHARDATAINDEX(int index) {
...
BOOL ENEMYTEMP_setChar(int index, ENEMYTEMP_DATACHAR element,
                              char *new) {
  if (!ENEMYTEMP_CHECKINDEX(index))
    return FALSE;
  if (!ENEMYTEMP_CHECKCHARDATAINDEX(element))
...
BOOL GROUP_CHECKINDEX(int index) {
  if (index < 0 || index >= GROUP_groupnum)
    return FALSE;
  return TRUE;
}

static BOOL GROUP_CHECKINTDATAINDEX(int index) {
  if (index < 0 || index >= GROUP_DATAINTNUM)
    return FALSE;
  return TRUE;
}

static BOOL GROUP_CHECKCHARDATAINDEX(int index) {
...
BOOL ENEMYTEMP_initEnemy(char *filename) {
  ...
  char line[256];
  int linenum = 0;           //行数助记用变量
  int enemytemp_readlen = 0; //有效宠物数量变量
  int i, j;
  ...
  linenum = 0;
#ifdef _NB_SQL_EnemyBase
  for (j = 0; j < ENEMYTEMP_enemynum; j++) {
    char buf[2048];
    sprintf(buf, "%s", sasql_getEnemybaseData(j));
    {
      char token[256];
      int ret;
      //给CHAR常量赋值
      for (i = 0; i < E_T_DATACHARNUM; i++) {
        ret =
            getStringFromIndexWithDelim(buf, ",", i + 1, token, sizeof(token));
        if (ret == FALSE) {
...
#define ENEMYTEMP_STARTINTNUM (E_T_DATACHARNUM + 1)
      ...
      for (i = ENEMYTEMP_STARTINTNUM;
        ...
        if (strlen(token) != 0) {
          ENEMYTEMP_setInt(enemytemp_readlen, i - ENEMYTEMP_STARTINTNUM,
                           atoi(token));
        }
      }
      enemytemp_readlen++;
...
BOOL ENEMY_initEnemy(char *filename) {
...
#ifdef _NB_SQL_Enemy
  ENEMY_enemynum = 0;
  ENEMY_enemynum = sasql_getDateNum("Data_enemy");
  ...
  {
    for (j = 0; j < ENEMY_enemynum; j++) {
      char buf[2048];
      sprintf(buf, "%s", sasql_getEnemyData(j));
      char token[256];
      int ret;
...
#define ENEMY_STARTINTNUM 3
#endif
      for (i = ENEMY_STARTINTNUM; i < ENEMY_DATAINTNUM + ENEMY_STARTINTNUM;
           i++) {
        ret = getStringFromIndexWithDelim(buf, ",", i, token, sizeof(token));
        if (ret == FALSE) {
          fprint("数据语法错误:[第%d个数据第%d个字段]%s\n", j, i, buf);
      ...
      int enemyid = -1;
      for (i = 0; i < ENEMYTEMP_enemynum; i++) {
        enemybaseid = ENEMYTEMP_getInt(i, E_T_TEMPNO);
        enemyid = ENEMY_getInt(enemy_readlen, ENEMY_TEMPNO);
        if (enemybaseid == enemyid) {
          break;
        }
      }
...
#define ENEMY_STARTINTNUM 3
      ...
      for (i = 0; i < ENEMYTEMP_enemynum; i++) {
        if (ENEMYTEMP_getInt(i, E_T_TEMPNO) ==
            ENEMY_getInt(enemy_readlen, ENEMY_TEMPNO)) {
          break;
        }
      }
      if (i == ENEMYTEMP_enemynum) {
...
#define GROUP_STARTINTNUM 2
      for (i = GROUP_STARTINTNUM; i < GROUP_DATAINTNUM + GROUP_STARTINTNUM;
        ...
        if (strlen(token) != 0) {
          ...
          if (i - GROUP_STARTINTNUM >= 3 && i - GROUP_STARTINTNUM <= 12) {
            if (checkNoEnemyIdGroup(atoi(token)) != TRUE) {
...
int ENEMY_getRank(int array, int tarray) {
  ...
  struct {
    ...
    float rank;
      ...
      {95, 2.0},  {90, 1.5}, {85, 1.0}, {80, 0.5}, {0, 0.0},
  };
  ...
  /* 仿件弁毛煌遥允月 */
  /* 仇及  及酷  反酷  踏毛辅哔及仪［ */
  paramsum = *(tp + E_T_BASEVITAL) + *(tp + E_T_BASESTR) + *(tp + E_T_BASETGH) +
             *(tp + E_T_BASEDEX);
...
int ENEMY_createEnemy(int array, int baselevel) {
  ...
  int enemyrank;
#ifdef _CHAR_PETDATA
  int basevit, basestr, basetgh, basedex;
#endif
...
#define PARAM_CAL(l)                                                           \
...
#ifdef _CHAR_PETDATA
  basevit = tp[E_T_BASEVITAL];
  basestr = tp[E_T_BASESTR];
  basetgh = tp[E_T_BASETGH];
  basedex = tp[E_T_BASEDEX];
#endif

#ifdef _NB_LV1PETBASE
  if (level == 1) { //如果怪物等级为1时，提高成长率
    tp[E_T_BASEVITAL] += RAND(0, 2);
    tp[E_T_BASESTR] += RAND(0, 2);
    tp[E_T_BASETGH] += RAND(0, 2);
    tp[E_T_BASEDEX] += RAND(0, 2);
  } else {
#endif
  ...
  /* 由仿丢□正本永玄 */
  CharNew.data[CHAR_VITAL] = PARAM_CAL(E_T_BASEVITAL); //血成长档
  CharNew.data[CHAR_STR] = PARAM_CAL(E_T_BASESTR);     //攻成长档
  CharNew.data[CHAR_TOUGH] = PARAM_CAL(E_T_BASETGH);   //防成长档
  CharNew.data[CHAR_DEX] = PARAM_CAL(E_T_BASEDEX);     //敏成长档
  CharNew.data[CHAR_FIREAT] = *(tp + E_T_FIREAT);      //火属性
  CharNew.data[CHAR_WATERAT] = *(tp + E_T_WATERAT);    //水属性
  CharNew.data[CHAR_EARTHAT] = *(tp + E_T_EARTHAT);    //地属性
  CharNew.data[CHAR_WINDAT] = *(tp + E_T_WINDAT);      //风属性
  ...
  CharNew.data[CHAR_SLOT] = *(tp + E_T_SLOT);             //技能格
  CharNew.data[CHAR_POISON] = *(tp + E_T_POISON);         //毒抗
  CharNew.data[CHAR_PARALYSIS] = *(tp + E_T_PARALYSIS);   //麻抗
  CharNew.data[CHAR_SLEEP] = *(tp + E_T_SLEEP);           //睡抗
  CharNew.data[CHAR_STONE] = *(tp + E_T_STONE);           //石抗
  CharNew.data[CHAR_DRUNK] = *(tp + E_T_DRUNK);           //醉抗
  CharNew.data[CHAR_CONFUSION] = *(tp + E_T_CONFUSION);   //乱抗
  CharNew.data[CHAR_RARE] = *(tp + E_T_RARE);             //珍稀度
  CharNew.data[CHAR_PETID] = *(tp + E_T_TEMPNO);          //编号
  CharNew.data[CHAR_CRITIAL] = *(tp + E_T_CRITICAL);      //会心率
  CharNew.data[CHAR_CRIT_RATE] = *(tp + E_T_CRITICAL);    //会心率
  CharNew.data[CHAR_COUNTER] = *(tp + E_T_COUNTER);       //反击率
  CharNew.data[CHAR_HIT_RATE] = *(tp + E_T_COUNTER);      //反击率
  CharNew.data[CHAR_LIMITLEVEL] = *(tp + E_T_LIMITLEVEL); //级限
#ifdef _CHAR_PETDATA
  CharNew.data[CHAR_DATAALL] = *(tp + E_T_INITNUM);
  CharNew.data[CHAR_DATAVIT] = basevit;
  CharNew.data[CHAR_DATASTR] = basestr;
  CharNew.data[CHAR_DATATGH] = basetgh;
  CharNew.data[CHAR_DATADEX] = basedex;
#endif
#ifdef _PET_FUSION
  CharNew.data[CHAR_FUSIONCODE] = *(tp + E_T_FUSIONCODE); //融合码
#endif
#ifdef _LOCK_PET_ITEM
  if (ENEMY_enemy[array].chardata[ENEMY_NAME].string[0] == '*') {
    CharNew.data[CHAR_PETLOCK] = 1; //绑定状态
  }
#endif
...
#undef PARAM_CAL
...
#ifdef _CHAR_PETDATA //
        CHAR_setInt(newindex, CHAR_PETDATAH,
                    CHAR_getWorkInt(newindex, CHAR_WORKMAXHP));
        CHAR_setInt(newindex, CHAR_PETDATAS,
                    CHAR_getWorkInt(newindex, CHAR_WORKATTACKPOWER));
        CHAR_setInt(newindex, CHAR_PETDATAT,
                    CHAR_getWorkInt(newindex, CHAR_WORKDEFENCEPOWER));
        CHAR_setInt(newindex, CHAR_PETDATAD,
                    CHAR_getWorkInt(newindex, CHAR_WORKQUICK));
#endif
      }
    }
  }
...
#ifdef _ENEMY_FALLGROUND
  {
    ...
    for (i = 0; i < arraysizeof(ridePetTable); i++) {
      if (CHAR_getInt(newindex, CHAR_BASEBASEIMAGENUMBER) ==
  ...
  CHAR_setWorkInt(newindex, CHAR_WORK_PROVOKEFIGHT, 0);
#endif
#ifdef _PETSKILL_RIDE
  CHAR_setWorkInt(newindex, CHAR_WORK_RIDE, MAKE2VALUE(-1, 0));
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
  CHAR_setWorkInt(newindex, CHAR_WORK_ABSOLUTE, 0);
  CHAR_setWorkInt(newindex, CHAR_WORK_ABSOLUTE2, 0);
#endif
#ifdef _PRO3_ADDSKILL
  CHAR_setWorkInt(newindex, CHAR_WORK_CRACKBONE, 0);
  CHAR_setWorkInt(newindex, CHAR_WORK_CRACKBONE2, 0);     //裂骨断筋
  CHAR_setWorkInt(newindex, CHAR_WORK_POISON_DRAGNET, 0); //毒素之网
  CHAR_setWorkInt(newindex, CHAR_WORK_PACIFY, 0);         //抚慰心灵
  CHAR_setWorkInt(newindex, CHAR_WORK_DARK_SEAL, 0);      //?灵封印
#endif
#ifdef _NB_PETINITBILITY // 记录宠物初始能力值变量
  CHAR_setInt(newindex, CHAR_PETINITLV, CHAR_getInt(newindex, CHAR_LV));
  CHAR_setInt(newindex, CHAR_PETATT1,
              CHAR_getWorkInt(newindex, CHAR_WORKMAXHP));
  CHAR_setInt(newindex, CHAR_PETATT2,
              CHAR_getWorkInt(newindex, CHAR_WORKATTACKPOWER));
  CHAR_setInt(newindex, CHAR_PETATT3,
              CHAR_getWorkInt(newindex, CHAR_WORKDEFENCEPOWER));
  CHAR_setInt(newindex, CHAR_PETATT4,
              CHAR_getWorkInt(newindex, CHAR_WORKQUICK));
#endif
  return newindex;
}
...
#undef PARAM_CAL
  ...
  CHAR_complianceParameter(newindex);
#ifdef _CHAR_PETDATA
  CHAR_setInt(newindex, CHAR_DATAVIT, basevit);
  CHAR_setInt(newindex, CHAR_DATASTR, basestr);
  CHAR_setInt(newindex, CHAR_DATATGH, basetgh);
  CHAR_setInt(newindex, CHAR_DATADEX, basedex);
  CHAR_setInt(newindex, CHAR_PETDATAH,
              CHAR_getWorkInt(newindex, CHAR_WORKMAXHP));
  CHAR_setInt(newindex, CHAR_PETDATAS,
              CHAR_getWorkInt(newindex, CHAR_WORKATTACKPOWER));
...
int ENEMY_TEST_createPetIndex(int array) {
  ...
  int basevit, basestr, basetgh, basedex, baseall;
...
#ifdef _CHAR_PETDATA
  CharNew.data[CHAR_DATAALL] = baseall;
  CharNew.data[CHAR_DATAVIT] = basevit;
  CharNew.data[CHAR_DATASTR] = basestr;
  CharNew.data[CHAR_DATATGH] = basetgh;
  CharNew.data[CHAR_DATADEX] = basedex;
...
#ifdef _PET_EVOLUTION
#ifdef _PET_FUSION_2
  if ((base2 = EVOLUTION_getPetTable(charaindex, petindex1, petindex2, type)) <
      0) {
#else
...
#ifdef _PET_FUSION_2
  int fusiontype = CHAR_getInt(baseindex, CHAR_FUSIONTYPE);
#endif
  Char CharNew;
  int newindex;
  int *p;
...
#define PARAM_CAL(l)                                                           \
  ...
  //------------------------------------------------------
  {
    ...
    for (i = 0; i < CHAR_MAXPETSKILLHAVE; i++) { //宠技
      ...
      if (FreeFusionSkill(charaindex, petskill[i]) == 1) {
...
#ifdef _PET_FUSION_2
  CHAR_setInt(newindex, CHAR_FUSIONTYPE, fusiontype);
#endif
...
#ifdef _PET_FUSION_LUA
      if (FreeFusionSkill(toindex, skill1[i]) == 0) {
        skill1[i] = -1;
      }
#else
      for (j = 0; j < 15; j++) { //检查非法技能
...
#ifdef _PET_FUSION_2 // 设定喂养次数
  if (fusion_type == FUSIONTYPE_LAND)
    CHAR_setInt(petindex, CHAR_FUSIONRAISE, PETFEEDCOUNT);
  else if (fusion_type == FUSIONTYPE_OCEAN)
    CHAR_setInt(petindex, CHAR_FUSIONRAISE, PETFEEDCOUNT2);
#else
...
#endif
#ifdef _NB_PETINITBILITY // 记录宠物初始能力值变量
  CHAR_setInt(petindex, CHAR_PETINITLV, CHAR_getInt(petindex, CHAR_LV));

  CHAR_setInt(petindex, CHAR_PETATT1,
              CHAR_getWorkInt(petindex, CHAR_WORKMAXHP));
  CHAR_setInt(petindex, CHAR_PETATT2,
              CHAR_getWorkInt(petindex, CHAR_WORKATTACKPOWER));
  CHAR_setInt(petindex, CHAR_PETATT3,
              CHAR_getWorkInt(petindex, CHAR_WORKDEFENCEPOWER));
  CHAR_setInt(petindex, CHAR_PETATT4,
...
BOOL PETFUSION_AddEgg(int toindex, int petID, int PetCode) {
  ...
  LogPet(CHAR_getChar(toindex, CHAR_NAME), CHAR_getChar(toindex, CHAR_CDKEY),
         ...
         "融合获得蛋", CHAR_getInt(toindex, CHAR_FLOOR),
         CHAR_getInt(toindex, CHAR_X), CHAR_getInt(toindex, CHAR_Y),
         CHAR_getChar(petindex2, CHAR_UNIQUECODE), petindex2, toindex);
...
#define PARAM_CAL(l) ((level - 1) * ET_PAR(E_T_LVUPPOINT) + ET_PAR(E_T_INITNUM))
  ...
  work[0] += FreeCtrlTransDevelop(toindex, petindex, 0) - 2;
  work[1] += FreeCtrlTransDevelop(toindex, petindex, 1) - 2;
  work[2] += FreeCtrlTransDevelop(toindex, petindex, 2) - 2;
  work[3] += FreeCtrlTransDevelop(toindex, petindex, 3) - 2;
#else
  work[0] += (RAND(0, 4) - 2);
  work[1] += (RAND(0, 4) - 2);
  work[2] += (RAND(0, 4) - 2);
...
int PETFUSION_LuaSetNewEgg(int toindex, int petindex, int array, int petwork) {
  int *p;
  int tp[E_T_DATAINTNUM];
  int tarray, i;
  int level, workrank, petrank = 0;
  int LevelUpPoint;
  int work[4] = {0, 0, 0, 0};
  work[0] = ((petwork >> 24) & 0xFF);
  work[1] = ((petwork >> 16) & 0xFF);
  work[2] = ((petwork >> 8) & 0xFF);
  work[3] = ((petwork >> 0) & 0xFF);
  struct {
    int num;
    float rank;
  } ranktbl[] = {
...
int ENEMY_createPets(int charaindex, int array) {
  Char CharNew;
  int newindex;
  int *p;
  int tp[E_T_DATAINTNUM];
