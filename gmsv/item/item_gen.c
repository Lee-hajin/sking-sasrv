...
// shan begin
#define FOOD_HP_RATE 200 //家族的料理补体功能多一倍
#define FOOD_MP_RATE 200 //家族的料理补气功能多一倍
#define FOOD_AI_RATE 200 //家族的料理补忠功能多一倍
...
#ifdef _ALCHEMIST
char alchetest_list[25][16] = {
    "石",   "木",   "骨",   "牙", "皮", "贝壳", "壳",   "爪",   "花",
    "叶",   "线",   "黏土", "鳞", "毒", "石化", "混乱", "酒醉", "昏睡",
    "回复", "复活", "水晶", "地", "水", "火",   "风"};
#endif
...
PET_ADD_INGRED( nm,vl1,vl2,vl3)
{
        ...
        {
                fixatom[ingnum] =ITEM_getAtomIndexByName( ENEMYTEMP_getChar(
petarray, nm) ); if( fixatom[ingnum] < 0 )
                {
                        print( "\nfucking atom:[%s] for pet id %d",
ENEMYTEMP_getChar( petarray, nm), petid ); continue;
                }
                ...
                if( petindex != -1)
                {
                        if( CHAR_getInt(petindex, CHAR_PETFAMILY) == 1 )
                        {
                                int ownerindex = CHAR_getWorkInt(petindex,
CHAR_WORKPLAYERINDEX); if (!CHAR_CHECKINDEX(ownerindex))
                                {
...
#ifdef _COMPOUND_EXP
char mergech[][16] = {"石",   "木",   "骨",   "牙",   "皮",   "贝壳", "壳",
                      "爪",   "花",   "叶",   "线",   "黏土", "鳞",   "毒",
                      "石化", "混乱", "酒醉", "睡眠", "回复", "复活", "水晶",
                      "地",   "水",   "火",   "风"};
#endif
...
int ITEM_mergeItem_merge(int charaindex, int petid, char *data, int petindex,
  ...
  int ret;
  int cnt = 0;
  int itemindexs[CHAR_getMaxInventory(charaindex) - CHAR_STARTITEMARRAY];
  int haveitemindexs[CHAR_getMaxInventory(charaindex) - CHAR_STARTITEMARRAY];
  // Nuke +2 1026: Avoid merge cheat
  int itemflag[CHAR_getMaxInventory(charaindex) - CHAR_STARTITEMARRAY];
  int j, collision = 0;

  char buff[64];

  ITEM_Item items[CHAR_getMaxInventory(charaindex) - CHAR_STARTITEMARRAY];
  // Nuke +1 1026
...

  {
    ...
    if (emptyindex == -1) {
      CHAR_talkToCli(charaindex, -1, "合成时，最少需空一格物品栏位！",
                     CHAR_COLORYELLOW);
      return -1;
    }
  }
    ...
    if (CHAR_CHECKITEMINDEX(charaindex, haveitemindex)) {
      ...
      if (ITEM_CHECKINDEX(itemindex)) {
        ...
        if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
          CHAR_talkToCli(charaindex, -1, "【温馨提示】绑定的物品无法合成。",
                         CHAR_COLORYELLOW);
          return FALSE;
        }
#endif
        ...
        if (ITEM_getmergeItemFromFromITEMtabl(
                ITEM_getInt(itemindex, ITEM_ID)) == TRUE) { //检查是否可合成
          ITEM_Item *itm;
  ...
  if (cnt > 1) {
    ...
    for (i = 0; i < cnt; i++) {
      ...
      if (pilenum <= 0) {
        ...
        LogItem(CHAR_getChar(charaindex, CHAR_NAME), /* 平乓仿   */
                CHAR_getChar(charaindex, CHAR_CDKEY), itemindexs[i],
                "合成删除道具", CHAR_getInt(charaindex, CHAR_FLOOR),
                CHAR_getInt(charaindex, CHAR_X),
                CHAR_getInt(charaindex, CHAR_Y),
                ITEM_getChar(itemindexs[i], ITEM_UNIQUECODE),
                ITEM_getChar(itemindexs[i], ITEM_NAME),
                ITEM_getInt(itemindexs[i], ITEM_ID), charaindex);
    ...
    if (ret >= 0) {
          ...
          if (CHAR_getInt(petindex, CHAR_PETFAMILY) == 1) {
            ...
            if (randtable == 1) {
              ...
              if (ITEM_getInt(createitemindex, ITEM_TYPE) == 20) {
                ...
                // Change fix
                // 原本少加了忠诚的功能,结果要登出入后,道具才有加忠诚效果,这里补上
                // 2005/01/04
                if ((p = strstr(ITEM_getChar(createitemindex, ITEM_ARGUMENT),
                                "忠"))) {
                  strcpy(argbuf, (p + 2));
                  arg3 = atoi(argbuf);
                  arg3 = arg3 * FOOD_AI_RATE / 100;
                }
                if (arg1 != 0 && arg2 != 0) {
                ...
                if (arg3 != 0) {
                  sprintf(argbuf, "忠%d", arg3);
                  strcat(charbuf1, argbuf);
                }
                if (arg1 != 0 && arg2 != 0) {
      ...
      char nowstr[64], nowtgh[64], nowdex[64], nowvit[64], nowcharm[64],
          ...
          nowpenetration[64], nowshield[64];

      sprintf(nowstr, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYATTACK));
      sprintf(nowtgh, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE));
      sprintf(nowdex, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYQUICK));
      sprintf(nowvit, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYHP));
      sprintf(nowcharm, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYCHARM));
      sprintf(nowcriti, "%d", ITEM_getInt(createitemindex, ITEM_CRITICAL));
      sprintf(nowyoid, "%d", ITEM_getInt(createitemindex, ITEM_MODIFYAVOID));
      sprintf(nowpenetration, "%d",
              ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE));
      sprintf(nowshield, "%d", ITEM_getInt(createitemindex, ITEM_OTHERDEFC));

      memset(Mname, 0, sizeof(Mname));
      if (ITEM_getInt(createitemindex, ITEM_MAGICID) > 0) {
        int marray =
            MAGIC_getMagicArray(ITEM_getInt(createitemindex, ITEM_MAGICID));
        if (marray != -1) {
      ...
      sprintf(
          ...
          (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? "傍" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? nowstr : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? "规" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? nowtgh : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? "刮" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? nowdex : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? "郴" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYHP) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? nowvit : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? "概" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? nowcharm : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? "农府" : "",
          (ITEM_getInt(createitemindex, ITEM_CRITICAL) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? nowcriti : "",
          (ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? " " : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) != 0) ? "雀乔" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) > 0) ? "+" : "",
          (ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) != 0) ? nowyoid : "",
          (ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE) > 0) ? "包烹+" : "",
          (ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE) > 0) ? nowpenetration
                                                               : "",
          (ITEM_getInt(createitemindex, ITEM_OTHERDEFC) > 0) ? "蒋靛+" : "",
          (ITEM_getInt(createitemindex, ITEM_OTHERDEFC) > 0) ? nowshield : "",
          Mname);
      if (ITEM_getInt(createitemindex, ITEM_TYPE) == 16 ||
          ITEM_getInt(createitemindex, ITEM_TYPE) >= 20 ||
          ITEM_getInt(createitemindex, ITEM_TYPE) == 5) {
      } else {
        ITEM_setChar(createitemindex, ITEM_EFFECTSTRING, set);
      }
      if (createitemindex != -1) {
        ...
        if (rc >= 0 && rc < CHAR_getMaxInventory(charaindex)) {
          ...
          LogItem(CHAR_getChar(charaindex, CHAR_NAME),
                  CHAR_getChar(charaindex, CHAR_CDKEY), createitemindex,
                  "合成出来道具", CHAR_getInt(charaindex, CHAR_FLOOR),
                  CHAR_getInt(charaindex, CHAR_X),
                  CHAR_getInt(charaindex, CHAR_Y),
                  ITEM_getChar(createitemindex, ITEM_UNIQUECODE),
                  ITEM_getChar(createitemindex, ITEM_NAME),
                  ITEM_getInt(createitemindex, ITEM_ID), charaindex);
          result = TRUE;
        } else {
...
#define MAXCANINSLAY 3
  ...
  if (i == MAXCANINSLAY) {
    char token[256];
    sprintf(token, "武器或防具已经镶满%d颗。", MAXCANINSLAY);
  ...
  sprintf(buf2, "%s|%s|%s|%s|%s|%s|%s|%s|%s|%s", codeTemp[0], codeTemp[1],
          codeTemp[2], codeTemp[3], codeTemp[4], codeTemp[5], codeTemp[6],
          codeTemp[7], codeTemp[8], codeTemp[9]);
#else
