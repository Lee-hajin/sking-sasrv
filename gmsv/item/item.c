...
//{"canpile",	0,		ITEM_CANBEPILE },
    ...
    {"lock", 0, ITEM_ItemLock}, /*  ITEM_USETIME  */
#endif
#ifdef _ITEM_TIME_LIMIT
    {"itime", -1, ITEM_TIMELIMIT},
#endif
...
#endif
#ifdef _ITEM_UPGRADE
    {"upgradno", 0, ITEM_UPGRADENO},
    {"upgradstr", 0, ITEM_UPGRADENO1},
    {"upgradtgh", 0, ITEM_UPGRADENO2},
    {"upgraddex", 0, ITEM_UPGRADENO3},
    {"upgradvit", 0, ITEM_UPGRADENO4},
    {"upgradcriti", 0, ITEM_UPGRADENO5},
    {"upgradyoid", 0, ITEM_UPGRADENO6},
#endif
};
...
//	{"bemerge", 0},					//ITEM_BEMERGE
    ...
    {"itime", -1},
#endif
#ifdef _ITEM_NAMECOLOR
    {"col", 0}, /*  ITEM_COLOER  */
#endif
#ifdef _ITEM_METAMOGET
    {"facedata", 0, ITEM_FACEDATA},
#endif
};
...
#ifdef _ITEM_NAMECOLOR
    {"itemcolor", ITEM_INTENTRY, ITEM_COLOER, NULL},
#endif
};
...
static BOOL ITEM_CHECKARRAYINDEX(int index) {
...
static int ITEM_CHECKINTDATAINDEX(int index) {
  if (ITEM_DATAINTNUM <= index || index < 0)
    return FALSE;
  return TRUE;
}
...
void _ITEM_endExistItemsOne(int index, char *file, int line) {
  ...
  for (i = 0; i < playernum; i++) {
    if (CHAR_CHECKINDEX(i)) {
      for (j = 0; j < CHAR_getMaxInventory(i); j++) {
  ...
  if (hitcnt < 1) {
    ...
    ITEM_UseItemnum--;
#ifdef _NB_FIX_POOLITEM
    if (ITEM_UseItemnum < 0) {
      ITEM_UseItemnum = 0;
    }
#endif
  }
...
int _ITEM_setInt(char *file, int line, int index, ITEM_DATAINT element,
                        int data) {
  int buf;
  if (!ITEM_CHECKINDEX(index)) {
    return -1;
...
int ITEM_getITEM_itemnum(void) { return ITEM_itemnum; }

int ITEM_getITEM_UseItemnum(void) { return ITEM_UseItemnum; }
...
#ifdef _ALLBLUES_LUA_1_2
BOOL ITEM_setLUAFunction(int itemindex, int functype,
...
#define ITEM_ID_TOKEN_INDEX 17
  ...
  print("ITEM_tbl分配 %4.2f MB 空间...",
        sizeof(ITEM_table) * ITEM_tblen / 1024.0 / 1024.0);
  print("ITEM_idx分配 %4.2f MB 空间...",
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    }
    {
      ...
      for (i = 0; i < ITEM_DATAINTNUM; i++) { //初始化
        intdata[i] = 0;
      }
      itemid = 0;
...
tmpbuf ); stringlen +=strlen(tmpbuf); if( stringlen >= arraysizeof( paramshow ))
  ...
  makeEscapeString(token, escapeeffectstring, sizeof(escapeeffectstring));
#else
...
#endif
#ifdef _ITEM_NAMECOLOR
  if (ITEM_getInt(itemindex, ITEM_COLOER) == 1) {
    itemcolor = CHAR_COLORGREEN;
  }

  if (ITEM_getInt(itemindex, ITEM_COLOER) == 2) {
    itemcolor = CHAR_COLORCYAN;
  }

  if (ITEM_getInt(itemindex, ITEM_COLOER) == 3) {
    itemcolor = CHAR_COLORPURPLE;
  }

  if (ITEM_getInt(itemindex, ITEM_COLOER) == 4) {
    itemcolor = CHAR_COLORBLUE2;
  }

  if (ITEM_getInt(itemindex, ITEM_COLOER) == 5) {
    itemcolor = CHAR_COLORGRAY;
  }

  if (ITEM_getInt(itemindex, ITEM_COLOER) == 6) {
    itemcolor = CHAR_COLORBLUE;
  }
  if (ITEM_getInt(itemindex, ITEM_COLOER) == 7) {
    itemcolor = CHAR_COLORYELLOW;
  }
#endif
  flg = 0;
...
#ifdef _LOCK_PET_ITEM
  {
    if (itm->string[ITEM_NAME].string[0] == '*') {
      char TM_EFFECT_Buff[256];
      itm->data[ITEM_ItemLock] = 1;
    } else {
      itm->data[ITEM_ItemLock] = 0;
    }
  }
#endif
...
void ITEM_equipEffect(int index) {
  ...
  if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      CHAR_getInt(index, CHAR_NB_NAMEMOTO) > 0) {
    char msg[256];
    sprintf(msg, "4|%d", CHAR_getInt(index, CHAR_NB_NAMEMOTO));
    lssproto_CHAREFFECT_send(getfdFromCharaIndex(index), msg);
  }
#endif
#ifdef _NB_CHARHALO
  if (CHAR_getInt(index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      CHAR_getInt(index, CHAR_NB_HALO) > 0) {
    char msg[256];
    sprintf(msg, "5|%d", CHAR_getInt(index, CHAR_NB_HALO));
    lssproto_CHAREFFECT_send(getfdFromCharaIndex(index), msg);
  }
#endif
  ...
  if (angelmode == FALSE && angelequip == TRUE) {
    ...
    CHAR_talkToCli(index, -1, "你受到了精灵保护，不会被敌人攻击。",
                   CHAR_COLORYELLOW);
    print(" 装备使者信物 ");
  }
#endif
...
void Other_DefcharWorkInt(int index) {
    ...
    CHAR_setWorkInt(index, CHAR_WORKFIXDEX,
                    CHAR_getWorkInt(index, CHAR_WORKFIXDEX) +
                        mdex * CHAR_getWorkInt(index, CHAR_WORKSUITDEX_P) /
                           ...
                           index, CHAR_WORK_F_SUIT ) );                        CHAR_setWorkInt( index, CHAR_WORK_I_RESIST,
                           i_res+CHAR_getWorkInt( index, CHAR_WORK_I_SUIT ) );                        CHAR_setWorkInt( index,
                           CHAR_WORK_T_RESIST, t_res+CHAR_getWorkInt( index, CHAR_WORK_T_SUIT ) );
                           }*/
#endif
  ...
  if (CHAR_getWorkInt(index, CHAR_WORK_ADDATTCRAZED) > 0) {
    int temp2 = CHAR_getWorkInt(index, CHAR_WORK_ADDATTCRAZED2);
    int old_dex = CHAR_getWorkInt(index, CHAR_WORKFIXDEX);
    CHAR_setWorkInt(index, CHAR_WORKFIXDEX,
                    old_dex + old_dex * (temp2 / 100.0));
  }
#endif
#ifdef _PRO3_ADDSKILL
  if (GETLOWVALUE(CHAR_getWorkInt(index, CHAR_WORK_CRACKBONE)) !=
      0) { //攻击力下降
    int temp2 = GETLOWVALUE(CHAR_getWorkInt(index, CHAR_WORK_CRACKBONE));
    CHAR_setWorkInt(index, CHAR_WORKFIXSTR,
                    CHAR_getWorkInt(index, CHAR_WORKFIXSTR) - temp2);
  }
#endif
...
char *ITEM_getEffectString(int itemindex) {
  ...
  if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
    sprintf(token, "[绑]%s",
            ITEM_item[itemindex].itm.string[ITEM_EFFECTSTRING].string);
  } else {
    sprintf(token, "%s",
            ITEM_item[itemindex].itm.string[ITEM_EFFECTSTRING].string);
  }
...
char *_ITEM_getSecretNameFromNumber(char *file, int line, int itemid) {
  ...
  return ITEMTBL_getChar(itemid, ITEM_SECRETNAME);
}
...
int ITEMTBL_getInt(int ItemID, ITEM_DATAINT datatype) {
  if (datatype >= ITEM_DATAINTNUM || datatype < 0)
    return -1;
  if (ITEM_CHECKITEMTABLE(ItemID) == FALSE)
    return -1;
...
char *ITEM_petmakeItemStatusString(int petindex, int itemid) {
  ...
  makeEscapeString(mybuff, escapeeffectstring, sizeof(escapeeffectstring));
#else
  makeEscapeString(ITEM_getChar(itemindex, ITEM_EFFECTSTRING),
                   escapeeffectstring, sizeof(escapeeffectstring));
#endif
