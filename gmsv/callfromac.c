...
void saacproto_ACServerLogin_recv(int fd, char *result, char *data) {
  {
    ...
    print("摆摊税收:");
    {
      int i;
      for (i = 1; i <= 7; i++) {
        print("%s:%d%,", multiStreetType(i), getStreetFax(i - 1));
      }
    }
    print("\n");
    initStreetEnable();
...
#ifdef _NB_DragonFunsionId //自定义2D人龙编号
    print("2D人龙进化是否随机: %s\n",
          getDragonFunsionRand() == 0 ? "正常" : "随机");
    print("任务给的2D人龙转数: %d转\n", getDragonTrans());
    {
      int i;
      char DragonName[5][56] = {"绿", "蓝", "灰", "红", "黄"};
      for (i = 1; i <= 5; i++) {
        if (getDragonFunsionId(i) >= 0) {
          print("2D%sId:%d,", DragonName[i - 1], getDragonFunsionId(i));
        }
      }
    }
    print("\n");
...
#endif
    print("\n※\033[31m%-66s\033[35m※",
          "本端只限技术交流，请勿用于商业用途，触及一切法律责任，均与作者无关");
    print("\n※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※※\033[0m\n");
  }
}
...
void saacproto_Broadcast_recv(int fd, char *id, char *charname, char *message) {
  /*
          if( strstr( message, "online") == 0 ||
                  ...
                  saacproto_DBGetEntryString_send( acfd, DB_ADDRESSBOOK,
     escapebuf, 0,0);
          }
  */
}
...
void saacproto_ACFixFMData_recv(int fd, char *result, int kindflag, char *data1,
  ...
  } else if (kindflag == FM_FIX_BADGE) {
    setFamilyBadge(charaindex);
...
void saacproto_ACKick_recv(int fd, int act, char *data, int retfd) {
  ...
  case 1: //跨星球踢人
  {
    ...
    /*
       ...
       szName ) ); if( retfd == -1 ){ print("因重复登入"); CHAR_talkToCli(
       fd_charaindex, -1, "因重复帐号登入而断线！",CHAR_COLORYELLOW); }else{
                                                            print("因做坏事 ");
                                                            CHAR_talkToCli(fd_charaindex,
       -1, "因做坏事而断线。",CHAR_COLORYELLOW);
                                                    }
...
void saacproto_ACMissionTable_recv(int fd, int num, int type, char *data,
  ...
  if (type == 1) { // get data list
    ...
    lssproto_WN_send(getfdFromCharaIndex(angelindex),
                     WINDOW_MESSAGETYPE_ANGELMESSAGE, WINDOW_BUTTONTYPE_YESNO,
                     CHAR_WINDOWTYPE_ANGEL_ASK, -1,
                     "目前魔族肆虐，精灵们需要你的帮忙，前往寻找勇者来消灭这些"
                     "魔族，你是否愿意帮忙？");
    ...
    sprintf(msg, " AC清除 任务:%d %s %s ", num, missiontable[num].angelinfo,
...
void saacproto_ACCharLogin_recv(int fd, int clifd, int flag) {
  if (flag == 0) {
    ...
    lssproto_ClientLogin_send(clifd, "账号或密码不得为空。");
    // CONNECT_endOne_debug(clifd);
    return;
  } else if (flag == 2) {
    lssproto_ClientLogin_send(
        clifd, "您的游戏账号已被锁定，无法登录游戏，如有疑问请联系客服。");
    // CONNECT_endOne_debug(clifd);
    return;
  } else if (flag == 3) {
    lssproto_ClientLogin_send(
        clifd, "您的IP已被锁定，无法登录游戏，如有疑问请联系客服。");
    // CONNECT_endOne_debug(clifd);
    return;
  } else if (flag == 4) {
    lssproto_ClientLogin_send(clifd,
                              "您在当前线路登录数量已满，请尝试登录其他线路。");
    // CONNECT_endOne_debug(clifd);
    return;
  } else if (flag == 5) {
    lssproto_ClientLogin_send(
        clifd, "您的账号尚未注册，请登录官方网站，注册后登录游戏。");
    // CONNECT_endOne_debug(clifd);
    return;
  } else if (flag == 6) {
    lssproto_ClientLogin_send(clifd,
                              "您的密码输入错误，请核对后登录。（注意大小写）");
    // CONNECT_endOne_debug(clifd);
    return;
  } else {
    lssproto_ClientLogin_send(clifd, "异常错误！");
    // CONNECT_endOne_debug(clifd);
    return;
  }
}
...
void saacproto_QueryPoint_recv(int fd, int point) {
  ...
  char token[64];
  sprintf(token, "你目前的个人金币为：%d", point);
...
void saacproto_NewVipShop_recv(int fd, int point, char *buf, int flag) {
  int charaindex = CONNECT_getCharaindex(fd);
  if (point == -1) {
    CHAR_talkToCli(charaindex, -1, "你的金币不足！", CHAR_COLORYELLOW);
    return;
  }
  if (strcmp(buf, "") == 0) {
  ...
  if (flag == 0) {
    ...
    LogSqlVipPoint(CHAR_getChar(charaindex, CHAR_NAME),
                   CHAR_getChar(charaindex, CHAR_CDKEY), "(购物剩点)", point,
                   CHAR_getInt(charaindex, CHAR_FLOOR),
                   CHAR_getInt(charaindex, CHAR_X),
                   CHAR_getInt(charaindex, CHAR_Y));
#endif
    ...
    if (CHAR_charSaveFromConnect(charaindex, FALSE)) {
      CHAR_talkToCli(charaindex, -1, "其飘 沫捞 何练钦聪促!", CHAR_COLORRED);
    }
#endif
  ...
  snprintf(token, sizeof(token), "您当前金币剩余 %d", point);
  CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
}
...
void saacproto_OnlineBuy_recv(int fd, char *data) {
  ...
  if (data[0] == '0') {
    ...
    if (itemindex != -1) {
      ...
      snprintf(token, sizeof(token), "领取物品 %s",
               ITEM_getChar(itemindex, ITEM_NAME));
      CHAR_talkToCli(charaindex, -1, token, CHAR_COLORGREEN);
    } else {
      CHAR_talkToCli(charaindex, -1, "此物品不存在，请与本服管理员联系！",
                     CHAR_COLORYELLOW);
    }
...
void saacproto_gm_shutdown_recv(int itype, char *msg) {
  if (itype == 1) {
    lssproto_Shutdown_recv(0, "hogehoge", atoi(msg));
  } else if (itype == 2) {
