...
void NPC_ItemStrStr(int itemID, double rate, char *name, char *token2,
                    int iCostFame, int iChangeItemCost, int iEventPoint);
...
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate,
                    int iCostFame, int iChangeItemCost, int EventPoint);
...
void NPC_ItemShop_BuyMain(int meindex, int talker, int before) {
  ...
  int point;
  char buff3[256];
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "EventPoint", buff3,
                                      sizeof(buff3)) != NULL) {
    point = CHAR_getInt(talker, CHAR_EVENTPOINT);
  }
...
void NPC_GetItemList(char *argstr, char *argtoken) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "ChangeItemCost", buff5,
                                      sizeof(buff5)) == NULL)
    iChangeItemCost = -1;
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "EventPoint", buff7,
                                      sizeof(buff7)) == NULL)
    iEventPoint = -1;
...
void NPC_ItemStrStr(int itemID, double rate, char *name, char *token2,
  ...
  if (iChangeItemCost > -1)
    iCostFame = iChangeItemCost;
  else if (iEventPoint > -1)
    iCostFame = iEventPoint;
          ...
          if (ITEM_getcostFromITEMtabl(start) != -1) {
            if (i == select) {
              ...
              if (NPC_AddItemBuy(meindex, talker, start, kosuu, rate, iCostFame,
                                 iChangeItemCost, iEventPoint) != TRUE) {
                return FALSE;
              }
              return TRUE;
            }
...
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate,
  ...
  if (iTotalEventPoint > 0) {
    iTotalEventPoint = iEventPoint * kosuu;
    if (CHAR_getInt(talker, CHAR_EVENTPOINT) < iEventPoint)
      return FALSE;
  }
  ...
  */
  ...
  if (iTotalEventPoint > 0) {
    CHAR_setInt(talker, CHAR_EVENTPOINT,
                CHAR_getInt(talker, CHAR_EVENTPOINT) - iTotalEventPoint);
    ...
    if (CHAR_getInt(talker, CHAR_FMINDEX) != -1 &&
      ...
      int fd = getfdFromCharaIndex(talker);
      char buf[256];
      sprintf(buf, "%d", CHAR_getInt(talker, CHAR_EVENTPOINT));
