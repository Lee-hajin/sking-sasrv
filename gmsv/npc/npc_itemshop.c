...
void NPC_ItemStrStr(int itemID, double rate, char *name, char *token2,
                    int iCostFame, int iChangeItemCost, int eventcost);
...
int NPC_GetSellItemList(int itemindex, int flg, char *argstr, char *argtoken,
                        int select, int sell);
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate,
                    int iCostFame, int iChangeItemCost, int eventcost);
int NPC_SellItemstrsStr(int itemindex, int flg, double rate, char *argtoken,
                        int select, int sell);
...
static NPC_Shop TypeTable[] = {
    ...
    {"NOSERING", 14},                       //鼻环
    {"AMULET", 15},                         //护身符
    {"OTHER", 16},      {"BOOMERANG", 17},  //回力标
    {"BOUNDTHROW", 18}, {"BREAKTHROW", 19}, //投掷物
#ifdef _ITEM_TYPETABLE
    {"DISH", 20},       {"METAL", 21},      {"JEWEL", 22},  {"WARES", 23},
    {"WBELT", 24},      {"WSHIELD", 25},    {"WSHOES", 26}, {"WGLOVE", 27},
    {"ANGELTOKEN", 28}, {"HEROTOKEN", 29},
#endif
    {"ACCESSORY", 30}, //配件
    {"OFFENCE", 40},   //攻击
    {"DEFENCE", 50},   //防御
...
void NPC_ItemShopTalked(int meindex, int talker, char *szMes, int color) {
  ...
  if (NPC_Util_isFaceToFace(meindex, talker, 2) ==
      FALSE) { //交谈时检查是否面对面
  ...
  //取得NPC只能卖的讯息
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "sellonly_msg", token,
                                      sizeof(token)) != NULL) {
    sellonlyflg = TRUE; // NPC不能买玩家的东西
    strcpysafe(sellmsg, sizeof(sellmsg),
               token); //例如:sellmsg的值可能是->并不是专门收买东西的店。
  }
  ...
  //取得玩家卖东西的指令. buff为一串卖东西指令的字串,例如:卖东西,卖,sell,uru
  //当玩家打出这些字句时,就可以卖东西了
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "other_msg", buff,
    ...
    while (getStringFromIndexWithDelim(buff, ",", i, buf2, sizeof(buf2)) !=
      ...
      i++;
      if (strstr(szMes, buf2) != NULL) {
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "hint_msg", token,
                                            sizeof(token)) != NULL) {
...
static void NPC_ItemShop_selectWindow(int meindex, int talker, int num,
  ...
  case 0:
    CHAR_send_P_StatusString(
        talker, CHAR_P_STRING_GOLD); //传送金钱,若100元 送出去的资料格式可能为
                                     //P8Nz2|100|
...
void NPC_ItemShopWindowTalked(int meindex, int talkerindex, int seqno,
...
void NPC_ItemShop_BuyMain(int meindex, int talker, int before) {
...
#ifdef USE_EVENTPOINT
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostEvent", buff,
                                      sizeof(buff)) != NULL) {
    sprintf(token, "FAME|%d", CHAR_getInt(talker, CHAR_EVENTPOINT));
  ...
  if (before != -1) {
    ...
    NPC_Util_GetStrFromStrWithDelim(argstr, "itemfull_msg", buff2,
                                    sizeof(buff2));
  ...
  lssproto_WN_send(fd, WINDOW_MESSAGETYPE_ITEMSHOPMAIN, WINDOW_BUTTONTYPE_NONE,
                   CHAR_WINDOWTYPE_WINDOWITEMSHOP_BUY_MSG,
                   CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
}
void NPC_GetItemList(char *argstr, char *argtoken) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "buy_rate", buff2,
                                      sizeof(buff2)) != NULL) {
    rate = atof(buff2);
  }
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostFame", buff3,
                                      sizeof(buff3)) == NULL)
    iCostFame = -1;
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "ChangeItemCost", buff5,
                                      sizeof(buff5)) == NULL)
    iChangeItemCost = -1;
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostEvent", buff7,
                                      sizeof(buff7)) == NULL)
    eventcost = -1;
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemList", buff, sizeof(buff)) !=
    ...
    while (getStringFromIndexWithDelim(buff, ",", i, buff2, sizeof(buff2)) !=
      ...
      if (eventcost > -1) {
        if (getStringFromIndexWithDelim(buff7, ",", i, buff8, sizeof(buff8)) !=
            FALSE)
          eventcost = atoi(buff8);
      }
      i++;
      if (strstr(buff2, "-") == NULL) {
        ...
        if (name == "\0")
          continue;
        loopcnt++;
        if (loopcnt == MAXSHOPITEM)
          break;
        NPC_ItemStrStr(atoi(buff2), rate, name, token2, iCostFame,
                       iChangeItemCost, eventcost);
...
void NPC_ItemStrStr(int itemID, double rate, char *name, char *token2,
  ...
  if (eventcost > -1) {
    sprintf(token2, "%s|0|%d|%d|%d|%s|%d|", escape, level, gold, graNo, info,
            eventcost < 0 ? -1 : eventcost);
  } else {
    sprintf(token2, "%s|0|%d|%d|%d|%s|%d|", escape, level, gold, graNo, info,
            iCostFame < 0 ? -1 : iCostFame / 100);
  }
}
...
BOOL NPC_SetNewItem(int meindex, int talker, char *data) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemList", buf, sizeof(buf)) !=
    ...
    while (getStringFromIndexWithDelim(buf, ",", j, buff2, sizeof(buff2)) !=
      ...
      if (eventcost > -1) {
        if (getStringFromIndexWithDelim(buff7, ",", j, buff8, sizeof(buff8)) !=
            FALSE)
          eventcost = atoi(buff8);
      }
      j++;
        ...
        if (ITEM_getcostFromITEMtabl(atoi(buff2)) != -1) {
          if (i == select) {
            ...
            if (NPC_AddItemBuy(meindex, talker, atoi(buff2), kosuu, rate,
                               iCostFame, iChangeItemCost, eventcost) != TRUE) {
              return FALSE;
            }
            return TRUE;
          }
          ...
          if (ITEM_getcostFromITEMtabl(start) != -1) {
            if (i == select) {
              ...
              if (NPC_AddItemBuy(meindex, talker, start, kosuu, rate, iCostFame,
                                 iChangeItemCost, eventcost) != TRUE) {
                return FALSE;
              }
              return TRUE;
            }
...
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate,
  ...
  maxgold = gold * kosuu;
  if (eventcost > 0) {
    if (CHAR_getInt(talker, CHAR_EVENTPOINT) < maxgold) {
      CHAR_talkToCli(talker, -1, "<CHAR_EVENTPOINT", CHAR_COLORYELLOW);
      return FALSE;
    }
  } else {
    if (CHAR_getInt(talker, CHAR_GOLD) < maxgold) {
      CHAR_talkToCli(talker, -1, "您的石币不足，无法购买此物品。",
                     CHAR_COLORYELLOW);
      return FALSE;
    }
    ...
    if (iCostFame > 0) {
      ...
      if (CHAR_getInt(talker, CHAR_FAME) < iTotalCostFame) {
        CHAR_talkToCli(talker, -1, "您的声望不足，无法购买此物品。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
  ...
  */
  ...
  if (eventcost > 0) {
    CHAR_setInt(talker, CHAR_EVENTPOINT,
                CHAR_getInt(talker, CHAR_EVENTPOINT) - maxgold);
    CHAR_talkToCli(talker, -1, "BUY G -CHAR_EVENTPOINT", CHAR_COLORYELLOW);
  } else {
    CHAR_DelGold(talker, maxgold);

    if (iTotalCostFame > 0) {
      CHAR_setInt(talker, CHAR_FAME,
                  CHAR_getInt(talker, CHAR_FAME) - iTotalCostFame);

      char token[256];
      sprintf(token, "已扣除[%d]声望", iTotalCostFame / 100);
      CHAR_talkToCli(talker, -1, token, CHAR_COLORYELLOW);

      if (CHAR_getInt(talker, CHAR_FMINDEX) != -1 &&
          (strcmp(CHAR_getChar(talker, CHAR_FMNAME), ""))) {
        int fd = getfdFromCharaIndex(talker);
        char buf[256];
    ...
    char token[256];
    sprintf(token,
            "酒捞袍 [%s](阑)甫 %d俺 备概窍咯 %d胶沛阑 荤侩沁嚼聪促。 唱狼 "
            "胶沛: %d胶沛",
            ITEM_getChar(itemindex, ITEM_NAME), kosuu, maxgold,
            CHAR_getInt(talker, CHAR_GOLD));
    CHAR_talkToCli(talker, -1, token, CHAR_COLORYELLOW);
  }
...
void NPC_ItemShop_Menu(int meindex, int talker) {
  ...
  // argstr取得整个设定档的讯息:
  // 例如->buy_rate:1.0|sell_rate:0.2|buy_msg:买,购买,感谢您,kau,buy.............
  // (中间的分格号是读入时加入的)
  if (NPC_Util_GetArgStr(meindex, argstr, sizeof(argstr)) == NULL) {
    print("shop_GetArgStr_Err");
    return;
  ...
  NPC_Util_GetStrFromStrWithDelim(argstr, "main_msg", buff, sizeof(buff));
#ifdef USE_EVENTPOINT
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostEvent", buff2,
                                      sizeof(buff2)) != NULL) {
    snprintf(token, sizeof(token), "%s|%s|%d", CHAR_getChar(meindex, CHAR_NAME),
             buff, CHAR_getInt(talker, CHAR_EVENTPOINT));
  } else {
    snprintf(token, sizeof(token), "%s|%s|%d", CHAR_getChar(meindex, CHAR_NAME),
  ...
  lssproto_WN_send(fd, WINDOW_MESSAGETYPE_ITEMSHOPMENU, WINDOW_BUTTONTYPE_NONE,
...
void NPC_ItemShop_SellMain(int meindex, int talker, int before) {
...
#ifdef _NB_CDKEYLOCK
  {
    char *cdkey = CHAR_getChar(talker, CHAR_CDKEY);
    if (sasql_getLock(cdkey) > 0) {
      CHAR_talkToCli(talker, -1, "你的帐号存在非法操作，无法进行物品贩卖！", 6);
      return;
    }
  }
#endif
#ifdef _ITEM_PET_LOCKED
  if (CHAR_getInt(talker, CHAR_LOCKED) == 1 && getItemPetLocked() == 1) {
    char message[256];
    char buf[256];
...
#ifdef USE_EVENTPOINT
  char _buffer[1024];
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "CostEvent", _buffer,
                                      sizeof(_buffer)) != NULL) {
    sprintf(token, "FAME|%d", CHAR_getInt(talker, CHAR_EVENTPOINT));
    lssproto_S2_send(fd, token);
  } else {
  ...
  if (before != -1) {
    ...
    if (CHAR_getWorkInt(meindex, NPC_SHOP_WORK_EXPRESS) == 1) {
      NPC_Util_GetStrFromStrWithDelim(argstr, "exrealy_msg", buff2,
                                      sizeof(buff2));
    } else {
      NPC_Util_GetStrFromStrWithDelim(argstr, "realy_msg", buff2,
                                      sizeof(buff2));
    }
    sprintf(token2, "%s|%s|", buff, buff2);
...
int NPC_GetLimtItemList(int talker, char *argstr, char *token2, int sell) {
  ...
  if (sell == -1) {
    ...
    imax = CHAR_getMaxInventory(talker);
    flg = -1;
  } else {
    i = sell;
    imax = sell + 1;
  ...
  for (; i < imax; i++) {
    ...
    if (ITEM_CHECKINDEX(itemindex)) {
          ...
          for (j = 0; j < ITEM_CATEGORYNUM + 3; j++) {
            ...
            if (strcmp(TypeTable[j].arg, token) == 0) {
              ...
              if (ITEM_getInt(itemindex, ITEM_TYPE) == itemtype) {
                cost =
                    NPC_GetSellItemList(itemindex, 0, argstr, token3, i, sell);
                if (cost != -1)
                  return cost;
                ...
                if (8 <= ITEM_getInt(itemindex, ITEM_TYPE) &&
                  ...
                  cost = NPC_GetSellItemList(itemindex, 0, argstr, token3, i,
                                             sell);
                  if (cost != -1)
                    return cost;
                  strncat(token2, token3, sizeof(token3));
      ...
      if ((NPC_Util_GetStrFromStrWithDelim(argstr, "LimitItemNo", buff,
                                           sizeof(buff)) != NULL) &&
          okflg == 0) {
        k = 1;
        while (getStringFromIndexWithDelim(buff, ",", k, token,
...
int NPC_GetSellItemList(int itemindex, int flg, char *argstr, char *argtoken,
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "special_item", buff,
                                      sizeof(buff)) != NULL) {
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "special_rate", buff2,
                                        sizeof(buff2)) != NULL) {
      rate = atof(buff2);
    } else {
      rate = 1.2;
    ...
    while (getStringFromIndexWithDelim(buff, ",", k, buff2, sizeof(buff2)) !=
      ...
      if (strstr(buff2, "-") == NULL && strcmp(buff2, "") != 0) {
        if (ITEM_getInt(itemindex, ITEM_ID) == atoi(buff2)) {
          cost =
              NPC_SellItemstrsStr(itemindex, 0, rate, argtoken, select, sell);
          return cost;
        }
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "sell_rate", buff,
                                      sizeof(buff)) != NULL) {
    rate = atof(buff);
...
BOOL NPC_SellNewItem(int meindex, int talker, char *data) {
  ...
  if (select < CHAR_STARTITEMARRAY || select >= CHAR_getMaxInventory(talker))
    return FALSE;
  k = select;
  itemindex = CHAR_getItemIndex(talker, k);

  cost = NPC_GetLimtItemList(talker, argstr, token2, select);
  if (cost == -1 || (cost * sellnum) + MyGold >= MaxGold ||
      !ITEM_CHECKINDEX(itemindex)) {
    int fd = getfdFromCharaIndex(talker);
    sprintf(token, "\n\n哎呀!对不起"
                   "\n\n对不起啊 ! 可不可以再选一次呢？");
...
#ifdef _ITEM_SELLNUMERROR
  int maxpile = CHAR_getInt(talker, CHAR_TRANSMIGRATION) +
                (CHAR_getInt(talker, CHAR_TRANSMIGRATION) / 5) * 2 + 3;
  if (sellnum < 1 && sellnum > maxpile) {
    return FALSE;
  }
#endif
#ifdef _ITEM_PILENUMS
  if (NPC_DelItem(talker, k, sellnum) == FALSE)
    return FALSE;
#else
  {
    LogItem(CHAR_getChar(talker, CHAR_NAME), CHAR_getChar(talker, CHAR_CDKEY),
            itemindex, "卖道具", CHAR_getInt(talker, CHAR_FLOOR),
...
void NPC_LimitItemShop(int meindex, int talker, int select) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "sellonly_msg", buf,
                                      sizeof(buf)) != NULL) {
    sprintf(token, "\n\n%s", buf);
