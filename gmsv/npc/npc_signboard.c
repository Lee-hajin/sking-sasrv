...
static void NPC_SignBoard_Window(int meindex, int toindex, int num,
                                 int select) {
  ...
  // Arminius: manor
  {
    ...
    if (p != NULL) {
      ...
      if (q != NULL) {
        ...
        if ((a >= 1) && (a <= 4)) {
#endif
          getStringFromIndexWithDelim(fmpointlist.pointlistarray[a - 1], "|", 5,
                                      token, sizeof(token));
          if (atoi(token) >= 0) {
            getStringFromIndexWithDelim(fmpointlist.pointlistarray[a - 1], "|",
 ...
 *
-------------------------------------------*/
void NPC_SignBoardWindowTalked(int meindex, int talkerindex, int seqno,
