...
#define NPC_DOOR_OPENMSG "옥은 상호 치열합니다!"
#define NPC_DOOR_CLOSEMSG "옥 리프팅} 户 凶!"
#define NPC_DOOR_CANTOPENMSG                                                   \
  "글쎄, 상호, 상호, 씨} 곧 상호 상호 중간에 Maung Renyi를 잃어버린 씨!"

#define ROPEGRAPHIC 9259 /* 蔷  卞丐月骚木卅中夫□皿 */

enum { NPC_DOOR_FLG_SOONCLOSE = 1, NPC_DOOR_FLG_WATCHCLOSE = 2 };
...
BOOL NPC_DoorInit(int meindex) {
...

  {
    ...
    if (dir >= 0)
      CHAR_setInt(meindex, CHAR_DIR, dir);
  }
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORKDOORPASSFLG) == 1) {
    ...
    if (NPC_RoomAdminNew_ReadFile(CHAR_getWorkChar(meindex, CHAR_WORKDOORNAME),
                                  &roominfo) == TRUE) {
...
void NPC_DoorAddSwitchCounter(int meindex, int i, int doindex) {
  if (i != 0 && CHAR_getWorkInt(meindex, CHAR_WORKDOORSWITCHCOUNT) != 0) {
    CHAR_setWorkInt(meindex, CHAR_WORKDOORSWITCHCURRENT,
                    CHAR_getWorkInt(meindex, CHAR_WORKDOORSWITCHCURRENT) + i);
...
void NPC_DoorWatch(int myobjindex, int moveobjindex, CHAR_ACTION act, int x,
  ...
  } else if (CHAR_getInt(meindex, CHAR_MERCHANTLEVEL) >= 0) {
...
void NPC_DoorLooked(int meindex, int lookedindex) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORKDOORORIGINALGRAPHIC) == ROPEGRAPHIC) {
    return;
  }
...
#define NPC_DOOR_EXPIRE_MSG "%s 방 유효 기간 %d/%d %d:%d"
    else {
      if (CHAR_getWorkInt(meindex, CHAR_WORKDOOREXPIRETIME) != 0xffffffff) {
        ...
        snprintf(msgbuf, sizeof(msgbuf), NPC_DOOR_EXPIRE_MSG,
                 roominfo.charaname, tmwk.tm_mon + 1, tmwk.tm_mday,
...
#undef NPC_DOOR_EXPIRE_MSG
  ...
  if (needkey >= 0) {
    ...
    CHAR_talkToCli(lookedindex, -1, "이 문은 열 수없는 것 같습니다!", CHAR_COLORWHITE);
  }
}
void NPC_DoorTalked(int meindex, int talkerindex, char *msg, int color) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORKDOORPASSFLG) >= 1 &&
      ((unsigned long)NowTime.tv_sec) >
          (unsigned long)CHAR_getWorkInt(meindex, CHAR_WORKDOOREXPIRETIME)) {
  ...
  if (strcmp(m, msg) == 0) {
    ...
    CHAR_talkToCli(talkerindex, -1, "암호가 잘못되었습니다!", CHAR_COLORWHITE);
  }
}
...
int NPC_DoorSearchByName(char *name) {
  ...
  for (i = CHAR_getPlayerMaxNum() + CHAR_getPetMaxNum(); i < charnum; i++) {
    ...
    if (CHAR_getCharUse(i) && CHAR_getInt(i, CHAR_WHICHTYPE) == CHAR_TYPEDOOR) {
...
int NPC_DoorSearchByPosition(int fl, int x, int y) {
  ...
  for (i = CHAR_getPlayerMaxNum() + CHAR_getPetMaxNum(); i < charnum; i++) {
    if (CHAR_getCharUse(i) && CHAR_getInt(i, CHAR_WHICHTYPE) == CHAR_TYPEDOOR &&
        CHAR_getInt(i, CHAR_FLOOR) == fl && CHAR_getInt(i, CHAR_X) == x &&
...
void NPC_DoorClose(int meindex, int doindex) {
  ...
  if (doindex >= 0)
    CHAR_talkToCli(doindex, -1, NPC_DOOR_CLOSEMSG, CHAR_COLORWHITE);
  CHAR_setInt(meindex, CHAR_TALKCOUNT, 0);
}
