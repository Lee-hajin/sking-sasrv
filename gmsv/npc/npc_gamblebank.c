...
static void NPC_GambleBank_selectWindow(int meindex, int toindex, int num,
                                        int flg);
...
enum {
  ...
  NPC_WORK_WORKTYPE =
      CHAR_NPCWORKINT2, // 0,-1 = NULL 2 = 银行 3 = 换币 4 = BOTH
...
BOOL NPC_GambleBankInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "GAMBLE_TYPE", buf1,
                                      sizeof(buf1)) == NULL) {
...
void NPC_GambleBankTalked(int meindex, int talkerindex, char *szMes,
                          int color) {
  char buf1[256];
  char npcarg[NPC_UTIL_GETARGSTR_BUFSIZE];
  int work_type;
...
static void NPC_GambleBank_selectWindow(int meindex, int toindex, int num,
  ...
  switch (num) {
  case GAMBLE_START:
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "gamble_start", token,
                                        sizeof(token)) == NULL) {
  ...
  case GAMBLE_END:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, Gamble_End[flg], token,
                                        sizeof(token)) == NULL) {
...
void NPC_GambleBankWindowTalked(int meindex, int talkerindex, int seqno,
...
int NPC_GambleBank_DoGold(int meindex, int toindex, int Gold, int flg) {
  // flg = 1 正常 flg = 2 钱不够 flg <= 0 错误 3 = 个人存款将满 4 = 个人银行将满
  // 5 = 单次最高金额
  char buf1[256];
  ...
  if (Gold < 0) { //取款
    ...
    if ((player_gold + Gold) > CHAR_getMaxHaveGold(toindex)) {
      return 3; //取款後将超过个人金额
    } else if ((Gold + stone_def) >
               CHAR_getInt(toindex, CHAR_BANKGOLD)) { //银行存款不够
      return 2;
    }

    CHAR_setInt(toindex, CHAR_BANKGOLD,
                (CHAR_getInt(toindex, CHAR_BANKGOLD) - (Gold + stone_def)));

    CHAR_AddGold(toindex, Gold);

    sprintf(buf1, "取款：%d ，手续费：%d，银行剩余：%d 。", Gold, stone_def,
            CHAR_getInt(toindex, CHAR_BANKGOLD));
    CHAR_talkToCli(toindex, meindex, buf1, CHAR_COLORYELLOW);
    LogBankStone(CHAR_getChar(toindex, CHAR_NAME),
                 CHAR_getChar(toindex, CHAR_CDKEY), toindex, Gold,
                 "GB_Bank_Get(银行取款)", CHAR_getInt(toindex, CHAR_FLOOR),
    ...
    if (Gold > player_gold) {
      return 2;
    } else if ((Gold + CHAR_getInt(toindex, CHAR_BANKGOLD)) >
               CHAR_MAXPERSONAGOLD) {
      return 4;
    }
    ...
    CHAR_setInt(toindex, CHAR_BANKGOLD,
                (CHAR_getInt(toindex, CHAR_BANKGOLD) + Gold));
    sprintf(buf1, "存款：%d ，银行剩余：%d 。", Gold,
            CHAR_getInt(toindex, CHAR_BANKGOLD));
    CHAR_talkToCli(toindex, meindex, buf1, CHAR_COLORYELLOW);
    LogBankStone(CHAR_getChar(toindex, CHAR_NAME),
                 CHAR_getChar(toindex, CHAR_CDKEY), toindex, Gold,
                 "GB_Bank_save(银行存款)", CHAR_getInt(toindex, CHAR_FLOOR),
                 CHAR_getInt(toindex, CHAR_X), CHAR_getInt(toindex, CHAR_Y),
                 CHAR_getInt(toindex, CHAR_GOLD),
                 CHAR_getInt(toindex, CHAR_BANKGOLD));
    return 1;
...
BOOL NPC_GambleBank_AddItem(int meindex, int toindex, int itemId, int count) {
  ...
  LogItem(CHAR_getChar(toindex, CHAR_NAME), CHAR_getChar(toindex, CHAR_CDKEY),
          itemindex, "游乐场兑换道具", CHAR_getInt(toindex, CHAR_FLOOR),
          CHAR_getInt(toindex, CHAR_X), CHAR_getInt(toindex, CHAR_Y),
  ...
  CHAR_setInt(toindex, CHAR_GAMBLENUM,
              CHAR_getInt(toindex, CHAR_GAMBLENUM) - count);
