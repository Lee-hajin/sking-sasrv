...
BOOL NPC_TimeManInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "change_no", buf, sizeof(buf)) !=
      NULL) {
  ...
  CHAR_setWorkInt(meindex, E_INT_GRAPHIC,
                  CHAR_getInt(meindex, CHAR_BASEIMAGENUMBER));

  if (NPC_Util_GetStrFromStrWithDelim(argstr, "time", buf, sizeof(buf)) !=
      NULL) {
    for (i = 0; i < 9; i++) {
...
void NPC_TimeManTalked(int meindex, int talkerindex, char *msg, int color) {
  ...
  getStringFromIndexWithDelim(buf, ",", rand() % tokennum + 1, token,
                              sizeof(token));
  /*--仄扎屯月--*/
...
/** 引歹曰卞窒井互粟仇匀凶午五卞裟壬木月*/
void NPC_TimeManWatch(int meobjindex, int objindex, CHAR_ACTION act, int x,
