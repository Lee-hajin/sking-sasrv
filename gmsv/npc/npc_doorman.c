...
void NPC_DoormanTalked(int meindex, int talkerindex, char *msg, int color) {
  char mode[128];
  char opt[256];
  char arg[NPC_UTIL_GETARGSTR_BUFSIZE];
  ...
  if (strcmp(mode, "gold") == 0) {
    ...
    if (g > 0 && yn < 0) {
      snprintf(msg, sizeof(msg), "打开门需要给我%d的金子这样可以吗？", g);
      CHAR_talkToCli(talkerindex, meindex, msg, CHAR_COLORWHITE);
    } else if (g > 0 && yn == 0) {
      snprintf(msg, sizeof(msg), "打开门 %d的金子是必要的。", g);
    } else if (g > 0 && yn == 1) {
      ...
      if (now_g < g) {
        ...
        snprintf(msg, sizeof(msg), "%d 收到金子了。现在就来开门。", g);
        CHAR_talkToCli(talkerindex, meindex, msg, CHAR_COLORWHITE);
    ...
    CHAR_talkToCli(talkerindex, meindex, "尚在未支援模式。", CHAR_COLORWHITE);
    ...
    if (iNum >= maxnum) {
      ...
      NPC_DoormanOpenDoor(CHAR_getWorkChar(meindex, CHAR_WORKDOORMANDOORNAME));
    }
