...
BOOL NPC_Util_CharNearby(int ind1, int ind2) {
  ...
  return NPC_Util_Nearby(CHAR_getInt(ind1, CHAR_X), CHAR_getInt(ind1, CHAR_Y),
                         CHAR_getInt(ind2, CHAR_X), CHAR_getInt(ind2, CHAR_Y));
}
  ...
  for (i = 0;; i++) {
    ...
    for (j = 0; j < arraysizeof(yntable); j++) {
      if (input[i] == yntable[j].string[0] &&
          strncmp(input + i, yntable[j].string, strlen(yntable[j].string)) ==
              0) {
...
int NPC_Util_countHaveItem(int meindex, int itemid) {
  ...
  if (!CHAR_CHECKINDEX(meindex))
    return -1;
  for (i = 0; i < CHAR_getMaxInventory(meindex); i++) {
...
BOOL NPC_Util_isBackContact(int frontindex, int backindex) {
  ...
  VALIDATEDIR(dir);
  CHAR_getCoordinationDir(dir, CHAR_getInt(frontindex, CHAR_X),
                          CHAR_getInt(frontindex, CHAR_Y), 1, &x, &y);
  if (x == CHAR_getInt(backindex, CHAR_X) &&
      y == CHAR_getInt(backindex, CHAR_Y) &&
      CHAR_getInt(backindex, CHAR_DIR) == CHAR_getInt(frontindex, CHAR_DIR))
    return TRUE;
  else
    return FALSE;
}
...
BOOL NPC_Util_isFaceToFace(int index1, int index2, int distance) {
  ...
  for (i = 1; i <= distance; i++) {
    CHAR_getCoordinationDir(CHAR_getInt(index1, CHAR_DIR),
                            ...
                            CHAR_getInt(index1, CHAR_Y), i, &x, &y);
    ...
    CHAR_getCoordinationDir(CHAR_getInt(index2, CHAR_DIR),
                            ...
                            CHAR_getInt(index2, CHAR_Y), i, &x, &y);
...
BOOL NPC_Util_isFaceToChara(int index1, int index2, int distance) {
  ...
  for (i = 1; i <= distance; i++) {
    ...
    CHAR_getCoordinationDir(
        CHAR_getInt(index1, CHAR_DIR), CHAR_getInt(index1, CHAR_X),
        CHAR_getInt(index1, CHAR_Y), i, &walked_x, &walked_y);
...
int NPC_Util_GiveAllItemToChar(int give, int take) {
  ...
  for (i = 0; i < CHAR_getMaxInventory(give); i++) {
...
static int SearchNearLine(int xStart, int yStart, int floor, int xPlus,
                          int yPlus, int Count, int type) {
  OBJECT object;
...
int NPC_Util_SearchItemInChar(int charindex, int itemindex) {
  ...
  for (i = 0; i <= CHAR_getMaxInventory(charindex); i++) {
...
char *NPC_Util_GetStrFromStrWithDelim(char *srcstr, char *srhstr, char *buf,
                                      int buflen) {
  BOOL rc;
  char *cret = NULL;
  int i;
...
BOOL NPC_Util_IsVisiblePlayer(int meindex) {
  ...
  for (i = chr_x - CHAR_DEFAULTSEESIZ / 2;
    ...
    for (j = chr_y - CHAR_DEFAULTSEESIZ / 2;
         j <= chr_y + CHAR_DEFAULTSEESIZ / 2 && found == FALSE; j++) {
      OBJECT object;
      for (object = MAP_getTopObj(chr_fl, i, j); object;
           object = NEXT_OBJECT(object)) {
        int objindex = GET_OBJINDEX(object);
        if (OBJECT_getType(objindex) == OBJTYPE_CHARA) {
...
int NPC_Util_FrontItem(int meindex) {
  ...
  for (obj = MAP_getTopObj(CHAR_getInt(meindex, CHAR_FLOOR), front_x, front_y);
    ...
    if (strlen(CHAR_getChar(talker, CHAR_TEACHER_ID)) > 0 &&
      ...
      for (i = 0; i < iPlayernum; i++) {
        if (CHAR_getCharUse(i) == FALSE)
          continue;
        if (strcmp(CHAR_getChar(talker, CHAR_TEACHER_ID),
