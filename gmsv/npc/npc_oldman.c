...
BOOL NPC_OldmanInit(int meindex) { return FALSE; }
...
#define NPC_OLDMAN_TITLE_UNIT2 "KO"
      ...
      if (delcnt > 0 && addcnt > 0) {
        ...
        snprintf(message, sizeof(message), "你已丧失得到%d%s称号的资格。",
                 delcnt,
                 delcnt < 10 ? NPC_OLDMAN_TITLE_UNIT1 : NPC_OLDMAN_TITLE_UNIT2);
        CHAR_talkToCli(talkerindex, meindex, message, CHAR_COLORWHITE);
      } else if (addcnt > 0) {
        snprintf(message, sizeof(message), "给你%d%s的称号吧！",
                 addcnt, // CoolFish: d%s -> %d%s 2001/4/18
                 addcnt < 10 ? NPC_OLDMAN_TITLE_UNIT1 : NPC_OLDMAN_TITLE_UNIT2);
        CHAR_talkToCli(talkerindex, meindex, message, CHAR_COLORWHITE);
      }
      if (delcnt > 0) {
        snprintf(message, sizeof(message), "失去%d%s 称号！", delcnt,
                 delcnt < 10 ? NPC_OLDMAN_TITLE_UNIT1 : NPC_OLDMAN_TITLE_UNIT2);
        CHAR_talkToCli(talkerindex, -1, message, CHAR_COLORYELLOW);
      }
      if (addcnt > 0) {
        snprintf(message, sizeof(message), "获得%d%s 称号！", addcnt,
                 addcnt < 10 ? NPC_OLDMAN_TITLE_UNIT1 : NPC_OLDMAN_TITLE_UNIT2);
        CHAR_talkToCli(talkerindex, -1, message, CHAR_COLORYELLOW);
      }
    }
  ...
  if (title_change)
    CHAR_sendStatusString(talkerindex, "T");
}
