...
static void NPC_PetSkillShop_selectWindow(int meindex, int toindex, int num,
                                          int select);
    ...
    while (getStringFromIndexWithDelim(msg, ",", k, buf, sizeof(buf))) {
      ...
      if (PETSKILL_getInt(skillarray, PETSKILL_ILLEGAL) == 1) {
        if (NPC_Util_CheckAssignArgFile(meindex, filename) != NULL) {
          print("PetSkillShop illegal skill:[%d-%s] ->file:%s", skillID,
...
void NPC_PetSkillShopTalked(int meindex, int talkerindex, char *szMes,
                            int color) {
...
void NPC_PetSkillShopWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (FreePetSkillShop(talkerindex, petindex, CHAR_getPetSkill(petindex, slot),
                       skillID) == FALSE) {
    return;
  }
#endif
#ifdef _NB_PETKILL_COST
  iCostFame = 0;
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "cost_fame", msg, sizeof(msg)) !=
      NULL) {
    if (getStringFromIndexWithDelim(msg, ",", skill, buf, sizeof(buf)) != FALSE)
      iCostFame = atoi(buf) * 100;
  }
  if (CHAR_getInt(talkerindex, CHAR_FAME) < iCostFame)
    return;
  CHAR_setInt(talkerindex, CHAR_FAME,
              CHAR_getInt(talkerindex, CHAR_FAME) - iCostFame);
  if (CHAR_getInt(talkerindex, CHAR_FMINDEX) != -1 &&
      (strcmp(CHAR_getChar(talkerindex, CHAR_FMNAME), ""))) {
    int fd = getfdFromCharaIndex(talkerindex);
    char buf[256];
    sprintf(buf, "%d", CHAR_getInt(talkerindex, CHAR_FAME));
