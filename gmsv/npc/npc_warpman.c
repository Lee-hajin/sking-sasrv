...
static void NPC_WarpMan_selectWindow(int meindex, int toindex, int num,
  ...
  if (strstr(npcarg, "%4d") != NULL) {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "Floor", buf3, sizeof(buf3)) !=
        NULL) {
  ...
  if ((NPC_ActionPassCheck(meindex, toindex, buf) == TRUE) ||
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "PayMsg", buf2, sizeof(buf2)) !=
        NULL) {
    ...
    } else if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NomalMsg", token,
                                               sizeof(token)) != NULL) {
...
void NPC_WarpManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NEW_WARPMAN:
    ...
    checkp = TRUE;
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "CHECKPARTY", buff2,
                                        sizeof(buff2)) != NULL) {
...
BOOL NPC_WarpMsg(int meindex, int talker, char *arg) {
  ...
  if (check_TimeTicketMap(fl)) {
    ...
    for (havei = CHAR_STARTITEMARRAY; havei < CHAR_getMaxInventory(talker);
         havei++) {
      itemi = CHAR_getItemIndex(talker, havei);
      if (!ITEM_CHECKINDEX(itemi))
        continue;
    ...
    if (havei == CHAR_getMaxInventory(talker)) {
      CHAR_talkToCli(talker, -1, "请先准备专用门票才可进入。",
                     CHAR_COLORYELLOW);
      return FALSE;
    }
  }
...
static void NPC_NewWarpMan_selectWindow(int meindex, int toindex, int num,
  ...
  while (getStringFromIndexWithDelim(npcarg, "OVER", talkNo, buf,
    ...
    if (strstr(buf, buf1) != NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf, "FREE", token, sizeof(token)) ==
          NULL) {
        print("msgErr");
        return;
      }
      ...
      if ((NPC_ActionPassCheck(meindex, toindex, token) == TRUE) ||
        ...
        //若附合条件 取出 FreeMsg 字串
        if (NPC_Util_GetStrFromStrWithDelim(buf, "FreeMsg", token,
                                            sizeof(token)) == NULL)
          return;
        CONNECT_set_pass(fd, TRUE);
        CONNECT_set_first_warp(fd, TRUE);
...
int CheckWarpMsg(int meindex, int talkerindex, char *npcarg, char *TalkStr) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "CHECKPARTY", buf, sizeof(buf)) !=
      NULL) {
...
BOOL NPC_TreasureEventRunMsg(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "TREASURE_NUM", buf,
                                      sizeof(buf)) == NULL)
    return FALSE;
