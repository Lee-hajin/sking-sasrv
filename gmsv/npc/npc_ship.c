...
NPC_AIR_MSG airmsg[] = {{"msg_gettingon", "PAON！（你无法於中途加入我们唷！）"},
...
BOOL npc_ShipInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "WAVE", buf1, sizeof(buf1)) ==
      NULL) {
 ...
 *   仄井仃日木凶凛及质
 *********************************/
void npc_ShipTalked(int meindex, int talkerindex, char *szMes, int color) {
  int i;
  int partyflg = FALSE;
  ...
  if (!partyflg) {
    ...
    if (CHAR_getWorkInt(meindex, NPC_WORK_MODE) == 0) {
      ...
      if (strstr(szMes, "出发") || strstr(szMes, "出发") ||
          strstr(szMes, "Go") || strstr(szMes, "go")) {
        ...
        if (CHAR_getWorkInt(meindex, NPC_WORK_SEFLG)) {
          ...
          CHAR_sendSEoArroundCharacter(
              ...
              CHAR_getInt(meindex, CHAR_Y), npc_wave, TRUE);
        }
        /* 请  允月凛及丢永本□斥*/
  ...
  if (dir >= 0 && dir <= 7) {
    ...
    if (ret == CHAR_WALKSUCCESSED) {
      ...
      for (i = 1; i < CHAR_PARTYMAX; i++) {
        ...
        if (CHAR_CHECKINDEX(toindex) && (mefl == fl) &&
            (abs(xx - end.x) + abs(yy - end.y) < 10)) {
          int parent_dir;
...
static void npc_ShipSendMsg(int meindex, int talkerindex, int tablenum) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, airmsg[tablenum].option, buf,
                                      sizeof(buf)) != NULL) {
    strcpy(msg, buf);
  } else {
...
BOOL npc_ShipCheckJoinParty(int meindex, int charaindex, BOOL msgflg) {
  ...
  if (CHAR_getWorkInt(meindex, NPC_WORK_MODE) != 0) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_GETTINGON);
    return FALSE;
  }
  /* 天□化不分匀凶日分户 */
  if (CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_NOTPARTY);
    return FALSE;
  }
  /* 由□  奴及谛醒毛民尼永弁允月 */
  if (CHAR_getEmptyPartyArray(meindex) == -1) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_OVERPARTY);
    return FALSE;
  }
  /* 失奶  丞及民尼永弁毛允月(嗟鞅失奶  丞) */
  if (!npc_ShipCheckDeniedItem(meindex, charaindex, argstr)) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_DENIEDITEM);
    return FALSE;
  }
  ...
  if (!npc_ShipCheckAllowItem(meindex, charaindex, FALSE)) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_ALLOWITEM);
    return FALSE;
  }
  /* 伊矛伙及民尼永弁毛允月 */
  if (!npc_ShipCheckLevel(meindex, charaindex, argstr)) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_LEVEL);
    return FALSE;
  }
  /* 奶矛件玄  井民尼永弁允月 */
  ...
  //	{
  //		if( msgflg) npc_ShipSendMsg( meindex, charaindex,
  ...
  if (ret == -1) {
    if (msgflg)
      npc_ShipSendMsg(meindex, charaindex, NPC_AIR_MSG_GOLD);
    return FALSE;
  }
  if (ret != 0) {
    char msgbuf[128];
