...
static void NPC_DengonWriteString(int meindex, int talkerindex, int id,
                                  char *str);
...
BOOL NPC_DengonInit(int meindex) {
  ...
  snprintf(filename, sizeof(filename), "%s%d_%d_%d", DENGONFILEDIR,
  ...
  if (!(f = fopen(filename, "r"))) {
    ...
    for (j = 0; j < DENGONFILELINENUM; j++) {
      fwrite("0000000000:", sizeof(char), DENGONFILECOUNTERSIZE, f);
...
void NPC_DengonWindowTalked(int index, int talker, int seqno, int select,
                            char *data) {
  ...
  if (seqno == CHAR_WINDOWTYPE_DENGON) {
    ...
    case WINDOW_BUTTONTYPE_PREV:
      id = CHAR_getWorkInt(talker, CHAR_WORKSHOPRELEVANT) +
           MESSAGEINONEWINDOW * ((select == WINDOW_BUTTONTYPE_NEXT) ? 1 : -1);
      ...
      if (NPC_DengonReadString(index, id, NPC_sendbuf)) {
        ...
        lssproto_WN_send(fd, WINDOW_MESSAGETYPE_WIDEMESSAGEANDLINEINPUT,
                         buttontype, CHAR_WINDOWTYPE_DENGON,
...
static BOOL NPC_DengonReadString(int meindex, int id, char *str) {
    ...
    return FALSE;
  {
    ...
    for (i = 0; i < MESSAGEINONEWINDOW; i++) {
      fseek(f, ((sendid + i) % DENGONFILELINENUM) * DENGONFILEENTRYSIZE,
            SEEK_SET);
      fread(buf, sizeof(char), DENGONFILEENTRYSIZE, f);
...
static void NPC_DengonWriteString(int meindex, int talker, int id, char *str) {
  ...
  snprintf(buf, sizeof(buf), "%s%d_%d_%d", DENGONFILEDIR,
           CHAR_getInt(meindex, CHAR_FLOOR), CHAR_getInt(meindex, CHAR_X),
           CHAR_getInt(meindex, CHAR_Y));
  ...
  fprintf(f, "%s\n                             %2d/%02d %2d:%02d %s%c", str,
          tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min,
          CHAR_getChar(talker, CHAR_NAME), '\0');
  fclose(f);
}
