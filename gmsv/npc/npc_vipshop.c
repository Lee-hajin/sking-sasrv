...
static void NPC_VipShop_selectWindow(int meindex, int toindex, int num,
                                     int select);
BOOL VipShop_GetMenuStr(int meindex, int toindex, char *npcarg, char *token,
                        int index);
...
static void NPC_VipShop_selectWindow(int meindex, int toindex, int num,
  ...
  case WINDOW_SELECT:
    ...
    if (VipShop_GetMenuStr(meindex, toindex, npcarg, token,
...
BOOL VipShop_GetMenuStr(int meindex, int toindex, char *npcarg, char *token,
  ...
  sprintf(token, "儡咯vip器牢飘:%d        %d/%d页\n",
          CHAR_getInt(toindex, CHAR_AMPOINT), index + 1,
          CHAR_getWorkInt(meindex, NPC_WORK_MAXPAGE));
