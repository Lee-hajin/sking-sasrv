...
void NPC_PoolItemShopTalked(int meindex, int talkerindex, char *szMes,
  ...
  if (!NPC_Util_isFaceToFace(meindex, talkerindex, 2)) {
    if (strcmp(szMes, "poolitemshop") != 0) { //芒绊 龋免
...
//select, data);
  ...
  if (ITEM_getITEM_UseItemnum() >=
      getItemnum() - 100) { // Change fix 防止记忆体满了后,仓库道具会消失
    CHAR_talkToCli(talkerindex, -1,
                   "目前忙碌中，无法取出道具，建议至其它伺服器再试！",
                   CHAR_COLORYELLOW);
    return;
  }
#endif
  ...
  case CHAR_WINDOWTYPE_POOLITEMSHOP_START:
    ...
    case 3:
      if (!CHAR_CheckDepotItem(talkerindex)) {
        ...
        CHAR_talkToCli(talkerindex, -1, "拌沥芒绊，立加己傍！",
                       CHAR_COLORYELLOW);
      } else {
  ...
  case CHAR_WINDOWTYPE_DEPOTITEMSHOP_ADD: //放入
    ...
    if (atoi(data) == 0) {
      ...
      if (NPC_DepotItem_InsertItem(meindex, talkerindex, atoi(data) - 1) ==
          FALSE) {
        NPC_PoolItemShop_DepotItem_Menu(meindex, talkerindex);
        CHAR_talkToCli(talkerindex, -1, "存放道具失败，请稍后再试！",
                       CHAR_COLORYELLOW);
      }
      /*
      #ifdef _NB_FIX_POOLITEM
                                      if( !CHAR_CheckDepotItem( talkerindex) ){
                                              CHAR_GetDepotItem( meindex,
      talkerindex);
                                              //CHAR_talkToCli( talkerindex, -1,
      "取得道具，请稍后！", CHAR_COLORYELLOW); }else{
                                              NPC_PoolItemShop_DepotItem_Menu(
      meindex, talkerindex);
                                      }
      #endif*/
    }
  ...
  case CHAR_WINDOWTYPE_DEPOTITEMSHOP_GET:
    ...
    if (atoi(data) == 0) {
      ...
      if (NPC_DepotItem_gettItem(meindex, talkerindex, atoi(data) - 1) ==
          FALSE) {
        NPC_PoolItemShop_DepotItem_Menu(meindex, talkerindex);
        CHAR_talkToCli(talkerindex, -1, "取出道具失败，请稍后再试！",
                       CHAR_COLORYELLOW);
      }
      /*
      #ifdef _NB_FIX_POOLITEM
...
void NPC_PoolItemShop_printWindow_Start(int meindex, int talkerindex) {
  ...
  if (fd != -1) {
    ...
    if (CHAR_charSaveFromConnect(talkerindex, FALSE)) {
      CHAR_talkToCli(talkerindex, -1, "拌沥沥焊 历厘己傍!", CHAR_COLORRED);
    }
#endif
#ifdef _NPC_DEPOTITEM
    strcpy(message, "3\n\n"
...
void NPC_DepotItem_MakeItemString(int meindex, int talkerindex, char *retstring,
  ...
  for (i = CHAR_STARTITEMARRAY; i < CHAR_MAXITEMHAVE; i++) {
    ...
    snprintf(buff, sizeof(buff),
             ...
             "%s|%d|%d|%d|%s|%d|",
#endif
             ITEM_getChar(itemindex, ITEM_SECRETNAME), poolflg,
...
void NPC_DepotItem_CheckRepeat_Del(int charaindex, int ti, int itemindex) {
  ...
  LogItem(CHAR_getChar(charaindex, CHAR_NAME),
          CHAR_getChar(charaindex, CHAR_CDKEY), itemindex, "仓库重复道具",
...
void NPC_DepotItem_CheckRepeat(int talkerindex) {
  ...
  for (i = 0; i < (CHAR_MAXDEPOTITEMHAVE - 1); i++) {
    ...
    for (j = (i + 1); j < CHAR_MAXDEPOTITEMHAVE; j++) {
      ...
      if (itemindex == itemindex1)
        continue;
      if (!ITEM_CHECKINDEX(itemindex1))
        continue;
...
BOOL NPC_DepotItem_InsertItem(int meindex, int talkerindex, int num) {
  ...
  if (ITEM_getInt(itemindex, ITEM_DROPATLOGOUT) || // 登出後消失
    ...
    // print("\n 改封包!!非法存放道具:%s ", CHAR_getChar( talkerindex,
    // CHAR_CDKEY) );
    return FALSE;
  }
#endif
  if (FreeLockItemSave(talkerindex, itemindex, 2) == 1) {
    return FALSE;
  }
  ...
  LogItem(CHAR_getChar(talkerindex, CHAR_NAME),
          CHAR_getChar(talkerindex, CHAR_CDKEY), itemindex, "存放道具",
...
#ifdef _SAMETHING_SAVEPOINT
  // if(CHAR_charSaveFromConnect(talkerindex, FALSE)){
  // CHAR_talkToCli(talkerindex, -1, "!VIP!", CHAR_COLORRED);
  //}
...
BOOL NPC_DepotItem_gettItem(int meindex, int talkerindex, int num) {
...
#ifdef _AUTO_DEL_ITEM
  {
    ...
    for (j = 0; j < AUTODELITEMNUM; j++) {
      ...
      if (itemid != -1 && itemid == getAutoDelItem(j)) {
        {
          LogItem(CHAR_getChar(talkerindex, CHAR_NAME),
                  CHAR_getChar(talkerindex, CHAR_CDKEY), itemid, "GM删除道具",
                  CHAR_getInt(talkerindex, CHAR_FLOOR),
  ...
  if (ITEM_getInt(itemindex, ITEM_USETIME) > 0) {
    if (ITEM_getInt(itemindex, ITEM_USETIME) < (int)time(NULL)) {
      ITEM_endExistItemsOne(itemindex);
      CHAR_talkToCli(talkerindex, -1, "您的道具已过期，系统自动清除。",
                     CHAR_COLORRED);
      return TRUE;
    }
  }
  ...
  LogItem(CHAR_getChar(talkerindex, CHAR_NAME),
          CHAR_getChar(talkerindex, CHAR_CDKEY), itemindex, "取出道具",
          CHAR_getInt(talkerindex, CHAR_FLOOR),
          CHAR_getInt(talkerindex, CHAR_X), CHAR_getInt(talkerindex, CHAR_Y),
          ITEM_getChar(itemindex, ITEM_UNIQUECODE),
          ITEM_getChar(itemindex, ITEM_NAME), ITEM_getInt(itemindex, ITEM_ID),
...
static BOOL NPC_PoolItemShop_PoolItem(int meindex, int talkerindex, int num) {
...
#endif
  if (FreeLockItemSave(talkerindex, itemindex, 1) == 1) {
    return FALSE;
  }
  ...
  LogItem(CHAR_getChar(talkerindex, CHAR_NAME), /* 平乓仿   */
          CHAR_getChar(talkerindex, CHAR_CDKEY), itemindex, "寄放道具",
          CHAR_getInt(talkerindex, CHAR_FLOOR),
          CHAR_getInt(talkerindex, CHAR_X), CHAR_getInt(talkerindex, CHAR_Y),
          ITEM_getChar(itemindex, ITEM_UNIQUECODE),
          ITEM_getChar(itemindex, ITEM_NAME), ITEM_getInt(itemindex, ITEM_ID),
...
static BOOL NPC_PoolItemShop_DrawItem(int meindex, int talkerindex, int num) {
  ...
  LogItem(CHAR_getChar(talkerindex, CHAR_NAME),
          CHAR_getChar(talkerindex, CHAR_CDKEY), itemindex, "领取道具",
          CHAR_getInt(talkerindex, CHAR_FLOOR),
          CHAR_getInt(talkerindex, CHAR_X), CHAR_getInt(talkerindex, CHAR_Y),
          ITEM_getChar(itemindex, ITEM_UNIQUECODE),
          ITEM_getChar(itemindex, ITEM_NAME), ITEM_getInt(itemindex, ITEM_ID),
