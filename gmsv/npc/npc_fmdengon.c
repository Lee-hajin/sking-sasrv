...
extern struct FMMEMBER_LIST
    memberlist[FMMAXNUM];               // 接收 AC 成员列表资料的 ARRAY
...
unsigned long READTIME1 = 0, READTIME2 = 0, READTIME3 = 0, READTIME4 = 0;
...
// Select Event
void NPC_FmDengonWindowTalked(int index, int talker, int seqno, int select,
                              char *data) {
...
#define DENGONDISTANCE 3
  if (CHAR_getInt(index, CHAR_FLOOR) != 777)
    if (NPC_Util_CharDistance(index, talker) > DENGONDISTANCE)
      return;
#endif
  ...
  if (seqno == CHAR_WINDOWTYPE_FM_DENGON) {
    ...
    case WINDOW_BUTTONTYPE_PREV: {
      ...
      else if ((dengonindex - 7) <= memberlist[fmindex_wk].memoindex &&
               (dengonindex - 7) >= (memberlist[fmindex_wk].memoindex - 7) &&
               memberlist[fmindex_wk].memonum >= DENGONFILELINENUM)
      ...
      if (dengonindex < 6) {
        strcpy(NPC_sendbuf,
               memberlist[fmindex_wk]
                   .memo[memberlist[fmindex_wk].memonum + (dengonindex - 6)]);
        strcat(NPC_sendbuf, "\n");
        for (i = memberlist[fmindex_wk].memonum + (dengonindex - 5);
             i < memberlist[fmindex_wk].memonum; i++) {
          strcat(NPC_sendbuf, memberlist[fmindex_wk].memo[i]);
          strcat(NPC_sendbuf, "\n");
        }
      ...
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_DENGON, buttontype,
    ...
    case WINDOW_BUTTONTYPE_OK: {
      ...
      sprintf(m_buf, "%s|%2d/%02d %2d:%02d %s", tmp_buffer, tm1.tm_mon + 1,
              tm1.tm_mday, tm1.tm_hour, tm1.tm_min,
              CHAR_getChar(talker, CHAR_NAME));
      strcpy(memberlist[fmindex_wk].memo[memberlist[fmindex_wk].memoindex],
             m_buf);

      // send acsv
      saacproto_ACFMWriteMemo_send(
          acfd, CHAR_getChar(talker, CHAR_FMNAME),
          CHAR_getInt(talker, CHAR_FMINDEX),
          makeEscapeString(
              memberlist[fmindex_wk].memo[memberlist[fmindex_wk].memoindex],
              buf, sizeof(buf)),
          fmindex_wk);
  ...
  // 家族之间留言板
    ...
    case WINDOW_BUTTONTYPE_PREV: {
      ...
      dengonindex += 7 * ((select == WINDOW_BUTTONTYPE_NEXT) ? 1 : -1);
      if (dengonindex > fmsmemo.memoindex &&
      ...
      else if (dengonindex > fmsmemo.memonum &&
               fmsmemo.memonum >= FMSDENGONFILELINENUM)
      ...
      if (dengonindex == fmsmemo.memoindex &&
          fmsmemo.memonum >= FMSDENGONFILELINENUM)
        buttontype |= WINDOW_BUTTONTYPE_PREV;
      else if ((dengonindex - 7) <= fmsmemo.memoindex &&
               (dengonindex - 7) >= (fmsmemo.memoindex - 7) &&
               fmsmemo.memonum >= FMSDENGONFILELINENUM)
        buttontype |= WINDOW_BUTTONTYPE_NEXT;
      ...
      if (dengonindex < 6) {
        if ((fmsmemo.memonum + (dengonindex - 6)) >= 140 ||
            (fmsmemo.memonum + (dengonindex - 6)) < 0)
          return;

        strcpy(NPC_sendbuf, fmsmemo.memo[fmsmemo.memonum + (dengonindex - 6)]);

        strcat(NPC_sendbuf, "\n");
        for (i = fmsmemo.memonum + (dengonindex - 5); i < fmsmemo.memonum;
             i++) {
          strcat(NPC_sendbuf, fmsmemo.memo[i]);
          strcat(NPC_sendbuf, "\n");
        }
      ...
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_FMSDENGON, buttontype,
  ...
  // 说明视窗(家族据点)
    ...
    case WINDOW_BUTTONTYPE_OK: {
      ...
      if (CHAR_getInt(talker, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER &&
        ...
        if (check == TRUE) {
          for (i = CHAR_getWorkInt(talker, CHAR_WORK_LISTPAGE);
               i < CHAR_getWorkInt(talker, CHAR_WORK_LISTPAGE) + 5;
               i++) { // 10个庄园
            getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 5,
                                        fmindex, sizeof(fmindex));
            if (atoi(fmindex) <= 0) {
      ...
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[0], "|", 3, x,
                                  sizeof(x));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[0], "|", 4, y,
                                  sizeof(y));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[0], "|", 5,
                                  fmindex, sizeof(fmindex));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[0], "|", 6, name,
                                  sizeof(name));
      if (atoi(fmindex) <= 0)
        strcpy(name, "  未占领");
      sprintf(pointbuf, "%s  萨姆吉尔  %3s   %3s   %s\n", pointbuf, x, y, name);
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[1], "|", 3, x,
                                  sizeof(x));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[1], "|", 4, y,
                                  sizeof(y));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[1], "|", 5,
                                  fmindex, sizeof(fmindex));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[1], "|", 6, name,
                                  sizeof(name));
      if (atoi(fmindex) <= 0)
        strcpy(name, "  未占领");
      sprintf(pointbuf, "%s  玛丽娜斯  %3s   %3s   %s\n", pointbuf, x, y, name);
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[2], "|", 3, x,
                                  sizeof(x));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[2], "|", 4, y,
                                  sizeof(y));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[2], "|", 5,
                                  fmindex, sizeof(fmindex));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[2], "|", 6, name,
                                  sizeof(name));
      if (atoi(fmindex) <= 0)
        strcpy(name, "  未占领");
      sprintf(pointbuf, "%s  加　　加  %3s   %3s   %s\n", pointbuf, x, y, name);
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[3], "|", 3, x,
                                  sizeof(x));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[3], "|", 4, y,
                                  sizeof(y));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[3], "|", 5,
                                  fmindex, sizeof(fmindex));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[3], "|", 6, name,
                                  sizeof(name));
      if (atoi(fmindex) <= 0)
        strcpy(name, "  未占领");
      sprintf(pointbuf, "%s  卡鲁他那  %3s   %3s   %s\n", pointbuf, x, y, name);
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[4], "|", 3, x,
                                  sizeof(x));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[4], "|", 4, y,
                                  sizeof(y));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[4], "|", 5,
                                  fmindex, sizeof(fmindex));
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[4], "|", 6, name,
                                  sizeof(name));
      if (atoi(fmindex) <= 0)
        strcpy(name, "  未占领");
...
#else
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_POINTLIST, WINDOW_BUTTONTYPE_OK,
                       CHAR_WINDOWTYPE_FM_POINTLIST,
#endif
#ifndef _FM_MODIFY
                       CHAR_getWorkInt(index, CHAR_WORKOBJINDEX),
  ...
  // 说明视窗(成员列表)
    ...
    case WINDOW_BUTTONTYPE_OK: {
      ...
      // 增加程式码(向AC要召募人员的值)
      sprintf(enlistbuf, "何练 备己盔 葛笼 咯何|0|%d",
              memberlist[fmindex_wk].accept);
  ...
  // 强者表的选项视窗
    ...
    case 2: // 前十大家族冒险列表
    {
      ...
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_DP,
                       WINDOW_BUTTONTYPE_OK | WINDOW_BUTTONTYPE_PREV,
  ...
  // 选项视窗
...
#ifdef _FMVER21
      //              if( CHAR_getInt( talker, CHAR_FMLEADERFLAG ) ==
      //              FMMEMBER_LEADER ||
      //                  CHAR_getInt( talker, CHAR_FMLEADERFLAG ) ==
      //                  FMMEMBER_ELDER || CHAR_getInt( talker,
      //                  CHAR_FMLEADERFLAG ) == FMMEMBER_VICELEADER ){
      if (CHAR_getInt(talker, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER ||
          CHAR_getInt(talker, CHAR_FMLEADERFLAG) == FMMEMBER_ELDER) {
#else
      if (CHAR_getInt(talker, CHAR_FMLEADERFLAG) == FMMEMBER_MEMBER) {
    ...
    case FM_MEMBERMEMO: {
      ...
      dengonindex = memberlist[fmindex_wk].memoindex;
      if (memberlist[fmindex_wk].memoindex < 6 &&
          memberlist[fmindex_wk].memonum < DENGONFILELINENUM) {
        dengonindex = 6;
      }
    ...
    case FM_FMMEMO: {
      ...
      if (dengonindex < 6) {
        if ((fmsmemo.memonum + (dengonindex - 6)) < 0 ||
            (fmsmemo.memonum + (dengonindex - 6)) >= 140)
          return;
        strcpy(NPC_sendbuf, fmsmemo.memo[fmsmemo.memonum + (dengonindex - 6)]);
        strcat(NPC_sendbuf, "\n");
        for (i = fmsmemo.memonum + (dengonindex - 5); i < fmsmemo.memonum;
    ...
    case FM_FMPOINTPK: {
      ...
      memset(&NPC_sendbuf, 0, sizeof(NPC_sendbuf));
      {
        char fmpointname[10][16] = {
            "萨姆吉尔", "玛丽娜丝", "加    加", "卡鲁它那", "伊　　甸",
            "塔 尔 塔", "尼 克 斯", "弗 列 顿", "亚 伊 欧", "瑞尔亚斯"};
        int i, fmpks_pos;
        char tmpbuf[1024];
        char fmtimebuff[64];
        char fmnumbuff[64];
        ...
        strcpy(NPC_sendbuf, "[style c=4]                         庄 园 族 战 "
                            "排 程 表[/style]\n\n");
        ...
        /*
  sprintf(tmpbuf,"[style c=5]%-8s　%-15s　%-15s  %-15s  %-15s  %-8s %-8s
  %-8s[/style]\n"
                                                                                                                        ,"庄    园"//庄园名字
                                                                                                                        ,"攻方家族"//攻方家族
                                                                                                                        ,"守方家族"//守方家族
        ...
        for (i = 1; i <= FMPOINTNUM; i++) {
          ...
          case FMPKS_FLAG_MANOR_OTHERPLANET: {
            ...
            sprintf(fmtimebuff2, "%02d.%02d %d:%d", tm1->tm_mon + 1,
                    tm1->tm_mday, tm1->tm_hour, tm1->tm_min);

            if (dueltime > 0) {
              sprintf(tmpbuf,
                      "[style c=1]%-8s　%-16s　%-16s  %-16s  %-16s[/style]\n",
              ...
              /*
              sprintf(tmpbuf,"[style c=6]%-8s　%-15s　%-15s  %-15s  %-15s  %-8d
              %-8d %-8d[/style]\n" ,fmpointname[i - 1]//庄园名字
                                                                                      ,fmpks[fmpks_pos + 1].guest_name//攻方家族
                                                                                      ,fmpks[fmpks_pos + 1].host_name//守方家族
                                                                                      ,fmtimebuff2//族战时间
              ...
              sprintf(tmpbuf,
                      "[style c=6]%-8s　%-16s　%-16s  %-16s  %-16s[/style]\n",
                      fmpointname[i - 1] //庄园名字
                      ,
                      fmpks[fmpks_pos + 1].guest_name //攻方家族
                      ,
          ...
          default: {
            sprintf(tmpbuf,
                    "[style c=0]%-8s　%-16s　%-16s  %-16s  %-16s[/style]\n",
                    fmpointname[i - 1] //庄园名字
                    ,
                    "", "", "无排程", "");
            /*
            sprintf(tmpbuf,"[style c=0]%-8s　%-15s　%-15s  %-15s  %-15s  %-8d
            %-8d %-8d[/style]\n" ,fmpointname[i - 1]//庄园名字
                                                                                            ,""
                                                                                            ,""
                                                                                            ,"无排程"
      ...
      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_WIDEMESSAGE,
                       WINDOW_BUTTONTYPE_CANCEL, CHAR_WINDOWTYPE_FM_DPSELECT,
#ifndef _FM_MODIFY
                       CHAR_getWorkInt(index, CHAR_WORKOBJINDEX),
#else
                       -1,
...
#else
    if (buttonevent >= 1 && buttonevent <= 11 &&
        CHAR_getInt(talker, CHAR_FMLEADERFLAG) == FMMEMBER_MEMBER)
#endif
    {
      int fd, i;
      int int_status;
      ...
      strcpy(
          getstatus,
          memberlist[fmindex_wk]
                  .numberlistarray[numberlistindex + buttonevent - 1] +
              (strlen(memberlist[fmindex_wk]
                          .numberlistarray[numberlistindex + buttonevent - 1]) -
               1));
      ...
      if (buttonevent != 11)
        ...
        case 1:
        case 3:
          strcpy(memberlist[fmindex_wk]
                         .numberlistarray[numberlistindex + buttonevent - 1] +
                     (strlen(memberlist[fmindex_wk]
                                 .numberlistarray[numberlistindex +
                                                  buttonevent - 1]) -
                      1),
                 "4");
          break;
      ...
      if (buttonevent == 11) {
        ...
        case 1:
          ...
          sprintf(enlistbuf, "何练 备己盔 葛笼 咯何|%d|%d", numberlistindex,
                  memberlist[fmindex_wk].accept);
          break;
        case 0:
      ...
      strcpy(numberlistbuf,
             memberlist[fmindex_wk].numberlistarray[numberlistindex]);
      strcat(numberlistbuf, "\n");
      ...
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_SELECT, buttontype,
                       CHAR_WINDOWTYPE_FM_MEMBERLIST,
#ifndef _FM_MODIFY
                       CHAR_getWorkInt(index, CHAR_WORKOBJINDEX),
#else
  ...
  // 家族强者表(前叁十)
    ...
    case WINDOW_BUTTONTYPE_PREV: {
      ...
      lssproto_WN_send(fd, WINDOW_FMMESSAGETYPE_DP,
#endif
                       buttontype, CHAR_WINDOWTYPE_FM_DPTOP,
#ifndef _FM_MODIFY
                       CHAR_getWorkInt(index, CHAR_WORKOBJINDEX),
#else
                       -1,
  ...
  // 据点列表
    ...
    case WINDOW_BUTTONTYPE_PREV: {
      ...
      if ((pointlistindex + 5) > 5) {
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[5], "|", 3, x,
                                    sizeof(x));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[5], "|", 4, y,
                                    sizeof(y));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[5], "|", 5,
                                    fmindex, sizeof(fmindex));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[5], "|", 6, name,
                                    sizeof(name));
        if (atoi(fmindex) <= 0)
          strcpy(name, "  未占领");
        sprintf(pointbuf, "%s  塔 尔 塔  %3s   %3s   %s\n", pointbuf, x, y,
                name);
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[6], "|", 3, x,
                                    sizeof(x));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[6], "|", 4, y,
                                    sizeof(y));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[6], "|", 5,
                                    fmindex, sizeof(fmindex));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[6], "|", 6, name,
                                    sizeof(name));
        if (atoi(fmindex) <= 0)
          strcpy(name, "  未占领");
        sprintf(pointbuf, "%s  尼 克 斯  %3s   %3s   %s\n", pointbuf, x, y,
                name);
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[7], "|", 3, x,
                                    sizeof(x));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[7], "|", 4, y,
                                    sizeof(y));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[7], "|", 5,
                                    fmindex, sizeof(fmindex));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[7], "|", 6, name,
                                    sizeof(name));
        if (atoi(fmindex) <= 0)
          strcpy(name, "  未占领");
        sprintf(pointbuf, "%s  弗 列 顿  %3s   %3s   %s\n", pointbuf, x, y,
                name);
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[8], "|", 3, x,
                                    sizeof(x));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[8], "|", 4, y,
                                    sizeof(y));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[8], "|", 5,
                                    fmindex, sizeof(fmindex));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[8], "|", 6, name,
                                    sizeof(name));
        if (atoi(fmindex) <= 0)
          strcpy(name, "  未占领");
        sprintf(pointbuf, "%s  亚 伊 欧  %3s   %3s   %s\n", pointbuf, x, y,
                name);
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[9], "|", 3, x,
                                    sizeof(x));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[9], "|", 4, y,
                                    sizeof(y));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[9], "|", 5,
                                    fmindex, sizeof(fmindex));
        getStringFromIndexWithDelim(fmpointlist.pointlistarray[9], "|", 6, name,
                                    sizeof(name));
        if (atoi(fmindex) <= 0)
          strcpy(name, "  未占领");
...
#endif
                       buttontype, CHAR_WINDOWTYPE_FM_POINTLIST,
#ifndef _FM_MODIFY
                       CHAR_getWorkInt(index, CHAR_WORKOBJINDEX),
#else
                       -1,
    ...
    default:
#ifdef _FIX_FM_FMPOINT
      ApplyFamilyPoint(index, talker,
                       atoi(data) +
                           CHAR_getWorkInt(talker, CHAR_WORK_LISTPAGE) * 5);
#endif
      break;
    }
  }
...
void NPC_FmDengonLooked(int meindex, int lookedindex) {
  ...
  if (!CHAR_CHECKINDEX(lookedindex))
    return;
  ...
  // 必须站在布告栏的前面一格
  if (NPC_Util_CharDistance(lookedindex, meindex) > 1)
    return;
  // 空白处请勿更动
...
void ApplyFamilyPoint(int meindex, int toindex, int select) {
...
#endif
    getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 5, fmindex,
                                sizeof(fmindex));
    if (tkfmindex == atoi(fmindex) - 1) {
      return;
    }
  ...
  if (memberlist[tkfmindex].fmjoinnum < 10)
    ...
    check = 2;
  getStringFromIndexWithDelim(fmpointlist.pointlistarray[select - 1], "|", 5,
                              fmindex, sizeof(fmindex));
  if (atoi(fmindex) <= 0 && check == 0) {
    saacproto_ACFixFMPoint_send(
        acfd, CHAR_getChar(toindex, CHAR_FMNAME), tkfmindex + 1, tkfmindex,
        CHAR_getChar(toindex, CHAR_FMNAME), tkfmindex + 1, tkfmindex, select);
    sprintf(buf, "恭喜你！\n    该庄园已经是你的了。");
    sasql_updateFmPointData(select, NowTime.tv_sec, 0, 0);
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, 0,
                     CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), buf);
  } else {
    if (check == 0)
      sprintf(buf, "该庄园据点已有人占领了，请到该庄园的踢馆管理员处申请踢馆抢"
                   "夺庄园吧！");
    else if (check == 1)
