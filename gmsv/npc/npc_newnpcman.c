...
char UserBBIs[12][256]={
        "小矮子",	"赛亚人",	"辫子男孩",	"酷哥",
"熊皮男",	"大个", "小矮妹",	"熊皮妹",	"帽子妹",
"短法发夹妹",	"手套女",	"辣妹" }; char PetBBIs[4][256]={//格鲁西斯
贝鲁卡 金格萨贝鲁 贝鲁伊卡 "格鲁西斯-红虎",	"贝鲁卡-绿虎",
"金格萨贝鲁-金虎",	"贝鲁伊卡-黄虎" };
...
static void NPC_NewNpcMan_selectWindow(int meindex, int toindex, int num,
                                       int flg);
...
BOOL NPC_NewNpcManInit(int meindex) {
...

  {
    ...
    for (i = 0; i < 48; i++) {
      ...
      //checkPc[i][0], checkPc[i][1],	checkPc[i][2]);
    }
  }
...
static void NPC_NewNpcMan_selectWindow(int meindex, int toindex, int num,
            ...
            case WINDOW_SELECT3:
                            {
                                    int petnum, petindex;
                                    petnum = atoi( data)-1;
                                    if( petnum >= CHAR_MAXPETHAVE || petnum < 0
       ) return; petindex = CHAR_getCharPet( talker, petsel); if(
       !CHAR_CHECKINDEX(petindex) ) return; break;
                            }
    */
...
#ifdef _PET_LOSTPET
  {
    ...
    ti = flg;
    if (havepetelement < 0) {
      CHAR_talkToCli(toindex, -1, "其飘 沫捞 何练钦聪促！", CHAR_COLORYELLOW);
      return;
    }
    ...
    if (getStringFromIndexWithDelim(petstring[ti - 1], "#", 2, petstring1,
                                    sizeof(petstring1)) == FALSE) {
      CHAR_talkToCli(toindex, -1, "领取宠物失败！", CHAR_COLORYELLOW);
      return;
    } else {
      char buf1[256];
      if (getStringFromIndexWithDelim(petstring[ti - 1], "|", 6, buf1,
                                      sizeof(buf1)) == FALSE)
        return;
      ltime = atoi(buf1);
      ltime = time(NULL) - ltime;
      ltime = ltime / (60 * 60 * 24);
      if (getStringFromIndexWithDelim(petstring[ti - 1], "|", 4, buf1,
                                      sizeof(buf1)) == FALSE)
        return;
      cost = atoi(buf1);
  ...
  case WINDOW_DEFIND: {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "CHECK_MSG", token,
                                        sizeof(token)) == NULL) {
      return;
    }
    for (i = 0; i < 48; i++) {
...
void NPC_NewNpcManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NPC_PROGRAMEGINEER_SELECT1:
    NPC_NewNpcMan_selectWindow(meindex, talkerindex, WINDOW_SELECT1,
                               atoi(data));
    break;
  ...
  case NPC_PROGRAMEGINEER_SELECT3:
    NPC_NewNpcMan_selectWindow(meindex, talkerindex, WINDOW_SELECT3,
                               atoi(data));
    break;
  case NPC_PROGRAMEGINEER_SELECTLOSTPET:
    NPC_NewNpcMan_selectWindow(meindex, talkerindex, WINDOW_SELECTLOSTPET,
                               atoi(data));
    break;
...
void NPC_reCheckMyPetUnusual(int meindex, int toindex) //检查宠物异常
{
  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if (!CHAR_CHECKINDEX(petindex))
      continue;
    if ((buf = CHAR_getChar(petindex, CHAR_NAME)) == "\0" || strlen(buf) <= 0) {
      char *PetNAME;
      ...
      if ((PetNAME = ENEMYTEMP_getChar(parry, E_T_NAME)) == "\0" ||
        ...
        print("ANDY Pet(ID:%d) Array:%d err Name:%s (file:%s %d) !! \n", petID,
              array, (PetNAME == NULL) ? "NULL" : PetNAME, __FILE__, __LINE__);
        continue;
      }
...
BOOL CHECK_ReplacePET(int toindex) {
  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if (!CHAR_CHECKINDEX(petindex))
      continue;
    if (CHAR_getInt(petindex, CHAR_PETID) != 1133)
      continue;
    if (CHAR_getInt(petindex, CHAR_SLOT) != 7)
      continue;
    ...
    snprintf(szPet, sizeof(szPet), "[%s]阑(甫) 扒匙林菌嚼聪促。",
    ...
    snprintf(szPet, sizeof(szPet), "[%s]阑(甫) 裙垫窍看嚼聪促。",
             CHAR_getUseName(ret));
    CHAR_talkToCli(toindex, -1, szPet, CHAR_COLORWHITE);

    LogPet(CHAR_getChar(toindex, CHAR_NAME), CHAR_getChar(toindex, CHAR_CDKEY),
           CHAR_getChar(ret, CHAR_NAME), CHAR_getInt(ret, CHAR_LV), "任务得到",
...
void NPC_reCheckItemPilenum(int meindex, int toindex) {
  ...
  for (i = 0; i < CHAR_getMaxInventory(toindex); i++) {
    itemindex = CHAR_getItemIndex(toindex, i);
    if (!ITEM_CHECKINDEX(itemindex))
      continue; // ITEM_USEPILENUMS
    if (ITEM_getInt(itemindex, ITEM_ID) != 20284)
      continue;
...
BOOL NPC_reFindMyLostPet(int meindex, int toindex, char *buf) {
  ...
  while (fgets(line, sizeof(line), fp) && count < 7) {
    if (line == NULL)
      continue;
    if (getStringFromIndexWithDelim(line, "|", 1, buf1, sizeof(buf1)) == FALSE)
      continue;
    if (strcmp(CHAR_getChar(toindex, CHAR_CDKEY), buf1))
      continue;
    if (getStringFromIndexWithDelim(line, "|", 6, buf1, sizeof(buf1)) == FALSE)
      continue;
    ltime = atoi(buf1);
    ltime = time(NULL) - ltime;
    ltime = ltime / (60 * 60 * 24);
    ...
    if (getStringFromIndexWithDelim(line, "|", 2, petname, sizeof(petname)) ==
        FALSE)
      continue;
    if (getStringFromIndexWithDelim(line, "|", 3, buf1, sizeof(buf1)) == FALSE)
      continue;
    lv = atoi(buf1);
    if (getStringFromIndexWithDelim(line, "|", 4, buf1, sizeof(buf1)) == FALSE)
      continue;
    cost = atoi(buf1);
...
BOOL NPC_getLostPetString(int meindex, int toindex) {
  ...
  while (fgets(line, sizeof(line), fp) && count < 7) {
    if (strlen(line) <= 0)
      continue;
    //		if( getStringFromIndexWithDelim( line, "#", 2, buf1, sizeof( buf1)) ==
    //FALSE ) return FALSE;
...
static void NPC_NewNpcMan_selectWindow( int meindex, int toindex, int num, int
flg); int NPC_NewNpcManDelPet(int meindex,int talker, int petsel); BOOL
NPC_NewNpcManAddPet(int meindex, int talker, int petid);
...
BOOL NPC_NewNpcManInit( int meindex )
{
        ...
        if(NPC_Util_GetArgStr( meindex, npcarg, sizeof(npcarg)) == NULL)
{ print("GambleBank:GetArgStrErr"); return FALSE;
        }

        if( NPC_Util_GetStrFromStrWithDelim( npcarg, "REPET", buf1, sizeof(
buf1)) == NULL )	{ return FALSE;
        }

        while( getStringFromIndexWithDelim(buf1, ",", k, buf2, sizeof(buf2)) !=
FALSE )	{ Re_Pet[k-1] = atoi( buf2); if( Re_Pet[k-1] < 0 ) return FALSE; k++;
...
void NPC_NewNpcManLoop( int meindex)
{
        ...
        if( (CHAR_getWorkInt( meindex, NPC_WORK_CURRENTTIME) + fulltime) >
NowTime.tv_sec ) return; CHAR_setWorkInt( meindex , NPC_WORK_INDEX, -1);
        CHAR_setWorkInt( meindex, NPC_WORK_CURRENTTIME, NowTime.tv_sec);

        //test
        if(NPC_Util_GetArgStr( meindex, npcarg, sizeof(npcarg)) == NULL)
{ print("GambleBank:GetArgStrErr"); return;
        }
...
buf1)) == NULL )	{ return;
        ...
        while( getStringFromIndexWithDelim(buf1, ",", k, buf2, sizeof(buf2)) !=
FALSE )	{ Re_Pet[k-1] = atoi( buf2); if( Re_Pet[k-1] < 0 ) return; k++; if(
(k-1) >= arraysizeof( Re_Pet) ) break;
        }
        //test end
}
...
return;
        ...
        if( CHAR_getWorkInt( meindex , NPC_WORK_INDEX) >= 0 )	{
                        ...
                        print("\n NPC_WORK_INDEX = %d ", CHAR_getWorkInt(
meindex , NPC_WORK_INDEX)); CHAR_talkToCli( talkerindex, meindex, buf1,
CHAR_COLORYELLOW); return; }else	{ CHAR_setWorkInt( meindex ,
NPC_WORK_INDEX, talkerindex); print("\n start : NPC_WORK_INDEX = talkerindex");
                CHAR_setWorkInt( meindex, NPC_WORK_CURRENTTIME, NowTime.tv_sec);
                NPC_NewNpcMan_selectWindow( meindex, talkerindex, NEWNPC_START,
0);
        }
}
...
flg)
{
        ...
        if(NPC_Util_GetArgStr( meindex, npcarg, sizeof(npcarg)) == NULL)
{ print("GambleBank:GetArgStrErr"); CHAR_setWorkInt( meindex , NPC_WORK_INDEX,
-1); return ;
        }
        ...
        case NEWNPC_END:
                ...
                if( flg <= CHAR_MAXPETHAVE && flg >= 0 )	{
                        ...
                        if( ( petid = NPC_NewNpcManDelPet( meindex, toindex,
petno)) >= 0 )	{ if( NPC_NewNpcManAddPet( meindex, toindex, petid) == TRUE )
{ type_str = 2;
                                }
                        }
                }
        ...
        if(NPC_Util_GetStrFromStrWithDelim( npcarg, token_str[ type_str],
token,sizeof( token) ) == NULL) { print("token_str[%d] err !", type_str);
                CHAR_setWorkInt( meindex , NPC_WORK_INDEX, -1);
                return;
        }
...
void NPC_NewNpcManWindowTalked( int meindex, int talkerindex, int seqno, int
select, char *data)
{
...
return;
        ...
        case NPC_NEWNPCMAN_START:
                if( select != WINDOW_BUTTONTYPE_YES && select !=
WINDOW_BUTTONTYPE_OK )	{ CHAR_setWorkInt( meindex , NPC_WORK_INDEX, -1);
                        return;
                }
                if(select==WINDOW_BUTTONTYPE_CANCEL)	{
                        CHAR_setWorkInt( meindex , NPC_WORK_INDEX, -1);
                }else	{
                        NPC_NewNpcMan_selectWindow( meindex, talkerindex,
NEWNPC_MAN, -1);
                }
        break;
        ...
        case NPC_NEWNPCMAN_MAN:
                {
                        ...
                        if(select==WINDOW_BUTTONTYPE_CANCEL)	{
                                ...
                                NPC_NewNpcMan_selectWindow( meindex,
talkerindex, NEWNPC_END, petnum);
                        }
                }
                break;
...
BATTLE_CHARMODE_NONE ) return -1; defpet = CHAR_getInt( talker,
CHAR_DEFAULTPET); if(defpet == petsel)	{ CHAR_setInt( talker, CHAR_DEFAULTPET,
-1); lssproto_KS_send( fd, -1, TRUE);
        }
...
CHAR_getChar( petindex, CHAR_NAME));
        ...
        if(CHAR_getChar( petindex, CHAR_UNIQUECODE)!=NULL){
                strncpy(uStr, CHAR_getChar( petindex, CHAR_UNIQUECODE), sizeof(
STRING32)); }else{ strncpy(uStr, "Pet_Unicode_null", sizeof( STRING32));
        }
        ...
        CHAR_talkToCli( talker, -1, msgbuf,  CHAR_COLORWHITE);
                                LogPet(
                                        ...
                                        CHAR_getChar( petindex,
CHAR_UNIQUECODE), petindex, talker
                                );
...
CHAR_getChar(petindex2,CHAR_NAME)); CHAR_talkToCli( talker, -1, msgbuf,
CHAR_COLORWHITE); for(j = 0; j < CHAR_MAXPETHAVE; j++){ petindex =
CHAR_getCharPet(talker, j); if( !CHAR_CHECKINDEX( petindex) ) continue;
                CHAR_complianceParameter( petindex );
...
BOOL CHECK_YEARPET( int toindex)
{
                        ...
                        for( i=0;i<CHAR_MAXPETHAVE;i++)	{
                                ...
                                if( !CHAR_CHECKINDEX(petindex) )
                                        continue;
                                for( k=0;k<arraysizeof( Re_Pet); k++)	{
                                        if( CHAR_getInt( petindex, CHAR_PETID)
== Re_Pet[k] )	{ print("\n check yearpet find it"); return TRUE;
                                        }
                                }
                        }
                        return FALSE;
