...
#ifdef _CALLBUS
  NPC_WORK_STARTX = CHAR_NPCWORKINT11,
  NPC_WORK_STARTY = CHAR_NPCWORKINT12,
  NPC_WORK_STARTFLOOR = CHAR_NPCWORKINT13,
#endif
};
...
NPC_BUS_MSG busmsg[] = {{"msg_gettingon", "PAON！（你无法於中途加入我们唷！）"},
...
BOOL NPC_BusInit(int meindex) {
  ...
  for (i = 1; i <= routenum; i++) {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, routetostring, buf,
                                        sizeof(buf)) == NULL) {
  ...
  CHAR_setInt(meindex, CHAR_LOOPINTERVAL, NPC_BUS_WAITINGMODE_WAITTIME);
...
#ifdef _CALLBUS
    CHAR_setWorkInt(meindex, NPC_WORK_STARTFLOOR,
                    CHAR_getInt(meindex, CHAR_FLOOR));
    CHAR_setWorkInt(meindex, NPC_WORK_STARTX, CHAR_getInt(meindex, CHAR_X));
    CHAR_setWorkInt(meindex, NPC_WORK_STARTY, CHAR_getInt(meindex, CHAR_Y));
    CHAR_setWorkInt(meindex, CHAR_WORK_CALLBUSTIME, -1);
#endif
  }
 ...
 *   仄井仃日木凶凛及质
 *********************************/
void NPC_BusTalked(int meindex, int talkerindex, char *szMes, int color) {
  int i, routetemp = -1;
  int partyflg = FALSE;
...
#ifdef _CALLBUS
      NPC_Util_GetArgStr(meindex, argstr, sizeof(argstr));
      if (CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUS) == 1) {
        for (i = 1; i < CHAR_getWorkInt(meindex, NPC_WORK_ROUTEMAX) + 1; i++) {
          char routetostring[64];
          snprintf(routetostring, sizeof(routetostring), "routename%d", i);
          if (NPC_Util_GetStrFromStrWithDelim(argstr, routetostring, buf,
                                              sizeof(buf))) {
            if (strstr(buf, szMes)) {
              routetemp = i;
        ...
        if (routetemp == -1)
          routetemp = 1;
        CHAR_setWorkInt(meindex, NPC_WORK_CURRENTROUTE, routetemp);
      }
#endif
      //		#define NPC_BUS_DEBUGROUTINTG	"routingtable:"
      if (strstr(szMes, "啊") || strstr(szMes, "啊磊") || strstr(szMes, "Go") ||
          strstr(szMes, "go") || routetemp != -1) {
#ifdef _CALLBUS
        if ((CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUS) == 1) &&
            (CHAR_getInt(meindex, CHAR_FLOOR) !=
                 CHAR_getWorkInt(meindex, NPC_WORK_STARTFLOOR) ||
             CHAR_getInt(meindex, CHAR_X) !=
                 CHAR_getWorkInt(meindex, NPC_WORK_STARTX) ||
             CHAR_getInt(meindex, CHAR_Y) !=
                 CHAR_getWorkInt(meindex, NPC_WORK_STARTY))) {
          int startfloor = CHAR_getWorkInt(meindex, NPC_WORK_STARTFLOOR);
          int startx = CHAR_getWorkInt(meindex, NPC_WORK_STARTX);
          int starty = CHAR_getWorkInt(meindex, NPC_WORK_STARTY);
          for (i = 0; i < CallBusCount; i++) {
            if (CallBusIdx[i] == meindex && CallBusLeader[i] == talkerindex) {
              break;
            }
          }
          if (i >= CallBusCount)
            return;
          CHAR_warpToSpecificPoint(meindex, startfloor, startx, starty);
          for (i = 1; i < CHAR_PARTYMAX; i++) {
            int partyindex = CHAR_getWorkInt(meindex, CHAR_WORKPARTYINDEX1 + i);
            if (CHAR_CHECKINDEX(partyindex))
              CHAR_warpToSpecificPoint(partyindex, startfloor, startx, starty);
          }
          CHAR_setWorkInt(meindex, NPC_WORK_ROUTEPOINT, 2);
          NPC_BusSetPoint(meindex, argstr);
          NPC_BusSetDestPoint(meindex, argstr);
        ...
        if (CHAR_getWorkInt(meindex, NPC_WORK_SEFLG)) {
          CHAR_sendSEoArroundCharacter(CHAR_getInt(meindex, CHAR_FLOOR),
                                       CHAR_getInt(meindex, CHAR_X),
                                       CHAR_getInt(meindex, CHAR_Y), 60, TRUE);
        }
        /* 请  允月凛及丢永本□斥*/
        for (i = 1; i < CHAR_PARTYMAX; i++) {
          int partyindex = CHAR_getWorkInt(meindex, CHAR_WORKPARTYINDEX1 + i);
...
#ifdef _CALLBUS
    if (CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUS) == 1) {
      if (CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUSTIME) != -1) { //被呼叫了
        if (CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUSTIME) + 30 <
            NowTime.tv_sec) {
          CHAR_setWorkInt(meindex, CHAR_WORK_CALLBUSTIME, -1);
          CHAR_DischargeParty(meindex, 0);
          for (i = 0; i < CallBusCount; i++) {
            if (CallBusIdx[i] == meindex) {
              CallBusBusy[i] = 0;
              break;
            }
          }
  ...
  case 3:
    ...
    if (CHAR_getWorkInt(meindex, NPC_WORK_CURRENTTIME) + 3 < NowTime.tv_sec) {
      ...
      /* 伙□玄瑁烂允月 */
      {
        ...
        // NPC_WORK_CURRENTROUTE));
      }
      /* 垫五窖曰白仿弘  凳 */
#ifdef _CALLBUS
      if (CHAR_getWorkInt(meindex, CHAR_WORK_CALLBUS) != 1) //呼叫型不用逆走
#endif
...
static void NPC_BusSendMsg(int meindex, int talkerindex, int tablenum) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, busmsg[tablenum].option, buf,
                                      sizeof(buf)) != NULL) {
    strcpy(msg, buf);
  } else {
...
BOOL NPC_BusCheckJoinParty(int meindex, int charaindex, BOOL msgflg) {
  ...
  //	{
  //		if( msgflg) NPC_BusSendMsg( meindex, charaindex,
  //NPC_BUS_MSG_EVENT); 		return FALSE;
  //	}
