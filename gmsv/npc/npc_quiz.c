...
int NPC_QuizRandAnswer(int p_no, int q_no, int mode, char *work1, char *work2,
                       char *work3);
...
BOOL NPC_QuizInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "Quiznum", buf, sizeof(buf)) ==
      NULL) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM) > (tbl[0] - 1)) {
    print("%s不符合条件,圆圈quiz的问题数(%d? 比出题数(%d题)不足",
          CHAR_getChar(meindex, CHAR_NAME), (tbl[0] - 1),
...
void NPC_QuizTalked(int meindex, int talkerindex, char *szMes, int color) {
...
static void NPC_Quiz_selectWindow(int meindex, int talker, int num) {
  ...
  case 0:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "StartMsg", token,
                                        sizeof(token)) == NULL) {
  ...
  case 1:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "EntryItem", buf2,
                                        sizeof(buf2)) != NULL) {
      if (NPC_EntryItemCheck(talker, buf2) == FALSE) {
        entryflg = FALSE;
      }
    }
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "EntryStone", buf3,
                                        sizeof(buf3)) != NULL) {
    ...
    if (entryflg == FALSE) {
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "NoEntryMsg", token,
                                          sizeof(token)) != NULL) {
  ...
  case 3:
    ...
    /*-- 旦正□玄丢永本□斥 --*/
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemFullMsg", token,
                                        sizeof(token)) == NULL) {
  ...
  case 2:
    /*--丢奶件--*/
    {
      ...
      if (PLAYER.quizno == CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM)) {
        double avg;
        int okflg;
        char buf2[512];
        ...
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "GetItem", buf,
                                            sizeof(buf)) != NULL) {
        ...
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "Border", buf,
                                            sizeof(buf)) != NULL) {
          okflg = NPC_QuizBorderLine(PLAYER.answer, buf);
          if (okflg == -1) {
            NPC_Util_GetStrFromStrWithDelim(argstr, "FailureMsg", buf2,
                                            sizeof(buf2));
        ...
        CHAR_setWorkInt(meindex,
                        CHAR_WORK_PLAYER1 +
                            CHAR_getWorkInt(talker, CHAR_WORKSHOPRELEVANT),
                        -1);

        //				print("\n %s
        //%d开放",CHAR_getChar(talker,CHAR_NAME),(int)PLAYER.ptr);
      ...
      if (Quiz[questionno].answertype == 4) {
        ...
        sprintf(token,
                ...
                "\n２． %s"
                "\n３． %s",
                len, PLAYER.quizno, buf, work1, work2, work3);
        ...
        sprintf(token,
                ...
                "\n\n１． %s"
                "\n２． %s",
                len, PLAYER.quizno, buf, work1, work2);
      }
  ...
  case 4:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "Party", token,
                                        sizeof(token)) != NULL) {
      buttontype = WINDOW_BUTTONTYPE_OK;
      messagetype = WINDOW_MESSAGETYPE_MESSAGE;
      windowno = CHAR_WINDOWTYPE_QUIZ_START;
  ...
  /*-仇仇匹霜耨允月--*/
  lssproto_WN_send(fd, messagetype, buttontype, windowno,
                   CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
}

void NPC_QuizWindowTalked(int meindex, int talkerindex, int seqno, int select,
                          char *data) {
  int i = 0;
  int old_no;
  ...
  case CHAR_WINDOWTYPE_QUIZ_MAIN: {
    ...
    if (point < 0) {
      CHAR_setWorkInt(meindex,
                      CHAR_WORK_PLAYER1 +
                          CHAR_getWorkInt(talkerindex, CHAR_WORKSHOPRELEVANT),
                      -1);
      // free(PLAYER.ptr);
      return;
    }
    // print("\nlen=%d,size=%d,point=%d\n",sizeof(pl_ptr),sizeof(struct
    // pl),point);
    memcpy(&PLAYER, pl_ptr, sizeof(struct pl));
    ...
    if (select == WINDOW_BUTTONTYPE_OK) {
      if (i >= quizcnt || i < 0)
        return;
      if (Quiz == NULL || strlen(Quiz[i].select1) >= 128)
        return;
...
BOOL NPC_QuizAddItem(int talker, char *buf) {
  ...
  if (itemindex != -1) {
    LogItem(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
            CHAR_getChar(talker, CHAR_CDKEY), itemindex, "猜谜领到道具",
...
int *NPC_GetQuestion(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "Type", buf, sizeof(buf)) !=
      NULL) {
    type = atoi(buf);
  }
  if (type <= 0) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "Answer", buf, sizeof(buf)) !=
      NULL) {
    answer = atoi(buf);
  }
  if (answer <= 0) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "Level", buf, sizeof(buf)) !=
      NULL) {
    level = atoi(buf);
  }
  if (level <= 0) {
    ...
    if (strstr(buff3, "*") != NULL) {
      ...
      for (i = 0; i < CHAR_getMaxInventory(talker); i++) {
        ...
        if (ITEM_CHECKINDEX(itemindex)) {
          ...
          if (itemno == id) {
            cnt++;
            LogItem(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿o */
                    CHAR_getChar(talker, CHAR_CDKEY), itemindex, "猜谜交出道具",
