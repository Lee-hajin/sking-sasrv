...
static void NPC_FreePetSkillShop_selectWindow(int meindex, int toindex, int num,
                                              int select);
...
BOOL NPC_FreePetSkillShopInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "pet_skill", msg, sizeof(msg)) !=
      NULL) {
    ...
    while (getStringFromIndexWithDelim(msg, ",", k, buf, sizeof(buf))) {
      ...
      if (PETSKILL_getInt(skillarray, PETSKILL_ILLEGAL) == 1) {
        if (NPC_Util_CheckAssignArgFile(meindex, filename) != NULL) {
          print("\n宠物技能不规范:[%d-%s] ->文件:%s\n", skillID,
                PETSKILL_getChar(skillarray, PETSKILL_NAME), filename);
        }
      }
    }
  } else {
...
void NPC_FreePetSkillShopTalked(int meindex, int talkerindex, char *szMes,
                                int color) {
...
static void NPC_FreePetSkillShop_selectWindow(int meindex, int toindex, int num,
  ...
  case START_WINDOW: {
    ...
    //玩家条件判断
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "FREE", buf1, sizeof(buf1)) !=
        NULL) {
    ...
    if (Evflg == FALSE) {
      CHAR_talkToCli(toindex, -1, "你想让宠物学特殊技能？可你的条件还不够喔！",
                     CHAR_COLORYELLOW);
      CHAR_setWorkInt(toindex, CHAR_WORKSHOPRELEVANT, 0);
      return;
    } else {
...
void NPC_FreePetSkillShopWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case SELECTSKILL_TEACH:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "skill_rate", msg,
                                        sizeof(msg)) != NULL) {
      rate = atof(msg);
    }
...
#ifdef _NB_PETKILL_COST
    iCostFame = 0;
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "cost_fame", msg,
                                        sizeof(msg)) != NULL) {
      if (getStringFromIndexWithDelim(msg, ",", skill, buf, sizeof(buf)) !=
          FALSE) {
        iCostFame = atoi(buf);
      }
    }
    if (CHAR_getInt(talkerindex, CHAR_EVENTPOINT) < iCostFame) {
      CHAR_talkToCli(talkerindex, -1, "声望不足，无法学习！", CHAR_COLORYELLOW);
      return;
    }
#endif
#ifdef _PETSKILL_SHOP_LUA
    if (FreePetSkillShop(talkerindex, petindex,
                         CHAR_getPetSkill(petindex, slot), skillID) == FALSE) {
      return;
    }
#endif
    /*
...
#ifdef _NB_PETKILL_COST
    if (iCostFame > 0) {
      CHAR_setInt(talkerindex, CHAR_EVENTPOINT,
                  CHAR_getInt(talkerindex, CHAR_EVENTPOINT) - iCostFame);

      if (CHAR_getInt(talkerindex, CHAR_FMINDEX) != -1 &&
          (strcmp(CHAR_getChar(talkerindex, CHAR_FMNAME), ""))) {
        int fd = getfdFromCharaIndex(talkerindex);
        char buf[256];
        sprintf(buf, "%d", CHAR_getInt(talkerindex, CHAR_EVENTPOINT));
        saacproto_ACFixFMData_send(
            acfd, CHAR_getChar(talkerindex, CHAR_FMNAME),
            CHAR_getInt(talkerindex, CHAR_FMINDEX),
            CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI), FM_FIX_FAME, buf,
            "", CHAR_getWorkInt(talkerindex, CHAR_WORKFMCHARINDEX),
            CONNECT_getFdid(fd));
      }

      char famemsg[64];
      snprintf(famemsg, sizeof(famemsg), "扣除 %d 声望", (iCostFame / 100));
      CHAR_talkToCli(talkerindex, -1, famemsg, CHAR_COLORYELLOW);
    }
#endif
    CHAR_setPetSkill(petindex, slot, skillID);
    CHAR_setInt(talkerindex, CHAR_GOLD,
                (CHAR_getInt(talkerindex, CHAR_GOLD) - cost));
...
BOOL NPC_FreePetSkillMakeStr(int meindex, int toindex, int select) {
  ...
  if (select == 0) {
    ...
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PETSKILLSHOP,
                     WINDOW_BUTTONTYPE_NONE, CHAR_WINDOWTYPE_WINDOWPETSKILLSHOP,
                     CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
  }
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "main_msg", msg, sizeof(msg)) ==
      NULL) {
    print("\n CAN'T FIND main_msg !!");
    return FALSE;
  }
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "skill_rate", msg, sizeof(msg)) !=
      NULL) {
    rate = atof(msg);
  }
...
#ifdef _NB_PETKILL_COST
      iCostFame = 0;
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "cost_fame", msg2,
                                          sizeof(msg2)) != NULL) {
        if (getStringFromIndexWithDelim(msg2, ",", i, buf, sizeof(buf)) !=
            FALSE) {
          iCostFame = atoi(buf);
        }
      }
  ...
  lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PETSKILLSHOP, WINDOW_BUTTONTYPE_NONE,
                   SELECTSKILL_TEACH,
                   CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
#ifdef _NB_PETKILL_COST
  sprintf(token, "FAME|%d", CHAR_getInt(toindex, CHAR_EVENTPOINT));
...
BOOL NPC_SkillShopDelItems(int meindex, int talker, char *buf) {
  ...
  while (getStringFromIndexWithDelim(buf, ",", i, buf1, sizeof(buf1)) !=
    ...
    if (NPC_SkillShopItemCheck(meindex, talker, atoi(item), atoi(cout)) ==
        FALSE) {
      Evflg = FALSE;
      break;
    }
...
BOOL NPC_SkillShopPetCheck(int toindex, int petindex, int skillID) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(Free, "FREE", data, sizeof(data)) !=
      NULL) {
    BOOL EvFlg = TRUE;
    i = 1;
