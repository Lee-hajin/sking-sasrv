...
void NPC_Gamble_RouletteTalked(int meindex, int talkerindex, char *szMes,
                               int color) {
  return;
}

void NPC_Gamble_RouletteWindowTalked(int meindex, int talkerindex, int seqno,
    ...
    if (CHAR_getWorkInt(meindex, NPC_WORK_SEFLG) < 0) {
      ...
      if (CHAR_getWorkInt(index, CHAR_NPCWORKINT7) == 2) {
        ...
        if (CHAR_getInt(meindex, CHAR_X) == 14 &&
          ...
          print("\n\n####################\n 小猪不在原点!![%d,%d]",
                CHAR_getInt(meindex, CHAR_X), CHAR_getInt(meindex, CHAR_Y));
        }
      } else if (CHAR_getWorkInt(index, CHAR_NPCWORKINT7) == 1 ||
                 (CHAR_getWorkInt(index, CHAR_NPCWORKINT9) < 30 &&
                  CHAR_getWorkInt(index, CHAR_NPCWORKINT9) > 24)) { //准备
        if (CHAR_getInt(meindex, CHAR_X) == 14 &&
            CHAR_getInt(meindex, CHAR_Y) == 8) {
        } else {
...
static void Find_Master(int meindex) {
  ...
  for (object = MAP_getTopObj(floor, x, y); object;
    ...
    if (!CHAR_CHECKINDEX(findex))
      continue;
    if (CHAR_getInt(findex, CHAR_WHICHTYPE) != CHAR_GAMBLEMASTER)
      continue;
    if (CHAR_getWorkInt(findex, CHAR_NPCWORKINT8) != 10001)
      continue;
...
BOOL ReadPointData(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "roulette", buf1, sizeof(buf1)) ==
      NULL) {
  ...
  while (getStringFromIndexWithDelim(buf1, ";", k, buf2, sizeof(buf2)) !=
    ...
    if (getStringFromIndexWithDelim(buf2, ",", 1, buf3, sizeof(buf3)) ==
        FALSE) {
      return FALSE;
    }
    PointData[i].x = atoi(buf3);
    if (getStringFromIndexWithDelim(buf2, ",", 2, buf3, sizeof(buf3)) ==
        FALSE) {
      return FALSE;
    }
...
static int Gamble_RouletteSetPoint(int meindex) {
  ...
  { //检查路线
    struct tagWalkStartPoint {
      int x;
      int y;
    } WalkStart[] = {{22, 5}, {7, 5}, {7, 11}, {22, 11}, {22, 5}, {7, 5}};
    int ch_x, ch_y;
    int next_x, next_y;
    int h;
