...
static void NPC_FMPKCallMan_selectWindow(int meindex, int toindex, int num,
                                         int select);
 ...
 *   仄井仃日木凶凛及质
 *********************************/
void NPC_FMPKCallManTalked(int meindex, int talkerindex, char *szMes,
                           int color) {
  /* 皿伊奶乩□卞覆仄化分仃  杀允月 */
...
static void NPC_FMPKCallMan_selectWindow(int meindex, int toindex, int num,
  ...
  // print("meindex:%d toindex:%d num:%d select:%d", meindex, toindex, num,
  // select);
  ...
  case 0:
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "MainMsg", buf, sizeof(buf)) ==
        NULL)
      return;
    sprintf(token,
            "3\n               ★家族ＰＫ场★\n"
...
#endif
    {
      if (CHAR_getWorkInt(toindex, CHAR_WORKFMSETUPFLAG) != 1) {
        ...
        // WON ADD 增加族长召唤的限制
        {
          ...
          // Terry add for 索引值为 0 的家族会有问题,所以多加判断家族名称
          strncpy(szFMName, CHAR_getChar(toindex, CHAR_FMNAME),
                  sizeof(szFMName));
          // end

          for (j = 0; j <= MAX_SCHEDULEMAN * MAX_SCHEDULE; j++) {
            // Terry fix for 索引值为 0 的家族会有问题,所以多加判断家族名称
            //						if( (fmindex == fmpks[j].host_index ) || (fmindex ==
            //fmpks[j].guest_index ) ){
            if ((fmindex == fmpks[j].host_index &&
                 strcmp(szFMName, fmpks[j].host_name) == 0) ||
                (fmindex == fmpks[j].guest_index &&
                 strcmp(szFMName, fmpks[j].guest_name) == 0)) {
              // end
              fmpk_pos = j;
              break;
            }
          ...
          if (fmpk_pos != -1) {
#ifdef FAMLYNUM_CF
            if (fmpk_pos <= getfamlynum() * MAX_SCHEDULE + 1) {
#else
...
void NPC_FMPKCallManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case CHAR_WINDOWTYPE_FMPKCALLMAN_CALL:
    if (select == WINDOW_BUTTONTYPE_YES) {
      lssproto_WN_send(
          ...
          makeEscapeString("\n召唤中・・・请稍候！\n＠＃＄％↑＆＊・・・", buf,
                           sizeof(buf)));
      NPC_CallFMMember(meindex, CHAR_getInt(talkerindex, CHAR_FLOOR),
  ...
  case CHAR_WINDOWTYPE_FMPKCALLMAN_LEAVE:
    if (select == WINDOW_BUTTONTYPE_YES) {
      ...
      CHAR_setWorkInt(talkerindex, CHAR_WORKWARPCHECK, FALSE);
      CHAR_getElderPosition(CHAR_getInt(talkerindex, CHAR_LASTTALKELDER), &spfl,
                            &spx, &spy);
      ...
      if (CHAR_getCharUse(charindex)) {
        ...
        if (CHAR_getWorkInt(charindex, CHAR_WORKBATTLEMODE) ==
          ...
          if (CHAR_getInt(charindex, CHAR_FMLEADERFLAG) < 0 ||
              CHAR_getInt(charindex, CHAR_FMLEADERFLAG) == FMMEMBER_APPLY)
            continue;
#else
          if (CHAR_getInt(charindex, CHAR_FMLEADERFLAG) <= 0)
            continue;
          ...
          if (checkflag != 1)
            lssproto_WN_send(
                fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_YESNO,
                CHAR_WINDOWTYPE_FMPKCALLMAN_COME,
