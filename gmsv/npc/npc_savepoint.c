...
BOOL NPC_SavePointInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "ID", buf2, sizeof(buf2)) ==
      NULL) {
...
void NPC_SavePointTalked(int meindex, int talkerindex, char *msg, int color) {
...
#ifndef _FREE_SAVE
  if (NPC_CheckFlg(CHAR_getInt(talkerindex, CHAR_SAVEPOINT), oldmanid) ==
      FALSE) {
...
#endif
  {
    ...
    //  Robin 0619 display time
    {
      ...
      time(&new_t);
      new_t = new_t - initTime + serverruntimenum;
      dd = (int)new_t / 86400;
      new_t = new_t % 86400;
      hh = (int)new_t / 3600;
      new_t = new_t % 3600;
      ...
      if (dd > 0) {
        snprintf(timeMsg, sizeof(timeMsg),
                 "本星球%s已经启动了 %d 日 %d 小时 %d 分 %d 秒。",
                 getGameservername(), dd, hh, mm, ss);
      } else if (hh > 0) {
        snprintf(timeMsg, sizeof(timeMsg),
                 "本星球%s已经启动了 %d 小时 %d 分 %d 秒。", getGameservername(),
                 hh, mm, ss);
      } else {
        snprintf(timeMsg, sizeof(timeMsg), "本星球%s已经启动了 %d 分 %d 秒。",
                 getGameservername(), mm, ss);
      }
      strcpy(buf3, timeMsg);
#else
      if (dd > 0) {
        ...
        snprintf(timeMsg, sizeof(timeMsg), "本星球已经启动了 %d 分 %d 秒。", mm,
                 ss);
      }
      strcpy(buf3, timeMsg);
#endif
    }
    {
      ...
      if (getPlayerNum() > 0) {
        sprintf(temp, "目前线上人数为 %d 人。",
                abs(StateTable[LOGIN]) + luaplayernum + getPlayerNum());
      } else {
        sprintf(temp, "目前线上人数为 %d 人。", abs(StateTable[LOGIN]) + luaplayernum);
      }
#else
      extern unsigned int StateTable[];
      if (getPlayerNum() > 0) {
        sprintf(temp, "目前线上人数为 %d 人。",
                abs(StateTable[LOGIN]) + getPlayerNum() + rand() % 10);
      } else {
      ...
      if (fd != -1) {
        if (CHAR_getFlg(talkerindex, CHAR_ISSAVE) == TRUE) {
          ...
          if (!chwk)
            return;
          // strcat(buf3,"系统自动为你存档！");
          CHAR_charSaveFromConnectAndChar(fd, chwk, FALSE);
        }
      }
#endif
    }
    // CHAR_talkToCli( talkerindex , -1 , buf2 , color );
    CHAR_talkToCli(talkerindex, -1, buf3, color);
  }
}
 ...
 *
-------------------------------------------*/
void NPC_SavePointWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "GetItem", buf, sizeof(buf)) !=
    ...
    if (checkflg == 0)
      return FALSE;
		...
		return TRUE; // Arminius 2.5 如果没有设 GetItem 则一律储存
...
void NPC_MessageDisp(int meindex, int talker, int MesNo) {
  ...
  switch (MesNo) {
    ...
    CHAR_setInt(talker, CHAR_LASTTALKELDER,
                CHAR_getWorkInt(meindex, CHAR_WORKOLDMANID));
