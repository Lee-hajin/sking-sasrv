...
static void NPC_FMWarpMan_selectWindow(int meindex, int toindex, int num,
                                       int select);
...
void CHECK_FMPknumInFloor(int meindex) {
  ...
  if (fmpks_pos < 0 || fmpks_pos >= MAX_SCHEDULEMAN * MAX_SCHEDULE)
    return;
...
BOOL NPC_FMWarpManInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "WARP1", buf, sizeof(buf)) ==
      NULL) {
  ...
  // shan begin
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "WARP2", buf, sizeof(buf)) ==
      NULL) {
    print("FMWarpman Err is %s", npcarg);
    print("FMWarpman Err");
    return FALSE;
...
void NPC_FMWarpManLoop(int meindex) {
  ...
  if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE) {
    ...
    if ((fmpks[fmpks_pos].prepare_time > 0) &&
      ...
      if (tm1.tm_hour - (fmpks[fmpks_pos].dueltime / 100) < 0)
        clock = (fmpks[fmpks_pos].dueltime % 100) +
                fmpks[fmpks_pos].prepare_time - 60;
      else
        clock = fmpks[fmpks_pos].dueltime - (tm1.tm_hour * 100) +
                fmpks[fmpks_pos].prepare_time;
      ...
      if ((tm1.tm_min >= clock) && (fmpks[fmpks_pos].prepare_time > 0)) {
        ...
        sprintf(token, "庄园所有权争夺战『%s ＶＳ %s』开～战～罗～",
                fmpks[fmpks_pos].guest_name, fmpks[fmpks_pos].host_name);
    ...
    if (meid > MANORNUM) {
      ...
      if (tm1.tm_min == FMWARPMAN_CLEANPLACE) {
        // 判断胜负、WARP离场、设定ＮＰＣ状态
        NPC_BattleOut(fmpks[fmpks_pos].host_index,
                      fmpks[fmpks_pos].guest_index);
        NPC_FMBATTLESET(floor, fmpks[fmpks_pos].host_index,
                        fmpks[fmpks_pos].guest_index, -1);
        if (fmpks[fmpks_pos].win == 0) { // 生存战
          ...
          if ((CHAR_getWorkInt(meindex, NPC_WORK_FMNUMI) - num1) <
              (CHAR_getWorkInt(meindex, NPC_WORK_FMNUMII) - num2))
            winflag = 1;
          else if ((CHAR_getWorkInt(meindex, NPC_WORK_FMNUMI) - num1) >
                   (CHAR_getWorkInt(meindex, NPC_WORK_FMNUMII) - num2))
            winflag = 2;
          else
            winflag = 3;
    ...
    if (winflag > 0 && winflag < 3) {
      ...
      if (meid > MANORNUM) {
        ...
        // 流浪家族pk 过声望
        if (winflag == 1) {
          saacproto_ACFixFMPK_send(
              acfd, fmpks[fmpks_pos].host_name, fmpks[fmpks_pos].host_index + 1,
              fmpks[fmpks_pos].host_index, fmpks[fmpks_pos].guest_name,
              fmpks[fmpks_pos].guest_index + 1, fmpks[fmpks_pos].guest_index);
        } else {
          saacproto_ACFixFMPK_send(
              acfd, fmpks[fmpks_pos].guest_name,
              fmpks[fmpks_pos].guest_index + 1, fmpks[fmpks_pos].guest_index,
              fmpks[fmpks_pos].host_name, fmpks[fmpks_pos].host_index + 1,
              fmpks[fmpks_pos].host_index);
        }
#ifdef _MANOR_PKRULE
      }
      ...
      {
#ifdef FAMLYNUM_CF
        if (meid > 0 && meid <= getfamlynum()) {
#else
        if (meid > 0 && meid <= MANORNUM) {
#endif
          ...
          if (winflag == 1) {
            char token[256];
            sprintf(token, " (%d:%d) %d/%d/%d", tm1.tm_hour, tm1.tm_min,
                    tm1.tm_year + 1900, tm1.tm_mon + 1, tm1.tm_mday);
            saacproto_ACFixFMPoint_send(
                acfd, fmpks[fmpks_pos].host_name,
                fmpks[fmpks_pos].host_index + 1, fmpks[fmpks_pos].host_index,
                fmpks[fmpks_pos].guest_name, fmpks[fmpks_pos].guest_index + 1,
                fmpks[fmpks_pos].guest_index, meid);
            // Syu 增加庄园战胜负Log
            Logfmpk(fmpks[fmpks_pos].host_name, fmpks[fmpks_pos].host_index,
                    num1, fmpks[fmpks_pos].guest_name,
                    fmpks[fmpks_pos].guest_index, num2, token, "", "", 2);
            int fmid;
            char fmindex[4];
#ifdef FAMLYNUM_CF
            for (fmid = 0; fmid < getfamlynum(); fmid++) { // 10个庄园
#else
            for (fmid = 0; fmid < MANORNUM; fmid++) { // 10个庄园
#endif
              getStringFromIndexWithDelim(fmpointlist.pointlistarray[fmid], "|",
                                          5, fmindex, sizeof(fmindex));
              if (fmpks[fmpks_pos].host_index == atoi(fmindex) - 1)
                break;
            }
            int fmpknum = sasql_queryFmPointData(fmid + 1, 1);
            if (fmpknum > -1) {
              sasql_updateFmPointData(fmid + 1, 0, fmpknum + 1, 1);
            }
#ifdef _NEW_MANOR_LAW
            // 原家族守住了庄园,家族成员可得到石币
            ...
            for (i = 0; i < FAMILY_MAXMEMBER; i++) {
              ...
              if (iCharindex >= 0 && CHAR_getCharUse(iCharindex)) {
                // 获得金钱 = 个人气势 * 5000
                int iAddGold = ((float)CHAR_getInt(iCharindex, CHAR_MOMENTUM) /
                                100.0f) /* * 5000.0f */;
                ...
                if (iGold + iAddGold > CHAR_MAXBANKGOLDHAVE) {
                  ...
                  LogFMPKGetMomey(CHAR_getChar(iCharindex, CHAR_FMNAME),
                                  ...
                                  CHAR_getInt(iCharindex, CHAR_MOMENTUM),
                                  iAddGold, 0);
                } else {
                ...
                CHAR_talkToCli(iCharindex, -1,
                               "辛苦了!守护住庄园的奖金已汇入你的个人银行",
                               CHAR_COLORRED);
              }
            }
#else
                ...
                if (CHAR_getInt(meindex, CHAR_RIDEPET) != -1) {
                  ...
                  if (CHAR_getInt(rideindex, CHAR_BASEBASEIMAGENUMBER) ==
                    ...
                    CHAR_sendCToArroundCharacter(
                        CHAR_getWorkInt(iCharindex, CHAR_WORKOBJINDEX));
                  }
                }
              }
            }
            ...
            char fmindextmp[4];
#ifdef FAMLYNUM_CF
            for (fmidtmp = 0; fmidtmp < getfamlynum(); fmidtmp++) { // 10个庄园
#else
            for (fmidtmp = 0; fmidtmp < MANORNUM; fmidtmp++) { // 10个庄园
#endif
              getStringFromIndexWithDelim(fmpointlist.pointlistarray[fmidtmp],
                                          "|", 5, fmindextmp,
                                          sizeof(fmindextmp));
              if (fmpks[fmpks_pos].host_index == atoi(fmindextmp) - 1)
                break;
            }
            sasql_updateFmPointData(fmidtmp + 1, NowTime.tv_sec, 0, 0);
            saacproto_ACFixFMPoint_send(
                acfd, fmpks[fmpks_pos].guest_name,
                fmpks[fmpks_pos].guest_index + 1, fmpks[fmpks_pos].guest_index,
                fmpks[fmpks_pos].host_name, fmpks[fmpks_pos].host_index + 1,
            ...
            // Syu 增加庄园战胜负Log
            Logfmpk(fmpks[fmpks_pos].guest_name, fmpks[fmpks_pos].guest_index,
                    num2, fmpks[fmpks_pos].host_name,
                    fmpks[fmpks_pos].host_index, num1, token, "", "", 2);
#ifdef _RIDEMODE_20
            if (getRideMode() == 2)
#endif
            {
                  ...
                  if (CHAR_getInt(meindex, CHAR_RIDEPET) != -1) {
                    ...
                    if (CHAR_getInt(rideindex, CHAR_BASEBASEIMAGENUMBER) ==
                        ...
                        CHAR_getInt(rideindex, CHAR_BASEBASEIMAGENUMBER) ==
                            100373) {
                      CHAR_setInt(iCharindex, CHAR_RIDEPET, -1);
                      CHAR_send_P_StatusString(iCharindex,
          ...
          for (i = 0; i < FAMILY_MAXMEMBER; i++) {
            ...
            if (iCharindex >= 0 && CHAR_getCharUse(iCharindex)) {
              ...
              CHAR_talkToCli(iCharindex, -1, "庄园战後个人及家族气势归零",
                             CHAR_COLORRED);
            }
      ...
      NPC_WarpFamily(
          floor, fmpks[fmpks_pos].host_index, fmpks[fmpks_pos].guest_index,
      ...
      if (meid > 0 && meid <= MANORNUM) {
        ...
        saacproto_ACFixFMPoint_send(
            ...
            meid);
        // Syu 增加庄园战胜负Log
        Logfmpk(fmpks[fmpks_pos].guest_name, fmpks[fmpks_pos].guest_index, num2,
                fmpks[fmpks_pos].host_name, fmpks[fmpks_pos].host_index, num1,
                token, "", "", 2);
#ifdef _NEW_MANOR_LAW
        int i, iFmIndex1, iFmIndex2, iCharindex;
        // 双方的个人及家族气势都要归零
...
void NPC_FMWarpManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (!CHAR_CHECKINDEX(talkerindex))
    return;
  ...
  if (CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI) ==
      fmpks[fmpks_pos].host_index) {
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "WARP1", buf, sizeof(buf)) ==
        NULL) {
  ...
  } else if (CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI) ==
             fmpks[fmpks_pos].guest_index) {
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "WARP2", buf, sizeof(buf)) ==
        NULL) {
    ...
    CHAR_talkToCli(talkerindex, meindex, "你并非对战双方所属任一家族成员！",
                   CHAR_COLORYELLOW);
    return;
  }
  getStringFromIndexWithDelim(buf, ",", 1, buff2, sizeof(buff2));
  ...
  case CHAR_WINDOWTYPE_WINDOWWARPMAN_MAIN:
    if (select == WINDOW_BUTTONTYPE_YES) {
      ...
      if ((fmpks[fmpks_pos].host_index != -1) &&
        ...
        if (CHAR_getWorkInt(meindex, NPC_WORK_FMNUMI) >
          ...
          return;
                 ...
                 (CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI) ==
                  fmpks[fmpks_pos].guest_index)) {
        CHECK_FMPknumInFloor(meindex);
        if (CHAR_getWorkInt(meindex, NPC_WORK_FMNUMII) >
            (fmpks[fmpks_pos].max_player - 1)) {
          lssproto_WN_send(
              fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
              makeEscapeString(
                  "已经不能再进去～！\n家族进场人数已经到达设定上限了！", token,
                  sizeof(token)));
          return;
        }
      }
        ...
        if (CHAR_getWorkInt(talkerindex, CHAR_WORKWARPCHECK) == TRUE) {
          if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE) {
            ...
            /*						{
                                                                    FILE *fp;
                                                                    struct tm
               tm1; char szFileName[128];

                                                                    memcpy(&tm1,
               localtime((time_t *)&NowTime.tv_sec), sizeof(tm1));
                                                                    memset(szFileName,0,sizeof(szFileName));
                                                                    sprintf(szFileName,"FMPkWarp.%d%d%d.log",tm1.tm_mon,tm1.tm_hour,tm1.tm_min);
                                                                    fp =
               fopen(szFileName,"a+"); if(fp != NULL){
            ...
            return;
          {
            ...
            int clock = 0;
            int fmpks_pos =
                CHAR_getWorkInt(meindex, NPC_WORK_ID) * MAX_SCHEDULE;
            memcpy(&tm1, localtime((time_t *)&NowTime.tv_sec), sizeof(tm1));
            clock = ((fmpks[fmpks_pos].dueltime % 100) +
                     fmpks[fmpks_pos].prepare_time) -
                    tm1.tm_min;
...
void NPC_ERR_FMDiSP(int meindex, int talker, int errNO) {
  ...
  if (errNO == 1) {
    /*--由□  奴□匹  仄井仃凶--*/
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "PartyMsg", token,
                                        sizeof(token)) == NULL) {
    ...
    /*--嗯互凶曰卅中匹允［--*/
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "MoneyMsg", token,
                                        sizeof(token)) == NULL) {
      /*--云嗯互箫曰卅中及卞｝涩烂今木化卅井匀凶日｝票及丢永本□斥毛
       * 歹曰卞请允--*/
      sprintf(token, "\n\n金钱似乎不足唷，请存好钱後再过来。");
    }
  }
...
static void NPC_FMWarpMan_selectWindow(int meindex, int toindex, int num,
  ...
  /*--涩烂白央奶伙及
   * 卞白夫失谛醒互隙烂今木化中月井今木化中木壬白夫失谛醒及喃曰请仄*/
  ...
  if (CHAR_getInt(toindex, CHAR_FMLEADERFLAG) == FMMEMBER_NONE) {
    ...
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1,
                     -1, makeEscapeString(tmpbuf, token, sizeof(token)));
    return;
  }
  ...
  if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE) {
    if (((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
          fmpks[fmpks_pos].host_index) &&
         (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                 fmpks[fmpks_pos].host_name) == 0)) ||
        ((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
          fmpks[fmpks_pos].guest_index) &&
         (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                 fmpks[fmpks_pos].guest_name) == 0))) {
      if ((fmpks[fmpks_pos].host_index != -1) &&
          (CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
           fmpks[fmpks_pos].host_index)) {
        // WON ADD 修正家族pk场的约战问题

        if (fmpks[fmpks_pos].flag == -1)
          return;
      ...
      sprintf(token, buf2, fmpks[fmpks_pos].guest_name,
              fmpks[fmpks_pos].host_name, tm1.tm_hour, tm1.tm_min);
      CHAR_setWorkInt(toindex, CHAR_WORKFMMANINDEX, meindex);
      CHAR_setWorkInt(toindex, CHAR_WORKSHOPRELEVANTTRD, -1);
    } else if ((((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
                  fmpks[fmpks_pos + 1].host_index) &&
                 (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                         fmpks[fmpks_pos + 1].host_name) == 0)) ||
                ((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
                  fmpks[fmpks_pos + 1].guest_index) &&
                 (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                         fmpks[fmpks_pos + 1].guest_name) == 0))) &&
               (((fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_SCHEDULED) ||
                 (fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_MANOR_PREPARE)))) {
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NextMsg", buf2,
                                          sizeof(buf2)) == NULL)
        return;
      sprintf(token, buf2, fl, tm1.tm_hour, tm1.tm_min);
      CONNECT_set_pass(fd, FALSE);
      CONNECT_set_first_warp(fd, FALSE);
    } else if ((strcmp(fmpks[fmpks_pos].host_name, "") == 0) ||
               (strcmp(fmpks[fmpks_pos].guest_name, "") == 0)) {
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "TalkMsg", buf2,
                                          sizeof(buf2)) == NULL)
        return;
      sprintf(token, buf2);
      CONNECT_set_pass(fd, FALSE);
      ...
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "AskMsg", buf2,
                                          sizeof(buf2)) == NULL)
        return;
      CONNECT_set_pass(fd, FALSE);
      CONNECT_set_first_warp(fd, FALSE);
  ...
  } else if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) ==
             NPC_STATEBUSY) // 两家族战斗中
  {
    if (((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
          fmpks[fmpks_pos].host_index) &&
         (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
        ...
        ((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
         ...
         (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                 fmpks[fmpks_pos].guest_name) == 0))) {
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "BusyMsg", buf2,
                                          sizeof(buf2)) == NULL) {
      ...
      NPC_GetPKFMNum(floor, fmpks[fmpks_pos].host_index,
                     fmpks[fmpks_pos].guest_index, &num1, &num2);
      sprintf(token, buf2, fmpks[fmpks_pos].host_name, num1,
              fmpks[fmpks_pos].guest_name, num2, tm1.tm_hour, tm1.tm_min);
    } else if ((((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
                  fmpks[fmpks_pos + 1].host_index) &&
                 (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                         fmpks[fmpks_pos + 1].host_name) == 0)) ||
    ...
    if (((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
        ...
        ((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) ==
         ...
         (strcmp(CHAR_getChar(toindex, CHAR_FMNAME),
                 fmpks[fmpks_pos].guest_name) == 0))) {
      char tmpbuf[256];
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "EndMsg", buf2,
                                          sizeof(buf2)) == NULL) {
      ...
      if (strcmp(CHAR_getWorkChar(meindex, NPC_WORK_WINFMNAME), "") == 0)
        ...
        sprintf(tmpbuf, "%s获胜了！！",
                CHAR_getWorkChar(meindex, NPC_WORK_WINFMNAME));
      sprintf(token, buf2, fmpks[fmpks_pos].guest_name,
...
void NPC_BattleOut(int index1, int index2) {
  ...
  for (i = 0; i < FAMILY_MAXMEMBER; i++) {
    ...
    charindex2 = familyMemberIndex[index2][i];
    if (CHAR_getCharUse(charindex1)) {
      if (CHAR_getWorkInt(charindex1, CHAR_WORKBATTLEMODE) !=
          BATTLE_CHARMODE_NONE)
    ...
    if (CHAR_getCharUse(charindex2)) {
      if (CHAR_getWorkInt(charindex2, CHAR_WORKBATTLEMODE) !=
          BATTLE_CHARMODE_NONE)
