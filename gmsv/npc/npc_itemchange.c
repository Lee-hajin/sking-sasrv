...
static void NPC_ItemchangeMan_selectWindow(int meindex, int toindex, int num,
                                           int select);
BOOL ItemchangeMan_GetNeedStr(int meindex, int toindex, char *npcarg,
                              char *token, int num);
BOOL ItemchangeMan_GetMenuStr(int meindex, int toindex, char *npcarg,
                              char *token);
...
void NPC_ItemchangeManTalked(int meindex, int talkerindex, char *msg,
                             int color) {
...
void NPC_ItemchangeManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NPC_ITEMCHANGE_SELECT:
    NPC_ItemchangeMan_selectWindow(meindex, talkerindex, ITEMCHANGE_SELECT,
                                   flg);
    break;
  case NPC_ITEMCHANGE_MESSAGE:
    flg = atoi(data);
    CHAR_setWorkInt(talkerindex, CHAR_WORKSHOPRELEVANT, flg);
    NPC_ItemchangeMan_selectWindow(meindex, talkerindex, ITEMCHANGE_MESSAGE,
                                   flg);
    break;
...
static void NPC_ItemchangeMan_selectWindow(int meindex, int toindex, int num,
  ...
  case ITEMCHANGE_MESSAGE:
    if (ItemchangeMan_GetNeedStr(meindex, toindex, npcarg, token, select) ==
        FALSE)
      return;
    buttontype = WINDOW_BUTTONTYPE_YESNO;
  ...
  case ITEMCHANGE_END:
    if (ActionNpc_CheckMenuFree(meindex, toindex, npcarg, select) == FALSE) {
      memset(token, 0, sizeof(token));
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "FAIL_MSG", token,
                                          sizeof(token)) != NULL) {
...
BOOL ItemchangeMan_GetMenuStr(int meindex, int toindex, char *npcarg,
  ...
  while (getStringFromIndexWithDelim(npcarg, "}", talkNo, buf1, sizeof(buf1)) !=
         FALSE) {
    char *ItemName;
    talkNo++;

    if (NPC_Util_GetStrFromStrWithDelim(buf1, "CHANGEITEM", buf2,
                                        sizeof(buf2)) != NULL) {
      addID = atoi(buf2);
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf1, "CHANGEMSG", buf3,
                                          sizeof(buf3)) == NULL)
        continue;
      strcat(buf3, "\n");
    }
...
BOOL ItemchangeMan_GetNeedStr(int meindex, int toindex, char *npcarg,
  ...
  if (npcarg == NULL)
    return FALSE;
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NeedHead", buf1, sizeof(buf1)) ==
      NULL) {
    print("ItemchangeMan Get NeedHead ERROR!\n%s", npcarg);
    return FALSE;
  }
  sprintf(token, "%s\n", buf1);
  if (getStringFromIndexWithDelim(npcarg, "}", num, buf1, sizeof(buf1)) ==
      FALSE) {
    printf("Get NeedMess error !!\n");
    return FALSE;
  }

  if (NPC_Util_GetStrFromStrWithDelim(buf1, "NeedItem", buf2, sizeof(buf2)) !=
      NULL) {
    while (getStringFromIndexWithDelim(buf2, ",", talkNo, buf1, sizeof(buf1)) !=
           FALSE) {
      char *ItemName = NULL;
      talkNo++;
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "NeedMsg", buf2, sizeof(buf2)) ==
        NULL)
      return FALSE;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf2, "DelGold", buf1, sizeof(buf1)) !=
      NULL) {
    char goldmess[256];
    int gold = atoi(buf1);
    if (gold > 0) {
...
// void ItemchangeMan_CheckMenuFree( int meindex, int toindex, char *npcarg, int
// nums)
#endif
