...
void NPC_FmLetterTalked(int meindex, int talkerindex, char *szMes, int color) {
...
#endif
    getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 9, token,
                                sizeof(token));
    village = atoi(token);
    if (village == CHAR_getInt(meindex, CHAR_FMINDEX)) {
      getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 5, token,
                                  sizeof(token));
      fmindex = atoi(token);
      break;
    }
  ...
  if (CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) == FMMEMBER_MEMBER
    ...
    lssproto_WN_send(
        getfdFromCharaIndex(talkerindex), WINDOW_MESSAGETYPE_MESSAGE,
        WINDOW_BUTTONTYPE_OK, -1, -1,
        makeEscapeString("\n只有本庄园的族长才能制作邀请函及同意书！", buf,
                         sizeof(buf)));
}
/*********************************
 * 苇日木凶凛及质
...
void NPC_FmLetterWindowTalked(int meindex, int talkerindex, int seqno,
