...
static void NPC_AutoPk_selectWindow(int meindex, int toindex, int num,
                                    int select);
...
void NPC_AutoPkWindowTalked(int meindex, int talkerindex, int seqno, int select,
                            char *data) {
  ...
  case NPC_AUTOPK_JOINPK:
    if (autopktime > 2) {
      ...
      for (i = 0; i < GMMAXNUM; i++) {
        if (strcmp(CHAR_getChar(talkerindex, CHAR_CDKEY), gminfo[i].cdkey) ==
            0) {
...
#endif
      {
        ...
        sprintf(
            buf2,
            "玩家%s参加比赛,你还没参加吗？本次奖品丰富，你还等什么？快来吧！",
  ...
  case NPC_AUTOPK_HISTORY:
    ...
    if (select == WINDOW_BUTTONTYPE_NEXT) {
      NPC_AutoPk_selectWindow(meindex, talkerindex, WINDOW_HISTORY_NEXT,
                              atoi(data));
      return;
    }
    if (select == WINDOW_BUTTONTYPE_PREV) {
      NPC_AutoPk_selectWindow(meindex, talkerindex, WINDOW_HISTORY_PREV,
                              atoi(data));
      return;
    }
    break;
...
static void NPC_AutoPk_selectWindow(int meindex, int toindex, int num,
  ...
  case WINDOW_SELECT:
    ...
    if (select == 1) {
      ...
      if (AutoPk_GetHistoryStr(token,
                               CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE))) {
        buttontype = WINDOW_BUTTONTYPE_CANCEL;
      } else {
  ...
  case WINDOW_HISTORY_NEXT:
    windowtype = WINDOW_MESSAGETYPE_MESSAGE;
    CHAR_setWorkInt(toindex, CHAR_WORK_LISTPAGE,
                    CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE) + 1);
    if (AutoPk_GetHistoryStr(token,
                             CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE)))
  ...
  case WINDOW_HISTORY_PREV:
    ...
    CHAR_setWorkInt(toindex, CHAR_WORK_LISTPAGE,
                    CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE) - 1);
    if (CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE) < 1)
      CHAR_setWorkInt(toindex, CHAR_WORK_LISTPAGE, 1);
    if (AutoPk_GetHistoryStr(token,
                             CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE)) ||
        CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE) == 1)
...
BOOL AutoPk_GetHistoryStr(char *token, int page) {
  ...
  sprintf(buf,
          ...
          "　优胜%7d     %s\n"
          "　优胜%7d     %s\n",
          atoi(history[0].winnum), history[0].name, atoi(history[1].winnum),
          history[1].name, atoi(history[2].winnum), history[2].name,
          atoi(history[3].winnum), history[3].name, atoi(history[4].winnum),
          history[4].name);
  strcat(token, buf);
  if (fgets(line, sizeof(line), fp) == NULL) {
    fclose(fp);
  ...
  for (i = 0; i < playernum; i++) {
    ...
    if (CHAR_getInt(i, CHAR_FLOOR) == 20000) {
      ...
      sprintf(buf, "%3d%5d    %s\n", CHAR_getInt(i, CHAR_TRANSMIGRATION),
              CHAR_getInt(i, CHAR_LV), CHAR_getChar(i, CHAR_NAME));
      num++;
      strcat(token, buf);
    }
  }
...
int AutoPk_PKTimeGet(void) { return autopktime; }
...
void AutoPk_GetChampionShip(void) {
  ...
  if (strlen(ship[0].cdkey) > 0) {
    sprintf(token, "恭喜%s获得本次PK比赛冠军，杀人得分总数%d分！", ship[0].name,
            ship[0].winnum);
    AutoPk_PKSystemTalk(token, token);
  }
  if (strlen(ship[1].cdkey) > 0) {
    sprintf(token, "恭喜%s获得本次PK比赛亚军，杀人得分总数%d分！", ship[1].name,
            ship[1].winnum);
    AutoPk_PKSystemTalk(token, token);
  }
  if (strlen(ship[2].cdkey) > 0) {
    sprintf(token, "恭喜%s获得本次PK比赛季军，杀人得分总数%d分！", ship[2].name,
            ship[2].winnum);
    AutoPk_PKSystemTalk(token, token);
  }
  sprintf(token, "恭喜%s和%s获得本次PK比赛优胜奖！", ship[3].name,
          ship[4].name);
  AutoPk_PKSystemTalk(token, token);
...
BOOL AutoPk_AddItem(int charindex, int index) {
  ...
  if (ret < 0 || ret >= CHAR_getMaxInventory(charindex)) {
...
void setAward(BOOL val) { getaward = val; }

BOOL getAward() { return getaward; }

BOOL getBAward() { return baward; }
