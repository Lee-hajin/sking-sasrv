...
void NPC_RidermanWindowTalked(int meindex, int talkerindex, int seqno,
...

{
  ...
  if (buttonproc[button].use == TRUE) {
    ...
    if (newwin == 6) {
      ...
      if (CHAR_getInt(talkerindex, CHAR_GOLD) < w.takegold) {
        ...
        lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK,
                         -1, -1, makeEscapeString(buf2, buf, sizeof(buf)));
        return;
      }
    ...
    if (newwin == 5) {
      ...
      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_SHOWRIDEPET, WINDOW_BUTTONTYPE_OK,
                       -1, -1, "");
      return;
    }
