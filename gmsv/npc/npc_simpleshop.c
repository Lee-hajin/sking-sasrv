...
static void NPC_SimpleShopOpenShopWindow(int meindex, int cliindex,
                                         char *npcarg);
static void NPC_SimpleShopGetEscapedItemString(int shopindex, int cliindex,
                                               char *str);
...
static void NPC_SimpleShopNormalWindow(int meindex, int playerindex, char *str);
...
#define SIMPLESHOP_FATALERRSTR "그것은 아파요!죄송합니다, 최근 상황이 좋지 않습니다....에...에"
...
#define SHOPRANGE 3
  ...
  if (NPC_Util_charIsInFrontOfChar(talker, meindex, SHOPRANGE) &&
    ...
    getStringFromIndexWithDelim(npcarg, "|", OPENSHOPTOKEN, token,
                                sizeof(token));
    if (NPC_SimpleShopOpenShopOrNot(msg, token) &&
      ...
      getStringFromIndexWithDelim(token, ",", rand() % tokennum + 1, tmp,
                                  sizeof(tmp));
...
void NPC_SimpleShopSpecialTalked(int meindex, int talker, char *msg,
                                 int color) {
  if (CHAR_getInt(talker, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      NPC_Util_isFaceToFace(meindex, talker, SHOPRANGE)) {
    char npcarg[NPC_UTIL_GETARGSTR_BUFSIZE], token[NPC_UTIL_GETARGSTR_LINEMAX];
    ...
    case 's': {
      ...
      if (CHAR_getWorkInt(meindex, CHAR_WORKSHOPCLIENTINDEX) != talker) {
        getStringFromIndexWithDelim(npcarg, "|", ANOTHERPLAYERTOKEN, token,
                                    sizeof(token));
      ...
      case 'b':
        if ((ret = NPC_SimpleShopProcessBuyMsg(msg, meindex, talker)) < 0) {
          switch (ret) {
          case -2:
            getStringFromIndexWithDelim(npcarg, "|", POORTOKEN, token,
                                        sizeof(token));
            NPC_SimpleShopNormalWindow(meindex, talker, token);
            break;
          case -3:
            getStringFromIndexWithDelim(npcarg, "|", ITEMFULLTOKEN, token,
                                        sizeof(token));
            NPC_SimpleShopNormalWindow(meindex, talker, token);
            break;
          case -1:
          ...
          getStringFromIndexWithDelim(npcarg, "|", THANKYOUTOKEN, token,
                                      sizeof(token));
          NPC_SimpleShopNormalWindow(meindex, talker, token);
        }
        break;
      case 's':
        ...
        if ((ret = NPC_SimpleShopProcessSellMsg(msg, meindex, talker)) < 0) {
          switch (ret) {
          case -2:
            getStringFromIndexWithDelim(npcarg, "|", RAREITEMTOKEN, token,
                                        sizeof(token));
            NPC_SimpleShopNormalWindow(meindex, talker, token);
            break;
          case -1:
...
void NPC_LimitBuyInShopSpecialTalked(int meindex, int talker, char *msg,
                                     int color) {
  if (CHAR_getInt(talker, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      NPC_Util_isFaceToFace(meindex, talker, SHOPRANGE)) {
    char npcarg[NPC_UTIL_GETARGSTR_BUFSIZE], token[NPC_UTIL_GETARGSTR_LINEMAX];
    ...
    case 's': {
      ...
      case 's':
        ...
        if ((ret = NPC_LimitBuyInShopProcessSellMsg(msg, meindex, talker)) <
            0) {
          switch (ret) {
          case -2:
            getStringFromIndexWithDelim(npcarg, "|", RAREITEMTOKEN, token,
...
static BOOL NPC_SimpleShopProcessBuyMsg(char *msg, int shopindex,
  ...
  itemtableindex =
      NPC_ShopWindowIndexToItemTableIndex(shopindex, shopwindowindex);
...
static BOOL NPC_SimpleShopProcessSellMsg(char *msg, int shopindex,
  ...
  {
    LogItem(CHAR_getChar(playerindex, CHAR_NAME), /* 平乓仿   */
            CHAR_getChar(playerindex, CHAR_CDKEY), itemind, "Sell",
            CHAR_getInt(playerindex, CHAR_FLOOR),
            CHAR_getInt(playerindex, CHAR_X), CHAR_getInt(playerindex, CHAR_Y),
            ITEM_getChar(itemind, ITEM_UNIQUECODE),
            ITEM_getChar(itemind, ITEM_NAME), ITEM_getInt(itemind, ITEM_ID),
            playerindex);
  }
...
static BOOL NPC_LimitBuyInShopProcessSellMsg(char *msg, int shopindex,
                                             int playerindex) {
  ...
  getStringFromIndexWithDelim(npcarg, "|", BUYINITEMLISTTOKEN, token,
                              sizeof(token));
  buyokflag = FALSE;
  for (i = 1; getStringFromIndexWithDelim(token, ",", i, buf, sizeof(buf));
       i++) {
...
static int NPC_SimpleShopGetItemNum(int meindex) {
  ...
  counter = 0;
  for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(meindex); i++) {
...
static void NPC_SimpleShopOpenShopWindow(int meindex, int cliindex,
  ...
  getStringFromIndexWithDelim(npcarg, "|", MAINTOKEN, maincaption,
                              sizeof(maincaption));
  getStringFromIndexWithDelim(npcarg, "|", BUYTOKEN, buycaption,
                              sizeof(buycaption));
  getStringFromIndexWithDelim(npcarg, "|", OKBUYTOKEN, okbuycaption,
                              sizeof(okbuycaption));
  getStringFromIndexWithDelim(npcarg, "|", SELLTOKEN, sellcaption,
                              sizeof(sellcaption));
  getStringFromIndexWithDelim(npcarg, "|", OKSELLTOKEN, oksellcaption,
                              sizeof(oksellcaption));
  getStringFromIndexWithDelim(npcarg, "|", POORTOKEN, poorcaption,
                              sizeof(poorcaption));
  getStringFromIndexWithDelim(npcarg, "|", ITEMFULLTOKEN, itemfullcaption,
                              sizeof(itemfullcaption));
  ...
  snprintf(tmp, sizeof(tmp),
           ...
           "%s|%s|%s|%s|"
           "-1|%d%s",
           objindex, maincaption, buycaption, okbuycaption, sellcaption,
           oksellcaption, poorcaption, itemfullcaption,
           NPC_SimpleShopGetItemNum(meindex), itemstr);
...
static void NPC_SimpleShopGetEscapedItemString(int shopindex, int cliindex,
  ...
  for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(shopindex); i++) {
    ...
    if (ITEM_CHECKINDEX(itemindex)) {
      ...
      snprintf(tmp, sizeof(tmp), "|%s|%d|%d|%d|", escapedname, cost, cl,
               imageno);
      strcat(str, tmp);
    }
  }

  for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(cliindex); i++) {
...
static BOOL NPC_SimpleShopFillItem(int meindex, char *npcarg) {
  ...
  getStringFromIndexWithDelim(npcarg, "|", ITEMLISTTOKEN, token, sizeof(token));
