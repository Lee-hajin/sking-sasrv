...
/*********************************
*********************************/
void NPC_TransmigrationTalked(int meindex, int talkerindex, char *szMes,
                              int color) {
#ifdef _PET_TRANS
  int i, petindex;
#endif
...
//玛雷菲雅判断 成立 宠物转生 不成立 人物转生
  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if (CHAR_getInt(petindex, CHAR_PETID) == 718
      ...
      if (CHAR_getInt(talkerindex, CHAR_LV) < 80) {
        CHAR_talkToCli(talkerindex, -1, "??? 80 ????? ??? ??? ? ????.！",
                       CHAR_COLORWHITE);
        return;
        ...
        CHAR_talkToCli(talkerindex, -1, "清除你身上非法玛雷菲雅",
                       CHAR_COLORRED);
        return;
      } else if (CHAR_getInt(petindex, CHAR_LV) == 79) {
        if (strcmp(CHAR_getChar(petindex, CHAR_OWNERCDKEY),
          ...
          CHAR_talkToCli(talkerindex, -1, "你身上的玛雷菲雅，并不是你的啊！",
                         CHAR_COLORWHITE);
          return;
        }
        petNum++;
  ...
  if (petNum > 1) {
    CHAR_talkToCli(talkerindex, -1, "你怎会有那麽多玛雷菲雅呢？",
                   CHAR_COLORWHITE);
    return;
  } else if (petNum == 1) {
...
static void NPC_Transmigration_selectWindow(int meindex, int toindex, int num) {
  ...
  case 0:
    ...
    if (
      ...
      //鳖戏毛ㄤ荚支匀凶
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "maxmsg", token,
                                          sizeof(token)) == NULL) {
        print("maxmsgErr");
        return;
      }
  ...
  case 2:
    ...
    sprintf(tmp, "yesmsg%d", CHAR_getWorkInt(toindex, CHAR_WORKSHOPRELEVANT));
  ...
  case 5:
    ...
    CHAR_setInt(toindex, CHAR_SAVEPOINT,
                CHAR_getInt(toindex, CHAR_SAVEPOINT) | (1 << 5));
    CHAR_setInt(toindex, CHAR_LASTTALKELDER, 5);
#endif
    //			buttontype = WINDOW_BUTTONTYPE_OK;
    //	  		windowno = CHAR_WINDOWTYPE_TRANSMIGRATION_END;
    return;
...
void NPC_TransmigrationWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case CHAR_WINDOWTYPE_TRANSMIGRATION_START:
    if (select == WINDOW_BUTTONTYPE_NEXT) {
      // ttom
      if (CONNECT_get_state_trans(fd) == 0) {
      } else
        return;
      // ttom end
  ...
  case CHAR_WINDOWTYPE_TRANSMIGRATION_MAIN:
    if (select == WINDOW_BUTTONTYPE_OK) {
      // ttom
      if (CONNECT_get_state_trans(fd) == 1) {
      } else
        return;
      // ttom end
  ...
  case CHAR_WINDOWTYPE_TRANSMIGRATION_END:
    if (select == WINDOW_BUTTONTYPE_NEXT) {
      // ttom
      if (CONNECT_get_state_trans(fd) == 2) {
      } else
        return;
      // ttom end
      ...
      // ttom
      if (CONNECT_get_state_trans(fd) == 4) {
      } else
        return;
      // ttom end
...
static int NPC_TransmigrationCheck(int meindex, int talker) {
...
#endif
    for (i = 0; i < 4; i++)
      if (NPC_EventCheckFlg(talker, ch_no[i]) == FALSE)
        return -1;
  }
  ...
  if (num < 4) {
    ...
    for (i = 0; i < CHAR_MAXPETHAVE; i++) {
      ...
      if (CHAR_getInt(petindex, CHAR_PETID) != petid[num])
        continue;

      //			if(CHAR_getInt( petindex, CHAR_LV) < PETLEVEL)
      //continue;
      break;
    }
...
#endif
  {
    for (j = 0; j < 4; j++) {
      for (i = 0; i < CHAR_MAXPETHAVE; i++) {
        ...
        if (petindex == -1)
          continue;
        //		print("petid=%d ",CHAR_getInt( petindex, CHAR_PETID));
        if (CHAR_getInt(petindex, CHAR_PETID) != petidfinal[j])
          continue;
        k++;
        break;
      }
    ...
    for (i = 0; i < CHAR_MAXPETHAVE; i++) {
      ...
      if (CHAR_getInt(petindex, CHAR_TRANSMIGRATION) == 2 &&
          (CHAR_getInt(petindex, CHAR_PETID) == 2 ||
           ...
           CHAR_getInt(petindex, CHAR_PETID) == 34))
        break;
      // if(CHAR_getInt( petindex, CHAR_TRANSMIGRATION) != 2 || CHAR_getInt(
      // petindex, CHAR_PETID) != lasttalkelder ) continue;
    }
    if (i == CHAR_MAXPETHAVE) {
      CHAR_talkToCli(talker, -1, "??? ???? ??? ????!", CHAR_COLORWHITE);
      return -1;
    }

    if (NPC_EventCheckFlg(talker, 63) == FALSE) {
      print("\nerr63");
      return -1;
    }
    if (NPC_EventCheckFlg(talker, 136) == FALSE) {
      print("\nerr136");
      return -1;
    }
    if (NPC_EventCheckFlg(talker, 175) == FALSE) {
      print("\nerr175");
      return -1;
    }
    if (NPC_EventCheckFlg(talker, 176) == FALSE) {
      print("\nerr176");
      return -1;
    }
    //检查有无15个魔界之门碎片
    ...
    if (j < 15) {
      CHAR_talkToCli(talker, -1, "你没有带15个魔界之门的碎片哦!",
                     CHAR_COLORWHITE);
      print("\nitem:%d", j);
      return -1;
    }
    ...
    if (getCancelAngleTrans() < 1)
      ...
      if (CHAR_getInt(talker, CHAR_HEROCNT) <= 0) {
        CHAR_talkToCli(talker, -1, "你没有执行过精灵召唤的任务!",
                       CHAR_COLORWHITE);
        return -1;
      }
#endif
...
BOOL NPC_TransmigrationMain(int meindex, int toindex, char *buf) {
  ...
  // if(CHAR_getInt(toindex, CHAR_SKILLUPPOINT) > 0) { //修复转生异点
  // CHAR_talkToCli(
  // toindex,-1,"你身上的点数没加完，所以我不能让你转生！",CHAR_COLORWHITE);
  // return FALSE;
  //}
  ...
  if (NPC_TransmigrationDelPet(meindex, toindex) == FALSE)
    return FALSE;
  if (NPC_TransmigrationAddPet(meindex, toindex, present) == FALSE)
    return FALSE;
  // Robin 0718
  {
    int tenseiNo;
  ...
  // 鳖戏夫弘毛潸月
  LogTensei(CHAR_getChar(toindex, CHAR_NAME), /* 平乓仿   */
            CHAR_getChar(toindex, CHAR_CDKEY), "Tensei", work[0],
            CHAR_getInt(toindex, CHAR_TRANSMIGRATION), work[1],
            NPC_StartpointCheck(meindex, toindex), item, pet, work[2], work[3],
            work[4], work[5], work[6], work[7], work[8], work[9]);

  //凳蕙
  CHAR_complianceParameter(toindex);
  // HP毛荚汊今六化丐仆月
  CHAR_setInt(toindex, CHAR_HP, CHAR_getWorkInt(toindex, CHAR_WORKMAXHP));
  CHAR_setInt(toindex, CHAR_LASTTALKELDER,
              NPC_StartpointCheck(meindex, toindex));
  CHAR_sendStatusString(toindex, "P");

  //转后强制传送，防止原地转生
  if (CHAR_getInt(toindex, CHAR_LASTTALKELDER) == 1) { //玛丽娜丝
    CHAR_warpToSpecificPoint(toindex, 2006, 20, 16);
  } else if (CHAR_getInt(toindex, CHAR_LASTTALKELDER) == 2) { //加加
    CHAR_warpToSpecificPoint(toindex, 3006, 21, 16);
  } else if (CHAR_getInt(toindex, CHAR_LASTTALKELDER) == 3) { //卡鲁它那
    CHAR_warpToSpecificPoint(toindex, 4006, 14, 20);
  } else {
    CHAR_warpToSpecificPoint(toindex, 1006, 15, 22);
  }
  /*
...
BOOL NPC_TransmigrationStatus(int meindex, int toindex, int work[10]) {
  ...
  CHAR_setInt(toindex, CHAR_TRANSEQUATION, (t_quest << 16) + (t_level << 0));
...
BOOL NPC_TransmigrationAddPet(int meindex, int talker, int petid) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex2, CHAR_NAME),
         CHAR_getInt(petindex2, CHAR_LV), "任务得到",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y), CHAR_getChar(petindex2, CHAR_UNIQUECODE),
         petindex2, talker);
...
int NPC_TransmigrationFlg_CLS(int meindex, int toindex) {
...
#ifdef _PET_TRANS
#ifdef _PROSK99
  int endevflg[] = {69,  70,  71,  72,  109, 118, 122,
                    131, 127, 158, 159, 181, -1}; // 181:白狼奖品
#else
  ...
  if (iGetFame > 0) {
    // 检查有没有导师
    if (strlen(CHAR_getChar(toindex, CHAR_TEACHER_ID)) > 0 &&
        strlen(CHAR_getChar(toindex, CHAR_TEACHER_NAME)) > 0) {
      ...
      for (i = 0; i < iPlayernum; i++) {
        if (CHAR_getCharUse(i) == FALSE)
          continue;
        if (strcmp(CHAR_getChar(toindex, CHAR_TEACHER_ID),
                   CHAR_getChar(i, CHAR_CDKEY)) == 0 &&
            strcmp(CHAR_getChar(toindex, CHAR_TEACHER_NAME),
          ...
          sprintf(szMsg, "获得学生 %s %.2f 点声望",
                  CHAR_getChar(toindex, CHAR_NAME), fGetFame);
...
BOOL NPC_TransmigrationDelPetDel(int meindex, int talker, int petsel) {
  ...
  CHAR_talkToCli(talker, -1, msgbuf, CHAR_COLORWHITE);
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿o */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "任务交出",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y), CHAR_getChar(petindex, CHAR_UNIQUECODE),
         petindex, talker);
...
BOOL NPC_TransmigrationDelPet(int meindex, int talker) {
  ...
  if (num < 4) {
    ...
    if (petsel == CHAR_MAXPETHAVE)
      return FALSE;
    if (NPC_TransmigrationDelPetDel(meindex, talker, petsel) == FALSE)
      return FALSE;
...
#endif
  {
    ...
    for (k = 0; k < 4; k++) {
      if (NPC_TransmigrationDelPetDel(meindex, talker, petwork[k]) == FALSE)
        return FALSE;
    }
  }
#ifdef _TRANS_6
...
#ifdef _PET_TRANS
void NPC_PetTransMan_selectWindow(int meindex, int toindex, int num,
                                  int select) {
  ...
  case 1: //是否让宠物转生
    ...
    if (errtype >= 0) {
      if (NPC_Util_GetStrFromStrWithDelim(buf, errchar[errtype], token,
                                          sizeof(token)) == NULL) {
        print("\n ERROR str errchar !!");
        return;
      }
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "selectmsg", token,
                                        sizeof(token)) == NULL) {
      print("\n ERROR selectmsg !!");
      return;
    }
  ...
  case 2: //选宠
    if (NPC_Util_GetStrFromStrWithDelim(buf, "setpetmsg", token,
                                        sizeof(token)) == NULL) {
      print("\n ERROR setpetmsg !!");
      return;
    }
  ...
  case 3: //宠物转生	确定/取消
    if (NPC_Util_GetStrFromStrWithDelim(buf, "transmsg", buf1, sizeof(buf1)) ==
        NULL) {
      print("\n ERROR transmsg !!");
      return;
    }
    ...
    if (CHAR_getInt(petindex, CHAR_FUSIONBEIT) == 1) {
      ...
      CHAR_setWorkInt(toindex, CHAR_WORKSHOPRELEVANT, 0);
        ...
        if (CHAR_getInt(toindex, CHAR_RIDEPET) ==
            select -
                1 /*&& CHAR_getInt( petindex, CHAR_TRANSMIGRATION ) > 0*/) { //骑乘中的宠物不能转生
      sprintf(token, "\n\n骑乘中的宠物不能转生喔！");
    } else
#endif
      ...
      if (NPC_PetTransManStatus(meindex, toindex, petindex) == FALSE)
        return;
      if (NPC_Util_GetStrFromStrWithDelim(buf, "main1msg", token,
                                          sizeof(token)) == NULL)
        return;
#ifdef _FUSIONBEIT_TRANS
      if (CHAR_getInt(petindex, CHAR_FUSIONBEIT) >= 1)
        CHAR_setInt(petindex, CHAR_FUSIONBEIT,
                    CHAR_getInt(petindex, CHAR_FUSIONBEIT) + 1);
#endif
    }
  ...
  case 5: //宠物转生取消
    if (NPC_Util_GetStrFromStrWithDelim(buf, "cancelmsg", token,
                                        sizeof(token)) == NULL)
      return;
    CHAR_setWorkInt(toindex, CHAR_WORKSHOPRELEVANT, 0);
    buttontype = WINDOW_BUTTONTYPE_OK;
...
#define RAND(x, y)                                                             \
  ...
  ans = NPC_PetTransManGetAns(total1, total2, petLV, petrank,
                              CHAR_getInt(petNo, CHAR_TRANSMIGRATION));
#else
  ans = NPC_PetTransManGetAns(total1, total2, petLV, petrank);
#endif
...

  {
    ...
    LogPetTrans(
        CHAR_getChar(toindex, CHAR_CDKEY), CHAR_getChar(petNo, CHAR_UNIQUECODE),
        CHAR_getChar(petindex, CHAR_UNIQUECODE), //玛蕾
        CHAR_getChar(toindex, CHAR_NAME), CHAR_getInt(toindex, CHAR_FLOOR),
        CHAR_getInt(toindex, CHAR_X), CHAR_getInt(toindex, CHAR_Y),
        CHAR_getInt(petNo, CHAR_PETID), CHAR_getChar(petNo, CHAR_NAME), petLV,
        petrank, vital2, str2, tgh2, dex2, total2,
        CHAR_getInt(petindex, CHAR_PETID), CHAR_getChar(petindex, CHAR_NAME),
        vital1, str1, tgh1, dex1, total1, work[0], work[1], work[2], work[3],
        ans, CHAR_getInt(petNo, CHAR_TRANSMIGRATION));
  }
  ...
  for (count = 0; count < CHAR_MAXPETHAVE; count++) {
    if (CHAR_getCharPet(toindex, count) == ret) {
      CHAR_send_K_StatusString(toindex, count,
  ...
  for (i = 0; i < CHAR_getMaxInventory(toindex); i++) {
    ...
    if (ITEM_CHECKINDEX(itemindex)) {
      ...
      for (j = 0; j < 3; j++) {
        if ((itemID >= DelItem[j].minItemID) &&
            (itemID <= DelItem[j].maxItemID)) {
          LogItem(CHAR_getChar(toindex, CHAR_NAME),
                  CHAR_getChar(toindex, CHAR_CDKEY), itemindex, "传送扣除道具",
  ...
  if (CHAR_charSaveFromConnect(toindex, FALSE)) {
    CHAR_talkToCli(toindex, -1, "矫胶袍阑 磊悼栏肺 历厘钦聪促!", CHAR_COLORRED);
  }
...
#define RAND(x, y)                                                             \
  ...
  int tran = CHAR_getInt(petindex, CHAR_TRANSMIGRATION);
  //融合宠转数-1
  if (CHAR_getInt(petindex, CHAR_FUSIONBEIT) != 0) {
    tran = CHAR_getInt(petindex, CHAR_FUSIONBEIT) - 1;
  }
  //获取MM数值
  vital1 = att1;
  str1 = att2;
  ...
  LevelUpPoint = CHAR_getInt(petindex, CHAR_ALLOCPOINT);
  petrank = CHAR_getInt(petindex, CHAR_PETRANK);
  petLV = CHAR_getInt(petindex, CHAR_LV);
  vital2 = (float)((LevelUpPoint >> 24) & 0xFF);
  str2 = (float)((LevelUpPoint >> 16) & 0xFF);
  tgh2 = (float)((LevelUpPoint >> 8) & 0xFF);
  dex2 = (float)((LevelUpPoint >> 0) & 0xFF);
  ...
  //融合宠
  if (CHAR_getInt(petindex, CHAR_FUSIONBEIT) != 0) {
    tran = CHAR_getInt(petindex, CHAR_FUSIONBEIT); // 2012/10/24 修正
  }
  ans = NPC_PetTransManGetAns(total1, total2, petLV, petrank, tran);
  ...
  enemynum = ENEMY_getEnemyNum();
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    if (CHAR_getCharPet(meindex, i) == petindex) {
      break;
    }
  }
  ...
  ret = GetNewPet(meindex, petindex, k, work);
  if (ret < 0)
    return FALSE;
  for (count = 0; count < CHAR_MAXPETHAVE; count++) {
    if (CHAR_getCharPet(meindex, count) == ret) {
      CHAR_send_K_StatusString(meindex, count,
                               CHAR_K_STRING_HP | CHAR_K_STRING_AI);
    }
  }
  sprintf(token, "%s 接受了精灵王的祝福！", CHAR_getUseName(ret));
  CHAR_talkToCli(meindex, -1, token, CHAR_COLORWHITE);
  sprintf(token, "K%d", i);
  CHAR_sendStatusString(meindex, token);
  // NPC_NowEventSetFlg( toindex, 71 );
  return TRUE;
}
