...
static BOOL NPC_generateNPC(int createindex, int createtemplateindex) {
  ...
  if (cr->chardata[NPC_CREATENEWNAME].string[0] != '\0') {
    strcpysafe(one.string[CHAR_NB_NAME].string,
               sizeof(one.string[CHAR_NB_NAME].string),
               cr->chardata[NPC_CREATENEWNAME].string);
  }
  one.data[CHAR_NB_NAMECOLOR] = cr->intdata[NPC_CREATECOLOR];
#endif
#ifdef _NB_CHARHALO
  one.data[CHAR_NB_HALO] = cr->intdata[NPC_CREATEHALO];
#endif
#ifdef _NB_CHARDISPTITLE
  one.data[CHAR_NB_NAMEMOTO] = cr->intdata[NPC_CREATENAMEMOTO];
#endif
  // Robin 0731
