...
static void NPC_Welfare_selectWindow(int meindex, int toindex, int num,
                                     int select);
...
void delprosk(int talkerindex, int delskid) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    int skillid = -1;
    skillid = CHAR_getCharSkill(talkerindex, i); // 技能ID
    if (skillid <= 0 || skillid == delskid)
      continue; //不储存需遗忘的技能
    hskill = CHAR_getCharHaveSkill(talkerindex, i); // 技能等级
    skill_level = SKILL_getInt(&hskill->skill, SKILL_LEVEL);
    skill_id[count] = skillid;
    skill_temp[count] = skill_level;
    count++;
  }
  CHAR_CHAT_DEBUG_delsk(talkerindex, "all"); // 删除所有技能
  for (i = 0; i < count; i++)                // 还原技能
...
void NPC_WelfareWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "profession_skill", msg,
                                      sizeof(msg)) != NULL) {
  ...
  // 判断职业
  {
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "profession_class", msg,
                                        sizeof(msg)) == NULL) {
      return;
    } else {
      p_class = atoi(msg);
    ...
    // 技能所需职业
    need_class =
        PROFESSION_SKILL_getInt(skillarray, PROFESSION_SKILL_PROFESSION_CLASS);
    ...
    if (CHAR_getInt(talkerindex, PROFESSION_CLASS) != need_class &&
        need_class != 4) {
      char token[256];
      memset(token, -1, sizeof(token));
  ...
  // 判断是否达成学习的条件
  {
    ...
    for (i = 0; i < 4; i++) {
      ...
      flag = -1;
      limit =
          PROFESSION_SKILL_getInt(skillarray, PROFESSION_SKILL_LIMIT1 + i * 2);
      need_percent = PROFESSION_SKILL_getInt(
          skillarray, PROFESSION_SKILL_LIMIT1 + i * 2 + 1);

      if (limit != 0 && need_percent == 0)
        need_count++;
      ...
      if (limit == 0) {
        ...
        for (j = 0; j < PROFESSION_MAX_LEVEL; j++) {
          if (CHAR_getCharSkill(talkerindex, j) > 0)
            break;
        }
        ...
        if (flag == -1 && need_percent == 0) {
          ...
          char token[256];
          memset(token, -1, sizeof(token));
          sprintf(token, "尚未学会%s!",
                  PROFESSION_SKILL_getChar(limit, PROFESSION_SKILL_NAME));
    ...
    if (count == need_count && need_count != 0) {
      ...
      for (i = 0; i < count; i++) {
        if (temp[i] != 0) {
          char token[50] = {0};
          sprintf(token, "%s ",
                  PROFESSION_SKILL_getChar(temp[i], PROFESSION_SKILL_NAME));
          strcat(msg, token);
        }
      }
  ...
  //判断转生
  {
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "trans", msg, sizeof(msg)) !=
        NULL) {
      int trans = atoi(msg);
      // print("\n转生要求:%d,%d",CHAR_getInt( talkerindex,
      // CHAR_TRANSMIGRATION),trans);
      if (CHAR_getInt(talkerindex, CHAR_TRANSMIGRATION) < trans) {
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "trans_msg", msg,
                                            sizeof(msg)) != NULL) {
          CHAR_talkToCli(talkerindex, meindex, msg, CHAR_COLORYELLOW);
          return;
        } else {
          CHAR_talkToCli(talkerindex, meindex, "您的转生次数不够哦。",
                         CHAR_COLORYELLOW);
          return;
        }
      }
  ...
  // 增加技能
  {
    ...
    if (PROFESSION_SKILL_ADDSK(talkerindex, skillID, skill_level) == -1)
      return;
  ...
  // 传送讯息
  {
    ...
    CHAR_setInt(talkerindex, PROFESSION_SKILL_POINT,
...
void NPC_WelfareMakeStr(int meindex, int toindex, int select) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "profession_skill", msg,
    ...
    while (getStringFromIndexWithDelim(msg, ",", i, buf, sizeof(buf)) !=
      ...
      if (PROFESSION_SKILL_CHECKINDEX(skillarray) == FALSE)
        continue;
      ...
      sprintf(
          token2, "|%s|%d|%s|%d",
          PROFESSION_SKILL_getChar(skillarray,
  ...
  lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PROFESSIONSHOP,
                   WINDOW_BUTTONTYPE_NONE, WINDOW_MESSAGETYPE_PROFESSIONSHOP,
                   CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
