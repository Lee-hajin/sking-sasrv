  ...
  while (getStringFromIndexWithDelim(argstr, "EventEnd", i, buf, sizeof(buf)) !=
    ...
    if (strstr(buf, "GetItem") != NULL) {
      if (strstr(buf, "ItemFullMsg") == NULL) {
        ...
        print("\n### EventNo=%d %s:ItemFullMsg 脱离###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
    ...
    if (strstr(buf, "GetPet") != NULL) {
      if (strstr(buf, "PetFullMsg") == NULL) {
        NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf4, sizeof(buf4));
        print("\n### EventNo=%d %s:PetFullMsg 脱离 ###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
    ...
    if (strstr(buf, "GetEp") != NULL) {
      if (strstr(buf, "EpFullMsg") == NULL) {
        NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf4, sizeof(buf4));
        print("\n### EventNo=%d %s:EpFullMsg 脱离 ###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
    if (strstr(buf, "GetStone") != NULL) {
      if (strstr(buf, "StoneFullMsg") == NULL) {
        NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf4, sizeof(buf4));
        print("\n### EventNo=%d %s:StonFullMsg 脱离 ###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
    ...
    if (strstr(buf, "DelStone") != NULL) {
      if (strstr(buf, "StoneLessMsg") == NULL) {
        NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf4, sizeof(buf4));
        print("\n### EventNo=%d %s:StoneLessMsg 脱离 ###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
  }
 ...
 *   仄井仃日木凶凛及质
 *********************************/
void NPC_ExChangeManTalked(int meindex, int talkerindex, char *szMes,
                           int color) {
...
BOOL NPC_TypeCheck(int meindex, int talker, char *szMes) {
  ...
  while (getStringFromIndexWithDelim(argstr, "EventEnd", i, buf, sizeof(buf)) !=
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf2, sizeof(buf2)) ==
        NULL)
      continue;
    ...
    if (evch != -1) {
      if (NPC_Util_GetStrFromStrWithDelim(buf, "TYPE", buf2, sizeof(buf2)) !=
          NULL) {
        ...
        if (strstr(buf2, "REQUEST") != NULL) {
          ...
          char token[14 * 100];
          char token2[14 * 50];
          if (NPC_Util_GetStrFromStrWithDelim(buf, "NomalMsg", token,
                                              sizeof(token)) != NULL) {
            ...
            if (NPC_Util_GetStrFromStrWithDelim(buf, "NpcWarp", token2,
                                                sizeof(token2)) != NULL) {
              NPC_EventWarpNpc(meindex, token2);
            }
            if (NPC_Util_GetStrFromStrWithDelim(buf, "EndSetFlg", buf2,
                                                sizeof(buf2)) != NULL) {
              char buf3[16];
              int k = 1;
              if (EvNo != -1)
            ...
            if (NPC_Util_GetStrFromStrWithDelim(buf, "CleanFlg", buf2,
                                                sizeof(buf2)) != NULL) {
              char buf3[16];
              int k = 1;
              k = 1;
...
#ifdef _ANGEL_SUMMON
            // 完成召唤任务
            if (NPC_Util_GetStrFromStrWithDelim(buf, "MISSIONOVER", buf2,
                                                sizeof(buf2)) != NULL) {
              int mindex;
              int mission;
              char nameinfo[64];
              ...
              if (mission == missiontable[mindex].mission) {
                ...
                saacproto_ACMissionTable_send(acfd, MISSION_HERO_COMPLETE, 4,
                                              nameinfo, "");
                sprintf(msg, " 完成任务 i:%d m:%d %s ", mindex, mission,
                        nameinfo);
                print(msg);
                LogAngel(msg);
              }
            ...
            // 清除召唤任务
            if (NPC_Util_GetStrFromStrWithDelim(buf, "MISSIONCLEAN", buf2,
                                                sizeof(buf2)) != NULL) {
              int mindex;
              int mission;
              char nameinfo[64];
              ...
              if (mission == missiontable[mindex].mission) {
                ...
                sprintf(msg, " 放弃任务 i:%d m:%d %s ", mindex, mission,
                        nameinfo);
                print(msg);
                LogAngel(msg);
              }
          ...
          } else if (NPC_Util_GetStrFromStrWithDelim(
                         buf, "NomalWindowMsg", token, sizeof(token)) != NULL) {
            /*--它奶件玉它匹丢永本□斥毛忒允--*/
            int fd = getfdFromCharaIndex(talker);
#ifdef _NEWEVENT
            if (NPC_Util_GetStrFromStrWithDelim(buf, "NomalWindowMsg1", buf2,
                                                sizeof(buf2)) != NULL) {
              lssproto_WN_send(
                  fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_NEXT,
              ...
              lssproto_WN_send(
                  fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_YES,
                  CHAR_WINDOWTYPE_WINDOWEVENT_STARTMSG,
                  CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
#ifdef _NEWEVENT
            }
  ...
  strcpy(buf, "\0");
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "NomalMainMsg", buf,
                                      sizeof(buf)) == NULL) {
...
-------------------------------------------*/
void NPC_ExChangeManWindowTalked(int meindex, int talkerindex, int seqno,
 ...
 * 嫩  毛  凶仄化中月井及民尼永弁
 *-----------------------------------*/
BOOL NPC_EventFreeIfCheck(int meindex, int talker, char *buf, int kosuu,
                          int flg) {
...
#ifdef _PROSK99
  if (strstr(buf, "PROSK") !=
      NULL) { // PROSK99>16 职技等级大於等於99%的数量要超过16个
    char *p = NULL;
    ...
    if ((baseno == petno) && (CHAR_getInt(petindex, CHAR_ENDEVENT) == mode)) {
      ...
      if (NPC_EventBigSmallLastCheck(petlevel, mypetlevel, flg) == TRUE) {
        ...
        if (CHAR_getWorkInt(talker, CHAR_WORKSHOPRELEVANTSEC) == 1) {
          ...
          NPC_Util_GetStrFromStrWithDelim(argstr, "Pet_Name", name,
                                          sizeof(name));
 ...
 *  矢永玄及伊矛伙毛民尼永弁允月
 ---------------------------------*/
int NPC_PetLvCheckType2(int petindex, int meindex, int talker, char *buf,
                        int mode) {
  char argstr[NPC_UTIL_GETARGSTR_BUFSIZE];
  char buff2[16];
  char buf3[8];
...
void NPC_MsgDisp(int meindex, int talker, int num) {
  ...
  if (num == 22 || num == 82 || num == 55 || num == 66) {
    ...
    while (getStringFromIndexWithDelim(argstr, "EventEnd", i, buf,
      ...
      /*--平□伐□玉及民尼永弁毛垫丹--*/
      if (NPC_Util_GetStrFromStrWithDelim(buf, "Pet_Name", buf2,
                                          sizeof(buf2)) != NULL) {
        CHAR_setWorkInt(talker, CHAR_WORKSHOPRELEVANTSEC, 1);
      }
  ...
  case 0:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "StopMsg", buf2, sizeof(buf2)) !=
        NULL) {
  ...
  case 1:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "NoStopMsg", buf2, sizeof(buf2)) !=
        NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
    }
    break;
  ...
  case 7:
    /*--蜇箕及奶矛件玄毛鞅户凶午五及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "EndStopMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
  ...
  case 2:
    ...
    if (strstr(buf, "ThanksMsg2") != NULL) {
      /*仇仇匹霜耨允月*/
      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_NEXT,
  ...
  case 22:
    ...
    if (work == 1) {
      if (NPC_Util_GetStrFromStrWithDelim(buf, "ThanksMsg", buf2,
                                          sizeof(buf2)) != NULL) {
        strcpysafe(token, sizeof(buf2), buf2);
      }
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf, tmp, buf2, sizeof(buf2)) !=
          NULL) {
        strcpysafe(token, sizeof(buf2), buf2);
      }
  ...
  case 3:
    /*--  骚及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "NomalMsg", buf2, sizeof(buf2)) !=
        NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
    }
    break;
  ...
  case 4:
    /*--失奶  丞互中匀天中及凛及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "ItemFullMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
    }
    break;
  ...
  case 5:
    /*--匙  毛允月端及丢奶件及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "RequestMsg", buf2,
                                        sizeof(buf2)) != NULL) {
  ...
  case 6:
    // print(" 6_答应? ");
    /*--熬仃潸曰及端及丢奶件及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "AcceptMsg", buf2, sizeof(buf2)) !=
        NULL) {
      if (strstr(buf2, "%8d") != NULL) {
        char buf3[64];
        int cost;
  ...
  case 66:
    ...
    if (work == 1) {
      if (NPC_Util_GetStrFromStrWithDelim(buf, "AcceptMsg", buf2,
                                          sizeof(buf2)) != NULL) {
        strcpysafe(token, sizeof(buf2), buf2);
      }
  ...
  case 8:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "ThanksMsg", buf2, sizeof(buf2)) !=
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf, "Charm", buf2, sizeof(buf2)) !=
          NULL) {
        /*--仇仇引匹  凶日｝    毛晓仆月---*/
      ...
      // Robin add 清除职业
      if (NPC_Util_GetStrFromStrWithDelim(buf, "PROFESSION", buf2,
                                          sizeof(buf2)) != NULL) {
        ...
        for (i = 0; i < CHAR_STARTITEMARRAY; i++) { //检查装备中道具
          if (ITEM_CHECKINDEX(CHAR_getItemIndex(talker, i))) {
            if (ITEM_getInt(CHAR_getItemIndex(talker, i), ITEM_NEEDPROFESSION) >
                0) {
              p_class = 1;
              CHAR_talkToCli(talker, -1, "请先把职业装备通通卸下！",
                             CHAR_COLORWHITE);
              break;
            }
          }
  ...
  case 9:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf, "NomalWindowMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
      if (EvNo != -1)
        NPC_NowEventSetFlg(talker, EvNo);
  ...
  case 99:
    ...
    if (work == 1) {
      ...
      if (strstr(buf, tmp) != NULL) {
        ...
        lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_YES,
                         CHAR_WINDOWTYPE_WINDOWEVENT_STARTMSG,
                         CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
        return;
      }
  ...
  case 10:
    ...
    /*--矢永玄互中匀天中及凛及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "PetFullMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
  ...
  case 11:
    /*--白仿弘毛壅允午五及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "CleanMainMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
      buttontype = WINDOW_BUTTONTYPE_YESNO;
  ...
  case 12:
    /*--白仿弘毛壅仄凶  及永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "CleanFlgMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      char buf3[64];
      int loop = 1;
  ...
  case 13:
    /*--云嗯互中匀天中及丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "StoneFullMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
    }
    break;
  ...
  case 14:
    /*--云嗯互凶曰卅中丢永本□斥--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "StoneLessMsg", buf2,
                                        sizeof(buf2)) != NULL) {
      strcpysafe(token, sizeof(buf2), buf2);
    }
    break;
  ...
  if (num != 8)
#endif
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, buttontype, windowtype,
                     CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
}
...
BOOL NPC_EventAdd(int meindex, int talker, int mode) {
  ...
  /*--矢永玄及  毛馨笛允月--*/
  if ((NPC_Util_GetStrFromStrWithDelim(buff, "pet_skill", buff2,
                                       sizeof(buff2)) != NULL) &&
      mode == 0) {
    if (mode == 0) {
...
#endif
  if (NPC_Util_GetStrFromStrWithDelim(buff, "DelItem", buff2, sizeof(buff2)) !=
      NULL) {
    if (strstr(buff2, "EVDEL") != NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buff, "NotDel", nbuf, sizeof(nbuf)) !=
          NULL) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buff, "DelStone", buff2, sizeof(buff2)) !=
    ...
    sprintf(token, "[%d]胶沛阑 扒匙林菌嚼聪促。", stone);
  ...
  /*--  骚及失奶  丞必永玄--*/
  if (NPC_Util_GetStrFromStrWithDelim(buff, "GetItem", buf, sizeof(buf)) !=
      NULL) {
    if (mode == 0) {
      j = 1;
      /*--窒蜊及失奶  丞毛  允井＂--*/
      ...
      if (kosuucnt < j) {
        ...
        if (NPC_Util_GetStrFromStrWithDelim(buff, "GetRandItem", buff2,
                                            sizeof(buff2)) != NULL) {
          rand_flg = 1;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buff, "DelPet", buff2, sizeof(buff2)) !=
          NULL &&
      mode == 0) {
    char buf4[32];
    char buf5[256];
...
BOOL NPC_AcceptDel(int meindex, int talker, int mode) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "GetStone", buff2, sizeof(buff2)) !=
    ...
    if ((CHAR_getInt(talker, CHAR_GOLD) + stone) >=
        CHAR_getMaxHaveGold(talker)) {
  ...
  /*--矢永玄及  毛馨笛允月--*/
  if ((NPC_Util_GetStrFromStrWithDelim(buf, "pet_skill", buff2,
                                       sizeof(buff2)) != NULL) &&
      mode == 0) {
    if (mode == 0) {
      CHAR_setWorkInt(talker, CHAR_WORKSHOPRELEVANTTRD, 1);
  ...
  /*--矢永玄毛壅允---*/
  if (NPC_Util_GetStrFromStrWithDelim(buf, "DelPet", buff2, sizeof(buff2)) !=
      NULL) {
    char buf4[32];
    char buf5[256];
    int i = 1;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "GetEp", buff2,
                                      sizeof(buff2)) //捞亥飘 器牢飘
      != NULL) {
    int ep;
    char token[128];
    ...
    ep = atoi(buff2);
    CHAR_setInt(talker, CHAR_EVENTPOINT,
                CHAR_getInt(talker, CHAR_EVENTPOINT) + ep);
    sprintf(token, "%dＥＰ甫 颊俊 持菌促. 醚 %dＥＰ", ep, point + ep);
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "GetStone", buff2, sizeof(buff2)) !=
    ...
    sprintf(token, "[%d]胶沛阑 裙垫窍看嚼聪促", stone);
    CHAR_talkToCli(talker, -1, token, CHAR_COLORWHITE);
  }
...
#endif
  /*--失奶  丞毛壅允--*/
  if (NPC_Util_GetStrFromStrWithDelim(buf, "DelItem", buff2, sizeof(buff2)) !=
      NULL) {
    if (strstr(buff2, "EVDEL") != NULL) {
      char work[256];
      char nbuf[256];
  ...
  /*--仿件母丞失奶  丞毛馨笛允月---*/
  if (NPC_Util_GetStrFromStrWithDelim(buf, "GetRandItem", buff2,
                                      sizeof(buff2)) != NULL) {
    char buf3[32];
    j = 1;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf, "GetShowItem", buff2,
                                      sizeof(buff2)) != NULL) {
...
BOOL NPC_EventAddPet(int meindex, int talker, char *buff2, int mode) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "任务得到",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y), CHAR_getChar(petindex, CHAR_UNIQUECODE),
         petindex, talker);
        ...
        for (i = 0; i < CHAR_getMaxInventory(talker); i++) {
          ...
          if (ITEM_CHECKINDEX(itemindex)) {
            ...
            if (itemno == id) {
              cnt++;
              LogItem(CHAR_getChar(talker, CHAR_NAME), // 平乓仿
                      CHAR_getChar(talker, CHAR_CDKEY), itemindex,
                      "任务收回道具", CHAR_getInt(talker, CHAR_FLOOR),
      ...
      for (j = 0; j < CHAR_getMaxInventory(talker); j++) {
        ...
        if (ITEM_CHECKINDEX(itemindex)) {
          if (atoi(buff3) == ITEM_getInt(itemindex, ITEM_ID)) {
            LogItem(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
                    CHAR_getChar(talker, CHAR_CDKEY), itemindex, "任务收回道具",
...
BOOL NPC_EventDelItemEVDEL(int meindex, int talker, char *buf, char *nbuf,
                           int breakflg) {
...
int NPC_EventFile(int meindex, int talker, char *arg) {
  ...
  while (getStringFromIndexWithDelim(argstr, "EventEnd", i, buf, sizeof(buf)) !=
    ...
    /*--平□伐□玉及民尼永弁毛垫丹--*/
    if (NPC_Util_GetStrFromStrWithDelim(buf, "Pet_Name", buff2,
                                        sizeof(buff2)) != NULL) {
      CHAR_setWorkInt(talker, CHAR_WORKSHOPRELEVANTSEC, 1);
    }
    /*--平□伐□玉及民尼永弁毛垫丹--*/
    ...
    if (evch != -1) {
      /*--奶矛件玄椭瘀及民尼永弁--*/
      //		if(NPC_ExChangeManEventCheck( meindex, talker, buf) ==
      //TRUE) {
 ...
 *  失奶  丞互中匀天中井升丹井及民尼永弁
----------- ------------------------------*/
BOOL NPC_ItemFullCheck(int meindex, int talker, char *buf, int mode,
                       int evcnt) {
      ...
      while (getStringFromIndexWithDelim(buf3, "&", j, buff4, sizeof(buff4))) {
        ...
        if (strstr(buff4, "ITEM") != NULL) {
          if (strstr(buff4, "*") != NULL) {
            ...
            if (l == -1)
              continue;
            for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(talker);
                 i++) {
              itemindex = CHAR_getItemIndex(talker, i);
              if (ITEM_CHECKINDEX(itemindex)) {
