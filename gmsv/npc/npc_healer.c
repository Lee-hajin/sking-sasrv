...
void NPC_HealerTalked(int meindex, int talker, char *msg, int color) {
  ...
  if ((CHAR_getWorkInt(talker, CHAR_WORKPARTYMODE) == 0) ||
    ...
    if (msgNo == 1) {
      CHAR_talkToCli(talker, meindex, "摹丰啊 肯丰 登菌嚼聪促！",
                     CHAR_COLORWHITE);

    } else if (msgNo == 2) {
      CHAR_talkToCli(talker, meindex, "由於你很诚实，让我帮你回复吧！",
                     CHAR_COLORWHITE);
    }
    ...
    for (i = 0; i < CHAR_PARTYMAX; i++) {
      ...
      if (otherindex != -1) {
        ...
        if (msgNo == 1) {
          CHAR_talkToCli(otherindex, meindex, "摹丰啊 肯丰登菌嚼聪促！",
                         CHAR_COLORWHITE);

        } else if (msgNo == 2) {
          CHAR_talkToCli(otherindex, meindex, "由於你很诚实，让我帮你回复吧！",
                         CHAR_COLORWHITE);
        }
      }
    }
