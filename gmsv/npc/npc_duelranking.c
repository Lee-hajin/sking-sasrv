...
static void NPC_Duelranking_selectWindow(int meindex, int toindex, int num,
                                         int select);
...
static void NPC_Duelranking_selectWindow(int meindex, int toindex, int num,
  ...
  if (NPC_Util_CharDistance(toindex, meindex) > 1)
    return;
  if ((fd = getfdFromCharaIndex(toindex)) == -1)
    return;
...
void NPC_DuelrankingWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case CHAR_WINDOWTYPE_DUELRANKING_START:
    if (atoi(data) == 3) {
      saacproto_DBGetEntryByCount_send(
          acfd, DB_DUELPOINT, 0, NPC_DUELRANKING_WINDOWLINENUM, fdid,
          CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX));
    }
    if (atoi(data) == 5) {
      char dbkey[256];
  ...
  case CHAR_WINDOWTYPE_DUELRANKING_TOPRANKING:
    if (select == WINDOW_BUTTONTYPE_PREV || select == WINDOW_BUTTONTYPE_NEXT) {
      ...
      if (count < 0)
        count = 0;
      saacproto_DBGetEntryByCount_send(
          acfd, DB_DUELPOINT, count, NPC_DUELRANKING_WINDOWLINENUM, fdid,
          CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX));
      ...
      if (strlen(data) != 0) {
        ...
        if (count < 1)
          count = 1;
        saacproto_DBGetEntryByCount_send(
            acfd, DB_DUELPOINT, count - 1, NPC_DUELRANKING_WINDOWLINENUM, fdid,
            CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX));
      }
    }
    break;
...
void NPC_Duelranking_PrintRanking(char *data, int count_start, int fdid,
                                  int msgid2) {
  char message[1024];
  char buf[2048];
  int i;
  ...
  for (i = 1;; i++) {
    ...
    }
    {
      ...
      snprintf(msgbuf, sizeof(msgbuf),
               "%s %5d%s         %10s        %3s    %s\n", hosi, atoi(rank) + 1,
               engnum[dsprank], score, lv, charaname);
    }
  ...
  if (cnt == 0) {
    ...
    button =
        WINDOW_BUTTONTYPE_NEXT | WINDOW_BUTTONTYPE_PREV | WINDOW_BUTTONTYPE_OK;
  }
  lssproto_WN_send(fd, WINDOW_MESSAGETYPE_WIDEMESSAGE, button,
                   CHAR_WINDOWTYPE_DUELRANKING_TOPRANKING, msgid2,
                   makeEscapeString(message, buf, sizeof(buf)));

  if (cnt == 0) {
    CHAR_setWorkInt(CONNECT_getCharaindex(fd), CHAR_WORKSHOPRELEVANT, -1);
  } else {
    CHAR_setWorkInt(CONNECT_getCharaindex(fd), CHAR_WORKSHOPRELEVANT,
                    count_start);
  }
}
...
void NPC_Duelranking_CallMyRanking(int count, int msgid, int msgid2) {
  ...
  if (count < 0)
    count = 0;
  saacproto_DBGetEntryByCount_send(
      acfd, DB_DUELPOINT, count, NPC_DUELRANKING_WINDOWLINENUM, msgid, msgid2);
}
