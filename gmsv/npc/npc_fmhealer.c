...
void NPC_FmHealerTalked(int meindex, int talker, char *msg, int color) {
  ...
  // if( CHAR_getInt(meindex, CHAR_FLOOR) != CHAR_getWorkInt(talker,
  // CHAR_WORKFMFLOOR)){ print("\n npc floor->%d player
  // floor->%d",CHAR_getInt(meindex, CHAR_FLOOR),
  ...
  if ((CHAR_getWorkInt(talker, CHAR_WORKPARTYMODE) == 0) ||
    ...
    if (msgNo == 1) {
      CHAR_talkToCli(talker, meindex, "眉仿苞 扁仿捞 葛滴 雀汗登菌嚼聪促！",
                     CHAR_COLORWHITE);
    } else if (msgNo == 2) {
      CHAR_talkToCli(talker, meindex, "由於你是本家族人员，请让我帮你回复吧！",
                     CHAR_COLORWHITE);
    }
  } else {
    int i = 0;
