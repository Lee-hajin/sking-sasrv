...
void NPC_PetRacePetTalked(int meindex, int talkerindex, char *szMes,
                          int color) {}
...
void NPC_PetRacePetLoop(int meindex) {
  ...
  case NPC_State_Run: {
    int interval = 50;
    int random = 0;
    if (CHAR_getWorkInt(meindex, NPC_WORK_ROUTETOX) <=
            CHAR_getWorkInt(meindex, NPC_WORK_STOPX) &&
        CHAR_getWorkInt(meindex, NPC_WORK_ROUTETOY) >=
            CHAR_getWorkInt(meindex, NPC_WORK_STOPY))
      random = 3;
    else
      random = RAND(0, 8);
    if (random == 0) // 攻击
