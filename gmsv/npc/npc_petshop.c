...
NPC_PETSHOP_MSG shopmsg[] = {{"over_msg", "钱用完罗!没问题吗?"},
                             {"main_msg", "欢迎"},
                             {"realy_msg", "真的可以吗?"},
                             {"thanks_msg", "谢谢!"},
                             {"cost_msg", "钱不够喔!"},
...
void NPC_getDrawPet(int meindex, int toindex, char *token, int *buttontype,
                    int select);
...
void NPC_DepotPet_List(int meindex, int toindex, char *token, int *buttontype,
                       int select);
...
#ifdef _CHAR_INFOSTORAGE
#else
  if (NPC_Util_CharDistance(lookedindex, meindex) > 2)
    return;
#endif
...
void NPC_PetShop_selectWindow(int meindex, int toindex, int num, int select) {
...
#else
  if (NPC_Util_CharDistance(toindex, meindex) > 2)
    return;
#endif
...
#ifdef _NB_CDKEYLOCK
  {
    ...
    if (sasql_getLock(cdkey) > 0) {
      CHAR_talkToCli(toindex, -1, "你的帐号存在非法操作，无法进行操作！", 6);
      return;
    }
  }
#endif
    ...
    if (CHAR_getInt(toindex, CHAR_LOCKED) == 1 && getItemPetLocked() == 1) {
      ...
      lssproto_WN_send(
          getfdFromCharaIndex(toindex), WINDOW_MESSAGETYPE_MESSAGEANDLINEINPUT,
  ...
  case 3:
    if ((CHAR_getInt(toindex, CHAR_GOLD) +
         CHAR_getWorkInt(toindex, CHAR_WORKSHOPRELEVANTSEC)) >
...
void NPC_PetShopWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (NPC_Util_CharDistance(toindex, meindex) > 2)
    return;
#endif
  // print("\n NPC_PetShopWindowTalked: data:%s seq:%d sel:%d ", data, seqno,
  // select);
  ...
  case CHAR_WINDOWTYPE_WINDOWPETSHOP_START: {
    ...
    if ((datanum == 3 && poolflg) ||  // 卖掉宠物
      ...
      if (!CHAR_CheckDepotPet(talkerindex)) {
        ...
        CHAR_talkToCli(talkerindex, -1, "拌沥傍蜡 芒绊 历厘 己傍！",
                       CHAR_COLORYELLOW);
      } else {
  ...
  case CHAR_WINDOWTYPE_DEPOTPETSHOP_HANDLE:
    ...
    case 1: //放入
      if (CHAR_findEmptyDepotPet(talkerindex) == -1) {
        CHAR_talkToCli(talkerindex, -1, "其飘 沫捞 何练钦聪促！",
                       CHAR_COLORYELLOW);
        return;
      }
...
void NPC_PetDel(int meindex, int talker, int select, char *token) {
  ...
  if(strlen(petnewname)>0)
          if(strcmp(petnewname,CHAR_getChar(petindex,CHAR_NAME))!=0 &&
  CHAR_getInt(petindex,CHAR_LV)>=100){ CHAR_talkToCli( talker, -1,
  "【宠物保护系统】有昵称的100级以上宠物无法卖入商店，如需操作请把宠物名称改为原名称.",
  ...
  for (i = 0; i < 64; i++) {
    if (getNoSellPetId(i) == petid) {
      CHAR_talkToCli(talker, -1, "【宠物保护系统】此宠物为珍稀宠物，不可贩卖。",
                     CHAR_COLORYELLOW);
      return;
    }
  }
...
void NPC_PetDel3(int meindex, int talker, int select, char *token) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), CHAR_getChar(talker, CHAR_CDKEY),
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "领取个仓宠物", CHAR_getInt(talker, CHAR_FLOOR),
         CHAR_getInt(talker, CHAR_X), CHAR_getInt(talker, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, talker);
  ...
  char depotbuff[128];
  sprintf(depotbuff, "[%s][LV:%d]其飘甫 茫疽嚼聪促。",
          CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
  CHAR_talkToCli(talker, -1, depotbuff, CHAR_COLORYELLOW);
#ifdef _SAMETHING_SAVEPOINT
  if (CHAR_charSaveFromConnect(talker, FALSE)) {
    CHAR_talkToCli(talker, -1, "其飘 沫捞 何练钦聪促!", CHAR_COLORRED);
  }
#endif
}
...
void NPC_MaxGoldOver(int meindex, int toindex, int select, char *token) {
  ...
  if (select < 0 || select >= arraysizeof(shopmsg))
    return;

  if (NPC_Util_GetStrFromStrWithDelim(npcarg, shopmsg[select].option, buf,
                                      sizeof(buf)) == NULL) {
...
void NPC_DepotPet_Add(int meindex, int talker, int select, char *token) {
...
#ifdef _PET_ITEM
  {
    ...
    for (i = 0; i < CHAR_MAXPETITEMHAVE; i++) {
      ...
      if (ITEM_getInt(itemindex, ITEM_ItemLock) == 1) {
        sprintf(token, "\n\n宠物身上有绑定的道具无法存入共同仓库！");
        CHAR_talkToCli(talker, -1, "宠物身上有绑定的道具无法存入共同仓库！",
                       CHAR_COLORYELLOW);
        return;
      }
    }
  ...
  {
    char depotbuff[128];
    sprintf(depotbuff, "[%s][LV:%d]其飘甫 拌沥芒绊俊 焊包窍看嚼聪促。",
            CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV));
    CHAR_talkToCli(talker, -1, depotbuff, CHAR_COLORYELLOW);
    LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
           CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
           CHAR_getInt(petindex, CHAR_LV), "存入公仓宠物",
           CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
           CHAR_getInt(talker, CHAR_Y), CHAR_getChar(petindex, CHAR_UNIQUECODE),
           petindex, talker);
  } else {
...
void NPC_DepotPet_Get(int meindex, int talker, int select, char *token) {
    ...
    for (j = 0; j < AUTODELITEMNUM; j++) {
      if (petid != -1 && petid == getAutoDelPet(j)) {
        ...
        LogPet(
            ...
            CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
            "清除CF设置的宠物", CHAR_getInt(talker, CHAR_FLOOR),
            CHAR_getInt(talker, CHAR_X), CHAR_getInt(talker, CHAR_Y),
            CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, talker);
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), CHAR_getChar(talker, CHAR_CDKEY),
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "领取公仓宠物", CHAR_getInt(talker, CHAR_FLOOR),
         CHAR_getInt(talker, CHAR_X), CHAR_getInt(talker, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE), petindex, talker);
#ifdef _PET_EVOLUTION
  if (CHAR_getInt(petindex, CHAR_FUSIONBEIT) == 1 &&
...
void NPC_DepotPet_CheckRepeat_del(int charaindex, int petindex) {
  LogPet(CHAR_getChar(charaindex, CHAR_NAME), /* 平乓仿   */
         ...
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "清除仓库洗宠", CHAR_getInt(charaindex, CHAR_FLOOR),
