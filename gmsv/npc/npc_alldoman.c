...
static void NPC_Alldoman_selectWindow(int meindex, int talker, int seqno,
                                      char *data);

void NPC_AlldomanWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NPC_ALLDOMAN_SELECT_WND:
    if (atoi(data) == 2) {
      ...
      if (CHAR_getInt(talkerindex, CHAR_FLOOR) != 8200) {
        CHAR_talkToCli(talkerindex, -1, "这里只能给真正的英雄刻写！",
                       CHAR_COLORRED);
        break;
      }
      ...
      sprintf(buf2, "你拿出了打败守灵者%3d的证明，试图将自己的姓名刻上石碑！",
      ...
      if (CHAR_getWorkInt(talkerindex, CHAR_WORKHEROFLOOR) == 132) {
        ...
        //到达顶层对全星系广播
        sprintf(buf,
                "%s征服了英雄战场，由风之精灵将这个消息通知全世界，新的英雄诞生"
                "了!!",
      ...
      saacproto_UpdataStele_send(
          acfd, CHAR_getChar(talkerindex, CHAR_CDKEY),
          CHAR_getChar(talkerindex, CHAR_NAME), buf2,
          CHAR_getInt(talkerindex, CHAR_LV),
          CHAR_getInt(talkerindex, CHAR_TRANSMIGRATION), 10,
          CHAR_getWorkInt(talkerindex, CHAR_WORKHEROFLOOR));
      //清除WorkFloor
      CHAR_setWorkInt(talkerindex, CHAR_WORKHEROFLOOR, 0);
    }
...
static void NPC_Alldoman_selectWindow(int meindex, int talker, int num,
                                      char *data) {
  ...
  case 4:
    if (showpage == 1) {
      ...
      sprintf(token,
              "４８５４４７圣饼时，吉鲁出现大批机暴横行，所到之处皆无一幸免，尼"
              "斯各村的长老达成共识，调集各村勇士，前往消灭这群发狂的机暴，这群"
              "勇士在海底隧道入口跟这群机暴激战，不料，实力相差悬殊，勇士们伤亡"
              "惨重，残余的勇士，选择在霍特尔海底通路的入口做最");
    } else if (showpage == 2) {
      buttontype = WINDOW_BUTTONTYPE_NEXT;
      sprintf(token,
              "後的的抵抗，各村长老召集大批人力，封死所有通往吉鲁的海底通道，终"
              "於在大批机暴涌入前，将入口封死，此地为４８５５５０圣饼时所建造，"
              "为纪念４８５４４７圣饼时所牺牲的勇士，立此石碑，建此地洞，命名为"
              "“英雄战场”，望後世人效法先烈的行径，达到强种强");
    } else if (showpage == 3) {
      buttontype = WINDOW_BUTTONTYPE_OK;
      sprintf(token, "族为目的，让尼斯大陆的人们生生不息，上面的英雄是最近勇闯“"
                     "英雄战场”所留下来的，特此诏告全尼斯英雄！\n\n            "
                     "          英雄战场制作委员会");
    }
...
BOOL NPC_AlldomanInit(int meindex) {
  if (loadherolist == 0) {
    loadherolist = 1;
    saacproto_UpdataStele_send(acfd, "FirstLoad", "LoadHerolist", "华义", 0, 0,
                               0, 999);
  }
...
void NPC_AlldomanWriteStele(char *token) {
  ...
  while (linenum < 100) {
    for (i = 0; i < MAX_HERO_COLUMN; i++) {
      if (getStringFromIndexWithDelim(token, "|", i + 1, Herolist[linenum][i],
                                      sizeof(Herolist[linenum][i])) == FALSE) {
...
void NPC_Alldoman_S_WriteStele(char *ocdkey, char *oname, char *ncdkey,
                               char *nname, char *title, int level, int trns,
                               int floor) {
  int i, j, temp, temp1, flag;
  for (i = 0; i < MAX_HERO_COLUMN; i++) {
    if ((strcmp(Herolist[i][0], ocdkey) == 0) &&
        (strcmp(Herolist[i][1], oname) == 0)) {
