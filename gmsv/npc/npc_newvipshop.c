...
static void NPC_NewVipShop_selectWindow(int meindex, int toindex, int num,
                                        int select);
BOOL NewVipShop_GetMenuStr(int meindex, int toindex, char *npcarg, char *token,
                           int index);
...
void NPC_NewVipShopWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NPC_TRANSERMAN_SELECT:
    NPC_NewVipShop_selectWindow(meindex, talkerindex, WINDOW_SELECT,
                                atoi(data));
    break;
  case NPC_TRANSERMAN_WARP:
    ...
    if (select == WINDOW_BUTTONTYPE_NEXT) {
      NPC_NewVipShop_selectWindow(meindex, talkerindex, WINDOW_NEXT,
                                  atoi(data));
      return;
    }
    if (select == WINDOW_BUTTONTYPE_PREV) {
      NPC_NewVipShop_selectWindow(meindex, talkerindex, WINDOW_PREV,
                                  atoi(data));
      return;
    }
...
static void NPC_NewVipShop_selectWindow(int meindex, int toindex, int num,
  ...
  case WINDOW_START:
    ...
    saacproto_QueryPoint_send(acfd, getfdFromCharaIndex(toindex),
                              CHAR_getChar(toindex, CHAR_CDKEY));
    CHAR_talkToCli(toindex, -1, "获取金币中，请稍候...", CHAR_COLORRED);
  ...
  case WINDOW_SELECT:
    ...
    if (NewVipShop_GetMenuStr(meindex, toindex, npcarg, token,
    ...
    break;
  case WINDOW_NEXT:
    if (CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE) <=
        CHAR_getWorkInt(meindex, NPC_WORK_MAXPAGE))
  ...
  case WINDOW_WARP:
    ...
    if (NPC_Util_GetStrFromStrWithDelim(npcarg, "END_MSG", buf1,
                                        sizeof(buf1)) == NULL) {
      print("NewVipShop Get START_MSG ERROR !");
      return;
    }
    if (getStringFromIndexWithDelim(npcarg, "}", select1, buf2, sizeof(buf2)) !=
        FALSE)
    ...
    break;
  case WINDOW_END:
    ActionNpc_CheckMenuFree(meindex, toindex, npcarg,
                            CHAR_getWorkInt(toindex, CHAR_WORK_LISTPAGE));
    return;
    break;
  }
...
BOOL NewVipShop_GetMenuStr(int meindex, int toindex, char *npcarg, char *token,
  ...
  while (getStringFromIndexWithDelim(npcarg, "}", talkNo, buf1, sizeof(buf1)) !=
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "MenuStr", buf2, sizeof(buf2)) ==
        NULL)
      continue;
    FINDS = TRUE;
