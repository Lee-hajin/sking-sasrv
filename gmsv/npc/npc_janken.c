...
void NPC_WarpPointGet(int meindex, int talker, int *fl, int *x, int *y,
                      int judge);
 ...
 *   仄井仃日木凶凛及质
 *********************************/
void NPC_JankenTalked(int meindex, int talkerindex, char *szMes, int color) {
  /* 皿伊奶乩□卞覆仄化分仃  杀允月 */
...
static void NPC_Janken_selectWindow(int meindex, int talker, int num) {
  ...
  case 0:
    ...
    /*-- 旦正□玄及丢永本□斥互  仃木壬巨仿□  憎 --*/
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "MainMsg", token,
                                        sizeof(token)) == NULL) {
  ...
  case 1:
    ...
    //失奶  丞及民尼永弁
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "EntryItem", buf,
                                        sizeof(buf)) != NULL) {
 ...
 * 它奴件玉它丢永本□斥卞覆允月质
 *********************************/
void NPC_JankenWindowTalked(int meindex, int talkerindex, int seqno, int select,
                            char *data) {
  /* ㄠ弘伉永玉动  及心 */
  if (NPC_Util_CharDistance(talkerindex, meindex) > 1)
    return;
...
void NPC_JnakenJudge(int meindex, int talker, int sel) {
  ...
  if (shouhai == 1) {
    ...
    //  仃凶午五反＞  仄中＝失弁扑亦件毛允月
    CHAR_sendWatchEvent(CHAR_getWorkInt(talker, CHAR_WORKOBJINDEX), CHAR_ACTSAD,
                        NULL, 0, TRUE);
