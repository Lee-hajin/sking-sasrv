...
BOOL NPCEnemy_FreeIfCheck(int meindex, int talker, char *buf, int kosuu,
                          int flg, int temp);
...
BOOL NPCEnemy_CheckMyPet(int meindex, int talker, int petLv, int flg,
                         int petid);
...
BOOL NPC_NPCEnemyInit(int meindex) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "sktype", buf, sizeof(buf)) !=
      NULL) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "steal", buf, sizeof(buf)) !=
      NULL) {
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "item", buf, sizeof(buf)) ==
        NULL) {
  ...
  if (strstr(argstr, "NEWNPCENEMY") != 0) {
    ...
    if (CHAR_getWorkInt(meindex, CHAR_WORK_DIEACT) == 1) {
      char buff1[256];
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "warpfl", buff1,
                                          sizeof(buff1)) == NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "warpx", buff1,
                                          sizeof(buff1)) == NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "warpy", buff1,
                                          sizeof(buff1)) == NULL) {
      ...
      if (MAP_IsValidCoordinate(CHAR_getWorkInt(meindex, CHAR_WORK_WARP_FLOOR),
        ...
        print("NPCENEMY: %s: .\n %s.\nInvalid fl=%d,x=%d,y=%d\n", filename,
              argstr, CHAR_getWorkInt(meindex, CHAR_WORK_WARP_FLOOR),
              CHAR_getWorkInt(meindex, CHAR_WORK_WARP_X),
              CHAR_getWorkInt(meindex, CHAR_WORK_WARP_Y));
        return FALSE;
      }
...
void NPC_NPCEnemyTalked(int meindex, int talkerindex, char *szMes, int color) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NEWTIME", buf, sizeof(buf)) !=
    ...
    if (CHAR_getWorkInt(meindex, NPC_TIME_MODE) <= 0) {
      if (NPC_Util_GetStrFromStrWithDelim(npcarg, "Time_Msg", buf,
                                          sizeof(buf)) != NULL) {
...
int NPC_NPCEnemy_Encount(int meindex, int charaindex, int mode) {
  ...
  if (!flg) {
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "deniedmsg", buf,
                                        sizeof(buf)) != NULL) {
  ...
  while (1) {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "noitem", buf, sizeof(buf)) !=
        NULL) {
      char data[128];
      BOOL found = FALSE;
...

    {
      ...
      BOOL Evflg = TRUE;
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "B_evend", buf,
                                          sizeof(buf)) != NULL) {
        int i = 1, event = -1;
        ;
      ...
      if (Evflg == FALSE)
        break;
      i = 1;
      if (NPC_Util_GetStrFromStrWithDelim(argstr, "B_evnow", buf,
                                          sizeof(buf)) != NULL) {
        int i = 1, event = -1;
        ;
        while (getStringFromIndexWithDelim(buf, ",", i, data, sizeof(data))) {
    ...
    if (CHAR_getWorkInt(meindex, CHAR_WORK_ONEBATTLE) == 1) {
      ...
      if (i != battlemax) {
        if (NPC_Util_GetStrFromStrWithDelim(argstr, "alreadymsg", buf,
                                            sizeof(buf)) != NULL) {
          CHAR_talkToCliAndParty(charaindex, meindex, buf, CHAR_COLORYELLOW);
        }
        return FALSE;
...
int NPC_NPCEnemy_Dying(int battleindex, int meindex) {
...
#endif
    /* 蝇    及陲太 */
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "endmsg", buf, sizeof(buf)) !=
        NULL) {
      for (i = 0; i < 5; i++) {
        /* 爵  卞辅笛仄化中月蝈够卞丢永本□斥 */
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "herobattlefield", buf,
                                      sizeof(buf)) != NULL) {
    int charaindex = BattleArray[battleindex].Side[0].Entry[0].charaindex;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "AddItem", buf, sizeof(buf)) !=
      NULL) {
    for (i = 0; i < 5; i++) {
      /* 爵  卞辅笛仄化中月蝈够卞丢永本□斥 */
      int toindex = BattleArray[battleindex].Side[0].Entry[i].charaindex;
    ...
    for (i = 0; i < 5; i++) {
      ...
      if (CHAR_CHECKINDEX(toindex)) {
        ...
        if (strstr(argstr, "NEWNPCENEMY")) {
          ...
          if (NPC_Util_GetStrFromStrWithDelim(argstr, "PROBCHANCE", buffer,
                                              sizeof(buffer)) != FALSE) {
            prob = atoi(buffer);
          }

          if (NPC_Util_GetStrFromStrWithDelim(argstr, "TIMECHANCE", buffer,
                                              sizeof(buffer)) != FALSE) {
            timesec = atoi(buffer);
          }

          if (prob >= chance) {
            if (NPC_Util_GetStrFromStrWithDelim(argstr, "WARPCHANCE", buffer,
                                                sizeof(buffer)) != FALSE) {

              getStringFromIndexWithDelim(buffer, ",", 1, buffer2,
                                          sizeof(buffer2));
              fl = atoi(buffer2);
              getStringFromIndexWithDelim(buffer, ",", 2, buffer2,
                                          sizeof(buffer2));
              x = atoi(buffer2);
              getStringFromIndexWithDelim(buffer, ",", 3, buffer2,
                                          sizeof(buffer2));
              y = atoi(buffer2);
            }
            ...
            if (CHAR_getWorkInt(toindex, CHAR_WORKPARTYMODE) ==
              ...
              for (i = 0; i < CHAR_PARTYMAX; i++) {
                int subindex =
                    CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i);
                if (CHAR_CHECKINDEX(subindex) == FALSE)
                  continue;

                CHAR_warpToSpecificPoint(subindex, fl, x, y);
                int nownow = (int)time(NULL) + timesec;
                CHAR_setInt(subindex, CHAR_EVOLUTION_STATS, nownow);
              }
            } else {
              int nownow = (int)time(NULL) + timesec;
              CHAR_setInt(toindex, CHAR_EVOLUTION_STATS, nownow);
              CHAR_warpToSpecificPoint(toindex, fl, x, y);
            }
          } else {
            if (NPC_Util_GetStrFromStrWithDelim(argstr, "WARP", buffer,
                                                sizeof(buffer)) != FALSE) {
...
BOOL NPC_NPCEnemy_BattleIn(int meindex, int charaindex) {
  ...
  if (ret == 0) {
    BattleArray[CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX)].WinFunc =
        NPC_NPCEnemy_Dying;
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "startmsg", buf, sizeof(buf)) !=
        NULL) {
      CHAR_talkToCliAndParty(charaindex, meindex, buf, CHAR_COLORYELLOW);
    }
    if (NPC_Util_GetStrFromStrWithDelim(argstr, "steal", buf, sizeof(buf)) !=
...
#ifdef _NB_NOBATTLEBUTTON //此NPC禁止求救
    if (CHAR_getWorkInt(meindex, CHAR_WORK_NOHELP) == 1) {
      CHAR_setWorkInt(charaindex, CHAR_WORK_NOHELP, 1);
    }
    if (CHAR_getWorkInt(meindex, CHAR_WORK_NOESCAPE) == 1) {
      CHAR_setWorkInt(charaindex, CHAR_WORK_NOESCAPE, 1);
    }
    lssproto_BattleButton_send(getfdFromCharaIndex(charaindex),
                               CHAR_getWorkInt(charaindex, CHAR_WORK_NOHELP),
                               CHAR_getWorkInt(charaindex, CHAR_WORK_NOESCAPE));
    //组队
    if (CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_LEADER) {
      int i;
      for (i = 1; i < CHAR_PARTYMAX; i++) {
        int toindex = CHAR_getWorkInt(charaindex, i + CHAR_WORKPARTYINDEX1);
        if (CHAR_CHECKINDEX(toindex)) {
          if (CHAR_getWorkInt(meindex, CHAR_WORK_NOHELP) == 1) {
            CHAR_setWorkInt(toindex, CHAR_WORK_NOHELP, 1);
          }
          if (CHAR_getWorkInt(meindex, CHAR_WORK_NOESCAPE) == 1) {
            CHAR_setWorkInt(toindex, CHAR_WORK_NOESCAPE, 1);
          }
          CHAR_setWorkInt(toindex, CHAR_WORK_NOHELP, 1);
          lssproto_BattleButton_send(
              getfdFromCharaIndex(toindex),
              CHAR_getWorkInt(toindex, CHAR_WORK_NOHELP),
              CHAR_getWorkInt(toindex, CHAR_WORK_NOESCAPE));
        }
      }
    }
#endif
...
void NPC_NPCEnemyWindowTalked(int meindex, int talkerindex, int seqno,
...
static int NPC_NPCEnemy_StealItem(char *argstr, int meindex, int charaindex) {
  char itembuf[1024];
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "item", itembuf,
                                      sizeof(itembuf)) != NULL) {
    char data[128];
    int delitemgroup[CHAR_getMaxInventory(charaindex)];
    int found = 0;
    int i;
    for (i = 0; i < CHAR_getMaxInventory(charaindex); i++) {
      int j;
...
static BOOL NPC_NPCEnemy_CheckFree(int meindex, int toindex, BOOL *Party) {
  ...
  if ((strstr(npcarg, "NEWNPCENEMY") == NULL) &&
      (strstr(npcarg, "OVER") == NULL)) {
    return FALSE;
  } else {
  }
  ...
  while (getStringFromIndexWithDelim(npcarg, "OVER", talkNo, buf1,
    ...
    if (strstr(buf1, temp1) != NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf1, "endmsg", token,
                                          sizeof(token)) != NULL) {
        CHAR_talkToCli(toindex, meindex, token, CHAR_COLORYELLOW);
      }
#ifdef _ALLDOMAN // (不可开) Syu ADD 排行榜NPC
      if (NPC_Util_GetStrFromStrWithDelim(buf1, "herobattlefield", token,
                                          sizeof(token)) != NULL) {
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf1, "WARP", buf2, sizeof(buf2)) ==
          FALSE) {
        continue;
        ...
        type = RAND(0, i - 1);
        if (AllWarPoint[type].floor <= 0 ||
            type > arraysizeof(AllWarPoint) - 1) {
          type = 0;
        }
      ...
      if (NPC_Util_GetStrFromStrWithDelim(buf1, "CHECKPARTY", buf4,
                                          sizeof(buf4)) != NULL) {
...
BOOL NPCEnemy_BSCheck(int meindex, int talker, char *buf) {
  ...
  if (strstr(buf, "<") != NULL) {
    ...
    } else if (NPCEnemy_FreeIfCheck(meindex, talker, buff2, kosuu, flg, temp) ==
               TRUE) {
      return TRUE;
    }
  }
...
void NPC_NPCEnemyWatch(int meobjindex, int objindex, CHAR_ACTION act, int x,
...
void Check_EnemyWarpMe(int meindex, char *args) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(args, "REPLACEMENT", buf1,
                                      sizeof(buf1)) == NULL) {
    return;
  }
