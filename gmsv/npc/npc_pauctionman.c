...
static void NPC_Pauction_selectWindow(int meindex, int toindex, int seqno,
...
void NPC_PauctionWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case NPC_PAUCTION_SELECT:
    NPC_Pauction_selectWindow(meindex, talkerindex, WINDOW_PAUCTION_SELECT,
                              select, data);
    break;
  case NPC_PAUCTION_NEW:
    NPC_Pauction_selectWindow(meindex, talkerindex, WINDOW_PAUCTION_NEW, select,
                              data);
    break;
  case NPC_PAUCTION_AUCTIONSURVEY:
    NPC_Pauction_selectWindow(meindex, talkerindex,
                              WINDOW_PAUCTION_AUCTIONSURVEY, select, data);
    break;
  case NPC_PAUCTION_LIST_BUY_ALL:
    NPC_Pauction_selectWindow(meindex, talkerindex,
                              WINDOW_PAUCTION_LIST_BUY_ALL, select, data);
    break;
  case NPC_PAUCTION_LIST_BUY_PET:
    NPC_Pauction_selectWindow(meindex, talkerindex,
                              WINDOW_PAUCTION_LIST_BUY_PET, select, data);
    break;
  case NPC_PAUCTION_LIST_BUY_ITEM:
    NPC_Pauction_selectWindow(meindex, talkerindex,
                              WINDOW_PAUCTION_LIST_BUY_ITEM, select, data);
    break;
  case NPC_PAUCTION_LIST_MODIFY:
    NPC_Pauction_selectWindow(meindex, talkerindex, WINDOW_PAUCTION_LIST_MODIFY,
                              select, data);
    break;
  }
}
...
static void NPC_Pauction_selectWindow(int meindex, int toindex, int seqno,
  ...
  case WINDOW_PAUCTION_SELECT: {
    ...
    if (atoi(data) == 1) {
      ...
      num =
          sasql_getPauctionList(token, 0, 0, CHAR_getChar(toindex, CHAR_CDKEY));
  ...
  case WINDOW_PAUCTION_NEW: {
    ...
    sprintf(token, "成功为您登记了一条委托交易信息,我们会尽心尽力为您服务,"
                   "如有一切动态,我们会第一时间通知您!");
  } break;
