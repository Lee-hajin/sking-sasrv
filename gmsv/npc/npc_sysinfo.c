...
CMD_MSG cmd_msg[] = {
    {"startinfo"},    {"endinfo"},         {"help"},       {"player"},
    {"list"},         {"sendno"},          {"sendmsg"},    {"shutdown"},
    {"msgcol"},       {"sysinfo"},         {"info"},       {"allowaccept"},
    {"denyaccept"},   {"closeallsockets"}, {"allnosee"},   {"allnobody"},
    {"movenum"},      {"bornnum"},         {"enemystate"}, {"setwalktime"},
    {"setcatime"},    {"setcdtime"},       {"setoneloop"}, {"titlerestart"},
...
static char *NPC_Sysinfo_GetMsg(int meindex, char *msgindexstr, char *out,
                                int outlen, int num);
...
BOOL NPC_SysinfoInit(int meindex) {
  ...
  if (tmp == -1)
    tmp = NPC_SYSINFO_TIMELIMIT_DEFAULT;
...
void NPC_SysinfoTalked(int meindex, int tindex, char *msg, int color) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORK_MODE) == 0) {
    ...
    if (strncmp(msgwk, cmd_msg[NPC_SYSINFO_MSG_STARTINFO].msg,
      ...
      if (getStringFromIndexWithDelim(msgwk, " ", 2, buff, len) == TRUE) {
...
static void NPC_Sysinfo_Msg_EndInfo(int meindex, int tindex, char *msg) {
  CHAR_setWorkInt(meindex, CHAR_WORK_MODE, 0);
  CHAR_setWorkInt(meindex, CHAR_WORK_PLAYER, -1);
  CHAR_talkToCli(tindex, meindex, "已移到一般模式",
                 CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
}
/*------------------------------------------------------------------------
 * HELP丢永本□斥
...
static void NPC_Sysinfo_Msg_Help(int meindex, int tindex, char *msg) {
  ...
  SYSINFO_MSGWK msgwk[] = {
      ...
      {"list                  讯息一览。"},
      {"send number          「number」 "
       "是数值。指定讯息的号码後将该讯息内容传送给全部的玩家。"},
      {"sendmsg msgs          msgs为想传送的讯息。在send "
       "之後空一格，即可将之後的文章送信给全部的玩家。"},
      {"shutdown min          min 代表分。"},
      {"msgcol col            "
       "设定传送给玩家讯息的颜色。颜色的指定有red,green,yellow,blue,purple,"
       "cyan,white。"},
      {"sysinfo               系统状态"},
      {"info                  玩家状态"},
      {"endinfo               回复普通的状态。"},
...
static void NPC_Sysinfo_Msg_SendNo(int meindex, int tindex, char *msg) {
  ...
  if (getStringFromIndexWithDelim(msg, " ", 2, buff, sizeof(buff)) == TRUE) {
    ...
    if (num >= 1 && num < NPC_SYSINFO_MSGMAX) {
      if (NPC_Sysinfo_GetMsg(meindex, "sendno", msgbuf, sizeof(msgbuf), num) !=
          NULL) {
...
static void NPC_Sysinfo_Msg_Msgcol(int meindex, int tindex, char *msg) {
  ...
  SYSINFO_COLSET colset[] = {
      {249, "red"},    {250, "green"}, {251, "yellow"}, {252, "blue"},
      {253, "purple"}, {254, "cyan"},  {255, "white"},
  };
...
static void NPC_Sysinfo_SendMsg(int meindex, int pindex, int tblnum) {
  ...
  NPC_Util_GetArgStr(meindex, argstr, sizeof(argstr));
  if (NPC_Util_GetStrFromStrWithDelim(argstr, msgtable[tblnum].arg, buff,
                                      sizeof(buff)) == NULL) {
...
static void NPC_Sysinfo_All_NoSee(int meindex, int tindex, char *msg) {
  ...
  if (getStringFromIndexWithDelim(msg, " ", 2, buff, sizeof(buff)) == TRUE) {
    if (strncmp(buff, "on", strlen(buff)) == 0) {
      ...
      CHAR_talkToCli(tindex, meindex, buff,
                     CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
    } else if (strncmp(buff, "off", strlen(buff)) == 0) {
      all_nosee = 0;
...
static void NPC_Sysinfo_Move_Num(int meindex, int tindex, char *msg) {
  ...
  if (getStringFromIndexWithDelim(msg, " ", 2, buff, sizeof(buff)) == TRUE) {
    ...
    EnemyMoveNum = work;
    snprintf(buff, sizeof(buff), "让敌人同时%d动作。", EnemyMoveNum);
    CHAR_talkToCli(tindex, meindex, buff,
                   CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
  } else {
    snprintf(buff, sizeof(buff), "现在敌人同时在进行%d动作。", EnemyMoveNum);
    CHAR_talkToCli(tindex, meindex, buff,
                   CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
  }
}
...
static void NPC_Sysinfo_Born_Num(int meindex, int tindex, char *msg) {
  ...
  if (getStringFromIndexWithDelim(msg, " ", 2, buff, sizeof(buff)) == TRUE) {
    ...
    one_loop_born = work;
    snprintf(buff, sizeof(buff), "敌人同时派出最高的%d。", one_loop_born);
    CHAR_talkToCli(tindex, meindex, buff,
                   CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
  } else {
    snprintf(buff, sizeof(buff), "现在敌人同时派出最高的%d。", one_loop_born);
    CHAR_talkToCli(tindex, meindex, buff,
                   CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
  }
}
...
static void NPC_Sysinfo_Enemy_State(int meindex, int tindex, char *msg) {
  ...
  snprintf(buff, sizeof(buff), "bornnum 现在敌人同时派出最高的%d。",
           one_loop_born);
  CHAR_talkToCli(tindex, meindex, buff,
                 CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));

  snprintf(buff, sizeof(buff), "movenum 现在敌人同时在进行%d动作。",
           EnemyMoveNum);
  CHAR_talkToCli(tindex, meindex, buff,
                 CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
}
static void NPC_Sysinfo_SetWalkTime(int meindex, int tindex, char *msg) {
  ...
  snprintf(msgbuf, sizeof(msgbuf), "WalkInterval = %d 用",
           getWalksendinterval());
...
static void NPC_Sysinfo_SetOneloop(int meindex, int tindex, char *msg) {
  ...
  snprintf(msgbuf, sizeof(msgbuf), "Onelooptime = %d 用", getOnelooptime_ms());
  CHAR_talkToCli(tindex, meindex, msgbuf,
                 CHAR_getWorkInt(meindex, CHAR_WORK_MSGCOLOR));
}
static void NPC_Sysinfo_TitleRestart(int meindex, int tindex, char *msg) {
