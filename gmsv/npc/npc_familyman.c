...
static void NPC_Familyman_selectWindow(int meindex, int toindex, int num) {
  ...
  if (fd != -1) {
    lssproto_WN_send(fd, w.windowtype, w.buttontype, w.windowno + 100,
                     CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
                     makeEscapeString(w.message, buf, sizeof(buf)));
  }
}

void NPC_FamilymanWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  if (buttonproc[button].use == TRUE) {
    ...
    if (newwin == 5) {
      if (CHAR_getInt(talkerindex, CHAR_FMINDEX) > 0) {
        // CHAR_talkToCli( talkerindex, -1, "资格不符！已经加入家族。",
        // CHAR_COLORWHITE );
      ...
      if ((CHAR_getInt(talkerindex, CHAR_TRANSMIGRATION) == 0) &&
        ...
        // CHAR_talkToCli( talkerindex, -1, "很抱歉！等级不足。",
        // CHAR_COLORWHITE );
        lssproto_WN_send(
            fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
            makeEscapeString("\n何练阑 父甸扁俊 饭骇捞 何练钦聪促！", buf,
                             sizeof(buf)));
        return;
      }
      ...
      if (CHAR_getInt(talkerindex, CHAR_GOLD) < 10000) {
        lssproto_WN_send(
            fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
            makeEscapeString(
                "\n何练阑 积己窍妨搁 弥家 10，000胶沛捞 鞘夸钦聪促！", buf,
                sizeof(buf)));
        return;
      }

      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_FAMILYADD, WINDOW_BUTTONTYPE_NONE,
                       CHAR_WINDOWTYPE_FAMILYMAN_ADD,
    ...
    if (newwin == 7) {
      if (CHAR_getInt(talkerindex, CHAR_FMINDEX) == -1) {
        // CHAR_talkToCli( talkerindex, -1, "你还未加入任何家族呀。",
        // CHAR_COLORWHITE );
        lssproto_WN_send(
            fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
            makeEscapeString("\n呕硼 且 何练捞 粮犁窍瘤 臼嚼聪促！", buf,
                             sizeof(buf)));
        return;
      }
      ...
      if (CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER) {
        lssproto_WN_send(
            ...
            CHAR_WINDOWTYPE_FAMILYMAN_OUT,
            CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
            makeEscapeString(
                "\n何练阑 秦眉 且 版快 何练扁技客 窃膊 葛电 牢盔捞\n何练俊辑 "
                "磊悼 呕硼邓聪促.\n肚茄 葛电 何练盔甸狼 啪铰其飘啊 "
                "郴妨笼聪促.\n何练 秦眉甫 柳青窍矫摆嚼聪鳖？",
                buf, sizeof(buf)));
        return;
...
static BOOL NPC_Familyman_readData(int meindex, int windowno, BOOL chkflg) {
  ...
  while (readflg == TRUE) {
    ...
    while (1) {
       ...
       * 公木动陆反  骰允月 */
      if ((chkflg == FALSE && winno == windowno) || chkflg == TRUE) {
