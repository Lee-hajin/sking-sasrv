...
BOOL NPC_AlreadyScheduled(int meindex,
                          int talkerindex); // 检查, 一个家族只能安排一场
// 产生排程表的 data
void NPC_LIST_gendata(int meindex, int talkerindex, int page, char *buf,
                      int size);
// 产生选择家族的 data
void NPC_SELECT_gendata(int meindex, int talkerindex, int page, char *buf,
                        int size);
// 产生排程详细的 data
...
void NPC_SchedulemanWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case CHAR_WINDOWTYPE_SCHEDULEMAN_START:
    ...
    case WINDOW_BUTTONTYPE_PREV:
      ...
      lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PKSCHEDULELIST, buttontype,
                       CHAR_WINDOWTYPE_SCHEDULEMAN_START,
                       CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), buf);
      break;
      ...
      if ((CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) == FMMEMBER_MEMBER) &&
        ...
        for (i = 0; i < MAX_SCHEDULE; i++) {
          if (fmpks[fmpks_pos + i].dueltime == dt) {
            if (i == 0) {
              CHAR_talkToCli(talkerindex, meindex, "这个时间无法进行预约。",
                             CHAR_COLORWHITE);
              break;
            }
            ...
            case FMPKS_FLAG_NONE:
              if (NPC_AlreadyScheduled(meindex, talkerindex)) {
                ...
                lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PKSCHEDULESELECTFAMILY,
                                 buttontype, CHAR_WINDOWTYPE_SCHEDULEMAN_SELECT,
                                 CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
                                 buf);
              }
              break;
            case FMPKS_FLAG_CHALLENGE: { // 主队族长可以重设条件，客队则是同意
                                         // pk，其他人滚蛋
              ...
              if (tkfmindex == fmpks[fmpks_pos + i].host_index) {
                ...
                lssproto_WN_send(
                    fd, WINDOW_MESSAGETYPE_PKSCHEDULEDETAIL,
                ...
                CHAR_talkToCli(talkerindex, meindex, "你的家族已经接受挑战。",
                               CHAR_COLORWHITE);
                // 重新送出 list
                NPC_LIST_gendata(meindex, talkerindex, page, buf, sizeof(buf));
                lssproto_WN_send(fd, WINDOW_MESSAGETYPE_PKSCHEDULELIST,
                                 buttontype, CHAR_WINDOWTYPE_SCHEDULEMAN_START,
                                 CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX),
                                 buf);
              }
            } break;
            case FMPKS_FLAG_SETTING: { // 主队族长可以重设条件
              int tkfmindex = CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI);
              if (tkfmindex == fmpks[fmpks_pos + i].host_index) {
                fmpks[fmpks_pos + i].flag = FMPKS_FLAG_SETTING;
                fmpks[fmpks_pos + i].setting_timeout =
        ...
        if (CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) != FMMEMBER_MEMBER) {
#endif
          CHAR_talkToCli(talkerindex, meindex, "只有族长才能预约家族ＰＫ喔。",
                         CHAR_COLORWHITE);
        } else if (CHAR_getWorkInt(talkerindex, CHAR_WORKFMSETUPFLAG) != 1) {
          CHAR_talkToCli(talkerindex, meindex, "你的家族还没有正式成立喔。",
                         CHAR_COLORWHITE);
        }
      }
      break;
  ...
  case CHAR_WINDOWTYPE_SCHEDULEMAN_SELECT:
    ...
    case WINDOW_BUTTONTYPE_NEXT:
      page =
          CHAR_getWorkInt(talkerindex, CHAR_WORK_PAGE) +
          MAXFAMILYINONEWINDOW * ((select == WINDOW_BUTTONTYPE_NEXT) ? 1 : -1);
      buttontype = WINDOW_BUTTONTYPE_NEXT | WINDOW_BUTTONTYPE_PREV;
      if (page < 1) {
        page = 1;
        buttontype = WINDOW_BUTTONTYPE_NEXT;
        ...
        for (i = 0; i < MAX_SCHEDULE; i++) {
          if (fmpks[fmpks_pos + i].dueltime == dt) {
            print("host_index: %d %d\n", fmpks[fmpks_pos + i].host_index,
                  CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI));
            if (fmpks[fmpks_pos + i].host_index ==
                CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI)) {
              print("data:%s\n", data);
              if (getStringFromIndexWithDelim(data, "|", 1, token,
                                              sizeof(token))) {
                a = atoi(token);
          ...
          if (fmpks[fmpks_pos + i].host_index ==
            ...
            case WINDOW_BUTTONTYPE_OK: {
              ...
              if (getStringFromIndexWithDelim(data, "|", 4, token,
                ...
                a = atoi(token);
                if ((a > 0) && (a <= 40))
                  fmpks[fmpks_pos + i].prepare_time = a;
              }
              // 最大人数
              if (getStringFromIndexWithDelim(data, "|", 5, token,
                ...
                a = atoi(token);
                if ((a > 0) && (a <= 50))
                  fmpks[fmpks_pos + i].max_player = a;
              }
              // 胜利条件
              ...
              CHAR_talkToCli(talkerindex, meindex, "家族挑战设定完成。",
                             CHAR_COLORWHITE);
            } break;
            case WINDOW_BUTTONTYPE_CANCEL:
              ...
              fmpks[fmpks_pos + i].guest_index = -1;
              strcpy(fmpks[fmpks_pos + i].guest_name, "");
              CHAR_talkToCli(talkerindex, meindex, "清除家族挑战。",
                             CHAR_COLORWHITE);
              break;
            }
            NPC_SavePKSchedule(meindex);
...
void NPC_SchedulemanLoop(int meindex) {
  ...
  /*
    print("Scheduleman: fmpks_pos=%d dueltime=%d host=%s guest=%s\n",
         fmpks_pos, fmpks[fmpks_pos].dueltime, fmpks[fmpks_pos].host_name,
...
void NPC_RemoveExpiredBattle(int meindex) {
  ...
  if (expired == -1) {
    for (i = 0; i < 3; i++) { // tolerance 是 3 个 fight interval
      keeptime += fin;
      if ((keeptime % 100) >= 60)
        keeptime = keeptime + 100 - 60;
    }
    if (fmpks[fmpks_pos].dueltime > keeptime)
      expired = 23;
  }
  ...
  if (expired >= 0) {
    ...
    for (i = expired + 1; i < MAX_SCHEDULE; i++) {
      memcpy(&fmpks[fmpks_pos + i - expired - 1], &fmpks[fmpks_pos + i],
    ...
    if (expired == 23)
      fmpks[fmpks_pos].dueltime =
          tm1.tm_hour * 100 + ((int)(tm1.tm_min / fin)) * fin;
...
BOOL NPC_AlreadyScheduled(int meindex, int talkerindex) {
  ...
  for (i = 0; i < MAX_SCHEDULE; i++) {
    if (fmpks[fmpks_pos + i].flag >= FMPKS_FLAG_SCHEDULED) {
      if (fmpks[fmpks_pos + i].host_index == tkfmindex)
        return TRUE;
...
void NPC_LoadPKSchedule(int meindex) {
  ...
  snprintf(filename, sizeof(filename), "%s%d_%d_%d", SCHEDULEFILEDIR,
  ...
  for (i = 0; i < MAX_SCHEDULE; i++) {
    ...
    */
    sprintf(tmp, "%d|%d|%s|%d|%s|%d|%d|%d|%d\n", fmpks[fmpks_pos + i].dueltime,
            fmpks[fmpks_pos + i].host_index, n1,
            fmpks[fmpks_pos + i].guest_index, n2,
            fmpks[fmpks_pos + i].prepare_time, fmpks[fmpks_pos + i].max_player,
            fmpks[fmpks_pos + i].flag, fmpks[fmpks_pos + i].win);
...
void NPC_LIST_gendata(int meindex, int talkerindex, int page, char *buf,
  ...
  for (i = page; i < MAX_SCHEDULE && i < page + 4; i++) {
    ...
    sprintf(tmp, "%d|%s|%s|%d|%d|%d|%d", fmpks[fmpks_pos + i].dueltime, n1, n2,
            fmpks[fmpks_pos + i].prepare_time, fmpks[fmpks_pos + i].max_player,
            flag, fmpks[fmpks_pos + i].win);
...
void NPC_SELECT_gendata(int meindex, int talkerindex, int page, char *buf,
  ...
  for (i = 0, j = 0; i < MAXFAMILYINONEWINDOW; i++, j++) {
    if (getStringFromIndexWithDelim(familyListBuf, "|", page + j, token,
                                    sizeof(token))) {
      if ((getStringFromIndexWithDelim(token, " ", 1, fmindex,
                                       sizeof(fmindex))) &&
          (getStringFromIndexWithDelim(token, " ", 2, fmname,
                                       sizeof(fmname)))) {
...
void NPC_DETAIL_gendata(int meindex, char *buf, int size, int dueltime) {
  ...
  if (fmpks[fmpks_pos + i].dueltime == dueltime) {
    makeEscapeString(fmpks[fmpks_pos + i].host_name, n1, sizeof(n1));
    makeEscapeString(fmpks[fmpks_pos + i].guest_name, n2, sizeof(n2));
    sprintf(buf, "%d|%s|%s|%d|%d|%d", fmpks[fmpks_pos + i].dueltime, n1, n2,
            fmpks[fmpks_pos + i].prepare_time, fmpks[fmpks_pos + i].max_player,
