...
static void NPC_MakePair_selectWindow(int meindex, int toindex, int num,
                                      int select);
BOOL MakePairMan_GetMenuStr(int meindex, int toindex, char *npcarg,
                            char *token);
...
int NPC_getTitleMsg(int meindex, int toindex, char *arg, char *token,
                    int title);
int NPC_getNextTitle(int meindex, int toindex, char *arg, int title,
                     int select);
...
void NPC_MakePairManWindowTalked(int meindex, int talkerindex, int seqno,
...
BOOL MakePairMan_GetMenuStr(int meindex, int toindex, char *npcarg,
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "MenuHead", buf1, sizeof(buf1)) ==
      NULL) {
    return FALSE;
  }
  sprintf(token, "%s\n", buf1);
  ...
  while (NPC_Util_GetStrFromStrWithDelim(npcarg, buf1, buf2, sizeof(buf2)) !=
         NULL) {
    talkNo++;
    sprintf(buf1, "MEMU%d", talkNo);
    FINDS = TRUE;
...
BOOL NPC_PairForage(int meindex, int toindex, char *arg, char *token) {
  ...
  for (i = 0; i < MAXPAIRNUMS; i++) { // forindex
    ...
    if (PairList[Psex][i].use <= 0)
      continue;
    if (!CHAR_CHECKINDEX(PairList[Psex][i].pindex))
      continue;
    if (CHAR_getWorkInt(PairList[Psex][i].pindex, CHAR_WORKBATTLEMODE) !=
        BATTLE_CHARMODE_NONE)
      continue;
    if (codes != CHAR_getWorkInt(PairList[Psex][i].pindex, CHAR_MYPAIRCODE))
      continue;
    if (CHAR_getWorkInt(PairList[Psex][i].pindex, CHAR_MYPAIRFLG) > 0)
      continue;
    name = CHAR_getChar(PairList[Psex][i].pindex, CHAR_NAME);
    cdKey = CHAR_getChar(PairList[Psex][i].pindex, CHAR_CDKEY);
    if (strcmp(PairList[Psex][i].name, name) ||
        strcmp(PairList[Psex][i].cdKey, cdKey)) {
      PairList[Psex][i].use = -1;
      PairList[Psex][i].forindex = -1;
      continue;
    }
    sprintf(token, "你的配对对象为： %s\n已送出邀请，请等候对方回应。", name);
    if (NPC_Util_GetStrFromStrWithDelim(arg, "InviteMsg", buf1, sizeof(buf1)) ==
        NULL) {
      break;
    }
    ...
    sprintf(buf, "%s：\n玩家%s\n%s", CHAR_getChar(meindex, CHAR_NAME), name,
            buf1);
    lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OKCANCEL,
                     WINDOW_TALKPAIR,
                     CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), buf);
    break;
  }
  if (i >= MAXPAIRNUMS) {
    if (NPC_Util_GetStrFromStrWithDelim(arg, "NoPairMsg", buf1, sizeof(buf1)) !=
        NULL) {
...
void NPC_PairUserAndWarp(int meindex, int toindex, int forindex, char *arg) {
  ...
  if (CHAR_getWorkInt(toindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
    return;
  if (CHAR_getWorkInt(forindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
    return;

  if (NPC_Util_GetStrFromStrWithDelim(arg, "PAIRTOPOINT", buf1, sizeof(buf1)) !=
      NULL) {
    char buf2[256];
...
int NPC_getPairCode(int toindex) {
  ...
  for (i = 0; i < 15; i++) {
    ...
    if (arg == "\0")
      continue;
    if (NPC_Util_GetStrFromStrWithDelim(arg, "PAIRCODE", buf1, sizeof(buf1)) ==
        NULL)
      continue;
    code = atoi(buf1);
    break;
...
int NPC_getNextTitle(int meindex, int toindex, char *arg, int title,
  ...
  if (getStringFromIndexWithDelim(arg, "PSYCHOMETRICS", 2, buf, sizeof(buf)) ==
      FALSE)
    return -1;
  if (getStringFromIndexWithDelim(buf, "}", title, buf1, sizeof(buf1)) == FALSE)
    return -1;
  if (select == WINDOW_BUTTONTYPE_YES) {
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "YESFOR", buf2, sizeof(buf2)) ==
        NULL)
      return -1;
  } else {
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "NOFOR", buf2, sizeof(buf2)) ==
        NULL)
      return -1;
  }
  toID = atoi(buf2);
...
int NPC_getTitleMsg(int meindex, int toindex, char *arg, char *token,
  ...
  if (getStringFromIndexWithDelim(buf, "}", title, buf1, sizeof(buf1)) == FALSE)
    return FALSE;
  if (NPC_Util_GetStrFromStrWithDelim(buf1, "TITLEMSG", buf2, sizeof(buf2)) ==
      NULL) {
    ...
    if (NPC_Util_GetStrFromStrWithDelim(buf1, "ENDMSG", buf2, sizeof(buf2)) ==
        NULL) {
      return -1;
    }
