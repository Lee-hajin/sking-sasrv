...
void NPC_MicTalked(int meindex, int talkerindex, char *msg, int color) {
  ...
  snprintf(buf, sizeof(buf), "%s\n%s", CHAR_getChar(talkerindex, CHAR_NAME),
           msg);
...
#endif
    // && ( CHAR_getWorkInt( talkerindex, CHAR_WORKFMFLOOR) == CHAR_getWorkInt(
    // meindex, CHAR_WORK_FLOOR ) ) )
    {
      ...
      saacproto_ACFMAnnounce_send(
          ...
          CHAR_getInt(talkerindex, CHAR_FMINDEX),
          CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI), message, color);
      return;
    }
  }
  ...
  for (i = 0; i < playernum; i++) {
    ...
    if (CHAR_getInt(i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
      continue;
    if (CHAR_getInt(i, CHAR_FLOOR) !=
        CHAR_getWorkInt(meindex, CHAR_WORK_FLOOR)) {
      continue;
    }
        ...
        if (CHAR_getWorkInt(i, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_NONE) {
          ...
          if (fd != -1) {
            lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE,
                             WINDOW_BUTTONTYPE_OK, CHAR_WINDOWTYPE_MICMESSAGE,
