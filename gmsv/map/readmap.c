...
BOOL MAP_readMapConfFile(char *filename) {
  ...
  char realopfile[256];
  BOOL crypto = FALSE;
  sprintf(realopfile, "%s.93", filename);
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    for (i = 0; i < MAP_DATAINT_NUM + MAP_DATACHAR_NUM; i++) {
      ...
      case MAP_INTFUNC: {
        ...
        one->data[MAP_confentries[i].index] = intfunction(token);
        break;
      }
      case MAP_CHARFUNC: {
...
BOOL MAP_readMapOne(char *filename) {
  ...
  /*
     ...
     showstring, filename ); fwrite(token, strlen(token), 1, fff); fclose(fff);
                  }
  */
...
#ifdef _CRYPTO_DATA
    BOOL crypto = FALSE;
    fp = fopen("./data/map/map_noexit.txt.93", "r");
    if (fp != NULL) {
      crypto = TRUE;
    } else
#endif
    ...
    if (fp != NULL) {
      while (fgets(mpexit, 128, fp) != NULL) {
        ...
        if (strstr(mpexit, "#") != NULL)
          continue;
        if (floorID == id) {
          MAP_map[mapindex].startpoint =
              (exfloor << 16) + (ex_X << 8) + (ex_Y << 0);
...
#define CHECKFLOORID(id)                                                       \
  if (MAP_mapnum <= (id) || (id) < 0)                                          \
    return (-1);
...
BOOL MAP_checkCoordinates(int mapid, int x, int y) {
  if (MAP_getfloorIndex(mapid) == -1 || x < 0 || x > MAP_getfloorX(mapid) ||
...
char *MAP_getdataFromRECT(int floor, RECT *seekr, RECT *realr) {
  ...
  if (seekr->width < 0 || seekr->height < 0 || MAP_GETMAXSIZE < seekr->width ||
...
char *MAP_getChecksumFromRECT(int floor, RECT *seekr, RECT *realr, int *tilesum,
                              int *objsum, int *eventsum) {
  int floorindex;
  int i, j;
  int floorx;
  int databufferindex = 0;
  ...
  *eventsum = 0;
  ...
  for (i = realr->y; i < realr->y + realr->height; i++) {
    for (j = realr->x; j < realr->x + realr->width; j++) {
      ...
      tilebuf[databufferindex] =
          (unsigned short)MAP_map[floorindex].tile[i * floorx + j];
      objbuf[databufferindex] =
          (unsigned short)MAP_map[floorindex].obj[i * floorx + j];
      databufferindex++;
    }
  }
...
BOOL MAP_makeVariousMap(char *atile, char *aobj, int floor, int startx,
  ...
  for (i = 0; i < ysiz; i++) {
    for (j = 0; j < xsiz; j++) {
      atile[i * xsiz + j] =
          MAP_getImageInt(MAP_map[findex].tile[startpoint + i * xsiz + j],
                          MAP_coKindAndInt[kind])
              ? 1
              : 0;

      aobj[i * xsiz + j] =
          MAP_getImageInt(MAP_map[findex].obj[startpoint + i * xsiz + j],
                          MAP_coKindAndInt[kind])
              ? 1
              : 0;
    }
...
void MAP_sendAroundMapdata(int fl, int fromx, int fromy) {
  ...
  if (mapdata != NULL) {
    ...
    for (i = x - MAP_CHAR_DEFAULTSEESIZ / 2;
         i <= x + MAP_CHAR_DEFAULTSEESIZ / 2; i++) {
      for (j = y - MAP_CHAR_DEFAULTSEESIZ / 2;
           j <= y + MAP_CHAR_DEFAULTSEESIZ / 2; j++) {
        OBJECT object;
...
int MAP_getMapMaxNum(void) { return MAP_idtblsize; }

int MAP_getMapNum() { return MAP_mapnum; }
...
#ifdef _NB_CreateTJMap //写出外挂地图格式
BOOL MAP_walkAbleForJL(int index, int ff) {
  ...
  if (floorindex == -1) {
    ...
    //打开文件
    char des_path[128];
    sprintf(des_path, "tjmap/ServerMap/TJMAP_%d.dat\0", ff);
    int fd;
    fd = fopen(des_path, "wb");
    int xsize = MAP_map[floorindex].xsiz;
    int ysize = MAP_map[floorindex].ysiz;

    char xy[20];
    sprintf(xy, "%d,%d,\0", xsize, ysize);
    fwrite(xy, strlen(xy), 1, fd);
    char flag[1];
    int offsetx = 0;
    int offsety = 0;
    for (offsety = 0; offsety < ysize; offsety++) {
      for (offsetx = 0; offsetx < xsize; offsetx++) {
        if (MAP_walkAbleFromPoint(
                ff, offsetx, offsety,
                CHAR_getFlg(index, CHAR_ISFLYING) |
                    CHAR_getWorkInt(index, CHAR_WORKSKYWALKER))) {
          // print("%d,%d,1\n",offsetx,offsety);
          sprintf(flag, "%d", 0);
          fwrite(flag, strlen(flag), 1, fd);
        } else {
          // print("%d,%d,0\n",offsetx,offsety);
          sprintf(flag, "%d", 1);
          fwrite(flag, strlen(flag), 1, fd);
...
void CHAR_CHAT_DEBUG_MAP_walkAbleForJL(int charaindex) {
  int i = 0;
  for (i = 0; i < 65536; i++) {
    MAP_walkAbleForJL(charaindex, i);
  }
}
#endif //写出外挂地图格式结束
