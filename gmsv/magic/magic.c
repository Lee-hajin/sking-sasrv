  ...
  if (func) {
    ...
    if (magic_use_type == 1) {
      if (!BATTLE_CHECKINDEX(battleindex)) {
        sprintf(magicbuff, "【友情提示】您的[%s]无法在平时使用哦！",
                MAGIC_getChar(marray, MAGIC_NAME));
        CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORYELLOW);
        return FALSE;
      }
    } else if (magic_use_type == 2) {
      if (BATTLE_CHECKINDEX(battleindex)) {
        sprintf(magicbuff, "【友情提示】您的[%s]只能在非战斗状态下使用哦！",
                MAGIC_getChar(marray, MAGIC_NAME));
        CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORYELLOW);
        return FALSE;
      }
      ...
      if (BATTLE_CHECKINDEX(battleindex)) {
        if (BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P) {
          sprintf(magicbuff, "【错误提示】您的[%s]无法在PK中使用哦！",
                  MAGIC_getChar(marray, MAGIC_NAME));
      ...
      if (!BATTLE_CHECKINDEX(battleindex)) {
        ...
        if (BattleArray[battleindex].type != BATTLE_TYPE_P_vs_P) {
          sprintf(magicbuff, "【错误提示】您的[%s]只能在PK中使用哦！",
                  MAGIC_getChar(marray, MAGIC_NAME));
          CHAR_talkToCli(charaindex, -1, magicbuff, CHAR_COLORRED);
          return FALSE;
        }
...
int MAGIC_DirectUse(int charaindex, int magicid, int toindex, int itemnum) {
  MAGIC_CALLFUNC func;
  int mp, ret, marray;
  int itemindex = -1;
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      ...
      if (BATTLE_CheckSameSide(charaindex, toindex) == 0) { //不同边
        ...
        CHAR_talkToCli(charaindex, -1, "恩惠精灵..不得施予非玩家敌方。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      ...
      if (BATTLE_CheckSameSide(charaindex, toindex) == 0) { //不同边
        int battleindex = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX);
        BATTLE_NoAction(battleindex, BATTLE_Index2No(battleindex, charaindex));
        CHAR_talkToCli(charaindex, -1, "滋润精灵..不得施予非玩家敌方。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      ...
      if (BATTLE_CheckSameSide(charaindex, toindex) == 1) { //同边
        int battleindex = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX);
        BATTLE_NoAction(battleindex, BATTLE_Index2No(battleindex, charaindex));
        CHAR_talkToCli(charaindex, -1, "异常状态..不得施予友方。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER
      ...
      if (BATTLE_CheckSameSide(charaindex, toindex) == 0) { //不同边
        int battleindex = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX);
        BATTLE_NoAction(battleindex, BATTLE_Index2No(battleindex, charaindex));
        CHAR_talkToCli(charaindex, -1, "光镜守..不得施予敌方。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
...
int MAGIC_AttSkill(int charaindex, int toindex, int marray, int mp) {
  ...
  if (CHAR_CHECKINDEX(charaindex) == FALSE)
    return FALSE;
  if (IsBATTLING(charaindex) == FALSE)
    return FALSE;
  ...
  if (func(charaindex, toindex, skillID, "MAGICSKILL") == FALSE)
    return FALSE;
  CHAR_setInt(charaindex, CHAR_MP, CHAR_getInt(charaindex, CHAR_MP) - mp);
  return TRUE;
}
