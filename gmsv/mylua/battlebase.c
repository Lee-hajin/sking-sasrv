...
static CharBase BattleBaseInt[] = {
    {{"索引"}, 0},     {{"回合"}, 1},     {{"计时"}, 2},     {{"下方队长"}, 3},
    {{"上方队长"}, 4}, {{"创建时间"}, 5}, {{"结束时间"}, 6}, {{"模式"}, 7},
    {{"类型"}, 8},     {{"胜方"}, 9}};

static int getInt(lua_State *L) {
  const int battleindex = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, BattleBaseInt, arraysizeof(BattleBaseInt), 1);
  int ret = -1;
  if (BATTLE_CHECKINDEX(battleindex) == TRUE) {
    BATTLE *TM_Battle = &BattleArray[battleindex];
    if (TM_Battle->use == TRUE) {
      switch (element) {
      case 0:
        ret = TM_Battle->battleindex;
        break;
      case 1:
        ret = TM_Battle->turn;
        break;
      case 2:
        ...
        break;
      case 3:
        ret = TM_Battle->leaderindex;
        break;
      case 4:
        ret = TM_Battle->rivalindex;
        break;
      case 5:
        ret = TM_Battle->CreateTime;
        break;
      case 6:
        ret = TM_Battle->EndTime;
        break;
      case 7:
        ret = TM_Battle->mode;
        break;
...
static int nohelpescsend(lua_State *L) {
  const int charaindex = luaL_checkint(L, 1);
  const int help = luaL_checkint(L, 2);
  const int escp = luaL_checkint(L, 3);

  CHAR_setWorkInt(charaindex, CHAR_WORK_NOHELP, help);
  CHAR_setWorkInt(charaindex, CHAR_WORK_NOESCAPE, escp);
  print("*************nohelpescsend************\n\n\n");
  print("*************charaindex:%d************\n\n\n", charaindex);
  print("*************help:%d************\n\n\n", help);
  print("*************escp:%d************\n\n\n", escp);
  lssproto_BattleButton_send(getfdFromCharaIndex(charaindex), help, escp);
}
...
static int CreateVsEnemy(lua_State *L) {
  ...
  if (ret == 0) {
    ...
    if (CHAR_getWorkInt(npcindex, CHAR_WORK_NOHELP) == 1) {
      CHAR_setWorkInt(charaindex, CHAR_WORK_NOHELP, 1);
    }
    if (CHAR_getWorkInt(npcindex, CHAR_WORK_NOESCAPE) == 1) {
...
static int CreateVsEnemyLv(lua_State *L) {
  ...
  int ret =
      BATTLE_CreateVsEnemyLvNew(charaindex, npcindex, enemytable, enemylvtable);
  if (ret == 0) {
    if (npcindex > -1) {
      CHAR_setWorkInt(npcindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_INIT);
      CHAR_sendBattleEffect(npcindex, ON);
...
static int CreateVsPlayer(lua_State *L) {
  ...
  if (CHAR_CHECKINDEX(charaindex1) && CHAR_CHECKINDEX(charaindex2)) {
    if (CHAR_getWorkInt(charaindex1, CHAR_WORKBATTLEMODE) ==
        ...
        CHAR_getWorkInt(charaindex2, CHAR_WORKBATTLEMODE) ==
            BATTLE_CHARMODE_NONE) {
...
static int setLUAFunctionPointer(lua_State *L) {
  ...
  const int battleindex = luaL_checkint(L, 1);
  const int functype =
      getCharBaseValue(L, 2, BattleBaseEvent, arraysizeof(BattleBaseEvent), 1);
...
static int WatchEntry(lua_State *L) {
  ...
  if (CHAR_CHECKINDEX(meindex) && CHAR_CHECKINDEX(toindex)) {
...
static int NewEntry(lua_State *L) {
  ...
  if (iRet == 0) {
    ...
    flg = (BattleArray[battleindex].Side[side].flg & BSIDE_FLG_HELP_OK) ? TRUE
                                                                        : FALSE;
    ...
    BATTLE_CommandSendHalo(charaindex, battleindex);
#endif
    lua_pushinteger(L, 1);
  } else {
    if (fd != -1) {
