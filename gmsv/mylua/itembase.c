...
extern MY_Lua MYLua;
static CharBase ItemBaseInt[] = {{{"序号"}, ITEM_ID},
...
static CharBase ItemBaseChar[] = {{{"名称"}, ITEM_NAME},
                                  {{"显示名"}, ITEM_SECRETNAME},
...
static CharBase ItemBaseValue[] = {{{"爪"}, ITEM_FIST},
                                   {{"斧"}, ITEM_AXE},
                                   {{"棍"}, ITEM_CLUB},
                                   {{"枪"}, ITEM_SPEAR},
                                   {{"弓"}, ITEM_BOW},
...
static CharBase ItemBaseWorkInt[] = {{{"对象"}, ITEM_WORKOBJINDEX},
                                     {{"玩家"}, ITEM_WORKCHARAINDEX}};

static CharBase ItemBaseEvent[] = {
    {{"未知事件"}, ITEM_PREOVERFUNC},  {{"邮件事件"}, ITEM_POSTOVERFUNC},
    {{"使用事件"}, ITEM_USEFUNC},      {{"装上事件"}, ITEM_ATTACHFUNC},
    {{"卸下事件"}, ITEM_DETACHFUNC},   {{"丢弃事件"}, ITEM_DROPFUNC},
    {{"检起事件"}, ITEM_PICKUPFUNC},   {{"死亡事件"}, ITEM_DIERELIFEFUNC},
    {{"ATTACHFUNC"}, ITEM_ATTACHFUNC}, {{"DETACHFUNC"}, ITEM_DETACHFUNC}};
...
static int getInt(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, ItemBaseInt, arraysizeof(ItemBaseInt), 1);
...
static int getChar(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, ItemBaseChar, arraysizeof(ItemBaseChar), 1);
...
static int getWorkInt(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, ItemBaseWorkInt, arraysizeof(ItemBaseWorkInt), 1);
...
static int setFunctionPointer(lua_State *L) {
  ...
  const int index = luaL_checkint(L, 1);
  const int functype =
      getCharBaseValue(L, 2, ItemBaseEvent, arraysizeof(ItemBaseEvent), 1);
