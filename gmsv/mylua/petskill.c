...
extern MY_Lua MYLua;
static CharBase PetSkillBaseInt[] = {{{"序号"}, PETSKILL_ID},
                                     {{"字段"}, PETSKILL_FIELD},
                                     {{"目标"}, PETSKILL_TARGET}
...
static CharBase PetSkillBaseChar[] = {{{"名称"}, PETSKILL_NAME},
                                      {{"注释"}, PETSKILL_COMMENT},
...
static int getInt(lua_State *L) {
  ...
  const int element =
      getCharBaseValue(L, 2, PetSkillBaseInt, arraysizeof(PetSkillBaseInt), 1);
...
static int getChar(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element = getCharBaseValue(L, 2, PetSkillBaseChar,
                                       arraysizeof(PetSkillBaseChar), 1);
...
static const luaL_Reg petskilllib[] = {
    {"getInt", getInt},   {"setInt", setInt},
    {"getChar", getChar}, {"setChar", setChar},
    {"check", check},     {"getPetskillArray", getPetskillArray},
    {NULL, NULL}};
