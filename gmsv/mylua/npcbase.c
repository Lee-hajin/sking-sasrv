...
static int NPC_GiveItem(int charaindex, int itemid) {
...
static int ClsEvent(lua_State *L) {
  size_t l;
  const int index = luaL_checkint(L, 1);
  char *data = luaL_checklstring(L, 2, &l);
...
static int SetPoint(lua_State *L) {
  ...
  CHAR_sendCDArroundChar(ff, fx, fy,
...
static int DelItemNum(lua_State *L) {
  ...
  for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(index); i++) {
    ...
    LogItem(CHAR_getChar(index, CHAR_NAME), CHAR_getChar(index, CHAR_CDKEY),
            itemindex, "LUA收取道具", CHAR_getInt(index, CHAR_FLOOR),
...
static int DelNpc(lua_State *L) {
  ...
  // 2018.08.06新增
  int TM_Objindex = CHAR_getWorkInt(index, CHAR_WORKOBJINDEX);
...
static int CheckEvent(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int eventid = luaL_checkint(L, 2);
  int ret;
  if (!CHAR_CHECKINDEX(index)) {
    print("CheckEvent ANDY index=%d err\n", index);
    ret = 0;
  }

  if (NPC_EventCheckFlg(index, eventid) == FALSE) {
    ret = 0;
  } else {
    ret = 1;
  }
