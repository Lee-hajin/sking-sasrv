...
static CharBase MagicBaseInt[] = {{{"ID"}, MAGIC_ID},
                                  {{"字段"}, MAGIC_FIELD},
                                  {{"目标"}, MAGIC_TARGET},
...
static CharBase MagicBaseChar[] = {{{"名字"}, MAGIC_NAME},
                                   {{"注释"}, MAGIC_COMMENT},
                                   {{"函数名"}, MAGIC_FUNCNAME},
                                   {{"选项"}, MAGIC_OPTION}};
  ...
  if (strlen(luafunctablepath) > 0) {
    ...
    while (mylua->next != NULL) {
      if (strcmp(mylua->luapath, luafunctablepath) == 0) {
        return MAGIC_addLUAListFunction(mylua->lua, luafuncname, luafunctable,
                                        gmlevel, usestring);
      }
      mylua = mylua->next;
    }
...
static int getChar(lua_State *L) {
  ...
  const int element =
      getCharBaseValue(L, 2, MagicBaseChar, arraysizeof(MagicBaseChar), 1);
...
static int getInt(lua_State *L) {
  const int magicid = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, MagicBaseInt, arraysizeof(MagicBaseInt), 1);
  int marray = MAGIC_getMagicArray(magicid);
  if (marray != -1) {
...
static const luaL_Reg magiclib[] = {{"addLUAListFunction", addLUAListFunction},
                                    {"getInt", getInt},
                                    {"getChar", getChar},
                                    {NULL, NULL}};
