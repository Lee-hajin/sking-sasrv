...
void List(char *path) {
  ...
  while (NULL != (ent = readdir(pDir))) {
    ...
    if (ent->d_type == 8) {
      char line[1024];
      if (ent->d_name == NULL || strlen(ent->d_name) < 1 ||
          ent->d_name[strlen(ent->d_name) - 1] == '~' ||
          ent->d_name[0] == '#' || strcmptail(ent->d_name, ".bak") == 0 ||
          strcmp(path, "attmagic.bin") == 0)
        continue;

      sprintf(filename, "%s/%s", path, ent->d_name);
      FILE *f1 = fopen(filename, "r");

      sprintf(filename, "93sa/%s/%s.93", path, ent->d_name);
      remove(filename);
      ...
      if (strcmp(path, "data") == 0 && (strcmp(ent->d_name, "autopk") == 0 ||
                                        strcmp(ent->d_name, "map") == 0))
        continue;
      sprintf(filename, "93sa/%s/%s", path, ent->d_name);
