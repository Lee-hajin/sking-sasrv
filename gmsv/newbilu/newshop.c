...
BOOL OnlineShop_init() {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (getStringFromIndexWithDelim(
            line, ",", 1, onlineshop[shoplist][list].name,
            sizeof(onlineshop[shoplist][list].name)) == FALSE) {
    ...
    if (getStringFromIndexWithDelim(
            line, ",", 5, onlineshop[shoplist][list].readme,
            sizeof(onlineshop[shoplist][list].readme)) == FALSE) {
      strcmp(onlineshop[shoplist][list].readme,
             onlineshop[shoplist][list].name);
    }
...
void OnlineShop_ShowList(int fd, int charaindex, int type, int page) {
  ...
  for (i = startpage; i < startpage + SHOPPAGEMAX; i++) {
    ...
    if (onlineshop[type - 1][i].use == TRUE) {
      sprintf(buf, "%d#%d#%s#%d#%d#%d#%s|", i, onlineshop[type - 1][i].picid,
              onlineshop[type - 1][i].name, onlineshop[type - 1][i].color,
              onlineshop[type - 1][i].percentage, onlineshop[type - 1][i].price,
  ...
  if ((type - 1) == ONLINESHOP_AMPOINT) {
    lssproto_VIP_SHOP_send(fd, availability,
                           CHAR_getInt(charaindex, CHAR_AMPOINT), type, page,
                           pagemax, token);
  } else {
#ifdef _ONLINE_SHOP_MYSQL
    lssproto_VIP_SHOP_send(fd, availability, sasql_getVipPoint(charaindex),
                           type, page, pagemax, token);
#else
    lssproto_VIP_SHOP_send(fd, availability, CHAR_getInt(charaindex, CHAR_BJ),
                           type, page, pagemax, token);
#endif
  }
}

void OnlineShop_Buy(int fd, int charaindex, int type, int page, int id,
                    int num) {
  switch (type - 1) {
  case ONLINESHOP_PET: {
#ifdef _ONLINE_SHOP_MYSQL
...
#endif
    int price = onlineshop[type - 1][id].price *
                onlineshop[type - 1][id].percentage / 100;
    if (bj >= price * num) {
      char token[64];
      int i, j;
...
#else
          CHAR_setInt(charaindex, CHAR_BJ,
                      CHAR_getInt(charaindex, CHAR_BJ) - price);
#endif
          char token[256];
...
#endif
    int price = onlineshop[type - 1][id].price *
                onlineshop[type - 1][id].percentage / 100 * num;
    if (bj >= price) {
#ifdef _ONLINE_SHOP_MYSQL
...
#ifdef _ONLINE_SHOP_MYSQL
      sasql_setVipPoint(charaindex, sasql_getVipPoint(charaindex) +
                                        onlineshop[type - 1][id].id * num);
#else
      CHAR_setInt(charaindex, CHAR_BJ,
                  CHAR_getInt(charaindex, CHAR_BJ) +
                      onlineshop[type - 1][id].id * num);
#endif
    }
  } break;
...
#ifdef _ONLINE_SHOP_MYSQL
    lssproto_VIP_SHOP_send(fd, 0,
                           (type - 1 == ONLINESHOP_AMPOINT)
                               ? CHAR_getInt(charaindex, CHAR_AMPOINT)
                               : sasql_getVipPoint(charaindex),
                           i, page, pagemax, "");
#else
    lssproto_VIP_SHOP_send(fd, 0,
                           (type - 1 == ONLINESHOP_AMPOINT)
