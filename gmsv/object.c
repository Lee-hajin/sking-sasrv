...
void endObjectOne(int index) {
  if (objnum <= index || index < 0)
    return;

  if (MAP_removeObj(obj[index].floor, obj[index].x, obj[index].y, index) ==
      FALSE) {
    //        fprint( "REMOVE OBJ ERROR  floor:%d  x:%d
...
BOOL CHECKOBJECTUSE(int index) {
  if (CHECKOBJECT(index) == FALSE)
    return FALSE;
  if (obj[index].type == OBJTYPE_NOUSE)
    return FALSE;
  return TRUE;
}

int OBJECT_getType(int index) { return obj[index].type; }

int OBJECT_getchartype(int index) { return obj[index].chartype; }
...
int OBJECT_getFloor(int index) { return obj[index].floor; }
...
int OBJECT_getX(int index) { return obj[index].x; }
...
int OBJECT_getY(int index) { return obj[index].y; }
...
int OBJECT_getTime(int index) { return obj[index].time; }
 ...
 *  int
 ------------------------------------------------------------*/
int OBJECT_getIndex(int index) { return obj[index].index; }
/*------------------------------------------------------------
 * y 毛涩烂允月
 * 娄醒
 *  index      int     奶件犯永弁旦
 ...
 *  int
 ------------------------------------------------------------*/
int OBJECT_getNum(void) { return objnum; }
  ...
  for (i = 0; i < objnum; i++) {
    ...
    case OBJTYPE_GOLD: {
      fprintf(igfile, STOREGOLDID "|x=%d|y=%d|floor=%d|%d\n", obj[i].x,
              obj[i].y, obj[i].floor, obj[i].index);
      break;
    }
    case OBJTYPE_CHARA: {
