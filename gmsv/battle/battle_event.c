...
char *aszParamChangeFull[] = {"无",   "攻击力", "防御力",
                              "速度", "魅力",   "捕获率"};
...
// 取得骑宠时的三围  正常版
float BATTLE_adjustRidePet3A(int charaindex, int petindex, int workindex,
                             int action) {
  float ret = CHAR_getWorkInt(charaindex, workindex);
  BOOL throwweapon =
      BATTLE_IsThrowWepon(CHAR_getItemIndex(charaindex, CHAR_ARM));
  ...
  case CHAR_WORKATTACKPOWER: // 攻击力
  {
    ...
    if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ABSOLUTE2)) > 0)
      absulte = GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ABSOLUTE2));
#endif
    if (throwweapon) // 投掷
  ...
  case CHAR_WORKDEFENCEPOWER: // 防御力
    ret = CHAR_getWorkInt(charaindex, CHAR_WORKDEFENCEPOWER);
    break;
...
int BATTLE_ItemCrushCheck(int charaindex, int flg) { // flg def 1 att 2
...
int BATTLE_ItemCrush(int charaindex, int ItemEquip, int Damages, int flg) {
  ...
  if (crushenum <= 0) { //损坏消失
    ...
    LogItem(CHAR_getChar(charaindex, CHAR_NAME),
            CHAR_getChar(charaindex, CHAR_CDKEY), itemindex, "因过度损坏而消失",
    ...
    if (level != crushlevel && defs < 70) {
      ...
      //++ ){ 				work = ITEM_getInt( itemindex, ItemBreakPos[i] ); 				if( work <= 0
      //)continue; 				work *=0.95; 				ITEM_setInt( itemindex, ItemBreakPos[i], work );
      //			}
      sprintf(szBuffer, "%s是%s的。", ITEM_getChar(itemindex, ITEM_NAME),
              aszCrushTbl[level]);
      CHAR_talkToCli(charaindex, -1, szBuffer, CHAR_COLORRED);
      //更改道具说明
      //			buf1 = ITEM_getChar( itemindex,
      //ITEM_SECRETNAME); 			if( strstr( buf1, "(") != 0 )	{ 				char buf5[256]; 				if(
      //getStringFromIndexWithDelim( buf1,"(",1, buf5, sizeof( buf5)) != FALSE )
      //{ 					sprintf( buf1, buf5);
      //				}
      //			}
      //			sprintf( buf2, "%s(%s)", buf1,
      //aszCrushTbl[level]); 			ITEM_setChar( itemindex, ITEM_SECRETNAME, buf2);
    }
  }
...
int BATTLE_ItemCrush(int charaindex) {
    ...
    return FALSE;
  {
    ...
    LogItem(CHAR_getChar(charaindex, CHAR_NAME),
            CHAR_getChar(charaindex, CHAR_CDKEY), itemindex, szBuffer,
            CHAR_getInt(charaindex, CHAR_FLOOR),
            CHAR_getInt(charaindex, CHAR_X), CHAR_getInt(charaindex, CHAR_Y),
            ITEM_getChar(itemindex, ITEM_UNIQUECODE),
            ITEM_getChar(itemindex, ITEM_NAME), ITEM_getInt(itemindex, ITEM_ID),
...
static int BATTLE_ItemCrushSeq(int charaindex) {
  ...
  if (BATTLE_ItemCrushCheck(charaindex) == TRUE) {
    if (BATTLE_ItemCrush(charaindex) == TRUE) {
      sprintf(szWork, "BK|%s的\t装备受到损伤。|", CHAR_getUseName(charaindex));
...
static BOOL BATTLE_DuckCheck(int attackindex, int defindex) {
  ...
  if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) ==
      BATTLE_COM_S_CHARGE_OK) { //技能状态判断
    return FALSE;
  }
  if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) ==
      BATTLE_COM_GUARD) { //技能状态判断
    return FALSE;
  }
  if (BATTLE_GetDamageReact(defindex) > 0) { //光镜守护的判断
    return FALSE;
  }
  if (BATTLE_CanMoveCheck(defindex) == FALSE) { //咒术状态的判断
#ifdef _PROFESSION_ADDSKILL
    if (CHAR_getWorkInt(defindex, CHAR_DOOMTIME) <=
      ...
      if (CHAR_getWorkInt(defindex, CHAR_WORKDRAGNET) == 0 &&
          ...
          &&
          GETHIGHVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_POISON_DRAGNET)) == 0
#endif
      ) {
      } else {
        return FALSE;
...
#endif
  //以上各种状态的判断就不一一解说了
  At_Dex = CHAR_getWorkInt(attackindex, CHAR_WORKFIXDEX); //取攻方对像敏捷
  Df_Dex = CHAR_getWorkInt(defindex, CHAR_WORKFIXDEX); //取防方对像敏捷
  if (CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
      CHAR_TYPEPLAYER) { //如果防方对像是玩家
    Df_Luck = CHAR_getWorkInt(defindex, CHAR_WORKFIXLUCK); //取幸运值
  }
  if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY &&
      CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
          CHAR_TYPEPET) { //如果攻方是怪物，防方是宠物，攻方敏捷*0.8
    At_Dex *= 0.8;
  } else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEENEMY &&
             CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
                 CHAR_TYPEPET //如果攻方不是怪物，防方是宠物，防方敏捷*0.8
  ) {
    Df_Dex *= 0.8;
  } else if (
      CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER &&
      CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
          CHAR_TYPEPLAYER) { //如果攻方不是玩家，防方不是玩家，攻方敏捷*0.6
    At_Dex *= 0.6;
  } else if (
      CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
      CHAR_getInt(defindex, CHAR_WHICHTYPE) !=
          CHAR_TYPEPLAYER) { //如果攻方是玩家，防方不是玩家，防御方敏捷*0.6
    Df_Dex *= 0.6;
  }

  if (Df_Dex >= At_Dex) { //上面计算后，防方敏捷大于攻方
    Big = Df_Dex;         // BIG值
    Small = At_Dex;       // SMALL值
    wari = 1.0;           //修正值1
  } else {
  ...
  //宠物技能相关的特殊修正，宠物使用咒术，回复等技能才出发这个修正
  if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_JYUJYUTU ||
      CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_JYUJYUTU2) {
    gKawashiPara = 0.027; //修正值2
  } else {
    gKawashiPara = 0.02; //修正值2
  }
  //先计算特殊修正
  //结果 = (BIG值 - SMALL值) / 修正值2
  ...
  if (Work <= 0)
    Work = 0;
  //回避 最终计算
  per = (float)(sqrt((double)Work)); //平方根计算
  per *= wari;                       //回避 * 修正值1
  per += Df_Luck;                    //回避 + 运气
  per += CHAR_getInt(defindex, CHAR_AVOID_RATE) -
         CHAR_getInt(attackindex, CHAR_HIT_RATE); //回避 + 运气
  per += gBattleDuckModyfy; //回避 + 外部传来的命中率
  if (CHAR_getWorkInt(attackindex, CHAR_WORKDRUNK) > 0) { //如果攻方处于混乱状态
    per += RAND(20, 30); //回避率直接等于 20~30之间随机数
  }
  if (gWeponType == ITEM_BOW) { //如果攻方为远程武器
    per += 20;                  //回避 + 20
  }
  if (CHAR_getWorkInt(defindex, CHAR_WORKBATTLECOM1) ==
      BATTLE_COM_S_NOGUARD) { //如果防方用不防守战法
    per +=
        CHAR_GETWORKINT_HIGH(defindex, CHAR_WORKBATTLECOM3); //回避+技能的回避值
  }
  ...
  if (per > KAWASHI_MAX_RATE * 100)
    per = KAWASHI_MAX_RATE * 100; //如果回避大于 7500则直接等于7500
  if (per <= 0)
    per = 1;

  gDuckPer = per * 0.01; //
#ifdef _EQUIT_HITRIGHT   //攻方装备命中降低防方回避
  if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
    int AddHit = CHAR_getWorkInt(attackindex, CHAR_WORKHITRIGHT); //取得装备命中
    per -=
        RAND(AddHit * 0.8,
             AddHit * 1.2); //回避率 - (装备命中*0.8 ~ 装备命中*1.2之间的随机值)
    per = (per < 0) ? 0 : per; //防止溢出
  }
#endif
#ifdef _PROFESSION_SKILL // WON ADD 人物职业技能
  per = BATTLE_check_profession_duck(defindex, per); //防方直接技能的回避加成
#endif
#ifdef _PROFESSION_ADDSKILL
  if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) ==
      BATTLE_COM_S_CHAOS) { //攻房混乱攻击时防方回避增加
    per += (per * 0.4);     //回避 * 0.4
  }
#endif

  if (RAND(1, 10000) <=
      per) { //取1~10000的随机数，如果随机数小于或等于回避率，攻方命中成功，防方回避失败
    flg = TRUE;
#ifdef _PROFESSION_SKILL // WON ADD 人物职业技能
    // 回避技能升级
 ...
 */
    ...
    T_pow[3] = (CHAR_getWorkInt(charaindex, CHAR_WORKFIXWINDAT) +
               ...
               2;
    /*
                    *pAt_Fire = ( CHAR_getWorkInt( charaindex,
       CHAR_WORKFIXFIREAT )
                            + CHAR_getWorkInt( petindex, CHAR_WORKFIXFIREAT ))
       /2; *pAt_Water = ( CHAR_getWorkInt( charaindex, CHAR_WORKFIXWATERAT )
                            + CHAR_getWorkInt( petindex, CHAR_WORKFIXWATERAT ))
       /2; *pAt_Earth = ( CHAR_getWorkInt( charaindex, CHAR_WORKFIXEARTHAT )
                            + CHAR_getWorkInt( petindex, CHAR_WORKFIXEARTHAT ))
       /2; *pAt_Wind = ( CHAR_getWorkInt( charaindex, CHAR_WORKFIXWINDAT )
                            + CHAR_getWorkInt( petindex, CHAR_WORKFIXWINDAT ))
       /2;
    */
  }
...
#ifdef _PROFESSION_ADDSKILL
  //加上结界
  // T_pow[0] += T_pow[0]*( (CHAR_getWorkInt( charaindex,
  // CHAR_WORKFIXEARTHAT_BOUNDARY )>>16&0x0000ffff) /100 ); T_pow[1] +=
  // T_pow[1]*( (CHAR_getWorkInt( charaindex, CHAR_WORKFIXWATERAT_BOUNDARY
  // )>>16&0x0000ffff) /100 ); T_pow[2] += T_pow[2]*( (CHAR_getWorkInt(
  // charaindex, CHAR_WORKFIXFIREAT_BOUNDARY )>>16&0x0000ffff) /100 ); T_pow[3]
  // += T_pow[3]*( (CHAR_getWorkInt( charaindex, CHAR_WORKFIXWINDAT_BOUNDARY
  // )>>16&0x0000ffff) /100 );
#endif
#ifdef _SUIT_TWFWENDUM
  T_pow[0] = (T_pow[0] + CHAR_getWorkInt(charaindex, CHAR_WORK_EA)) > 100
                 ? 100
                 : (T_pow[0] + CHAR_getWorkInt(charaindex, CHAR_WORK_EA));
  T_pow[1] = (T_pow[1] + CHAR_getWorkInt(charaindex, CHAR_WORK_WR)) > 100
                 ? 100
                 : (T_pow[1] + CHAR_getWorkInt(charaindex, CHAR_WORK_WR));
  T_pow[2] = (T_pow[2] + CHAR_getWorkInt(charaindex, CHAR_WORK_FI)) > 100
                 ? 100
                 : (T_pow[2] + CHAR_getWorkInt(charaindex, CHAR_WORK_FI));
  T_pow[3] = (T_pow[3] + CHAR_getWorkInt(charaindex, CHAR_WORK_WI)) > 100
                 ? 100
                 : (T_pow[3] + CHAR_getWorkInt(charaindex, CHAR_WORK_WI));
#endif
  {
  ...
  /*
          ...
          *pAt_None = ATTR_MAX - *pAt_Fire - *pAt_Water - *pAt_Earth -
     *pAt_Wind; if( *pAt_None < 0 )*pAt_None = 0;
  */
}
...
static int BATTLE_AttrAdjust(int attackindex, int defindex, int damage) {
  ...
  */
  damage = BATTLE_AttrCalc( //火2水1第0封3
      At_pow[2], At_pow[1], At_pow[0], At_pow[3], At_pow[4], Dt_pow[2],
      Dt_pow[1], Dt_pow[0], Dt_pow[3], Dt_pow[4]);
...
int BATTLE_DamageCalc(int attackindex, int defindex) {
...
#ifdef _PRO_ABSOLUTE_DEFENSE
    if (GETHIGHVALUE(CHAR_getWorkInt(attackindex, CHAR_WORK_ABSOLUTE2)) > 0)
      absulte = GETLOWVALUE(CHAR_getWorkInt(attackindex, CHAR_WORK_ABSOLUTE2));
    attack -= absulte;
#endif
  } else {
...
#ifdef _PROFESSION_ADDSKILL
  {
    ...
    // T_pow[1] += T_pow[1]*( (CHAR_getWorkInt( charaindex,
    // CHAR_WORKFIXWATERAT_BOUNDARY )>>16&0x0000ffff) /100 ); T_pow[2] +=
    // T_pow[2]*( (CHAR_getWorkInt( charaindex, CHAR_WORKFIXFIREAT_BOUNDARY
    // )>>16&0x0000ffff) /100 ); T_pow[3] += T_pow[3]*( (CHAR_getWorkInt(
    // charaindex, CHAR_WORKFIXWINDAT_BOUNDARY )>>16&0x0000ffff) /100 );
  }
#endif
...
#ifdef _DAMMAGE_CALC
  // attackindex攻方  defindex防方
  if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
    damage = damage * getPlayDamageCalc() / 100; //取得自定义伤害比例
    // print("对方为人物:伤害值百分比：%d\n",getPlayDamageCalc());
  } else if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET) {
    // print("对方为宠物:伤害值百分比：%d\n",getPetDamageCalc());
    damage = damage * getPetDamageCalc() / 100; //取得自定义伤害比例
  } else {
    // print("对方啥都不是！\n");
  }
#endif
...
static int BATTLE_CriticalCheckPlayer(int attackindex,
                                      int defindex) //会心一击的计算
{
  ...
  At_Dex =
      CHAR_getWorkInt(attackindex, CHAR_WORKFIXDEX); //得到发起攻击对象的敏捷
  Df_Dex = CHAR_getWorkInt(defindex, CHAR_WORKFIXDEX); //得到受到攻击对象的敏捷

  if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) ==
      CHAR_TYPEPLAYER) { //如果攻击对象为玩家
    At_Luck = CHAR_getWorkInt(attackindex, CHAR_WORKFIXLUCK); //会心率取幸运值
  }
  At_SoubiIndex = CHAR_getItemIndex(attackindex, CHAR_ARM); //取武器索引
  At_Kiryoku = CHAR_getInt(attackindex, CHAR_MP); //取攻击对象的MP

  if (ITEM_CHECKINDEX(At_SoubiIndex) == TRUE) {
    At_Soubi = ITEM_getInt(At_SoubiIndex, ITEM_CRITICAL); //取武器会心率
  }
  if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPET &&
      CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
          CHAR_TYPEENEMY) { //如果攻击对象为宠物，受到对象为怪物
    Df_Dex *= 0.8;          //受到对象敏捷*0.8
  } else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY &&
             CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
                 CHAR_TYPEPET) { //如果攻击对象为怪物，受到对象为宠物
    divpara = 10.0;              //修正值1
    root = 0;                    //标识
  } else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER &&
             CHAR_getInt(defindex, CHAR_WHICHTYPE) ==
                 CHAR_TYPEPLAYER) { //如果攻击对象不是玩家，受到对象为玩家
    divpara = 10.0;                 //修正值1
    root = 0;                       //标识
  } else if (CHAR_getInt(attackindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER &&
             CHAR_getInt(defindex, CHAR_WHICHTYPE) !=
                 CHAR_TYPEPLAYER) { //如果攻击对象为玩家，受到对象不是玩家
    Df_Dex *= 0.6;                  //受到对象敏捷*0.6
  }

  if (At_Dex >= Df_Dex) { //如果攻击对象敏捷大于或等于受到对象的情况下
    Big = At_Dex;   // BIG值
    Small = Df_Dex; // Small值
    wari = 1.0;     //修正值2
  } else {          //如果攻击对象敏捷小于受到对象的情况下
    Big = Df_Dex;   // BIG值
    Small = At_Dex; // Small值
    if (Big <= 0) { // BIG小于或等于0
      wari = 0.0;   //修正值2
  ...
  if (Work <= 0)
    Work = 0;
  if (root ==
      1) { //如果标识为1时进行 平方根计算Work 之后加 (武器的会心率 * 0.5)
    per = (float)(sqrt((double)Work)) + At_Soubi * 0.5;
  } else { //否则普通计算Work 之后加 (武器的会心率 * 0.5)
    per = (float)Work + At_Soubi * 0.5;
  }
  // per为最终返回的会心率
  per *= wari;                                     //会心率 *修正值2
  per += At_Luck;                                  //会心率 + 幸运值
  per += CHAR_getInt(attackindex, CHAR_CRIT_RATE); //会心率 + 幸运值
  per *= 100;                                      //会心率 * 100
  if (per < 0)
    per = 1; //防溢出设置
  if (per > 10000)
...
static int BATTLE_GuardianCheck(int attackindex, int defindex) {
  ...
  if (CHAR_getWorkInt(GuardianIndex, CHAR_WORKSLEEP) > 0 ||
      ...
      || GETTHIRDVALUE(CHAR_getWorkInt(GuardianIndex, CHAR_WORK_PROVOKEFIGHT)) >
             0 //挑畔反击
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
      || CHAR_getWorkInt(GuardianIndex, CHAR_WORK_ABSOLUTE) > 0
#endif
#ifdef _PRO3_ADDSKILL
      ||
      GETHIGHVALUE(CHAR_getWorkInt(GuardianIndex, CHAR_WORK_POISON_DRAGNET)) > 0
#endif
  ) {
...
static int BATTLE_AttackSeq(int attackindex, int defindex, int *pDamage,
  ...
  if (opt ==
      BATTLE_COM_S_DAMAGETOHP2) { //暗月狂狼 (会心一击上升30%)  && CHAR_getInt(
                                  //attackindex, CHAR_HP) <= CHAR_getWorkInt(
                                  //attackindex, CHAR_MAXMP)*0.5
    // print("\n1会心一击:%d",perCri);
...
#ifdef _PRO_ABSOLUTE_DEFENSE
  if (Absolute_Defense_Check(defindex) == TRUE)
    *pDamage = 0;
#endif
    ...
    if (*pRefrect == BATTLE_MD_TRAP) { //陷阱
...
#endif
#ifdef BOSS_AI
        if (CHAR_getWorkInt(defindex, CHAR_WORKTACTICS) == TACTICS_BOSS) {
          int attackSide = 0, SubNo;
          int attackNo = BATTLE_Index2No(battleindex, attackindex);
          ...
          BattleArray[battleindex].Side[attackSide].Entry[SubNo].hatred +=
              damage;
        }
#endif
        playerdamage = damage;
      ...
      *pRefrect = BATTLE_MD_VANISH;
      ...
      if (CHAR_getWorkInt(defindex, CHAR_WORKTACTICS) == TACTICS_BOSS) {
        ...
        BattleArray[battleindex].Side[attackSide].Entry[SubNo].hatred += damage;
      }
#endif
      hp = CHAR_getInt(defindex, CHAR_HP);
      if (defpet == -1) {
    ...
    if (pethp <= 0) {
      ...
      CHAR_setWorkInt(
          ...
          MAKE2VALUE(CHAR_getInt(defindex, CHAR_RIDEPET),
                     GETHIGHVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_RIDE))));
#endif
...
int BATTLE_Attack(int battleindex, int attackNo, int defNo) {
  ...
  if (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) ==
    ...
    if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
      ...
      if (CHAR_CHECKITEMINDEX(defindex, itemindex) == FALSE) {
        ...
        if (toid == -1) {
          sprintf(token, "BK|%s无空位卸下%s|", CHAR_getUseName(defindex),
                  item[fromid]);
        } else {
          if (rand() % 100 <= lj) {
...
#endif
    iRet = FALSE;
#ifdef _FIX_ACUPUNCTURE2 //修复传死亡旗给错的CLIENT
    if (react != BATTLE_MD_ACUPUNCTURE)
      flg |= BCF_DEATH;
#else
    flg |= BCF_DEATH;
...
#ifdef _PETSKILL_PLUS1
  int successFlg = 0;
  if (gBattleStausSuccess > 0 && RAND(1, 100) < gBattleStausSuccess)
    successFlg = 1;
  //确认有无其他状态
  if (gBattleStausChange >= BATTLE_ST_END || gBattleStausChange <= 0)
    successFlg = 0;
  ...
  if ((damage > 0) && (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) ==
    ...
    int array = -1, temp2;
    char *pszOption = NULL, temp[64] = "";
    array = CHAR_GETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM3);
    pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
    strcpy(temp, pszOption);
    if ((pszOption = strstr(temp, "乱%"))) {
      pszOption += 3;
      sscanf(pszOption, "%d", &temp2);
      gBattleStausChange = BATTLE_ST_CONFUSION;
      suitpoison = temp2;
      gBattleStausTurn = 3;
    }
    if ((pszOption = strstr(temp, "回%"))) {
      pszOption += 3;
      sscanf(pszOption, "%d", &temp2);
      if (CHAR_getWorkInt(defindex, CHAR_MYSKILLDUCK) > 0) {
        int ducktemp = CHAR_getWorkInt(defindex, CHAR_MYSKILLDUCKPOWER);
        CHAR_setWorkInt(defindex, CHAR_MYSKILLDUCKPOWER,
                        ducktemp + (ducktemp * (temp2 / 100.0)));
      }
    }
  }
#endif
  ...
  if ((damage > 0) && (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) ==
    ...
    if (RAND(1, 5) > 4) {
      ...
      CHAR_setWorkInt(defindex, CHAR_WORKBATTLECOM1,
                      BATTLE_COM_NONE); //因为设定了昏睡,所以设定无动作
    }
  }
#endif
  if (damage > 0) {
...
int BATTLE_Attack_FIREKILL(int battleindex, int attackNo, int defNo) {
  ...
  //#ifdef _PETSKILL_ACUPUNCTURE
  //    if( react == BATTLE_MD_ACUPUNCTURE ){
  //    //针刺外皮时会无法正常解除被打方的状态,所以把index换回来
  //	    defindex = toindex = BATTLE_No2Index( battleindex, defNo );
  //        statusDefNo = defNo;
  //	}
  //#endif
...
static int IsNeedCaptureItem(int charaindex) {
  ...
  for (i = 0; i < arraysizeof(NeedEnemy); i++) {
    if (NeedEnemy[i].EnemyId != CHAR_getInt(charaindex, CHAR_PETID))
      continue;
#ifdef _CAPTURE_FREES
    return i;
#else
...
BOOL BATTLE_CaptureItemCheck(int attackindex, int defindex) {
  ...
  for (i = 0; i < MAXCAPTRUEFREE; i++) {
    if (NeedEnemy[ti].ItemId[i] == -1)
      break;
    for (j = 0; j < CHAR_getMaxInventory(attackindex); j++) {
    ...
    //没找到
    if (j >= CHAR_getMaxInventory(attackindex))
      return FALSE;
  }
  return TRUE;
#else
  for (j = CHAR_STARTITEMARRAY; j < CHAR_getMaxInventory(attackindex); j++) {
    itemindex = CHAR_getItemIndex(attackindex, j);
    if (ITEM_CHECKINDEX(itemindex) == FALSE)
      continue;
    ...
    for (j = 0; j < CHAR_getMaxInventory(attackindex); j++) {
      ...
      if (ITEM_getInt(itemindex, ITEM_ID) == NeedEnemy[ti].ItemId[i]) {
        LogItem(CHAR_getChar(attackindex, CHAR_NAME),
                CHAR_getChar(attackindex, CHAR_CDKEY), itemindex,
                "使用道具捕获宠物", CHAR_getInt(attackindex, CHAR_FLOOR),
                CHAR_getInt(attackindex, CHAR_X),
                CHAR_getInt(attackindex, CHAR_Y),
                ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                ITEM_getChar(itemindex, ITEM_NAME),
                ITEM_getInt(itemindex, ITEM_ID), attackindex);
        {
...
BOOL BATTLE_Capture(int battleindex, int attackNo, int defNo) {
  ...
  if (CaptureCheckFunction(attackindex, defindex) == 0
#else
  ...
  if (flg == 1) {
    ...
    if (pindex == -1) {
      ...
      LogPet(CHAR_getUseName(attackindex), CHAR_getUseID(attackindex),
             ...
             "捕获宠物", CHAR_getInt(attackindex, CHAR_FLOOR),
             CHAR_getInt(attackindex, CHAR_X), CHAR_getInt(attackindex, CHAR_Y),
             CHAR_getChar(defindex, CHAR_UNIQUECODE), defindex, attackindex);
      {
        int array = ENCOUNT_getEncountAreaArray(
            CHAR_getInt(attackindex, CHAR_FLOOR),
            CHAR_getInt(attackindex, CHAR_X), CHAR_getInt(attackindex, CHAR_Y));
        if (array != -1) {
          int enemyentrymax = ENCOUNT_getCreateEnemyMaxNumFromArray(array);
          if (enemyentrymax != -1) {
            int i;
            for (i = 0; i < ENCOUNT_GROUPMAXNUM; i++) {
              if (ENCOUNT_getGroupIdFromArray(array, i) != -1) {
                int itemid;
                int groupid = ENCOUNT_getGroupIdFromArray(array, i);
                extern int GROUP_getGroupArray(int);
                int g_array = GROUP_getGroupArray(groupid);
                itemid = GROUP_getInt(g_array, GROUP_APPEARBYITEMID);
                if (itemid != -1) {
                  int j;
                  for (j = 0; j < CHAR_getMaxInventory(attackindex); j++) {
                    int itemindex = CHAR_getItemIndex(attackindex, j);
                    if (ITEM_CHECKINDEX(itemindex)) {
      ...
      if (needItemId >= 0) {
        for (i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(attackindex);
             i++) {
          ...
          if (ITEM_getInt(itemindex, ITEM_ID) == needItemId) {
            {
              ...
              LogItem(CHAR_getChar(attackindex, CHAR_NAME),
                      CHAR_getChar(attackindex, CHAR_CDKEY), itemindex,
                      "使用料理捕获宠物", CHAR_getInt(attackindex, CHAR_FLOOR),
                      CHAR_getInt(attackindex, CHAR_X),
                      CHAR_getInt(attackindex, CHAR_Y),
                      ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                      ITEM_getChar(itemindex, ITEM_NAME),
...
BOOL BATTLE_EscapeCheck(int attackindex, int attackNo, int *pPar) {
  ...
  int mylevel, enemylevel = 0, enemycnt = 0, i, enemyindex, luck, escape_cnt,
               rare, SubNo;
  BATTLE_ENTRY *pEntry;
...
BOOL BATTLE_Escape(int battleindex, int attackNo, int flag) {
  ...
  if (flag == 0) {
    if (CHAR_getWorkInt(attackindex, CHAR_WORK_NOESCAPE)) {
      flg = 0;
      CHAR_talkToCli(attackindex, -1, "禁止逃跑", CHAR_COLORRED);
    }
  }
#endif
...
int BATTLE_PetOut(int battleindex, int attackNo) {
  ...
  if (iRet < 0) {
    // snprintf( szBuffer, sizeof(szBuffer),
    // "(%s)呼唤宠物,可是没有出现其踪迹。", 	CHAR_getUseName( attackindex )
    //);
...
int BATTLE_S_Roar(int battleindex, int attackNo, int defNo, int marray) {
  ...
  if (FINDPET == TRUE) { //若为年兽
    ...
    BATTLESTR_ADD(szWork);
    CHAR_talkToCli(CHAR_getWorkInt(charaindex, CHAR_WORKPLAYERINDEX), -1, buf4,
                   CHAR_COLORYELLOW);
  }
...
#ifdef BOSS_AI //魔王不会中状态
  if (CHAR_getWorkInt(defindex, CHAR_WORKTACTICS) == TACTICS_BOSS)
    return FALSE;
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
  if (CHAR_getWorkInt(defindex, CHAR_WORK_ABSOLUTE) > 0)
    return FALSE;
#endif
...
//职业技能状态攻击命中判定
int PROFESSION_BATTLE_StatusAttackCheck(int charaindex, int toindex, int status,
                                        int Success) {
  ...
  if (status == CHAR_WORKICECRACK || status == CHAR_WORKICECRACK2 ||
      status == CHAR_WORKICECRACK3 || status == CHAR_WORKICECRACK4 ||
      status == CHAR_WORKICECRACK5 || status == CHAR_WORKICECRACK6 ||
      status == CHAR_WORKICECRACK7 || status == CHAR_WORKICECRACK8 ||
      status == CHAR_WORKICECRACK9 || status == CHAR_WORKICECRACK10) {
    return 1;
  }
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
  if (status == BATTLE_ST_ABSOLUTE_DEFENSE) {
    for (i = 1; i < BATTLE_ST_END; i++)
      CHAR_setWorkInt(toindex, StatusTbl[i], 0);
    return 1;
  }
#endif
  for (i = 1; i < BATTLE_ST_END; i++) { //若已有异常状态则return
...
BOOL BATTLE_LostEscape(int battleindex, int attackNo) {
  ...
  LogPet(CHAR_getUseName(oyaindex), CHAR_getUseID(oyaindex),
         ...
         CHAR_getInt(attackindex, CHAR_LV), "战斗逃走回收",
         CHAR_getInt(oyaindex, CHAR_FLOOR), CHAR_getInt(oyaindex, CHAR_X),
         CHAR_getInt(oyaindex, CHAR_Y),
         CHAR_getChar(attackindex, CHAR_UNIQUECODE), attackindex, oyaindex);
...
void BATTLE_Steal(int battleindex, int attackNo, int defNo) {
  ...
  if (RAND(1, 100) < per) {
    ...
    if (RAND(1, 100) < 50) {
      ...
      stealmode = 1;
      for (j = 0, i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory(defindex);
           i++) {
      ...
      if (itemindex == -1) {
        ...
        CHAR_sendItemDataOne(defindex, itemtbl[i]);
        {
          LogItem(CHAR_getChar(defindex, CHAR_NAME),
                  CHAR_getChar(defindex, CHAR_CDKEY), itemindex, "偷窃的道具",
                  CHAR_getInt(defindex, CHAR_FLOOR),
                  CHAR_getInt(defindex, CHAR_X), CHAR_getInt(defindex, CHAR_Y),
                  ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                  ITEM_getChar(itemindex, ITEM_NAME),
                  ITEM_getInt(itemindex, ITEM_ID), defindex);
        }
...
int BATTLE_S_FallGround(int battleindex, int attackNo, int defNo,
  ...
  if (skill_type == BATTLE_COM_S_FALLRIDE && damage > 0 && react == 0
    ...
    int fallflg2 = 50;
    if (CHAR_GETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM3) > 0) {
      fallflg2 = CHAR_GETWORKINT_LOW(attackindex, CHAR_WORKBATTLECOM3);
      // print("落马几率%d",fallflg2);
    }
    if (
#ifdef _EQUIT_RESIST
        fallflg < fallflg2 + CHAR_getWorkInt(defindex, CHAR_WORKEQUITFALLRIDE)
#else
        fallflg < fallflg2
#endif
    ) {
      if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
        if (CHAR_getInt(defindex, CHAR_RIDEPET) >= 0) {
          flg |= BCF_FALL;
#ifdef _PETSKILL_RIDE
          fallflg = CHAR_getWorkInt(defindex, CHAR_WORK_RIDE);
    ...
    if (
      ...
      if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
        if (CHAR_getInt(defindex, CHAR_RIDEPET) >= 0) {
          ...
          BATTLE_changeRideImage(defindex);
#ifdef _PETSKILL_RIDE
          CHAR_setWorkInt(defindex, CHAR_WORKPETFALL, 5);
#else
          CHAR_setWorkInt(defindex, CHAR_WORKPETFALL, 1);
#endif
        }
...
void BATTLE_S_ToothCrushe(int battleindex, int attackindex, int defindex,
  ...
  if ((crushindex = BATTLE_ItemCrushCheck(defindex, 1)) >= 0) {
    ...
    if (crushenum <= 0) {
      ...
      LogItem(CHAR_getChar(defindex, CHAR_NAME),
              CHAR_getChar(defindex, CHAR_CDKEY), itemindex, "因过度损坏而消失",
      ...
      char buf2[256];
      sprintf(buf2, "%s是%s的。", ITEM_getChar(itemindex, ITEM_NAME),
              aszCrushTbl[level]);
      CHAR_talkToCli(defindex, -1, buf2, CHAR_COLORYELLOW);
      //更改道具说明
      //			buf1 = ITEM_getChar( itemindex,
...
int BATTLE_S_AttackDamage(int battleindex, int attackNo, int defNo,
  ...
  case BATTLE_COM_S_2TIMID: {
    ...
    if (rand() % 100 < timid && damage > 1) {
      if (CHAR_getInt(defindex, CHAR_WHICHTYPE) == CHAR_TYPEPET) {
        // print("%d",BATTLE_BattleUltimate(battleindex,defNo));
        // if (BATTLE_BattleUltimate(battleindex,defNo) == FALSE){
        int defkoyaindex = CHAR_getWorkInt(defindex, CHAR_WORKPLAYERINDEX);
        ...
        lssproto_KS_send(getfdFromCharaIndex(defkoyaindex),
...
int PETSKILL_SetDuckChange_Battle(int battleindex, int attackNo, int toNo,
  ...
  if (!CHAR_CHECKINDEX(charaindex))
    return FALSE;
  if (BATTLE_No2Index(battleindex, toNo) != charaindex)
    return FALSE;
...
int BATTLE_S_Barrier(int battleindex, int attackNo, int defNo, int marray) {
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    for (i = 0; ToList[i] != -1; i++) {
      ...
      if (BATTLE_StatusAttackCheck(charaindex, toindex, BATTLE_ST_BARRIER,
...
int BATTLE_S_Nocast(int battleindex, int attackNo, int defNo, int marray) {
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    for (i = 0; ToList[i] != -1; i++) {
      toindex = BATTLE_No2Index(battleindex, ToList[i]);
      if (BATTLE_StatusAttackCheck(charaindex, toindex, BATTLE_ST_NOCAST,
                                   Success, 30, 1.0, &perStatus) == TRUE &&
...
int battle_profession_attack_fun(int battleindex, int attackNo, int defNo,
  ...
  char *pszP = NULL;
  char *pszOption = NULL;
  int effect = 0, img1 = 0, img2 = 0, atk_num = 0, skill_level, cooltime = 0;
  int iDamage = 0; // 要传给client用的
  ...
  if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL)) >
          0 //?灵封印
      && CHAR_getInt(charaindex, PROFESSION_CLASS) == 2) {
    sprintf(szCommand, "BD|r%X|0|C|%X|", attackNo,
            PROFESSION_SKILL_getskillArray(skill));
    BATTLESTR_ADD(szCommand);
    return iRet;
  }
#endif
  ...
  if ((pszP = strstr(pszOption, "效%")) != NULL)
    sscanf(pszP + 3, "%d", &effect);
#ifdef _PRO3_ADDSKILL
  if ((pszP = strstr(pszOption, "冷%")) != NULL)
    sscanf(pszP + 3, "%d", &cooltime);
#endif
  // 准备期的动画
  ...
  case BATTLE_COM_S_CHAIN_ATK_2: // 双重攻击
  {
    ...
    // if( reflec_num > 0 )	CHAR_setWorkInt( defindex, CHAR_WORKDAMAGEREFLEC,
    // max(reflec_num - 1, 0)  );//让镜有效,这行要拿掉
    if (evanish_num > 0)
    ...
    // 增加攻击力
    atkpower = (int)(CHAR_getWorkInt(charaindex, CHAR_WORKFIXSTR) *
  ...
  case BATTLE_COM_S_DEAD_ATTACK: // 濒死攻击
  {
    ...
    CHAR_setWorkInt(charaindex, CHAR_WORKHITRIGHT,
                    CHAR_getWorkInt(charaindex, CHAR_WORKHITRIGHT) + hit);
  ...
  case BATTLE_COM_S_ATTACK_WEAK: // 弱点攻击
  {
    ...
    CHAR_setWorkInt(charaindex, CHAR_WORKQUICK,
                    CHAR_getWorkInt(charaindex, CHAR_WORKFIXDEX) * (dex / 100));
    sprintf(szCommand, "BD|r%X|0|6|%X|", attackNo, (-1) * (skill_level + 10));
    BATTLESTR_ADD(szCommand);

    break;
  }
  case BATTLE_COM_S_PLUNDER: // 铺迓佣
  {
  ...
  {
    if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_CRACKBONE2)) > 0) {
      sprintf(szBuffer, "裂骨断筋于 %d 回合后才可使用。",
              GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_CRACKBONE2)));
      CHAR_talkToCli(charaindex, -1, szBuffer, CHAR_COLORYELLOW);
      return iRet;
    }
    if ((pszP = strstr(pszOption, "DAM")) != NULL)
      sscanf(pszP + 3, "%d", &otherdamage);
    break;
  }
#endif
  default:
...
#ifdef _PRO3_ADDSKILL
  if (skill_type == BATTLE_COM_S_CRACK_BONE && damage > 0) { //裂骨断筋
    int ltemp;
    damage += otherdamage;
    ltemp = GETLOWVALUE(CHAR_getWorkInt(attackindex, CHAR_WORK_CRACKBONE2));
    CHAR_setWorkInt(attackindex, CHAR_WORK_CRACKBONE2,
                    MAKE2VALUE(cooltime, ltemp));
  }
#endif
  ...
  case BATTLE_COM_S_CHAOS: // 混乱攻击
  {
    ...
    // 减30%攻击力
    CHAR_setWorkInt(charaindex, CHAR_WORKATTACKPOWER,
                    CHAR_getWorkInt(charaindex, CHAR_WORKATTACKPOWER) * 70 /
                        100);
  ...
  case BATTLE_COM_S_CHAIN_ATK: // 连环攻击
  {
    ...
    if (atk_num == 2 && (CHAR_getInt(charaindex, CHAR_HP) > 0) &&
        (CHAR_getInt(defindex, CHAR_HP) > 0)) {
      BATTLE_Attack(battleindex, attackNo, defNo);
    }
  ...
  //	(attackNo >= 10)? CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
#ifdef _PRO3_ADDSKILL
  if (skill_type == BATTLE_COM_S_CRACK_BONE &&
      CHAR_getInt(defindex, CHAR_HP) > 0 && iDamage > 0) { //裂骨断筋
    int str = 0, turn = 0, per = 0;
    int htemp, ltemp;
    if ((pszP = strstr(pszOption, "STR")) != NULL)
      sscanf(pszP + 3, "%d", &str);
    if ((pszP = strstr(pszOption, "回%")) != NULL)
      sscanf(pszP + 3, "%d", &turn);
    if ((pszP = strstr(pszOption, "避%")) != NULL)
      sscanf(pszP + 3, "%d", &per);
    htemp = GETHIGHVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_CRACKBONE));
    ltemp = GETLOWVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_CRACKBONE));
    str = abs(str);
    if (htemp > 0)
      turn = htemp;
    sprintf(szCommand, "BD|r%X|0|D|%X|", defNo, str); //降攻
    BATTLESTR_ADD(szCommand);
    str += ltemp;
    if (str > 300)
      str = 300;
    CHAR_setWorkInt(defindex, CHAR_WORK_CRACKBONE, MAKE2VALUE(turn + 1, str));
    htemp = GETHIGHVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_CRACKBONE2));
    ltemp = GETLOWVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_CRACKBONE2));
    sprintf(szCommand, "BD|r%X|0|3|%X|", defNo, per);
    BATTLESTR_ADD(szCommand);
    ltemp += per;
    if (ltemp < -40)
      ltemp = -40;
    CHAR_setWorkInt(defindex, CHAR_WORK_CRACKBONE2, MAKE2VALUE(htemp, ltemp));
  }
#endif
  i = defNo - DefSide * SIDE_OFFSET;
  if (ultimate > 0) {
...
int battle_profession_attack_magic_fun(int battleindex, int attackNo, int defNo,
  ...
  if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL)) >
      ...
      && CHAR_getInt(charaindex, PROFESSION_CLASS) == 2) {
    char szBuffer[64] = "";
    sprintf(szBuffer, "BD|r%X|0|C|%X|", attackNo,
            PROFESSION_SKILL_getskillArray(skill));
    BATTLESTR_ADD(szBuffer);
    return FALSE;
  }
...
#ifdef BOSS_AI
    if (BattleArray[battleindex]
            .Side[CHAR_getWorkInt(charaindex, CHAR_WORKBATTLESIDE) ^ 1]
            .type == BATTLE_S_TYPE_ENEMY) {
      int isearch, bossindex, npcside;
      npcside = CHAR_getWorkInt(charaindex, CHAR_WORKBATTLESIDE) ^ 1;
      for (isearch = 0; isearch < BATTLE_ENTRY_MAX;
           isearch++) { //找看看有没有boss
        bossindex =
            BattleArray[battleindex].Side[npcside].Entry[isearch].charaindex;
        if (CHAR_getWorkInt(bossindex, CHAR_WORKTACTICS) == TACTICS_BOSS) {
          print("\n boss ai 使用移行换位");
          BattleArray[battleindex]
              .Side[npcside ^ 1]
              .Entry[attackNo]
              .proskilluse = 21;
        }
...
int battle_profession_status_chang_fun(int battleindex, int attackNo, int defNo,
  ...
  int flg = 0, DefSide = 0, ultimate = 0;
  int ReactType = 0, effect = 0, needitem = -1, itemindex, cooltime;
  ...
  if ((pszP = strstr(pszOption, "冷%")) != NULL)
    sscanf(pszP + 3, "%d", &cooltime);
  if ((pszP = strstr(pszOption, "道")) != NULL)
    sscanf(pszP + 2, "%d", &needitem); //消耗道具
#endif
  // 目标地球一周，不动作
  if (defNo >= 0 && defNo <= 19) {
    defindex = BATTLE_No2Index(battleindex, defNo);
...
#endif
  {
...
#ifdef _PRO_ABSOLUTE_DEFENSE
    toindex = BATTLE_No2Index(battleindex, defNo2);
    if (skill_type == BATTLE_COM_S_ABSOLUTE_DEFENSE) {
      int itemok = -1;
      if (GETHIGHVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_ABSOLUTE2)) > 0) {
        sprintf(szCommand, "绝对防御于 %d 回合后才可使用。",
                GETHIGHVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_ABSOLUTE2)));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    }
#endif
#ifdef _PRO3_ADDSKILL
    if (skill_type == BATTLE_COM_S_PACIFY) { //抚慰心灵
      if (GETFIRSTVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_PACIFY)) > 0) {
        sprintf(szCommand, "抚慰心灵于 %d 回合后才可使用。",
                GETFIRSTVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_PACIFY)));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    } else if (skill_type == BATTLE_COM_S_DARK_SEAL) { //?灵封印
      if (GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL)) > 0) {
        sprintf(szCommand, "?灵封印于 %d 回合后才可使用。",
                GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL)));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    } else if (skill_type == BATTLE_COM_S_DEMON_SPEECH) { //恶魔诅咒
      if (GETTHIRDVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH)) >
          0) {
        sprintf(
            szCommand, "恶魔诅咒于 %d 回合后才可使用。",
            GETTHIRDVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH)));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    } else if (skill_type == BATTLE_COM_S_FIRST_AID) { //战场急救
      if (CHAR_getWorkInt(charaindex, CHAR_WORK_FIRST_AID) > 0) {
        sprintf(szCommand, "战场急救于 %d 回合后才可使用。",
                CHAR_getWorkInt(charaindex, CHAR_WORK_FIRST_AID));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    } else if (skill_type == BATTLE_COM_S_PROAR) { //战狼怒吼
      if (GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ROAR)) > 0) {
        sprintf(szCommand, "战狼怒吼于 %d 回合后才可使用。",
                GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ROAR)));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    } else if (skill_type == BATTLE_COM_S_MEDICATION) { //制药学
      if (CHAR_getWorkInt(charaindex, CHAR_WORK_MEDICATION) > 0) {
        sprintf(szCommand, "制药学于 %d 回合后才可使用。",
                CHAR_getWorkInt(charaindex, CHAR_WORK_MEDICATION));
        CHAR_talkToCli(charaindex, -1, szCommand, CHAR_COLORYELLOW);
        return iRet;
      }
    }
...
#endif
#ifdef _PRO_KILLME
        || ((pszP = strstr(pszOption, "怒")) != NULL)
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
        || ((pszP = strstr(pszOption, "绝")) != NULL)
#endif
    ) {
      for (i = 1; i < BATTLE_ST_END; i++) {
        if (strncmp(pszP, aszStatus[i], 2) == 0) {
...
#ifdef _PRO_KILLME
    if (skill_type == BATTLE_COM_S_KILLME)
      Success *= 0.7;
#endif
    if (skill_type == BATTLE_COM_S_DRAGNET
#ifdef _PRO3_ADDSKILL
        || skill_type == BATTLE_COM_S_POISON_DRAGNET
#endif
    ) {
    ...
    case BATTLE_COM_S_RESIST_FIRE:
      CHAR_talkToCli(charaindex, -1, "火抗性提升", CHAR_COLORYELLOW);
      break;
    case BATTLE_COM_S_RESIST_ICE:
      CHAR_talkToCli(charaindex, -1, "冰抗性提升", CHAR_COLORYELLOW);
      break;
    case BATTLE_COM_S_RESIST_THUNDER:
      CHAR_talkToCli(charaindex, -1, "雷抗性提升", CHAR_COLORYELLOW);
      break;
#ifdef _PROFESSION_ADDSKILL
    case BATTLE_COM_S_RESIST_F_I_T:
      CHAR_talkToCli(charaindex, -1, "火冰雷抗性提升", CHAR_COLORYELLOW);
      break;
#endif
    }
    ...
    for (i = 0; ToList[i] != -1; i++) {
      ...
      if (skill_type == BATTLE_COM_S_INSTIGATE) {
        if (CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
          CHAR_talkToCli(charaindex, -1, "不可施于人物身上", CHAR_COLORYELLOW);
          break;
        }
      }
#ifdef _PRO3_ADDSKILL
      if (skill_type == BATTLE_COM_S_DRAGNET &&
          GETTHIRDVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_POISON_DRAGNET)) >
              0) {
        ;
        ;
      } else if (skill_type == BATTLE_COM_S_POISON_DRAGNET &&
                 (rand() % 100 < Success) //毒素之网
                 && CHAR_getWorkInt(toindex, CHAR_WORKDRAGNET) <= 0) {
        if (GETTHIRDVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_POISON_DRAGNET)) ==
            0) {
          int poisontemp = 10;
          if ((pszP = strstr(pszOption, "素%")) != NULL)
            sscanf(pszP + 3, "%d", &poisontemp);
          sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 12);
          BATTLESTR_ADD(szCommand);
          CHAR_setWorkInt(toindex, CHAR_WORK_POISON_DRAGNET,
                          MAKE2VALUE(turn + 1, poisontemp));
          BATTLE_BadStatusString(ToList[i], BATTLE_ST_POISON_DRAGNET);
#ifdef _PROFESSION_ADDSKILL
      ...
      } else if (skill_type == BATTLE_COM_S_PACIFY) { //抚慰心灵
        int pacifytemp = CHAR_getWorkInt(toindex, CHAR_WORK_PACIFY);
        int pacifyl1 = GETFOURTHVALUE(pacifytemp);
        int pacifyl2 = GETTHIRDVALUE(pacifytemp);
        int pacifyr1 = GETSECONDVALUE(pacifytemp);
        int pacifyr2 = GETFIRSTVALUE(pacifytemp);
        int pacifytemp2 = 5;
        if (CHAR_getInt(toindex, CHAR_WHICHTYPE) != CHAR_TYPEPET) {
          CHAR_talkToCli(charaindex, -1, "目标不是宠物!", CHAR_COLORYELLOW);
          return iRet;
        }
        if ((pszP = strstr(pszOption, "忠%")) != NULL)
          sscanf(pszP + 3, "%d", &pacifytemp2);
        sprintf(szCommand, "BD|r%X|0|B|%X|", ToList[i], pacifytemp2);
        BATTLESTR_ADD(szCommand);
        // if( pacifyl2 == 0 )
        CHAR_setWorkInt(toindex, CHAR_WORK_PACIFY,
                        MAKE4VALUE(pacifyl1, turn + 1, pacifytemp2, pacifyr2));
        // else
        //	CHAR_setWorkInt( toindex, CHAR_WORK_PACIFY,
        //MAKE4VALUE(pacifyl1,pacifyl2,pacifyr1+pacifytemp2,pacifyr2) );
        BATTLE_BadStatusString(ToList[i], BATTLE_ST_PACIFY);
        pacifytemp = CHAR_getWorkInt(charaindex, CHAR_WORK_PACIFY);
        pacifyl1 = GETFOURTHVALUE(pacifytemp);
        pacifyl2 = GETTHIRDVALUE(pacifytemp);
        pacifyr1 = GETSECONDVALUE(pacifytemp);
        CHAR_setWorkInt(charaindex, CHAR_WORK_PACIFY,
                        MAKE4VALUE(pacifyl1, pacifyl2, pacifyr1, cooltime));
      } else if (skill_type == BATTLE_COM_S_DARK_SEAL) { //?灵封印
        int darksealtemp = CHAR_getWorkInt(toindex, CHAR_WORK_DARK_SEAL);
        int darkseal = GETLOWVALUE(darksealtemp);
        if (CHAR_getInt(toindex, PROFESSION_CLASS) != PROFESSION_CLASS_WIZARD) {
          CHAR_talkToCli(charaindex, -1, "目标不是?灵法师!", CHAR_COLORYELLOW);
          return iRet;
        }
        CHAR_setWorkInt(toindex, CHAR_WORK_DARK_SEAL,
                        MAKE2VALUE(turn + 1, darkseal));
        darksealtemp = CHAR_getWorkInt(charaindex, CHAR_WORK_DARK_SEAL);
        darkseal = GETHIGHVALUE(darksealtemp);
        CHAR_setWorkInt(charaindex, CHAR_WORK_DARK_SEAL,
                        MAKE2VALUE(darkseal, cooltime));
        sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 15);
        BATTLESTR_ADD(szCommand);
      } else if (skill_type == BATTLE_COM_S_RESTATE) {
        if (strstr(pszOption, "darkseal")) { //斗气导引
          int darkseal;
          if (CHAR_getInt(toindex, PROFESSION_CLASS) !=
              PROFESSION_CLASS_WIZARD) {
            CHAR_talkToCli(charaindex, -1, "目标不是?灵法师!",
                           CHAR_COLORYELLOW);
            return iRet;
          }
          if ((pszP = strstr(pszOption, "率%")) != NULL)
            sscanf(pszP + 3, "%d", &darkseal);
          if (rand() % 100 > darkseal) {
            CHAR_talkToCli(charaindex, -1, "斗气导引失败!", CHAR_COLORYELLOW);
            return iRet;
          }
          darkseal = GETLOWVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_DARK_SEAL));
          CHAR_setWorkInt(toindex, CHAR_WORK_DARK_SEAL,
                          MAKE2VALUE(0, darkseal));
          sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 16);
          BATTLESTR_ADD(szCommand);
        } else if (strstr(pszOption, "demon_speech")) { //神灵之赐
          int dtemp = CHAR_getWorkInt(toindex, CHAR_WORK_DEMON_SPEECH);
          // int h1=GETFOURTHVALUE(CHAR_getWorkInt( toindex,
          // CHAR_WORK_DEMON_SPEECH ));
          int h2 =
              GETTHIRDVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_DEMON_SPEECH));
          // int l1=GETSECONDVALUE(CHAR_getWorkInt( toindex,
          // CHAR_WORK_DEMON_SPEECH )); int l2=GETFIRSTVALUE(CHAR_getWorkInt(
          // toindex, CHAR_WORK_DEMON_SPEECH ));
          if ((pszP = strstr(pszOption, "率%")) != NULL)
            sscanf(pszP + 3, "%d", &dtemp);
          if (rand() % 100 > dtemp) {
            CHAR_talkToCli(charaindex, -1, "神灵之赐失败!", CHAR_COLORYELLOW);
            return iRet;
          }
          CHAR_setWorkInt(toindex, CHAR_WORK_DEMON_SPEECH,
                          MAKE4VALUE(0, h2, 0, 0));
          sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 18);
          BATTLESTR_ADD(szCommand);
          BATTLE_BadStatusString(ToList[i], 0);
        }
      } else if (skill_type == BATTLE_COM_S_DEMON_SPEECH) { //恶魔诅咒
        // int dtemp = CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH );
        int h1 =
            GETFOURTHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH));
        int h2 =
            GETTHIRDVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH));
        int l1 =
            GETSECONDVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH));
        int l2 =
            GETFIRSTVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH));
        if (Absolute_Defense_Check(toindex) == TRUE)
          return iRet;
        CHAR_setWorkInt(charaindex, CHAR_WORK_DEMON_SPEECH,
                        MAKE4VALUE(h1, cooltime, l1, l2));

        if ((pszP = strstr(pszOption, "HP%")) != NULL)
          sscanf(pszP + 3, "%d", &l1);
        if ((pszP = strstr(pszOption, "MP%")) != NULL)
          sscanf(pszP + 3, "%d", &l2);
        h1 = GETFOURTHVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_DEMON_SPEECH));
        h2 = GETTHIRDVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_DEMON_SPEECH));
        CHAR_setWorkInt(toindex, CHAR_WORK_DEMON_SPEECH,
                        MAKE4VALUE(turn + 1, h2, l1, l2));
        sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 17);
        BATTLESTR_ADD(szCommand);
        BATTLE_BadStatusString(ToList[i], BATTLE_ST_DEMON_SPEECH);
      } else if (skill_type == BATTLE_COM_S_FIRST_AID) { //战场急救
        int addhp = 10, addhppet;
        int ridepet = BATTLE_getRidePet(toindex);
        if (CHAR_getInt(toindex, CHAR_HP) <= 0)
          return iRet;
        if ((pszP = strstr(pszOption, "血%")) != NULL)
          sscanf(pszP + 3, "%d", &addhp);
        addhppet = addhp;
        addhp = CHAR_getWorkInt(toindex, CHAR_WORKMAXHP) * (addhp * 0.01);
        if (ridepet == -1) {
          CHAR_setInt(toindex, CHAR_HP,
                      min(addhp + CHAR_getInt(toindex, CHAR_HP),
                          CHAR_getWorkInt(toindex, CHAR_WORKMAXHP)));
          ...
          CHAR_setInt(toindex, CHAR_HP,
                      min(addhp + CHAR_getInt(toindex, CHAR_HP),
                          CHAR_getWorkInt(toindex, CHAR_WORKMAXHP)));
          addhppet =
              CHAR_getWorkInt(ridepet, CHAR_WORKMAXHP) * (addhppet * 0.01);
          CHAR_setInt(ridepet, CHAR_HP,
                      min(addhppet + CHAR_getInt(ridepet, CHAR_HP),
                          CHAR_getWorkInt(ridepet, CHAR_WORKMAXHP)));
        }
        CHAR_setWorkInt(charaindex, CHAR_WORK_FIRST_AID, cooltime);
        snprintf(szCommand, sizeof(szCommand), "BD|r%X|%X|%X|d%X|p%X|",
                 ToList[i], BD_KIND_HP, 1, addhp, ridepet ? addhppet : 0);
        BATTLESTR_ADD(szCommand);
      } else if (skill_type == BATTLE_COM_S_PROAR) { //战狼怒吼
        int delhp, delmp;
        int old_hp = CHAR_getWorkInt(toindex, CHAR_WORKMAXHP);
        int old_mp = CHAR_getWorkInt(toindex, CHAR_WORKMAXMP);
        int ridepet = BATTLE_getRidePet(toindex);
        if ((pszP = strstr(pszOption, "HP%")) != NULL)
          sscanf(pszP + 3, "%d", &delhp);
        if ((pszP = strstr(pszOption, "MP%")) != NULL)
          sscanf(pszP + 3, "%d", &delmp);
        //减 MP
        if (CHAR_getInt(toindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
          delmp = old_mp * (delmp * 0.01);
          if (CHAR_getInt(toindex, CHAR_MP) - delmp < 0) {
            delmp = CHAR_getInt(toindex, CHAR_MP);
            old_mp = 0;
          } else {
            old_mp = CHAR_getInt(toindex, CHAR_MP) - delmp;
          }
          sprintf(szCommand, "BD|r%X|1|0|m%X|p%X|", ToList[i], delmp, 0);
          CHAR_setInt(toindex, CHAR_MP, old_mp);
          BATTLESTR_ADD(szCommand);
        }
        //减 HP
        if (ridepet == -1) {
          delhp = old_hp * (delhp * 0.01);
          old_hp = CHAR_getInt(toindex, CHAR_HP) - delhp;
          old_hp = old_hp < 0 ? 0 : old_hp;
          CHAR_setInt(toindex, CHAR_HP, old_hp);
          sprintf(szCommand, "BD|r%X|0|0|%X|", ToList[i], delhp);
        } else {
          int pet_old_hp = CHAR_getInt(ridepet, CHAR_HP);
          int petdelhp = pet_old_hp * (delhp * 0.01);
          delhp = old_hp * (delhp * 0.01);
          old_hp = CHAR_getInt(toindex, CHAR_HP) - delhp;
          old_hp = old_hp < 0 ? 0 : old_hp;
          CHAR_setInt(toindex, CHAR_HP, old_hp);
          pet_old_hp -= petdelhp;
          pet_old_hp = pet_old_hp < 0 ? 0 : pet_old_hp;
          CHAR_setInt(ridepet, CHAR_HP, pet_old_hp);
          if (pet_old_hp <= 0) {
          ...
          sprintf(szCommand, "BD|r%X|0|0|%X|p%X|", ToList[i], delhp, petdelhp);
        }
        BATTLESTR_ADD(szCommand);
        delhp = 0;
        if ((pszP = strstr(pszOption, "率%")) != NULL)
          sscanf(pszP + 3, "%d", &delhp);
        if (rand() % 100 < delhp && old_hp > 0) {
          if ((pszP = strstr(pszOption, "惊%")) != NULL)
            sscanf(pszP + 3, "%d", &delmp);
          delhp = GETHIGHVALUE(CHAR_getWorkInt(toindex, CHAR_WORK_ROAR));
          CHAR_setWorkInt(charaindex, CHAR_WORK_ROAR, MAKE2VALUE(delhp, delmp));
          sprintf(szCommand, "BD|r%X|0|A|%X|", ToList[i], 19);
        }
        if (old_hp <= 0) {
          CHAR_setFlg(toindex, CHAR_ISDIE, 1);
          CHAR_setWorkInt(toindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE);
          BATTLESTR_ADD("FF|");
        }
        delhp = GETLOWVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_ROAR));
        CHAR_setWorkInt(charaindex, CHAR_WORK_ROAR,
                        MAKE2VALUE(cooltime, delhp));
      } else if (skill_type == BATTLE_COM_S_MEDICATION) { //制药学
        int eitemno;
        if ((pszP = strstr(pszOption, "药%")) != NULL)
          sscanf(pszP + 3, "%d", &eitemno);
        for (j = CHAR_STARTITEMARRAY; j < CHAR_getMaxInventory(charaindex);
             j++) {
          itemindex = CHAR_getItemIndex(charaindex, j);
          if (itemindex == -1)
            break;
          else if (eitemno == ITEM_getInt(itemindex, ITEM_ID)) {
            CHAR_talkToCli(charaindex, -1, "有相同的药品存在!",
                           CHAR_COLORYELLOW);
            return iRet;
          }
        }
        if (j == CHAR_getMaxInventory(charaindex)) {
          CHAR_talkToCli(charaindex, -1, "道具栏没有空间!", CHAR_COLORYELLOW);
          return iRet;
        }
        itemindex = ITEM_makeItemAndRegist(eitemno); //给道具
        if (itemindex != -1) {
          int rc = CHAR_addItemSpecificItemIndex(charaindex, itemindex);
          if (rc >= 0 && rc < CHAR_getMaxInventory(charaindex)) {
            CHAR_sendItemDataOne(charaindex, rc);
            LogItem(CHAR_getChar(charaindex, CHAR_NAME),
                    CHAR_getChar(charaindex, CHAR_CDKEY), itemindex,
                    "制药学的道具", CHAR_getInt(charaindex, CHAR_FLOOR),
                    CHAR_getInt(charaindex, CHAR_X),
                    CHAR_getInt(charaindex, CHAR_Y),
                    ITEM_getChar(itemindex, ITEM_UNIQUECODE),
                    ITEM_getChar(itemindex, ITEM_NAME),
                    ITEM_getInt(itemindex, ITEM_ID), charaindex);
            CHAR_setWorkInt(charaindex, CHAR_WORK_MEDICATION, cooltime);
            CHAR_talkToCli(charaindex, -1, "制药完成!", CHAR_COLORYELLOW);
          }
        }
      } else
#endif
        // 状态命中检定
        if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, toindex, status,
                                                 Success) == 0) ||
            (CHAR_getInt(toindex, CHAR_HP) <= 0)) {

          // snprintf( szBuffer, sizeof(szBuffer),
          // "(%s)无法将(%s)做成(%s)(%d%%)", 	CHAR_getUseName( charaindex ),
...
#ifdef _PRO_KILLME
            if (StatusTbl[status] == CHAR_WORKANGER)
              CHAR_setWorkInt(toindex, CHAR_WORKANGER_TARGET, charaindex);
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
            if (StatusTbl[status] == CHAR_WORK_ABSOLUTE) {
              old_dex = 300;
              if ((pszP = strstr(pszOption, "减%")) != NULL)
                sscanf(pszP + 3, "%d", &old_dex);
              CHAR_setWorkInt(toindex, CHAR_WORK_ABSOLUTE2,
                              MAKE2VALUE(cooltime, old_dex));
            }
#endif
          }
          ...
          if (skill_type == BATTLE_COM_S_FIRE_ENCLOSE) { // 火附体
            ...
            // 加雷熟练度经验值
            PROFESSION_SKILL_LVEVEL_UP(charaindex,
  ...
  case BATTLE_COM_S_SHIELD_ATTACK: // 盾击
  {
    ...
    if ((itmid < 0) || ITEM_WSHIELD != ITEM_getInt(itmid, ITEM_TYPE)) {
      CHAR_talkToCli(charaindex, -1, "未装备盾牌，无法使用此技能",
                     CHAR_COLORYELLOW);
      iRet = FALSE;
      break;
    }
    ...
    if (skill_level != 10)
      CHAR_setWorkInt(
          ...
          (int)(CHAR_getWorkInt(charaindex, CHAR_WORKATTACKPOWER) * 0.5));
...
#ifdef BOSS_AI
    if (BattleArray[battleindex].Side[DefSide].type == BATTLE_S_TYPE_ENEMY) {
      int isearch, bossindex;
      for (isearch = 0; isearch < BATTLE_ENTRY_MAX;
           isearch++) { //找看看有没有boss
        bossindex =
            BattleArray[battleindex].Side[DefSide].Entry[isearch].charaindex;
        if (CHAR_getWorkInt(bossindex, CHAR_WORKTACTICS) == TACTICS_BOSS) {
          print("\n boss ai 使用盾击");
          BattleArray[battleindex]
              .Side[DefSide ^ 1]
    ...
    for (i = 0; ToList[i] != -1; i++) {
      ...
      //取得攻击後状态
      iWork = BATTLE_AttackSeq(charaindex, defindex, &damage, &Guardian,
                               skill_type);

      ultimate =
          BATTLE_DamageSub(charaindex, defindex, &damage, &petdamage, &react);
      ...
      iRet = BATTLE_DefineAttack(charaindex, defindex, iWork, szBuffer, damage,
                                 react, &flg);
      ...
      if (hit == 1) {
        if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, defindex, status,
                                                 Success) == 0) ||
            (CHAR_getInt(defindex, CHAR_HP) <= 0)) {

          // snprintf( szBuffer, sizeof(szBuffer),
          // "(%s)无法将(%s)做成(%s)(%d%%)", 	CHAR_getUseName( charaindex ),
          //CHAR_getUseName( defindex ), aszStatusFull[status], perStatus );
  ...
  case BATTLE_COM_S_TOXIN_WEAPON: // 毒素武器
  {
...

    {
      ...
      if (CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) > 0 ||
          CHAR_getWorkInt(charaindex, CHAR_BECOMEPIG) > 0)
        bChange = TRUE;
#else
      if (CHAR_getWorkInt(charaindex, CHAR_WORKFOXROUND) > 0)
        bChange = TRUE;
      ...
      for (i = 0; ToList[i] != -1; i++) {
        ...
        iRet = BATTLE_DefineAttack(charaindex, defindex, iWork, szBuffer,
                                   damage, react, &flg);

        if (damage > 0) {
          ultimate = BATTLE_DamageSub(charaindex, defindex, &damage, &petdamage,
                                      &react);
  ...
  case BATTLE_COM_S_BOUNDARY: // 四属性结界
  {
    ...
    analysis_profession_parameter(
        2, CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM3), defNo2,
        charaindex);
...
// 火、冰、雷附体附加攻击
int BATTLE_PROFESSION_RANG_ATTACK_DAMAGE(int charaindex, int attackNo,
  ...
  if (img1 != 0) {
    if ((PROFESSION_BATTLE_StatusAttackCheck(charaindex, defindex, status,
      ...
      //					CHAR_getUseName( charaindex ),
      //CHAR_getUseName( defindex ), aszStatusFull[status], perStatus );
      // BATTLE_BroadCast( battleindex, szBuffer,(attackNo >= 10)?
      // CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
    } else {
      CHAR_setWorkInt(defindex, StatusTbl[status], turn + 1);
...
void BATTLE_BatFly(int battleindex, int attackNo, int myside) {
  ...
  if ((CHAR_getInt(charaindex, CHAR_HP) + addhp) >
      CHAR_getWorkInt(charaindex, CHAR_WORKMAXHP)) {
    CHAR_setInt(charaindex, CHAR_HP,
                CHAR_getWorkInt(charaindex, CHAR_WORKMAXHP));
    addhp = 0;
  } else
    CHAR_setInt(charaindex, CHAR_HP, CHAR_getInt(charaindex, CHAR_HP) + addhp);
...
void BATTLE_BattleModel_ATTACK(int battleindex, int charaindex,
  ...
  if (CHAR_getWorkInt(iDefindex, CHAR_WORKBATTLECOM1) ==
      BATTLE_COM_S_EARTHROUND0) {
    print("\n[宠技特殊处理]目标死亡或不存在或地球一周了\n");
    return;
  }
  ...
  // 设定 CHAR_NPCWORKINT1 为 BATTLE_COM_S_BATTLE_MODEL 在 BATTLE_DamageSub
  // 会用到
  ...
  if (!(iDefState == BATTLE_RET_DODGE)) {
    ...
    // 物理攻击
    if (iType & 0x00000004)
      iUltimate = BATTLE_DamageSub(charaindex, iDefindex, &iDamage, &iPetDamage,
                                   &iTemp);
    // 非物理攻击
    else {
      iTemp = -1;
  ...
  if (CHAR_getInt(iDefindex, CHAR_HP) <= 0) {
    ...
    if (BATTLE_ItemCrushSeq(iDefindex) == TRUE)
      ...
      iFlg |= BCF_CRUSH;
    // 状态命中判定
    if (iDamage > 0 && BATTLE_StatusAttackCheck(charaindex, iDefindex, iEffect,
      ...
      if (iGuardian >= 0)
        BATTLE_BadStatusString(iGuardian,
                               iEffect); // iGuardian 大於等於 0
                                         // 表示有目标使用了忠犬,该目标状态改变
      else
        BATTLE_BadStatusString(pAAttackObject->target,
                               iEffect); // 否则原目标状态改变
    }
  }
...
void BATTLE_BattleModel(int battleindex, int attackNo, int myside) {
  int iToList[SIDE_OFFSET * 2 + 1], i, i0, i1, i2,
      iEffect = -1, iTurn = -1, iEffectHit = 0, iType, iObjectNum, charaindex;
  ...
  pszOption = PETSKILL_getChar(CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM3),
                               PETSKILL_OPTION);

  if (pszOption == "\0") {
    printf("BATTLE_BattleModel: read PETSKILL_OPTION "
           "error!!(array:%d,file:%s,line:%d)\n",
           CHAR_getWorkInt(charaindex, CHAR_WORKBATTLECOM3), __FILE__,
           __LINE__);
    return;
  }
  ...
  iObjectNum =
      CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM2); // 取得攻击物件数量
  ...
  if (iObjectNum >= i) {
    ...
    for (i = 0; i < iObjectNum; i++) {
      ...
      if (AAttackObject[i].index != -1) {
        ...
        BATTLE_BattleModel_ATTACK(battleindex, charaindex, &AAttackObject[i],
  ...
  // 当攻击物件数量小於在场目标数量，则以攻击类型决定攻击方式
    ...
    i0 = i; // 记录场上目标只数
    // 型态 2
    // :每个攻击物件打完该攻击目标後便不再攻击其它目标,每个目标只会受一次攻击
    ...
    // 型态 1
    // :每个目标都要打到,会有一个攻击物件打二个目标以上的情形,所以若为型态 1
    // 的话,除了型态 2 的攻击处理要执外,要再作以下处理
    if (iType & 0x00000001) {
      // 处理还没有被打的目标
      ...
      for (i1 = 0; i1 < i0 - iObjectNum; i1++) { // 计算剩几个目标没打
        for (i2 = 0; i2 < iObjectNum; i2++) {    // 攻击物件计数
          ...
          if (AAttackObject[i2].index != -1) {
            ...
            BATTLE_BattleModel_ATTACK(battleindex, charaindex,
