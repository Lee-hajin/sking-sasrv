...
int NowBattlerFd;
#if 1
int BATTLE_MpDown(int charaindex, int down) { return 0; }
#else
{
  int mp, battleindex;
  // 爵  反覆谛爵井｝???爵井
...
void BattleCommandDispach(int fd, char *command) {
  ...
  if (strncmp(command, "U", 1) == 0) {
    ...
    //charaindex ) ); sprintf( szBuffer, "Command(%s)(待机)", CHAR_getUseName(
    // charaindex ) );
    endFlg = 1;
    ...
    //) { 		print("\n 改封包!叫出非待机宠!!:%s ", CHAR_getChar( charaindex,
    //CHAR_CDKEY) ); 		iNum = -1;
    //	}
    // }
#endif
    ...
    if (!ITEM_CHECKINDEX(itemindex))
      valid = -1;
    if (valid >= 0)
      valid = ITEM_isTargetValid(charaindex, itemindex, ToNo);
    if (valid < 0) {
      CHAR_setWorkInt(charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_WAIT);
      CHAR_setWorkInt(charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
    } else {
...
//CHAR_getChar(charaindex, CHAR_CDKEY), CHAR_getChar(charaindex, CHAR_NAME) );
...
#ifndef _PROSKILL_OPTIMUM // Robin fix cancel 此处略过职业检查, 改在
                          // PROFESSION_SKILL_Use 中检查
      // 人物的职业
      ...
      if (1) {
        ...
        if (PROFESSION_SKILL_Use(charaindex, iNum, ToNo, NULL) == 1) {
          // sprintf( szBuffer, "Command(%s)(职业技能)", CHAR_getUseName(
          // charaindex ) );
          endFlg = 1;
        } else {
          //				print("\n职业技能失败!!\n");
  ...
  if (endFlg) {
    ...
    /*
                    if( endFlg==1 ){
                            int fd;
                            if( CHAR_getInt( charaindex,
       CHAR_WHICHTYPE)==CHAR_TYPEPLAYER ){ fd = CHAR_getWorkInt( charaindex,
...
//********************************************************
{
  if (CHAR_CHECKINDEX(charaindex) == FALSE)
    return FALSE;
  if (getfdFromCharaIndex(charaindex) < 0)
    return FALSE;
  ...
  char tmBuff2[256] = "";
  for (i = 0; i < 20; i++) {
    tmindex = BATTLE_No2Index(battleindex, i);
    if (CHAR_CHECKINDEX(tmindex)) {
      if (CHAR_getInt(tmindex, CHAR_NB_HALO) > 0) {
        sprintf(tmBuff2, "%d|%d|", i, CHAR_getInt(tmindex, CHAR_NB_HALO));
        strcat(tmBuff1, tmBuff2);
      }
    }
  }
  lssproto_B_send(getfdFromCharaIndex(charaindex), tmBuff1);
#endif
  return TRUE;
}
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
      ...
      if (CHAR_getFlg(charaindex, CHAR_ISDIE) == TRUE) {
        flg |= BC_FLG_DEAD;
      } else if (CHAR_getWorkInt(charaindex, CHAR_WORKPOISON) > 0) {
        flg |= BC_FLG_POISON;
      } else
        ...
        if (CHAR_getWorkInt(charaindex, CHAR_WORKWEAKEN) > 0) {
          ...
          if (CHAR_getWorkInt(charaindex, CHAR_WORKDEEPPOISON) > 0) {
            ...
            if (CHAR_getWorkInt(charaindex, CHAR_WORKNOCAST) > 0) {
              ...
              } else if (CHAR_getWorkInt(charaindex, CHAR_WORKSLEEP) > 0) {
                flg |= BC_FLG_SLEEP;
              } else if (CHAR_getWorkInt(charaindex, CHAR_WORKSTONE) > 0) {
                flg |= BC_FLG_STONE;
              } else if (CHAR_getWorkInt(charaindex, CHAR_WORKDRUNK) > 0) {
                flg |= BC_FLG_DRUNK;
              } else if (CHAR_getWorkInt(charaindex, CHAR_WORKCONFUSION) > 0) {
...
#endif
      makeEscapeString(CHAR_getUseName(charaindex), szEscapeName,
                       sizeof(szEscapeName));
      makeEscapeString(BATTLE_CharTitle(charaindex), szEscapeTitle,
      ...
      if (petindex != -1) {
        ...
        if (CHAR_getInt(charaindex, CHAR_RIDENAMEFLG) == 1)
          makeEscapeString(CHAR_getChar(petindex, CHAR_NAME), szEscapePetName,
...
#ifdef _PETSKILL_RIDE // 若中了爆裂攻击而落马时将讯息传给 Client
          if (CHAR_getWorkInt(charaindex, CHAR_WORKPETFALL) == 5) {
            rideflg = -4;
      ...
      sprintf(szBuffer, "%X|%s|%s|%X|%X|%X|%X|%X|%X|%s|%X|%X|%X|",
              ...
              CHAR_getWorkInt(charaindex, CHAR_WORKMAXHP), flg, rideflg,
...
void BATTLE_BpSendToWatch(BATTLE *pBattle, // 棋爵田玄伙正旦弁及禾奶件正
...
int checkErrorStatus(int charaindex) {
  ...
  if (CHAR_getWorkInt(charaindex, CHAR_WORKPARALYSIS) > 0 // 麻痹
  ...
  //|| CHAR_getWorkInt( charaindex, CHAR_DOOMTIME ) > 0 //世界末日集气
      ...
      || CHAR_getWorkInt(charaindex, CHAR_WORK_ABSOLUTE) > 0
#endif
#ifdef _PRO3_ADDSKILL
      || GETHIGHVALUE(CHAR_getWorkInt(charaindex, CHAR_WORK_POISON_DRAGNET)) > 0
#endif
  ) {
