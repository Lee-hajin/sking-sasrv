...
static PROFESSION_SKILL_skillFunctionTable PROFESSION_SKILL_functbl[] = {
...
#ifdef _PRO_KILLME
    {"PROFESSION_KILLME", PROFESSION_killme, 0}, // 舍身诱敌
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
    {"PROFESSION_ABSOLUTE_DEFENSE", PROFESSION_absolute_defense, 0}, // 绝对防御
#endif
    // 猎人
    ...
    {"PROFESSION_PLUNDER", PROFESSION_plunder, 0},               // 铺迓佣
    ...
    {"PROFESSION_CRACK_BONE", PROFESSION_crack_bone, 0},         // 裂骨断筋
    {"PROFESSION_POISON_DRAGNET", PROFESSION_poison_dragnet, 0}, // 毒素之网
    {"PROFESSION_PACIFY", PROFESSION_pacify, 0},                 // 抚慰心灵
    {"PROFESSION_DARK_SEAL", PROFESSION_dark_seal, 0},           // 灵封印
    {"PROFESSION_RESTATE", PROFESSION_restate, 0}, // 斗气导引,神灵之赐
    {"PROFESSION_DEMON_SPEECH", PROFESSION_demon_speech, 0}, // 恶魔诅咒
    {"PROFESSION_FIRST_AID", PROFESSION_first_aid, 0},       // 战场急救
    {"PROFESSION_ROAR", PROFESSION_roar, 0},                 // 战狼怒吼
    {"PROFESSION_MEDICATION", PROFESSION_medication, 0},     // 制药学
#endif
    // 共同技能
...
BOOL PROFESSION_initSkill(char *filename) {
  ...
  for (i = 0; i < arraysizeof(PROFESSION_SKILL_functbl); i++) {
    PROFESSION_SKILL_functbl[i].hash =
        hashpjw(PROFESSION_SKILL_functbl[i].functionname);
  }
  return TRUE;
}
...
BOOL PROFESSION_SKILL_CHECKINDEX(int index) {
  if (profession_skill_num < index || index < 0)
    return FALSE;
  return TRUE;
}

static BOOL PROFESSION_SKILL_CHECKINTDATAINDEX(int index) {
  if (PROFESSION_SKILL_DATAINTNUM < index || index < 0)
    return FALSE;
  return TRUE;
}

static BOOL PROFESSION_SKILL_CHECKCHARDATAINDEX(int index) {
  if (PROFESSION_SKILL_DATACHARNUM < index || index < 0)
    return FALSE;
  return TRUE;
}

int PROFESSION_SKILL_getInt(int index,
                                   PROFESSION_SKILL_DATAINT element) {
  return PROFESSION_skill[index].data[element];
}

int PROFESSION_SKILL_setInt(int index, PROFESSION_SKILL_DATAINT element,
                                   int data) {
  int buf;
...
char *PROFESSION_SKILL_getChar(int index,
...
BOOL PROFESSION_SKILL_setChar(int index,
                                     PROFESSION_SKILL_DATACHAR element,
                                     char *new) {
  if (!PROFESSION_SKILL_CHECKINDEX(index))
    return FALSE;
...
int _PROFESSION_SKILL_GetArray(char *file, int line, int charaindex,
                               int skill) {
  int skillid, array;
...
int PROFESSION_SKILL_Use(int charaindex, int skill, int toNo, char *data) {
  ...
  if (professionCallLua(charaindex, skillid, Pskillid) == 0) {
    return -1;
  }
#endif
  ...
  if (func) {
    ...
    // 耗损 MP
    if (PROFESSION_SKILL_DEC_COST_MP(charaindex, skill, Pskillid,
                                     skill_level) != 1) {
      //			print("\n Error:职技MP:%d:%d:%d ", skill,
      //Pskillid, skill_level);
      return ret;
    }
    ...
    if (Pskillid == 57 //激怒宠物 选宠物才有经验值
        && CHAR_getInt(
               BATTLE_No2Index(
                   CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEINDEX), toNo),
...
#ifdef BOSS_AI
int PROFESSION_SKILL_UseByBoss(int charaindex, int skillid, int toNo,
                               char *data) {
  int Pskillid;
  int ret = -1;
  PROFESSION_SKILL_CALLFUNC func;
  ...
  if (func) {
    // 使用职业技能
    ret = func(charaindex, toNo, Pskillid, data, 100);
    if (ret == -1) {
      int rands = rand() % 10;
      // andy_log
      // print("ret = %d, rands:%d \n", ret, rands);
...
// 耗损 MP
int PROFESSION_SKILL_DEC_COST_MP(int charaindex, int skill, int Pskillid,
                                 int skill_level) {
  ...
  if (skill_level > 0) {
    ...
    if (old_mp < dec_mp)
      return -1;
#ifdef _PRO3_ADDSKILL
    if (CHAR_getInt(charaindex, PROFESSION_CLASS) == PROFESSION_CLASS_WIZARD) {
    ...
    if (Pskillid != 11
#ifdef _PRO_ABSOLUTE_DEFENSE
        && Pskillid != 75 && Pskillid != 76 && Pskillid != 77
#endif
#ifdef _PROFESSION_ADDSKILL
...
void PROFESSION_LEVEL_CHECK_UP(int charaindex) {
  ...
  //	print("\n技能熟练度上升 ==> 当前熟练度点数(%d)
  //下一级熟练度升级点数(%d)", skill_level_sum, next_level_need_point );
  ...
  if (skill_level_sum >= next_level_need_point) {
    ...
    //		print("\n职业等级上升 ==> 当前职业等级点数(%d)
    //下一级职业等级升级点数(%d)", old_skill_point, next_skill_point );
...
void PROFESSION_LEVEL_CHECK_UP2(int charaindex) {
  int skill_level_add = 0, skill_level_sum = 0, i = 0;
  int old_level = CHAR_getInt(charaindex, PROFESSION_LEVEL);
  int next_level_need_point = 9999999;
  CHAR_HaveSkill *hskill;
  ...
  if (skill_level_sum >= next_level_need_point) {
    ...
    // CHAR_setInt( charaindex, PROFESSION_SKILL_POINT, next_skill_point );
    CHAR_sendStatusString(charaindex, "P");
    sprintf(msg, "职业等级上升为%d级！", old_level + 1);
    CHAR_talkToCli(charaindex, -1, msg, CHAR_COLORYELLOW);
    CHAR_sendCToArroundCharacter(
        CHAR_getWorkInt(charaindex, CHAR_WORKOBJINDEX));
  }
...
void profession_common_fun(int charaindex, int toNo, int skill_level, int array,
                           int com1) {
...
// 巫师技能-火山泉
int PROFESSION_volcano_springs(int charaindex, int toNo, int array, char *data,
                               int skill_level) {
  if (toNo > 19)
    toNo = 0;
...
// 巫师技能-火星球
int PROFESSION_fire_ball(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo < 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-火龙枪
int PROFESSION_fire_spear(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-召雷术
int PROFESSION_summon_thunder(int charaindex, int toNo, int array, char *data,
                              int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-电流术
int PROFESSION_current(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-暴风雨
int PROFESSION_storm(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-冰箭术
int PROFESSION_ice_arrow(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-冰爆术
int PROFESSION_ice_crack(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-冰镜术
int PROFESSION_ice_mirror(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-世界末日
int PROFESSION_doom(int charaindex, int toNo, int array, char *data,
                    int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-嗜血成性
int PROFESSION_blood(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-嗜血蛊
int PROFESSION_blood_worms(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-一针见血
int PROFESSION_sign(int charaindex, int toNo, int array, char *data,
                    int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-火附体
int PROFESSION_fire_enclose(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-冰附体
int PROFESSION_ice_eclose(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-雷附体
int PROFESSION_thunder_enclose(int charaindex, int toNo, int array, char *data,
                               int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-火魔法熟练度
int PROFESSION_fire_practice(int charaindex, int toNo, int array, char *data,
                             int skill_level) {
  return TRUE;
}

// 巫师技能-冰魔法熟练度
int PROFESSION_ice_practice(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  return TRUE;
}

// 巫师技能-雷魔法熟练度
int PROFESSION_thunder_practice(int charaindex, int toNo, int array, char *data,
                                int skill_level) {
  return TRUE;
}

// 巫师技能-附身术
int PROFESSION_enclose(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 巫师技能-移形换位
int PROFESSION_transpose(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-爆击
int PROFESSION_brust(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-连环攻击
int PROFESSION_chain_atk(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-回避
int PROFESSION_avoid(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-补血
int PROFESSION_recovery(int charaindex, int toNo, int array, char *data,
                        int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-武器专精
int PROFESSION_weapon_focus(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  return TRUE;
}

// 勇士技能-状态回复
int PROFESSION_reback(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-双重攻击
int PROFESSION_chain_atk_2(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-舍已为友
int PROFESSION_scapegoat(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-激化攻击
int PROFESSION_enrage(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-能量聚集
int PROFESSION_energy_collect(int charaindex, int toNo, int array, char *data,
                              int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-专注战斗
int PROFESSION_focus(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-盾击
int PROFESSION_shield_attack(int charaindex, int toNo, int array, char *data,
                             int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-二刀流
int PROFESSION_dual_weapon(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  return TRUE;
}

// 勇士技能-格档
int PROFESSION_deflect(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-贯穿攻击
int PROFESSION_through_attack(int charaindex, int toNo, int array, char *data,
                              int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-座骑攻击
int PROFESSION_cavalry(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-濒死攻击
int PROFESSION_dead_attack(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-回旋攻击
int PROFESSION_convolute(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo < 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 勇士技能-混乱攻击
int PROFESSION_chaos(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-陷阱
int PROFESSION_trap(int charaindex, int toNo, int array, char *data,
                    int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-驯伏宠物
int PROFESSION_docile(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-激怒宠物
int PROFESSION_enrage_pet(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-天罗地网
int PROFESSION_dragnet(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-树根缠绕
int PROFESSION_entwine(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-自给自足
int PROFESSION_autarky(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  return TRUE;
}

// 猎人技能-铺迓佣
int PROFESSION_plunder(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-毒素武器
int PROFESSION_toxin_weapon(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-火抗性提升
int PROFESSION_resist_fire(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-冰抗性提升
int PROFESSION_resist_ice(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-雷抗性提升
int PROFESSION_resist_thunder(int charaindex, int toNo, int array, char *data,
                              int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
#ifdef _PROFESSION_ADDSKILL
// 猎人技能-自然威能
int PROFESSION_resist_f_i_t(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-号召自然
int PROFESSION_call_nature(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-四属性结界
int PROFESSION_boundary(int charaindex, int toNo, int array, char *data,
                        int skill_level) {
  // if(toNo!=20 && toNo!=21)
  // toNo=0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
#endif
#ifdef _PRO_KILLME
int PROFESSION_killme(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_KILLME);
  return TRUE;
...
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
int PROFESSION_absolute_defense(int charaindex, int toNo, int array, char *data,
                                int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
...
#ifdef _PRO3_ADDSKILL
//裂骨断筋
int PROFESSION_crack_bone(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_CRACK_BONE);
  return TRUE;
}
//毒素之网
int PROFESSION_poison_dragnet(int charaindex, int toNo, int array, char *data,
                              int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_POISON_DRAGNET);
  return TRUE;
}
//抚慰心灵
int PROFESSION_pacify(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_PACIFY);
  return TRUE;
...
//灵封印
int PROFESSION_dark_seal(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_DARK_SEAL);
  return TRUE;
}
//斗气导引
int PROFESSION_restate(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_RESTATE);
  return TRUE;
}
//恶魔诅咒
int PROFESSION_demon_speech(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_DEMON_SPEECH);
  return TRUE;
}
//战场急救
int PROFESSION_first_aid(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_FIRST_AID);
  return TRUE;
}
//战狼怒吼
int PROFESSION_roar(int charaindex, int toNo, int array, char *data,
                    int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_PROAR);
  return TRUE;
...
//制药学
int PROFESSION_medication(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  profession_common_fun(charaindex, toNo, skill_level, array,
                        BATTLE_COM_S_MEDICATION);
  return TRUE;
}
#endif
// 猎人技能-团体火抗性提升
int PROFESSION_g_resist_fire(int charaindex, int toNo, int array, char *data,
                             int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-团体冰抗性提升
int PROFESSION_g_resist_ice(int charaindex, int toNo, int array, char *data,
                            int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-团体雷抗性提升
int PROFESSION_g_resist_thunder(int charaindex, int toNo, int array, char *data,
                                int skill_level) {
  if (toNo != 20 && toNo != 21)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-弱点攻击
int PROFESSION_attack_weak(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-挑拨
int PROFESSION_instigate(int charaindex, int toNo, int array, char *data,
                         int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-遗忘
int PROFESSION_oblivion(int charaindex, int toNo, int array, char *data,
                        int skill_level) {
  if (toNo > 19)
    toNo = 0;
  profession_common_fun(charaindex, toNo, skill_level, array,
...
// 猎人技能-追寻敌踪
int PROFESSION_track(int charaindex, int toNo, int array, char *data,
                     int skill_level) {
...
// 猎人技能-回避战斗
int PROFESSION_escape(int charaindex, int toNo, int array, char *data,
                      int skill_level) {
  char *pszP = NULL, *pszOption = "\0", msg[20];
  int per = 0, rate = 0, img1, ret = 1;
  skill_level /= 10;
...
// 共同技能-气力充沛
int PROFESSION_full_mp(int charaindex, int toNo, int array, char *data,
                       int skill_level) {
  return TRUE;
}
// 共同技能-负重增加
int PROFESSION_strong_back(int charaindex, int toNo, int array, char *data,
                           int skill_level) {
  return TRUE;
}
// 共同技能-自我强化
int PROFESSION_strengthen(int charaindex, int toNo, int array, char *data,
                          int skill_level) {
  return TRUE;
}
