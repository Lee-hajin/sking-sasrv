...
int BATTLE_ItemUseDelete(int charaindex, int haveitemindex) {
  int itemindex;
    ...
    return 0;
  {
    LogItem(CHAR_getChar(charaindex, CHAR_NAME),
            CHAR_getChar(charaindex, CHAR_CDKEY), itemindex, "战斗使用道具",
...
void ITEM_useMRecovery_Battle(int charaindex, int toNo, int haveitemindex) {
  ...
  if (BATTLE_CHECKINDEX(battleindex) == FALSE)
    return;
  if ((attackNo = BATTLE_Index2No(battleindex, charaindex)) == -1)
    return;
  ...
  if (IsBATTLING(charaindex) == TRUE) {
    ...
    if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER
      ...
      if (BATTLE_CheckSameSide(charaindex, toNo) == 0) { //不同边
        ...
        CHAR_talkToCli(charaindex, -1, "光镜守..不得施予非玩家敌方。",
                       CHAR_COLORYELLOW);
        return;
      }
    }
...
void ITEM_useStatusChange_Battle(int charaindex, int toNo, int haveitemindex) {
  int turn = 0, i;
...
void ITEM_useStatusRecovery_Battle(int charaindex, // 银匀凶谛及奶件犯永弁旦
                                   int toNo, // 银歹木月谛及  寞
...
void ITEM_useMagicDef_Battle(int charaindex, int toNo, int haveitemindex) {
  int turn = 0, i;
  int battleindex, attackNo, itemindex, status = -1;
  char *pszP = NULL, *arg;
...
//--------------------------------------------------------------
// 爵    及桦宁
void ITEM_useParamChange_Battle(int charaindex, // 银匀凶谛及奶件犯永弁旦
                                int toNo,       // 银歹木月谛及  寞
                                int haveitemindex // 银丹谛及失奶  丞及    寞
) {
  int i;
...
//--------------------------------------------------------------
// 爵    及桦宁
void ITEM_useFieldChange_Battle(int charaindex, // 银匀凶谛及奶件犯永弁旦
                                int toNo,       // 银歹木月谛及  寞
                                int haveitemindex // 银丹谛及失奶  丞及    寞
) {
  int itemindex;
...
//--------------------------------------------------------------
// 爵    及桦宁
void ITEM_useAttReverse_Battle(int charaindex, // 银匀凶谛及奶件犯永弁旦
                               int toNo,       // 银歹木月谛及  寞
                               int haveitemindex // 银丹谛及失奶  丞及    寞
) {
...
//--------------------------------------------------------------
// 爵    及桦宁
void ITEM_useRessurect(int charaindex,   // 银匀凶谛及奶件犯永弁旦
                       int toNo,         // 银歹木月谛及  寞
                       int haveitemindex // 银丹谛及失奶  丞及    寞
) {
...
//--------------------------------------------------------------
// 爵    及桦宁
void ITEM_useCaptureUp_Battle(int charaindex, // 银匀凶谛及奶件犯永弁旦
                              int toNo,       // 银歹木月谛及  寞
                              int haveitemindex // 银丹谛及失奶  丞及    寞
) {
...
void ITEM_useAddexp_Effect(int charaindex, int toindex, int haveitemindex) {
  ...
  if ((p = strstr(arg, "檬扁拳")) != NULL) {
    ...
    sprintf(szBuffer, "[%s](捞)狼 版氰摹 备浇 沥焊啊 檬扁拳 登菌嚼聪促.",
...
#ifdef _ITEM_ADDEXP2 // 智果时间可以累加
  {
    ...
    if (point >= 72000) {
      CHAR_talkToCli(charaindex, -1, "智慧之果累计时间及效果已达上限。",
                     CHAR_COLORYELLOW);
      return;
    }
    point += (power * vtime);
  ...
  //清除道具
  BATTLE_ItemUseDelete(charaindex, haveitemindex);
  sprintf(szBuffer, "[%s] [%d]%% [%d]", CHAR_getChar(toindex, CHAR_NAME), power,
          vtime);
  CHAR_talkToCli(charaindex, -1, szBuffer, CHAR_COLORYELLOW);
}
#endif
...
void ITEM_useFirecracker_Battle(int charaindex, int toindex,
                                int haveitemindex) {
  ...
  if ((buff1 = ITEM_getChar(itemindex, ITEM_ARGUMENT)) == "\0")
    return;
  ...
  sprintf(szWork, "BB|a%X|w3|r%X|f0|d0|p0|FF|",
          BATTLE_Index2No(battleindex, charaindex), toindex);
