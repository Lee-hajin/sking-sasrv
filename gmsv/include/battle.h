...
typedef enum
{
...
#endif
#ifdef _PETSKILL_RIDE //上马术
	BATTLE_COM_S_RIDE,
#endif
#ifdef _PETSKILL_PROVOKEFIGHT //挑衅反击
...
#endif
#ifdef _PRO_KILLME
	BATTLE_COM_S_KILLME, //舍身诱敌
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
	BATTLE_COM_S_ABSOLUTE_DEFENSE, //绝对防御
#endif
#ifdef _PETSKILL_ISULTIMATE
	BATTLE_COM_S_ISULTIMATE,
#endif
#ifdef _PRO3_ADDSKILL
	BATTLE_COM_S_CRACK_BONE,	 //裂骨断筋
	BATTLE_COM_S_POISON_DRAGNET, //毒素之网
	BATTLE_COM_S_PACIFY,		 //抚慰心灵
	BATTLE_COM_S_DARK_SEAL,		 //?灵封印
	BATTLE_COM_S_RESTATE,		 //斗气导引,神灵之赐
	BATTLE_COM_S_DEMON_SPEECH,	 //恶魔诅咒
	BATTLE_COM_S_FIRST_AID,		 //战场急救
	BATTLE_COM_S_PROAR,			 //战狼怒吼
	BATTLE_COM_S_MEDICATION,	 //制药学
#endif
	...
	BATTLE_COM_S_PLUS1, //增强技能	shen add 2012/07/10
#endif
	BATTLE_COM_END
} BATTLE_COM;
...
#define STRCPY_TAIL(_pszTop, _pszLast, _szBuffer)                                                                    \
	{                                                                                                                \
		...
		(_pszTop + _len < (_pszLast)-1) ? (memcpy(_pszTop, _szBuffer, _len), _pszTop += _len, _pszTop[0] = 0) : (0); \
	}

#define BATTLESTR_ADD(_szBuffer)                                                                                                              \
	{                                                                                                                                         \
		int _len = strlen(_szBuffer);                                                                                                         \
		(pszBattleTop + _len < (pszBattleLast)-1) ? (memcpy(pszBattleTop, _szBuffer, _len), pszBattleTop += _len, pszBattleTop[0] = 0) : (0); \
	}
...
#ifdef _ALLBLUES_LUA_1_4
BOOL BATTLE_setLUAFunction(int battleindex, int functype, lua_State *L, const char *luafunctable);
lua_State *BATTLE_getLUAFunction(int battle, int functype);
#endif
