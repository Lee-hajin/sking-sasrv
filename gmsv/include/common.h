...
/* MACROS */
#define RETURNFALSEIFFALSE(x) \
  if (!x)                     \
    return FALSE;
#define EXITWITHEXITCODEIFFALSE(x, code) \
  if (!x)                                \
    exit(code);
#ifdef __GNUC__
#define print(format, arg...) fprintf(stderr, format, ##arg)
#define fprint(format, arg...) fprintf(stderr, "\033[31m%s:%d " format "\033[0m", __FILE__, __LINE__, ##arg)
#endif
